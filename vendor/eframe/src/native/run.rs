...
impl<T: WinitApp> WinitAppWrapper<T> {
    ...
    fn check_redraw_requests(&mut self, event_loop: &ActiveEventLoop) {
        ...
        self.windows_next_repaint_times
            .retain(|window_id, repaint_time| {
                ...
                if let Some(window) = self.winit_app.window(*window_id) {
                    ...
                    // never be processed. Collect these windows to paint them directly.
        ...
        // Paint invisible windows directly because the OS won't send RedrawRequested.
        // This also lets queued viewport commands such as Visible(true) make progress.
        ...
        // Throttle only repaints already requested by the app. Do not insert a repaint
        // for an idle invisible window: an idle tray resident must remain asleep.
        if !invisible_window_ids.is_empty() {
            let now = Instant::now();
            for window_id in &invisible_window_ids {
                throttle_existing_repaint(&mut self.windows_next_repaint_times, *window_id, now);
            }
        }
            ...
            let event_result = match event {
                UserEvent::RequestRepaint {
                        ...
                        || current_pass_nr == cumulative_pass_nr + 1
                    {
                        ...
                        {
                            // A hidden/minimized window won't receive RedrawRequested on
                            // Windows. Bound app-requested work without creating an idle
                            // heartbeat or leaving the event loop in ControlFlow::Poll.
                            let when = if let Some(window) = self.winit_app.window(window_id)
                                && is_invisible_or_minimized(&window)
                            {
                                throttle_invisible_repaint_time(when, Instant::now())
                            } else {
                                when
                            };
