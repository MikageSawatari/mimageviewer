...
This directory is based on `eframe` 0.33.3. It backports the invisible/minimized-window repaint
scheduler fix from upstream [PR #7905](https://github.com/emilk/egui/pull/7905): due work is
consumed without leaving `ControlFlow::Poll` waiting for an OS redraw event that hidden Windows
HWNDs do not receive, and app-requested hidden repaints are throttled without creating an idle
heartbeat. No application-side hidden guard or sleep is part of this patch. See
`docs/tray-residency-cpu-spin-investigation.md` in the application repository.

Remove this vendored crate and the root `[patch.crates-io]` override after upgrading to an
upstream eframe release containing PR #7905 (0.34.0 or later), once the tray-residency regression
checks pass without the local patch.
