    ...
    extern "C" fn callback(
        ...
        match msg {
            native::UCM_CHANGEVOLUMEW => {
                // UnRAR guarantees that P1 points to a nul-terminated volume name. In
                // RAR_VOL_NOTIFY mode it can point directly into a std::wstring, whose
                // allocation is only as long as that particular name. Treating it as a
                // fixed-size buffer reads beyond the allocation before truncating at nul.
