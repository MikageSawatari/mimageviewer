...
しない。remote は PDF ページでは実際に描画した元ラスタのピクセル寸法を編集座標とし、通常画像では
従来どおり decode 後の元画素寸法を使う。両者を `StoredEditSpace` 境界で分け、crop とコミック注釈は
カタログ値を直接受け取れない形にした。カタログ値を使う remote の残り 1 箇所は縦横比による見開き
分類だけであり、絶対座標へは変換しない。
