...
- ZIP エントリ: 書庫を 1 回だけ開き、各エントリの**先頭 64 KiB だけ**を読む。全体を
  展開すると、寸法のためだけに 1 冊分の画像を伸長することになる。ネストパス・旧形式名・
  RAR はこの経路では解決しない (RAR は部分読みが無く 1 エントリごとに全体展開になる)。
- **寸法不明の件数はログへ出す**ので、次は推測から始めずに済む。

なお「分割のために全ページの寸法が要る」という説明は正確ではない。分割だけなら、ページが
届いた時点で左右に分ければよく、ページ番号は変わらないので利用者から見た位置も動かない
(位置がずれるのは、端末側が「表示単位の並びへの添字」で位置を持っているという**実装の
都合**であって、必然ではない)。**本当の理由は見開きのページ揃えが同じ情報を要ること。**
横長ページを単独表示にするかどうかが決まらないと組み合わせが決まらず、こちらはページ 1 枚
ずつでは解けない。カタログが無い本では見開きの揃えも同様に狂っていた。

横長判定を作る場所はコンテナと横断コレクションの 2 つある。どちらも同じ関数を通す。

## 6. 残り

- リリース時の更新履歴 (Phase 0 で利用者確認を挟む)。
- **未確認**: 編集ツール (消しゴム / 隠蔽 / 注釈 / 補正レイヤー / 切り取り) へ分割中に入ると
  分割前のページ全体で開くこと (2026-08-26 に修正、実機での再確認待ち)。

## 6.1 本体側の実機確認 (2026-08-26 完了)

ページ送り (左右の往復 / 混在 / 先頭・末尾 / 長押し / クリック・タップ)、着地 (一覧・
しおり・検索・シーク・フォルダ移動)、モード切替、ズーム・ナビゲータ・ルーペ、記録もの
(★ / タグ / しおり / 読書位置 / ページ番号)、コンテナ種別 (フォルダ / ZIP / PDF / ネスト /
変換書庫)、縦連結、回転を確認済み。実機確認用の素材は
[make-split-trim-sample.py](../scripts/make-split-trim-sample.py) と
[make-split-archive-sample.py](../scripts/make-split-archive-sample.py) で作れる。

### 実機で見つかり、直したもの

| 症状 | 原因 |
| --- | --- |
| 編集ツールへ入ると半ページのまま | 表示の左右が編集へ漏れていた。`page_edit_tool_owns_canvas` の間は左右を返さない |
| HUD からモードを選んでも綴じ方向が付いてこない | 対応表が片側 (方向 → モード) にしかなく、逆向きが分割を素通り。`SpreadMode::reading_direction` を正本にした |
| 矢印キー・タップの左右が反転しない | `is_rtl` (見開きのペア並び順) を入力の左右にも流用していた。**1 つの述語が 2 つの問いに使われていた**。読み順から導く `advances_right_to_left` を分けた |
| 非アクティブなウィンドウで分割が外れる / F12 分離中にちらつく | 非アクティブ窓は snapshot から描くのに、その `content_bbox` だけ分割を通していなかった (描き手の数え損ね、通算 3 度目) |
| スライドショーが分割を無視する | 自動送りは読み手の代わりに送る機能なので分割に従う (利用者判断) |

回転すると分割が解除され、半回転・1 回転で再び分割されるのは**仕様**として確認した
(横長でなくなれば分割しない、という判定がそのまま出ている)。

**リモートは 2026-08-26 に実機 (スマートフォン) で確認済み。**分割表示、モードの往復
(分割 左→右 ⇄ 右→左 ⇄ 1ページ表示 ⇄ 見開き)、PDF・ZIP 双方。製品ページとマニュアルは
追記済み。
