...
### 横長判定はサムネイルカタログから切り離した (2026-08-26)

出荷前の実機で「分割を選んでも横長のまま」という報告があり、**2 回続けて原因を外した**。
1 回目は query 文字列の解析漏れ (実在の欠陥だが症状の原因ではない)、2 回目は
「速い画像フォルダにはカタログが無いから」(対象が PDF だった)。3 回目にログへ件数を
出してから当てた。**推測の前に計装する。**

実際の原因は、横長判定がサムネイルカタログだけを見ていたこと。既定のキャッシュ方針
`Auto` は速くて小さい画像にカタログを作らず、端末から直接開いた PDF にはそもそも
サムネイルが無い。**表示の仕様がキャッシュ方針で変わってはいけない**ので、方針に
依存しない経路を 2 つ用意した。

- 画像ファイル: ヘッダだけ読む (`into_dimensions`、画素は読まない)。
- PDF: `MSG_PAGE_SIZES` を新設し、`FPDF_GetPageSizeByIndexF` で**文書ごとに 1 往復**。
  ページを読み込まないのでレンダリング 1 枚より安い。カタログに寸法がある間は呼ばない。
- ZIP エントリ: 未対応 (1 エントリごとの展開が要る)。`cache_zip_always` を切った場合
  だけ効かない。**寸法不明の件数はログへ出す**ので、次は推測から始めずに済む。

横長判定を作る場所はコンテナと横断コレクションの 2 つある。どちらも同じ関数を通す。
