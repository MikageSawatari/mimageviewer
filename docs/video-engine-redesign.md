...
- **ピン (📌)**: 動画再生中の現在位置をグリッドサムネとして固定する。1 動画につき
  最大 1 個。
  - 作成: フルスクリーン動画再生中の左ジャンプパネル上部のピンボタン、または
    シークバーホバー preview 上のピンボタン。
  - 更新: 既にピンがある動画でもう一度押すと新位置で上書き (= 常に SetPinAt)。
    `VideoPinDb::set_pin()` の SQL は `CASE WHEN excluded.thumb_webp` で
    新サムネが空 (= 新位置の seek thumbnail 未生成) なら既存 thumb_webp を保持する。
  - 削除: 左ジャンプパネルのピン行右側の × ボタン (`NativeOverlayCommand::DeletePin`)。
...
- **Ctrl+←→ は使わない**。見開き表示中はページ進行方向 (RTL/LTR) で「次へ」の意味が
  ...
  (= 既存 Ctrl+↑↓ ショートカットと整合)。
- **ツールバーボタンの取捨選択**: 環境設定の「表示 → ツールバー」ページで、表示する
