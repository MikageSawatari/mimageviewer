...
`draw_fs_spread` 側で合成ページ (左右幅合計×高さ) を対象に `zip_spread_zoom_pan` で同様にズーム/パン
(既定 = 単ページ幅の約 1.2 倍)。見開きズーム中は表示トリムを無効化して左右ページ全体を対象にする
(トリム UV と合成ページ座標の混在で端に切り落とし領域が見える事故を避けるため)。**PDF** (単ページ) は
`maybe_rerender_pdf(zoom)` でズーム倍率に応じ高解像度へ再レンダ ((idx, zoom) が変わったときだけ要求し、
in-flight キャンセルの無限ループを避ける)。**既知の制限**: PDF を見開きでズームしても再レンダせず
fit 解像度のまま (画像見開きは問題なし。PDF 見開きズームは niche のため後続)。**表示トリム** 中はパン対象を
トリム後 bbox に絞る (上記コンテンツ領域 `content_min`/`content_size`)。操作仕様は
[keymap-spec.md](keymap-spec.md) を参照。
...
`spread_shift_anchor_idx` に移動先ページを置き、そのページから先だけ一時的にペアリングを組み直す。
アンカーより前は基本の表紙あり/なしルール、アンカー以降はアンカー起点のルールになるため、
横長ページを自動的な表紙扱いにはしない:
