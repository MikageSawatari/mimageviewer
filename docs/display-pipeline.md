...
| カラー化 | **白黒 → カラー**。差が最も大きい | `colorize_apply_ms` 中央値 **7.9ms** / 最大 **135ms** (1123×1648 で 10ms) | **サムネイルに適用して使う。** フルサイズの実測値なので、サムネイル寸法では面積比でこれを大きく下回る見込み。**採用前にサムネイル寸法で実測すること** (トーン階調化のぼかしに固定費がある可能性)。実測して間に合わないと分かった場合に限り、前ページ保持 + 「カラー化中」表示へ落とす |
...
1. **サムネイル寸法での色処理コストを実測する。** カラー化の 7.9ms はフルサイズの値。
   トーン階調化のぼかしに固定費がある可能性があるため、面積比の推定で進めない。
   計測は `scripts/page-turn/measure.rhai` + `-Setup` で用意した実物の本で行う
   (合成 fixture はカラー化を再現しない)。
2. **通過表示に色処理を適用する**。色調補正 (`adjust_ms` 実測 0.00ms) → LUT → カラー化の順。
   これで R1 と R2 を同時に満たせる。「カラー化中」で前ページを保持する必要がなくなる。
3. **通過中はフルサイズのアップロードとデコードを始めない**。UI スレッドの 28.2ms は
   ほぼ全量がフルサイズ upload (§2.5.1.1 の実測)。通過するページに対してこれを走らせない。
4. **止まったページだけ実体化する** (R4)。
