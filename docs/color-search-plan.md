...
> ステータス: **Phase 1/2 + Phase 3 実装済み（Codex 実装 + ClaudeCode レビュー/追補、2026-06-23）**。今回リリースの対象ビューは通常フォルダ / ZIP / PDF 表示に固定（集約ビューは見送り確定）。Eagle 風の色指定 UI（SV 面 + Hue、プリセット、HEX/RGB/HSL 入力）まで実装済み。スポイトは実機確認で UI 応答と入力透過のリスクが大きかったため削除済み。マニュアル（grid.html）・製品ページ（index.html）更新済み。UI snapshot は任意（既存 facet バー snapshot は無く、追加は forward-looking なので未実施）。
...
>   メニュー内完結の SV 面 + Hue スライダ、通常色プリセット、HEX/RGB/HSL 入力へ置き換え。
>   Eagle 先頭 2 つの特殊色（透明/白黒系と推定）は通常 RGB クエリと意味が違うため初版では未実装。
> - **実機確認後の修正 (2026-06-23)**: 色操作（SV/Hue、プリセット、HEX/RGB/HSL、許容範囲）で
...
- **コントロール**: メニュー内完結のカラーピッカー（SV 面 + Hue スライダ）+ 通常色プリセット +
  HEX/RGB/HSL 入力 + 許容範囲スライダ + クリア。色や許容範囲を操作した時点でフィルタを有効化する。
- **スキャン中 UI**: 色フィルタを有効化した瞬間に「絞り込み準備中 N/M」+ プログレス +
...
- per-folder DB を引かなくなったので、**原理的には Ctrl+G / タグ / 閲覧履歴ビューでも動く**
...
- ただし今回リリースでは**通常フォルダ / ZIP / PDF 表示のみ有効**に固定する。
  ...
  `favsearch.on_results_grid()`、閲覧履歴、ドライブ一覧）では、画像色メニューを出さない。
...
3. **Phase 3 — companion + 仕上げ** 一部実装済み
   - 実装済み: メタデータパネルのパレット表示（§7.1、decode 済み CPU バッファのみ）+
     ...
     Eagle 風ピッカー（SV/Hue、プリセット、HEX/RGB/HSL）。
