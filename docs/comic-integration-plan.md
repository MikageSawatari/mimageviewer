...
ピッカー / 詳細編集 / 実画像ベイク**。
Inc 0/1/2/3/7 完了、Inc 4c スタンプ完了、Inc 6 の変形ハンドル部分完了。
残: Inc 4c オノマトペ (追加 DL パックのフォント前提) / Inc 5 プリセット・フォント /
...
CC-BY 帰属 + bootstrap 配線 [本コミット]。次は Inc 4c オノマトペ → 編集用追加パック
DL 基盤 → Inc 5 プリセット・フォント → Inc 6 Undo）
