...
| D3 | サムネ反映 | 当初は非反映。v2.5 以降は「編集結果をサムネイルに保持」が ON の場合に限り、編集プレビューキャッシュへテキスト／スタンプと切り取りを焼き込む。色調補正は表示時に適用し、最終 AI／ポストフィルタは含めない。 |
...
- **キャッシュ無効化に comic 段を足す**: master §3.1 のルール「crop 変更 → edit+final cache クリア」
  ...
  Inc 1/2 で用意する。当初はサムネ非反映だったが、v2.5 では注釈保存時に編集プレビューキャッシュを
  無効化し、編集終了時に専用 worker で注釈と切り取りまで再生成する（D3）。通常の thumb cache は
  直接書き換えない。
...
- **対策**: ユーザー画像スタンプは **選択時に長辺 1024px 上限へ縮小し、PNG/base64 として
  `StampSource::Embedded` に入れて注釈ドキュメントへ埋め込む**。絵文字は同梱アセットへのキー参照のまま。
  いったん配置されたユーザー画像スタンプは元ファイルが削除されても表示・保存・再編集できる。
- **再利用履歴**: 自前スタンプを毎回ファイル選択しなくて済むよう、配置時に同じ埋め込み PNG を
  `%APPDATA%/mimageviewer/comic_user_stamps.db` に MRU として保存する（上限 48、ピッカー表示 24）。
  履歴から選んだ場合も配置先の注釈には `Embedded` がコピーされるため、履歴 DB の削除や prune は
  既存画像上のスタンプを壊さない。旧 `StampSource::File` 参照だけは元ファイル欠落時に
  プレースホルダ表示になりうる。
...
> ②本体タブに形状別パラメータのスライダー群・seed が無く形状コンボのみだった（§7.2 の [x] が
> 実態と不一致）。③しっぽの tip(先端)・付け根(base) 編集 UI が無かった（§7.3 の [x] が不一致）。
> ④吹き出し/窓のセリフにテキストプリセットバーが出ていなかった。
> **これらは以下で全て解消**し、§7.1〜7.8 の [x] が実態と一致するようになった:
> `85e3327d`（2 層構造化＝常時表示エリア新設）/ `690d7a17`（形状別スライダー＋しっぽ tip/付け根）/
> `228786ca`（左追加 UI 全幅縦ボタン＋一覧本文抜粋＋ダイアログ題名）。
> フォントのカテゴリ UI（追加パック/ユーザー/システム）と追加ダイアログの暗フレーム統一は
> 編集パック連動・視認性の観点で **mIV 独自設計として維持**（ラボへ戻さない）。
...
- [x] half_w/half_h（アスペクト保持の一様スケール）/ opacity / flip_h / flip_v
- [x] outline（ステッカーフチ）
- [x] style_preset_link
- [x] ピッカー（カテゴリ / 検索 / 最近）/ ユーザー画像追加 / source 差し替え
- [x] デコードキャッシュ / 欠落プレースホルダ / GPU テクスチャ quad（多数複製でも軽い）
- [x] 絵文字アセット同梱＋CC-BY 帰属表記（ソフトウェア情報 / readme）

### 7.5 MessageWindowObject — ✅ 個別編集 UI 完了（Inc 4d `1ebe7ddb`/`b4a15d46`）
- [x] frame: None / SolidRounded / DoubleLine（+ frame_gap_px, outline）
- [x] fill: None / Solid / Translucent / GradientScrim（+ gradient_to 線形グラデ / scrim_dense_side）
- [x] position: Top / Middle / Bottom / Center / Free（`resolve_window_placement` で pivot 解決）
- [x] size: FullWidth / Inset / AutoFitText
- [x] vanchor / Insets（per-side padding）/ shadow（ドロップシャドウ）
- [x] name_plate: mode / 名前テキスト / 色 / fill / outline / corner / padding / offset（mIV は「部品」タブに集約。ラボの上部常時ヘッダとは配置のみ差）
- [x] portrait: side / width / fill / outline / margin（本文との非重なり、色付きプレースホルダ）
- [x] indicator: kind ＋ indicator_auto（溢れた時だけ表示）
- [x] 日本語禁則ワードラップ / AutoFitText / オーバーフロー警告（赤枠、comic-core 描画）
- [ ] [deferred] 9-slice 画像枠 / 実立ち絵画像 / Beveled 枠 / per-corner 角丸 / choice・NVL 複数エントリ

### 7.6 DecorationLayer（飾り）— ✅ 完了（Inc 4e `1ebe7ddb`、`bubble_deco_ui`）
- [x] kind: Sparkle(星) / Flower(花) / Bubble(泡)
- [x] placement: Outline / Outside / Inside / Tail
- [x] density / size_ratio / color / seed（追加時 seed=max+1 で位置衝突回避、再生成ボタン）
- [x] outline_width / outline_color / center_color（花中央）/ points（星）/ petals（花）/ gradient（泡）

### 7.7 共有 UI 挙動
- [x] オブジェクト一覧（上へ/下へ/複製/削除）、z normalize、enable/disable トグル
- [x] 選択 / 移動 / 四隅スケール / 回転ノブ / しっぽハンドル（当たり判定＝comic-core ジオメトリ一致）— Inc 6 `c6d4d7cb`
- [x] Undo / Redo（エディタ専用、coalesce）— Inc 6 `52b5f9f6`/`93a24c6c`
- [x] プリセット: セリフ / 本体 / ウィンドウ（system + user、適用 / 更新 / 削除 / 改名 / リンク点灯）— Inc 5
...
- [x] mimageviewer.dat ミラー（バックアップ ON 時のみ）。OFF→ON バックフィルは siblings 同様 **見送り**（§6.3）
- [x] ZIP / PDF 内画像での編集・保存（page_path_key）— Inc 2
- [x] フォルダ移動で保持（バックアップ ON）、ユーザー画像スタンプも保持（§6.4）
- [x] 非破壊オーバーレイ表示（最前面 D1）/ Ctrl+E 焼き込み出力（表示と一致）— Inc 1/7
- [x] 編集プレビューキャッシュ使用時のみサムネ反映（D3）/ 非破壊回転 DB との整合（D8）
...
- **Inc 4: オブジェクト種別の描画＋インライン編集を移植**（4a→4e、各独立コミット）
  ...
  - 受け入れ（各）: **当該種別の描画とインライン編集コントロール**がラボ一致。§7 の該当項目を消し込む。
    ...
    描画＋編集コントロールに限定して判定）。
  - ✅ **4d ウィンドウ詳細 完了（2026-06-06、`1ebe7ddb` + Codex `b4a15d46`）**: `window_body_ui`
    (枠タブ = 位置/サイズ/角丸/背景[単色/半透明/スクリム/2色グラデ]/枠[単線/二重線]/影/縦位置/
    折り返し[禁則]/per-side 余白) + `window_parts_ui` (部品タブ = 名前プレート/立ち絵枠/続き指標) +
...
- **Inc 7: 書き出し統合（Ctrl+E）** — ✅ **基本実装完了（2026-06-05）**
  ...
  - ✗ **D10（ダウンサンプル後に最終解像度直焼き）は不採用（2026-06-07）**: 現状は base 解像度（ダウンサンプル前）で
    焼くため、強い縮小指定時はテキストが下地と一緒に縮小されてやや甘くなる。**等倍/軽縮小では差は出ない**。
    厳密な D10（worker 後段で最終解像度ベイク）は source→base（AI 倍率）→crop オフセット→最終縮小 と座標が多段で
    掛かり（spread 合成オフセット + FontSet 非 Send も絡む）、crop×強縮小×注釈の隅でしか出ない座標ズレの視覚バグが
    入りやすい。最終出力の縮小は重視機能でないため複雑さ/リスクに見合わず **実装しない判断**（D10 行 + C5 エントリ参照）。
  - 受け入れ: 注釈付き画像が Ctrl+E で焼き込み出力される（位置・crop・回転反映）。強縮小時の鮮明化は不採用。
- **Inc 8: 仕上げ・回帰** ✅ 完了 (2026-06-07)
  ...
  - **UI スナップショットは対象外と判断**: `tests/ui_snapshot.rs` (egui_kittest) は
    ...
    切り出した純粋描画関数」のみを対象にする (App 丸ごとは状態組み合わせ爆発で不採用)。テキスト注釈の
    左右パネルは `show_viewport_immediate` 内で `self.*` (選択/ドラッグ/`comic_docs`/フォント/ライブ画像)
    に密結合しており純粋関数化されていないため、現状の harness では捕捉できない。加えて、見た目回帰が
    最も出やすい**注釈の描画 (吹き出し形状・文字効果・しっぽ・並列ベイク) は comic-core の単体テストで
    画素/バッファ単位に検証済み** (並列==逐次の決定性、回転 no-clip、dilate coverage、tail tip 投影 等)。
    よって egui スナップショット追加はリファクタコスト > 便益と判断し、見送る (将来パネルを純粋関数化
    したタイミングで再検討)。
