...
- **Inc 7: 書き出し統合（Ctrl+E）**
  ...
  - **テキストは最終出力解像度に直接ベイクして最後に合成（D10）**: 画像側を crop→色→AI→ポストフィルタ
    →（ダウンサンプル）まで確定 → `scale_scene(S=最終出力長辺/crop後ソース長辺)` → 最終解像度で
    `bake_overlay_with_stamps` → 1 回 over 合成 → 書き出し。ダウンサンプルが**テキストより前**であること。
  - 受け入れ: 出力画像がフルスクリーン表示と一致（くっきり・回転反映）。ダウンサンプル指定時もテキストが
    甘くならない（最終解像度直焼き）。
- **Inc 8: 仕上げ・回帰**
