...
| U3 | ストリップの高さ・セル幅・枚数 | `SEEK_STRIP_HEIGHT = 104.0` / `SEEK_STRIP_CELL_WIDTH = 152.0` (`render_core.rs`)。**枚数は既定値を持たない**。presenter が実際の帯幅から `ceil(strip_rect.width() / SEEK_STRIP_CELL_WIDTH)` で出し、`RequestSeekStripWindow` で App へ渡す。「11 枚前後」という出発点は、画面幅で決まる導出に置き換わった |
