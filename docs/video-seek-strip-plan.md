...
| D7 | 波形にも**新しいキャッシュ機構**を作る | 全尺デコードを待たせない。§5 で窓オンデマンドとして解決する |
...
- ドラッグ `dx` → `center_index -= dx / cell_w`。
- 中央 `|` の直下は `center_index` の位置。**選択時刻はセル内で線形補間する**:
  `t = kf[i] + frac * (kf[i+1] - kf[i])` (`i = floor(center_index)`, `frac = fract`)。
  - 補間ありにすると、絵はセルのキーフレーム、時刻はその GOP 内の任意点になる。絵と時刻が
    最大 1 GOP ずれる代わりに、ストリップから精密な位置へ飛べる。
  - 補間なし (キーフレーム時刻ちょうどへシーク) も 1 行の差なので、**実機で見比べて確定する**
    (§9 の未確定 U1)。
...
enum StripAxis {
    ...
    KeyframeIndex { keyframes: Vec<f64> },
    /// 索引が無い / 不完全なコンテナ。等時間グリッドに落とし、絵は直前キーフレーム。
    TimeGrid { interval_secs: f64 },
}
```

- `avformat_index_get_entries_count` が動画ストリームの尺に対して妥当な数を返せば
  `KeyframeIndex`、そうでなければ `TimeGrid`。判定式は §9 の U2 で確定する。
...
### 5.4 永続キャッシュ (フェーズ 2、v1 では作らない)

窓オンデマンドで即応するので v1 には要らない。次段として、**粗い全尺ピーク列**を持つと
「全尺を一目で見る」ができるようになる。
...
| # | 論点 | 決め方 |
| --- | --- | --- |
| U1 | 選択時刻をセル内で線形補間するか、キーフレーム時刻ちょうどにするか | 両方実装して実機で見比べる。1 行の差 |
