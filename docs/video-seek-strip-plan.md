...
| D11 | キーフレームは**間隔が一定以上あくように間引く**。しきい値は環境設定 | N 枚おきではない。既定 2.0 秒、0.0 で間引かない。大きくすると大雑把なシークがしやすくなる |
...
**D18 (未実装)**: **ストリップ上のホイールで範囲を段階的に変える。現在値は右上に表示する**
...
- **波形の上の段は §5.4 を前提にする。** 窓ごとの復号は範囲に比例するので、実測から外挿すると
  ...
  機構があるので、**埋まりながら見える**形にできる。上の段を削るのではなく §5.4 を実装する。

**D19 (未実装)**: **ストリップの右上に鍵アイコンを置き、固定できるようにする** (利用者要望
...
- **既存の `video_seek_bar_locked` (bool) は変えない。** これは**リリース済み**の設定なので、
  ...
  `video_seek_strip_locked` (bool、未リリース) を足し、**ストリップ固定を ON にしたら
  バー固定も ON にする**不変条件を持たせれば、到達できる状態は上の 3 つだけになる。
  移行コードは不要。
- ストリップを「なし」にしたときの扱いを決める。固定は**ストリップが出ているときだけ意味を持つ**
  形にし、なしにしたら領域を解放する。
- 右上には D18 の現在値表示も置くので、鍵と数値の配置を同時に決める。

**D17 (未実装)**: **波形モードでストリップ上を hover したら、その時刻のサムネイルを 1 枚、
ストリップの上に出す** (利用者要望 2026-08-25)。元の要望が「音声とサムネイルを一緒に見ながら
選びたい」なので、波形モードで絵が一切見えないと片手落ちになる。

- 見た目と位置はシークバーの hover 1 枚プレビューに合わせる。出す場所はストリップの上。
- **サムネイル列モードでは出さない** (セル自体が絵なので重複する)。
...
- 最小間隔は環境設定で変える (`Settings.video_seek_strip_min_interval_secs`、**既定 2.0 秒**、
  範囲 0.0〜60.0)。**大きくすると 1 画面が覆う時間が伸びて大雑把なシークがしやすくなる。**
  **0.0 は間引かない** (全キーフレームが並ぶ = 最も細かい)。
...
| U7 | 最小間隔の既定 2.0 秒が長尺で足りるか | 2 時間の動画では 11 セル = 22 秒しか覆わない。尺に応じた既定が要るかを実機で判断する (設定で上げられるので v1 は固定でよい) |
