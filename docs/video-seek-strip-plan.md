...
(ストリップ固定) まで実装。
残りは D17 (波形 hover サムネイル)、§5.4、§9 の未確定項目の実機調整、
...
- **A: 末尾セルだけ失敗 — 246 件 (87%)**。237 件は失敗セルがちょうど 1 個。最終セルから EOF までの
  差は中央値 0.21 秒で、**最小 -1.164 秒 = 宣言 duration を越えた位置にセルを置いている**。
  `cell_count` の切り上げと duration の誤差で、最後のフレームより後ろにセルが立つのが原因。
  軸種別は `time_grid` 239 / `keyframe_index` 90 の**両方**で出るので、TimeGrid 固有ではなく
  軸に依らない境界の問題。**ここを直せば失敗の 87% が消える。**
- **B: それ以外 — 37 件**。`.mpg` 16 / `.mp4` 12 が主。最悪例は duration 40 秒に対し
  **keyframe_count 3・index_last_secs 1.0・coverage 2.5%** の mp4 で、索引が先頭 1 秒しか無いため
  ...
  TimeGrid 側の限界。「索引がほぼ無い素材は事前に unavailable と断る」方向の判断が要る
  (D21 の最大 GOP 基準は、索引が密なまま早期で終わる素材を拾えない)。
