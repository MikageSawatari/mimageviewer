...
**未対応 (要判断)**: 間引きで**最後のキーフレームが採用されない**ことがある。実素材
`アカリがやってきたぞっ` は 301 秒だが軸の最終セルは 283.1 秒で、**末尾 18 秒にセルが無い**
(最後のキーフレーム 294.8 秒が、直前の採用セルから 11.7 秒しか離れておらず 15 秒未満で落ちる)。
「最後のキーフレームは間隔に関わらず必ず採用する」で届くが、末尾だけ間隔が詰まる。
