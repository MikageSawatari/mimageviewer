...
- サムネイル (最小間隔): 0.1 / 0.2 / 0.5 / 1 / 2 / 5 / 10 / 15 / 30 / 60 秒と 2 / 5 / 10 / 15 / 30 分
...
**構築速度の実測 (2026-08-26、release profile)**: 7.0GB / H.264 + AAC の MP4 で 60 秒 chunk を
10 個。**1 chunk あたり 69.4ms (復号 54ms + 解析 16ms)**。60 分ぶん (60 chunk) の完全被覆に
**4.2 秒**、3 時間の全尺で **12.5 秒**。

- **重いのは解析ではなく復号側**。解析は per-sample ループが全てで、bin 幅を変えても
  ほぼ変わらない (60 秒ぶんが 0.1 / 0.5 / 1.8 秒 bin のいずれでも 14ms 前後)。
  帯用に指標を減らしても取り返せるのは 69ms 中 16ms までで、**最適化する価値は薄い**。
- 60 分の窓復号は 3.1 秒 (§14) なので、完全被覆までの時間はほぼ同等。加えて粗い列は
  (1) 中心から順に埋まるので**見たい場所は即座に出る**、(2) 一度作れば span 変更もスクロールも
  **復号なし**、という差がある。窓復号は動かすたびに毎回払う。

> **測り方の注意**: この節の初版は **debug profile で測った数字** (1 chunk 177.5ms、解析 122ms)
> を載せており、「解析側が重い」と**逆の結論**を書いていた。float の詰まったループは
> profile で 8 倍以上変わる。§14 の既存値も最適化プロファイルなので、**比較する数字は
> 必ず同じ profile で取り直す** (`cargo test --release`)。
...
**D29 (末尾セルの位置、2026-08-27、利用者報告)**: **格子軸 (`TimeGrid`) の末尾セルは
`duration - interval` に置く。**

尺は間隔の整数倍とは限らないので、素直に `index * interval` を使うと最後のセルが尺の端数の
位置に来る。実機報告の素材は **172.73 秒 / 5 秒間隔で末尾セルが 170.0 秒**で、そこへスクラブすると
**残り 2.7 秒しか無く、すぐ次の動画へ移ってしまう**。末尾セルを 1 間隔ぶん手前へ寄せれば、
どのセルへ移っても最低 1 間隔ぶんは見られ、**セルの右端が動画の末尾と一致する**。

- **末尾以外は動かさない。** `count = ceil(duration / interval)` なので
  `(count - 2) * interval < duration - interval` が常に成り立ち、直前のセルと同時刻にはならない。
- 逆写像 (`center_index_for_time`) は `cell(i)` の二分探索なので、置き方を変えても自動で追従する。
  等間隔を仮定した式は使っていない。
- 尺が 1 間隔に満たない素材は寄せる余地が無いので 0 のまま。

**D26 (最後のキーフレームを必ず採用) は原因ではない。** 報告された素材の軸を実測したところ
`time_grid` で、D26 の間引きを通っていなかった。利用者からは「D26 をやめる方が素直なら
やめてよい」と提案があったが、**測ってみると別の経路だった**ので D26 は残す。D26 は
「ストリップが動画の終わりまで届かない」という別の問題への対処で、利用者自身が
「間隔が詰まるよりサムネが見えない方が影響が大きい」と選んだもの。
