...
(メモリ内の粗い全尺ピーク列) まで実装。
残りは MPEG-TS など `TimeGrid` 経路の実素材確認のみ (§9 は 4 件とも決着済み)。
...
実装する前に書いた並びなので、そこだけ古い。**§9 の U2〜U5 は 4 件とも決着済み**
(2026-08-27 にコードと突き合わせて確認。§9 に決まった値と在り処を書いた)。残りは
MPEG-TS など `TimeGrid` 経路の実素材確認のみ。
...
> ⚠️ **D 番号が重複している。** 本書は決定を §2 の表と各節の散文の両方へ書いてきた結果、
> **D23 / D24 / D25 / D28 / D29 が別々の決定へ二重に使われている** (例: D28 は「表示の 5 値」と
> 「帯の可読性」)。既存の参照を壊さないため振り直していない。**番号だけで参照せず、節も添える。**
...
| # | 決定 | 補足 |
| --- | --- | --- |
| D28 | 表示は **非表示 + (場面 / 波形) × (周辺 / 全体) の 5 値**。`SeekStripView` 1 つで持つ | 開閉 bool + 内容 + 範囲の 3 本立てにすると「非表示なのに全体表示」が表現できてしまう。D12 の 3 値を置き換える |
| D29 | 高さは **大 (104pt、従来) / 中 (72pt) / 小 (48pt) / 最小 (36pt)** の 4 段階。設定名に px を出さない | egui は論理座標なので、同じ pt でも DPI で見え方が変わる。中の値は実機で確認して決めた。**最小は実機確認で足した** (2026-09-01): 全体表示の枚数はセルの高さで決まるので、「小」でも思ったほど並ばなかった。36pt は鍵ボタン (28pt + 余白 4pt) が収まる下限に近い |
| D30 | **右下のストリップボタンはメニューを開く**。非表示・4 表示・3 高さを直接選べる | D13 の 1 クリック巡回を置き換える。巡回は環境設定で絞れるので、巡回だけだと到達できない表示が生まれる。`Shift+S` は引き続き巡回する |
| D31 | 全体表示に**表示範囲の設定は無い**。右上の値表示を出さず、**ホイールも効かない** | 範囲は尺と帯の幅で決まるので変えられる値が無い。ホイールは背面の項目送りへも渡さない (周辺表示と行き来したときに、スクロールが動画送りへ化けるのを防ぐ、利用者判断 2026-09-01) |

**寸法の解決は 1 か所**。`seek_strip_layout.rs` の `SeekStripLayout` が、選んだ高さと表示範囲から
帯の矩形・セル幅・セル高さ・全体表示のセル数を返し、**描画・入力・映像の予約・波形ラスタの
要求がすべてこの 1 つの値を見る**。旧実装は `SEEK_STRIP_HEIGHT` 定数を 4 経路が別々に読んで
いたので、定数の局所置換では必ずどれかが取り残される。高さは `NativeBarLockState` に載せて
presenter へ届ける (snapshot と一緒に運ぶと、動画の切替中に予約だけ外れる = §1.162 と同型)。
`VideoVisualLayout` は「表示中か」ではなく**占めている points** を持つ。

**全体表示は周辺表示と同じ座標系を使う。** 中心位置は状態ではなく導出値で、軸の真ん中 (場面) /
尺の真ん中 (波形) に固定する。すると `cell(i)` が帯の i 番目の区間に並び、セル ⇔ ポインタ ⇔
時刻の既存の写像がそのまま成り立つ。**動くのは赤線だけ** (`SeekStripMarker::Fraction`)。

- 場面の軸は `StripAxis::whole(duration, cells)` = 尺を帯のセル数で等分した `TimeGrid`。
  セル数は presenter が実寸から出して `RequestSeekStripWindow` で渡すので、リサイズや高さ変更で
  作り直す。許容幅が ±1 セルあるため、ワーカーは keyframe-only 復号のまま近傍のキーフレームを
  採る (**任意フレームの同期精密復号はしない**)。
- 波形は「1 画面の範囲 = 尺」にすることで同じ写像に載る。粗い全尺ピーク列 (D27) がそのまま効く。
- 入力は「ポインタが指した時刻へ移る」。帯が動かないので `MoveSeekStrip` は出さず、離した /
  クリックした時点で `CommitSeekStrip` だけを出す。周辺表示のセル吸着 (D16) は全体表示では
  使わない (1 セルが長い時間を表すので、吸着するとポインタの位置と一致しない)。
- **長さ不明・0 の動画では全体表示を成立させない。** 既存の `video_position_controls_available`
  でストリップ自体が開かないので、赤線が「動いていない位置」を指すことはない。
...
### 4.6 在メモリの復号済みセル上限 (2026-08-29、レビュー R-20)

`SharedState.cells` は決着したセルを足すだけで、worker が生きている間は 1 つも落として
いなかった。320x320 RGBA = 400 KiB/セルで、最小間隔 0.1 秒の長尺を行き来すると数 GiB に
届く。UI へ渡す snapshot も毎回この map 全体の clone なので、80-100ms ごとの複製コストも
累積セル数に比例していた。

`MAX_RETAINED_CELL_BYTES` (128 MiB、約 320 セル) を上限にし、要求ごとに窓が決まった
時点で `plan_strip_cell_eviction` が超過分を落とす。

- **窓 (可視 + 先読み) は落とさない。** 窓自体が上限を超える場合は超えたまま残す。
  今見ている画をその場で消さないための優先で、上限より正しさを上に置く。
- 落とすのは**窓から遠い順に、上限を下回るまで**。窓の外を一律に捨てると、少し戻る
  たびに直前まで見ていた近傍を読み直す往復になる (連結カラー化で同型の
  「予算超過 -> 破棄 -> 再計算」ループを踏んでいる)。
- 落として失うのは復号済み RGBA だけで、§4.5 の永続 WebP から数 ms で戻る。
  再復号ではないので、上限を低めに置いても体感の代償は小さい。
- 落としたセルは `settled_ids` から消えるので、次の要求が自然に拾い直す。
...
**永続化しない** (D7 の方針を維持)。動画を閉じたら捨てる。**→ D32 (§5.5) で覆した。**
...
### 5.5 粗い列の永続キャッシュ (D32、2026-09-02、利用者要望)

**D7 / D27 の「永続化しない」を覆す。** 粗い全尺ピーク列を SQLite へ chunk 単位で保存し、
次に同じ動画を開いたときは復号せずに読む。

**覆す理由**: D7 の根拠 (§14 の 4 番) は「波形の 60 秒窓は 55〜215ms なのでキャッシュは要らない」で、
これは**窓復号についての測定**。D18 で表示範囲の上限が全尺級 (最大 180 分) になり、D27 で粗い全尺列を
足した後は、同じ列の構築に 3 時間素材で **12.5 秒**かかる (§5.4 の実測)。D7 が測っていた対象とは
別物なので、前提ごと変わっている。

**作るのは「波形ストリップを見ている間」だけ** (利用者判断 2026-09-02)。これは新しい規則ではなく、
**既にそうなっている**: `wave_background_paused_for_mode` が波形モード以外で背景段を止め、
ストリップを閉じる経路も `set_background_paused(true)` を通る
([native_video.rs:7655](../src/app/native_video.rs) / 同 7926)。**したがって「計算できた chunk を
保存する」だけで、見ていない動画を勝手に走査しない**という性質が自動的に満たされる。閉じたら止まり、
そこまでの chunk は残る (chunk 単位なので部分でも有効)。**この不変条件を保存側で作り直さない。**

**表示範囲の閾値は動かさない。** 列を作り始めるのは `COARSE_BUILD_MIN_SPAN_SECS = 600` 秒以上のまま。
既定の 180 秒では列を作らないので、既定のまま使っている限りキャッシュは溜まらない。下げれば
「開いただけで溜まる」になるが、波形モードで動画を開くたびに全尺の音声走査が走り、HDD 上の大きな
ファイルでは利用者が頼んでいない I/O になる。しかも 180 秒 / 1920px は 0.094 秒 bin を要求するので
粗い列 (0.1 秒) では足りず、**180 秒の表示自体はどのみち窓復号**になる。閾値を下げるかは、この保存が
入った後に別途決める。

**置き場所**: `video_tile_thumbs.db` へ 1 テーブル足す。別ファイルにしない理由は、サイズ表示・全削除・
フォルダ単位削除・DB 破損時のファイル削除フォールバックが
[cache_maintenance.rs](../src/cache_maintenance.rs) に既にあり、同じファイルなら**そのまま効く**から。
追加は `clear_for` / `clear_all` / `clear_for_folder` の DELETE 文 1 つずつ。

**行の形** (主キーは `(path, bin_secs_millis, chunk_index)`):

| 列 | 内容 |
| --- | --- |
| `path` | `crate::path_key::normalize_keep_drive` の結果。**`adjustment_db::normalize_path` を使わない** — 実装は今たまたま同一だが、`clear_for_folder` の前方一致が見るのは前者なので、綴りの持ち主を 1 つにする |
| `chunk_index` | `COARSE_BINS_PER_CHUNK` 単位の添字 |
| `bins` | `QuantizedWaveformBin` を並べた BLOB。1 bin 7 バイト、通常 600 bin = 4,200 バイト。**末尾 chunk は短い**ので、読み出し時に長さがその添字の期待スロット数と一致することを検証する |
| `video_mtime` / `video_size` | `WaveFileIdentity` と同じ 2 値。片方だけにしない |
| `bin_secs_millis` / `bin_count` | 列の scale。`CoarseScale::for_duration` は尺から決まるので、尺の報告が変われば bin 数が変わる。**不一致の行は使わない** |
| `format_version` | 量子化フォーマットの版。開発中に 5 → 7 バイトへ変わった実績があるので、タイル側の `render_version` と同じものを持つ |

**失敗した chunk は保存しない。** 一過性の復号失敗を焼き付けないため。`failed` bitset はメモリのまま。

**容量**: 7 バイト/bin × 10 bin/秒 = **70 バイト/秒**。1 時間 250 KB / 3 時間 740 KB /
...
窓解析はメモリのまま**である旨へ書き換え、覆した理由を残す。

**計測**: `wave_coarse_cache` (読めた chunk 数 / 全 chunk 数 / 読み込み時間) を perf へ出す。
