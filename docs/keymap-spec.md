...
| <kbd>Alt</kbd>+<kbd>N</kbd> | `FsNavigatorToggle`。ナビゲータの固定表示を切り替える。単ページ / 見開きの一部だけが見えているとき、または 360 度パノラマ表示中に、縮小画像と現在の表示範囲を示す。左ドラッグは範囲指定ズーム（パノラマでは選択の高さを視野角にして中心を移動）、右ドラッグはパン、ダブルクリックはその位置を中央へ移動、ホイールはナビゲータ自体のサイズ変更。ヘッダーの四隅ボタンで配置を選ぶ。固定表示の状態・配置・サイズは保存する。平面で全体が見えているときは自動で隠すが、パノラマでは常に一部だけが見えるため隠さない。連結読み・動画・比較・編集 / 分析モードでは表示しない。ルーペとは用途が異なるため同時表示できる。既定 <kbd>Alt</kbd>+<kbd>N</kbd> は `FsImage` 文脈の既定 chord と衝突しない |
| <kbd>Alt</kbd> (押しっぱ) | `FsNavigatorHold`。固定表示が OFF でも、押している間だけナビゲータを表示する。ナビゲータ上でドラッグを始めた後は、操作を途中で切らないよう <kbd>Alt</kbd> を離してもドラッグ終了まで表示を保つ。表示できる画像・モードと平面の全体表示中に自動で隠す条件は `FsNavigatorToggle` と同じ |
