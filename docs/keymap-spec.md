...
| 360 度パノラマ表示中 | 左ドラッグの yaw/pitch、ホイールの FOV、上バーの 360 解除ボタン、上バーの投影方式ボタンとその一覧 | パノラマ表示中だけの連続操作 / 表示中限定のボタン。一覧の項目選択はマウス固定 (見開きボタンの一覧と同じ)。<kbd>V</kbd> の 360 度パノラマモード切替 (`FsPanorama`) と <kbd>Shift</kbd>+<kbd>V</kbd> の投影方式順送り (`FsPanoramaProjection`) は `KeyAction` であり、コマンド設定で変更できる |
