...
サムネイル一覧、通常の画像フルスクリーン、消しゴム / 隠蔽加工 / 切り取り / 補正レイヤーモードでは <kbd>?</kbd> で現在の文脈で使える
ショートカット一覧を表示する。keymap 化済み操作は現在の `keymap.ini` の実割り当てから
表示し、Enter / 矢印など当面固定扱いの操作は固定キーとして補助表示する。
キー未設定または明示無効化中の割り当て可能操作は別枠で表示する。
`?` 自体は初期版では固定キー扱い。動画フルスクリーンは egui 経路から初期対応し、
native 動画 overlay 上で直接開くヘルプとテキスト注釈のヘルプは後続。
...
| キー / 入力 | 動作 | 備考 |
|---|---|---|
| <kbd>?</kbd> | 現在の動画フルスクリーンコンテキストで使えるショートカット一覧を表示 | egui 経路の初期対応。native 動画 overlay 上で直接開くヘルプは後続 |
