...
| <kbd>Z</kbd> (ホールド/短押し) | ZipPla 風 全画面ズームモード (v2.0.0)。**押している間**は画像全体を表示してズーム範囲の枠を出し、カーソルで枠を移動・ホイールで枠サイズ (= 倍率) を変える。**離す**と枠の範囲を画面いっぱいに拡大し、以後マウス移動で表示範囲をパン (元画像範囲内のみ、余白なし)。ズーム中はホイールで倍率変更。**もう一度 <kbd>Z</kbd> で元の表示へ戻る**。短押しでもその場でズーム。前後ページへ移動してもズーム状態・倍率はセッション内で維持する。単ページ通常閲覧専用 (見開き / 連結 / パノラマ / 動画 / 分析モードでは無効)。現行ルーペ (<kbd>M</kbd> / <kbd>Shift</kbd>) とは別物 |
