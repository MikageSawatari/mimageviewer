...
| <kbd>Space</kbd> | 選択中アイテムをチェック ON/OFF。画像 / 動画 / ZIP・PDF 本体 / 変換前アーカイブ / ZIP 内画像 / PDF ページが対象 (**フォルダは v1.1.0 で整理対象外**) |
...
| マウスホイール | 前 / 次のファイル。縦読みモードでは縦スクロール |
...
| 入力 | 動作 |
|---|---|
| 方向パッド / 左スティック | グリッドでは選択移動。画像ではページ送り。縦読みモードでは上下が縦スクロール、左右がページ送り。動画では左右がシーク / タイルカーソル移動、上下が前後ファイル移動 |
...
| <kbd>Select</kbd> | 画像フルスクリーンの表示モード切り替え (単ページ / 見開き / 縦読み) |
...
| <kbd>↑</kbd> / <kbd>↓</kbd> | 前 / 次のファイル (= 一般慣例で左右と同義)。縦読みモードでは縦スクロール。スライドショー中もフォルダ内移動は再生を止めない |
| <kbd>PageUp</kbd> / <kbd>PageDown</kbd> | 縦読みモードでは画面単位で縦スクロール |
...
| <kbd>1</kbd> 〜 <kbd>6</kbd> | 表示モード切替 (<kbd>1</kbd>: 単ページ / <kbd>2</kbd>: 見開き 左開き / <kbd>3</kbd>: 見開き 左開き+表紙単独 / <kbd>4</kbd>: 見開き 右開き / <kbd>5</kbd>: 見開き 右開き+表紙単独 / <kbd>6</kbd>: 縦読み)。ホバーバーの表示モードボタンからも切替可 |
