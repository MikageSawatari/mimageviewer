...
サムネイル一覧では <kbd>?</kbd> で現在のグリッド文脈で使えるショートカット一覧を表示する。
keymap 化済み操作は現在の `keymap.ini` の実割り当てから表示し、Enter / 矢印など
当面固定扱いの操作は固定キーとして補助表示する。キー未設定または明示無効化中の
割り当て可能操作は別枠で表示する。`?` 自体は初期版では固定キー扱い。
