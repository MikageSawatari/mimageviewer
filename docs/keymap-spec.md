...
も標準キーなしのサムネイル一覧 Action として割り当てられる。設定名は旧設計との互換のため
`Global...` のまま残すが、競合判定とヘルプ表示では `Grid` 文脈として扱う。
...
| <kbd>PageUp</kbd> / <kbd>PageDown</kbd> | 縦/横連結モードでは画面単位で連結方向へスクロール。通常のページ単位表示では、環境設定の「ページジャンプ量」ぶん前 / 次へジャンプする。Action: `FsFixedJumpPrevNoRtl` / `FsFixedJumpNextNoRtl`。こちらは PageUp/PageDown へ割り当てる用途のため RTL でも前 / 次の意味を反転しない |
