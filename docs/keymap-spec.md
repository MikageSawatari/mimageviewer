...
| <kbd>Ctrl</kbd>+<kbd>←</kbd> / <kbd>→</kbd> | 見開きの「1 ページずらし」(現在ページを軸に見開きを 1 ページぶんずらす。空白/欠落ページでの綴じずれの応急補正。1 回押すごとに必ず 1 ページ動く。フォルダ/本を開き直すとリセットされ永続化しない)。Single モードでは前 / 次ファイル。RTL では左右の意味を反転 |
