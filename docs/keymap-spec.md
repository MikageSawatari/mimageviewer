...
| 360 度パノラマ表示中 | 左ドラッグの yaw/pitch、ホイールの FOV、上バーの 360 解除ボタン、上バーの投影方式ボタン | パノラマ表示中だけの連続操作 / 表示中限定のボタン。<kbd>V</kbd> の 360 度パノラマモード切替 (`FsPanorama`) と <kbd>Shift</kbd>+<kbd>V</kbd> の投影方式切替 (`FsPanoramaProjection`) は `KeyAction` であり、コマンド設定で変更できる |
