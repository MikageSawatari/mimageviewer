...
| <kbd>Alt</kbd>+<kbd>N</kbd> | `FsNavigatorToggle`。単ページ / 見開きの一部だけが見えているとき、または 360 度パノラマ表示中に、縮小画像と現在の表示範囲を示すナビゲータを開閉する。左ドラッグは範囲指定ズーム（パノラマでは選択の高さを視野角にして中心を移動）、右ドラッグはパン、ダブルクリックはその位置を中央へ移動、ホイールはナビゲータ自体のサイズ変更。ヘッダーの四隅ボタンで配置を選ぶ。表示状態・配置・サイズは保存する。平面で全体が見えているときは自動で隠すが、パノラマでは常に一部だけが見えるため隠さない。連結読み・動画・比較・編集 / 分析モードでは表示しない。ルーペとは用途が異なるため同時表示できる。既定 <kbd>Alt</kbd>+<kbd>N</kbd> は `FsImage` 文脈の既定 chord と衝突しない |
