...
| <kbd>T</kbd> | 左側のフォルダツリーペインの表示 / 非表示を切り替える。表示時は現在フォルダへツリーカーソルを移す |
...
| <kbd>Y</kbd> | グリッドではフォルダツリーペインを表示し、現在フォルダへツリーカーソルを移す。動画では S キー相当。画像では Y+左右で Ctrl+左右相当の見開き 1 ページずらし |
