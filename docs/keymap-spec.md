...
| キー | 動作 |
|---|---|
| <kbd>Backspace</kbd> | 親フォルダへ。検索 (Ctrl+S / Ctrl+G) 中は検索仮想階層を 1 段ドリルアップ、最上位 (集約ビュー / 結果一覧) では no-op (検索を閉じるには <kbd>Esc</kbd> / 検索バーの <kbd>×</kbd> / <kbd>Ctrl</kbd>+<kbd>G</kbd>・<kbd>Ctrl</kbd>+<kbd>S</kbd> 再押下)。Ctrl+F フィルタ中は、フィルタを実行したフォルダだけ親移動を no-op にする。検索結果から子フォルダへ入った後は通常どおり親へ戻れる |
| <kbd>Alt</kbd>+<kbd>↑</kbd> | 親フォルダへ (<kbd>Backspace</kbd> と同じ。Explorer 慣習に合わせた代替ショートカット。Ctrl+F フィルタ元フォルダでは no-op) |
...
| <kbd>Space</kbd> | 選択中アイテムをチェック ON/OFF。フォルダ / 画像 / 動画 / ZIP・PDF 本体 / 変換前アーカイブ / ZIP 内画像 / PDF ページが対象 |
| <kbd>Ctrl</kbd>+<kbd>A</kbd> | 表示中のチェック可能なアイテムを全選択 |
| <kbd>Ctrl</kbd>+<kbd>C</kbd> / <kbd>X</kbd> | チェック済み、または選択中の実フォルダ / 実ファイルをコピー / カット |
| <kbd>Ctrl</kbd>+<kbd>V</kbd> | クリップボードのファイル / フォルダを現在の実フォルダへペースト。ZIP/PDF/検索結果など実フォルダ以外では無効 |
| <kbd>Delete</kbd> | チェック済み、または選択中の実フォルダ / 実ファイルを削除 (通常はゴミ箱) |
| マウス左ドラッグ | グリッドのセルを掴んでエクスプローラ等へファイル D&D 送出 (コピー)。複数チェック選択時はその実パス群をまとめてドラッグ。フォルダ / ZIP・PDF 本体 / 変換前アーカイブも対象。ZIP/PDF 内画像 (仮想フォルダ) は対象外 |
| エクスプローラ等からのドロップ | mIV ウィンドウへファイル / フォルダをドロップすると現在表示中のフォルダへコピー。ZIP/PDF など実フォルダ以外を表示中は拒否 |
| グリッド空白の右クリック | 現在の実フォルダに新しいフォルダを作成、またはクリップボードのファイル / フォルダをペースト |
...
| 入力 | 動作 |
|---|---|
| 方向パッド / 左スティック | グリッドでは選択移動。画像ではページ送り。動画では左右がシーク / タイルカーソル移動、上下が前後ファイル移動 |
...
| <kbd>Select</kbd> | 画像フルスクリーンの見開きモード切り替え |
...
| <kbd>1</kbd> 〜 <kbd>5</kbd> | 見開きモード切替 (<kbd>1</kbd>: 単ページ / <kbd>2</kbd>: 見開き 左開き / <kbd>3</kbd>: 見開き 左開き+表紙単独 / <kbd>4</kbd>: 見開き 右開き / <kbd>5</kbd>: 見開き 右開き+表紙単独)。ホバーバーの見開きボタンからも切替可 |
