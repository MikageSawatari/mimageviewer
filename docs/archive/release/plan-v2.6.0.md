...
- definition の表示名は走査 identity に含めず、並び単位の変更も保存済み snapshot の
  ...
  開く／閉じる等のダイアログ操作時に一度だけ正規化・保存・必要な無効化を行う。管理画面を
  開いたまま終了またはトレイへ退避する場合も、共通の設定保存境界で有効な draft を Settings へ
  確定する。この境界では終了間際の再走査 worker を開始せず、トレイから復帰した場合だけ必要な
  snapshot 無効化 / prepare を1回適用する。
...
- 10 万件以上は既存サブ展開と同じ続行確認を使う。件数で黙って打ち切らない。
  走査候補が 10 万件以上でも、保存条件に ★ / タグ / 編集状態が含まれる場合は、
  background worker でその条件まで適用した最終一致件数を先に数え、続行確認の表示と
  閾値判定には候補件数ではなく最終一致件数を使う。件数確認用の候補 snapshot は本準備の
  snapshot として切り詰めず、同じ metadata revision なら評価結果を本準備へ引き渡す。
  revision が変わった場合は候補全体から数え直す。これにより、後から★等が変わった際に
  一度除外した候補を復帰できなくなることと、大量 DB 問い合わせの不要な反復を防ぐ。
...
- 名前条件を持つ有効ルールがその物理フォルダに無い場合、候補ごとの小文字文字列は生成しない。
...
- Ctrl+S / G / T の結果一覧からスマートフォルダへ直行するときは、検索元を一度再構築せず
  `ViewReturnContext`（検索開始前のpath + サブ展開snapshot + レーティング一覧の星数）を新しいビューへ移譲する。
...
- ソート変更は完成済み generation の正規化key配列・採用entry index・sparse metadataと共有
  ...
  移譲してUIスレッド外で破棄する。cancelしたscan / prepareのreceiverに到着済みの結果と、
  大件数確認待ちsnapshotも所有者ごと同じworkerへ移す。sort-onlyではDB照会用の全件key配列を
  生成しない。
