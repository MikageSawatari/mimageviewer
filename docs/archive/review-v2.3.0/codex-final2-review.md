...
`src/version_highlights.rs:248` の must_read 文言と `docs/archive/review-v2.3.0/release-note-drafts.md:8` の注意書きも、表示専用・ユーザー向け表現として問題ありません。確認として `cargo test --lib version_highlights::tests::embedded_table_contains_v2_3_0_entry -- --exact` は通過、対象コードの `git diff --check` も問題なしです。

補足: `docs/archive/review-v2.3.0/release-note-drafts.md` は現在 `??` の未追跡ファイルなので、コミット対象にする場合は `git add` が必要です。