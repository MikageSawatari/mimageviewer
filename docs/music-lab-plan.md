...
2. `music_lab`
  ...
  - timeline / spectrum は全尺 decode + timeline analysis 完了後に表示する。timeline 行テクスチャの raster は worker で行い、UI thread は完成画像を少量ずつ texture upload する。解析自体の行単位 incremental 化は後続
