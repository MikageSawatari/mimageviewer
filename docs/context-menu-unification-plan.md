...
**「保存先」の列を足す。** 実ファイルのタグは XMP (`dc:subject`)、★は `xmp:Rating` として
ファイル自身に書かれ他のソフトからも見える。**ZIP 内のページには書き込む先が無く、
mIV の DB にしか残らない。** これは欠陥ではなくそうするしかない性質だが、
「ZIP を別の PC へ持っていったらページのタグが消えた」と受け取られるので、
UI のラベルとマニュアルの両方で見えている必要がある。
