...
6. **旧 XMP タグは利用者向けの明示操作を全廃する** (棚卸し §5.5 の (b))。右クリック 2 項目に加え、
   上部「タグ」メニューの同等項目も落とす。本計画時点では自動seedを維持したが、2026-09-12に
   v1.0救済終了の利用者了承を得て別chunkで撤去した。`xmp:Rating`書込みなど一般XMP機能は残す。
...
12. **実ファイル / 実フォルダの「切り取り」「コピー」を最上位の先頭へ常に出す**
    (2026-09-05、バックログ §1.179)。Grid と Fullscreen の両面で出し、チェック時は件数を
    併記する。仮想単一項目では隠し、`VirtualOnly` / `Mixed` のチェック選択では共通理由付き
    disabled にする。Windows 項目のサブメニュー / 併記設定による出し分けはしない。
    dispatch はメニューを開いた時点で解決した `shell_paths` と、`GridCutFiles` /
    `GridCopyFiles` から呼ぶ共通 Shell clipboard helper を使う。
...
## 4.1 機能 × 対象種別の表 (2026-08-30 合意 / 同日 調査完了)

> **調査結果は [item-kind-capability-matrix.md](item-kind-capability-matrix.md) が正本。**
> 以下はその調査を出すに至った経緯と、表を作る際の判断根拠として残す。
...
以下は2026-08-30時点で明示操作を削除した際の境界記録である。当時は削除の意味を二段階に
分ける必要があった。
...
本計画 §1 の合意は「旧 XMP タグ関連の項目は削除」であり、自動seed廃止までは含まなかった。
その後2026-09-12に利用者が自動救済終了を明示了承したため、通常／prepared／metadata refreshの
全入口、context owner、quiescence、専用worker／helperを一体で撤去した。`tags.db`既存行、
`tag_item_state`、Tantivy一回移行、一般XMP metadata／ratingは維持する。
...
利用者向け設定ではさらに、通常メニューと右クリックメニューを別ページへ分けた。右クリック側は
固定の確認場面を同じproduction builderへ通し、内部の`Inherit`をその場面で実際に描かれる
「表示（既定）/非表示（既定）」へ投影する。利用者が選ぶ値は「表示/非表示」の2値で、明示値だけが
全場面共通で保存される。対象外、非表示、同階層の先頭を区別し、対象外・非表示ではその項目が
現れる別の確認場面もbuilder結果から案内する。場面選択は確認専用で、設定を保存・変更しない。
