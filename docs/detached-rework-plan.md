...
**再計測して撤去した (2026-09-04)**: `update_prefetch_window` の「現在ページがロード中なら
他の pending を全部キャンセルする」規則は、上限制御と同じ目的を二重に果たしていた。実機計測で
**害が確認できたので撤去した** — 取消済みでも枠は実終了まで空かないため、毎フレーム巻き込み
キャンセルすると予算全体が不要な仕事で埋まり、取消中が常時 5/6 枠、6 枠すべてが取消中になった
16 回では表示中ページ自身が最大 201ms 待たされていた。旧規則を固定していたテストは、新しい契約
(範囲内の先読みは止めない / 範囲外は従来どおり止める) の 2 本へ置き換えた。
