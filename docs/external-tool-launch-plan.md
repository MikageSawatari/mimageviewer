...
**未リリーステーブルの形は `schema_meta` の版で管理する (2026-09-02)。** 列の有無だけを
見ていると、`payload` の**値の綴り**を変えたときに検出できない。古い綴りが残った DB は
読み込みで `Incompatible` になり、**設定全体が保存されなくなる**
(`classify_settings_deserialization_error`)。`external_tools_unreleased_shape` の値を上げると
テーブルを作り直す。出荷済みの版にこのテーブルは無いので、作り直してよい。
