...
**結論: mIV は「仮想パスをそのまま渡す」方針を持たない。** 分解プレースホルダ
(`{container}` / `{entry}` / `{page}`) を用意してアプリ固有の書式を組み立てさせる案も
一度実装したが、**v1 では入れない** (理由は §4.4)。
...
pub struct ExternalTool {
    ...
    pub arguments: String,           // 引数テンプレート (既定 "{files}")
...
| 値 | UI の表示 | 動作 |
| --- | --- | --- |
| `TempEdited` (既定) | 一時ファイル (編集を反映) | ページに加えた編集を反映した PNG を一時ファイルへ。**常に再エンコード** |
| `TempOriginal` | 一時ファイル (編集前) | 編集前のデータをそのまま一時ファイルへ。**常に無劣化** |
...
**出力の形は編集の有無で変えない (利用者判断 2026-09-02)。** 一度は「無加工なら焼き込まず
元バイト列を出す」最適化を残したが、これも撤回した。理由は上と同じ形:

> 受け取る側から見ると、**同じ設定・同じツールなのに、そのページに編集が付いているか
> どうかで拡張子も EXIF の有無も変わる**。編集の有無はツール側から見えない。

したがって `TempEdited` は常に PNG、`TempOriginal` は常に元バイト列。値ごとに振る舞いが
1 つに決まり、「どちらが出るか」を利用者が事前に言える。無加工の JPEG を無劣化で渡したい
ときは `TempOriginal` を選ぶ。

- 編集を**適用するか**の判定にはこれまでどおり
  [`books::page_requires_full_composite()`](../src/books.rs:305) を使う (掛かっていなければ
  合成段を飛ばすだけで、出力が PNG であることは変わらない)。
- 実ファイルのコピーが増える分は、既存の再利用キャッシュ (`CacheKey` = source + policy +
  edit fingerprint、source の mtime / サイズで検証) が吸収する。2 回目以降は再コピーしない。

**`TempEdited` に表示専用効果は含まれない (2026-09-02 時点)。** カラー化 / Creative LUT /
ポストフィルタ / スマートシャープ / AI 拡大は反映されない。これは製本 (`compose_book_page`) と
同じ振る舞いで、意図的にそうなっている
(`full_composite_ignores_display_only_filter_and_sharpen`)。**だから値の名前を「表示どおり」に
しない** — 名前が嘘になる。焼き込み段階の統一は別作業とし、§7 に切り出した。
...
**`external_tools` は v3.5.0 で出荷済み (2026-09-03)。** 開発中に使っていた
`external_tools_unreleased_shape` によるテーブル作り直しは撤去済みで、今後は発火させない。
列追加は既存行を保持する `ALTER TABLE`、値の意味や綴りを変える場合は通常の移行として扱う。
...
**v1 で使える記法は `{files}` 1 つだけ** (2026-09-02 利用者判断)。

| 記法 | 展開されるもの |
| --- | --- |
| `{files}` | 実体化後の**実ファイルパス**。「1 件ずつ」なら 1 引数、「まとめて渡す」なら対象数ぶんの引数 |

**1 つに絞る理由は「書いたのに入らない」場合を作らないこと。** 引数は常に書いたとおりに
渡る。値が入らない記法があると、落とすか / 空で渡すか / 起動を止めるかを決めることになり、
どれも「その先で何が起きるか利用者が予測できない」。以下は一度実装したうえで撤去した。

- **対象の場所** (`{container}` / `{entry}` / `{page}` / `{time}` 系) — 値が入る条件が
  対象の種類ごとに違う (PDF ページ / 書庫内ページ / フォルダーの画像 / 再生中の動画 /
  見開き合成)。同じツールが、選んだものによって別の引数列で起動する。
- **ファイル名の部品** (`{dir}` / `{name}` / `{stem}` / `{ext}` / `{uri}`) —
  「まとめて渡す」で **どのファイルの値か決まらない**。実装は黙って先頭の対象を指しており、
  `--out {stem}.{ext} {files}` が 2 件目以降を 1 件目の名前で上書きさせ得た。
  `{ext}` は拡張子の無いファイルで空にもなる。

**綴りは `{files}` 1 つ。** 以前は「1 件ずつ」用の `{file}` と「まとめて渡す」用の
`{files}` に分かれていたが、複数選択を切り替えるたびに引数を書き直す必要があり、片方を
書き忘れると黙って 1 件しか渡らない。`{file}` は読み込み時点で `{files}` へ揃える
(開発中に書いたテンプレートが literal `{file}` を渡さないように) が、設定画面にも文書にも
出さない。

需要が確認できたら、**まず「値が無ければ起動しない」+ メニューでの事前グレーアウト**から
設計し直す。落として黙って動かす方針は採らない。

- **キーワードを 1 つも含まない引数テンプレートには `{files}` を自動で追加する**
  (NeeView と同じ事故防止)。綴りが 1 つなので、複数選択の設定によって足すものが変わらない。
- **`Batch` なのにテンプレートが `{files}` を含まない場合は起動しない** (2026-08-31 決定)。
  **5 件選んだのに 1 件だけ開く**という黙った食い違いになるので、`{files}` を使うよう
  促すトーストを出して断る。§4.5 の混在選択と同じ判断 (部分実行より、理由を言って止める)。
- **知らない記法は文字どおり残す。** 利用者が書いたものを黙って変えない。
- **引数テンプレートは先にトークン列へ分割してから各トークン内で置換する。**
  直接起動は `Command` に `OsString` 引数列を渡し、Shell のフォールバックだけは同じ引数列を
  CreateProcess 互換の quoting で `lpParameters` へ連結する。この順序なら置換値に空白が
  含まれても壊れず、利用者が引用符を自分で書く必要も無い。分割規則は
  `CommandLineToArgvW` 互換とする。
- **落として黙って起動する仕組みは持たない** (2026-09-02 に一度実装して撤去)。
  値の無いトークンを落とすと、option だけが残らないよう直前の option も落とすことになり、
  その結果が意図した動きである保証が無い。v1 の値はすべて「渡すファイル」由来で必ず入る。
...
```
既定 (引数なし)         → {files} が 1 つ付く
読み取り専用で開かせる    --readonly {files}
まとめて渡す (Batch)     --input {files}
```
...
- まず `Command::new(program)` + `OsString` 引数列で起動する。EXE / BAT / CMD の既存動作は
  この経路に残し、**`cmd /c` は mIV 側では経由しない。**
- Windows の `ERROR_BAD_EXE_FORMAT` (193) だけは、何も起動していないことが確定しているため、
  `ShellExecuteExW` の `open` verb へフォールバックする。関連付けは OS に解決させ、
  自前の `ftype` 相当解決や起動経路の選択 UI は持たない。
- Shell へ渡す引数列は `Command::args` と同じ quoting の正本から組み立て、コマンドライン長の
  判定もその生成結果を使う。
- パスを `to_string_lossy()` せず `OsStr` のまま扱う (現行 [open_with.rs:143](../src/open_with.rs:143) の修正)。
- ツールごとの「コンソール窓を表示する」が OFF なら両経路とも隠し、ON なら表示する。
  既定は OFF。GUI を自分で開くツールの表示を抑止する設定ではない。
- `spawn()` と Shell の失敗は**必ず**通知する。文面はツール名 + OS エラーで、Shell 自身の
  エラー UI は出さない。
...
- **次の要求が来て、前の要求が置き換わったとき** (世代番号)
  - 「同じツールのときだけ」ではない。準備中は進捗ダイアログが入力を止めるので
    (`modal_dialog_block_reason`)、**準備中の要求は常に高々 1 つ**。ツールごとに
    世代を分けても表せる状態が増えないので、世代は 1 本にする
    (2026-09-02、Codex Sol 指摘 #9 で当初「同じツールだけ」と書いていた誤りを訂正)
...
- 対象がそもそも扱えない項目 (実体化できない書庫内ディレクトリなど) では積まない
  (右クリックが伸びるのを避ける)。**例外は `OriginalFile` のツール**で、こちらは `MF_GRAYED` +
  理由のツールチップで出す (§4.8)。
...
- `external_tools` テーブルは **v3.5.0 で出荷済み**。`show_console` は
  `ALTER TABLE ... ADD COLUMN ... DEFAULT 0` で追加し、登録済みツールを保持する。
  開発中の shape marker を理由にテーブルを作り直してはならない。

### 4.11 安全性

- 引数テンプレートを `cmd` / PowerShell 等のコマンド文字列として実行しない。先にトークンへ
  分割し、Shell フォールバックの `lpParameters` だけを CreateProcess 互換で quote する (§4.4)。
- プログラム / スクリプトのパスがネットワーク上 (`\`) の場合は確認してから起動する。存在確認はワーカーで。
- プログラム / スクリプトの登録は利用者の明示操作なので、スクリプトも EXE と同じ信頼境界で扱う。
...
| **P4 (一部実装済み 2026-09-02)** | **`SpreadPolicy` 3 値** (`Merged` の合成を含む) は実装済み。**`VideoPolicy::CurrentFrame` が残り**。場所のプレースホルダは §4.4 のとおり v1 では作らない | M |
...
- **`{files}` が複数選択の切り替えで正しく広がること** (1 件ずつ = 1 引数 / まとめて渡す = 件数ぶん)
...
### 決定済み (2026-09-02 追補、利用者判断)

13. **見開き合成に隙間を入れない。** `spread_page_gap_px` は画面 px の表示設定で、画像へ
    焼くと表示倍率で意味が変わる。<kbd>Ctrl+E</kbd> / <kbd>Ctrl+S</kbd> の見開きキャプチャも
    隙間なしで、外部ツールだけ変えると食い違う。現状維持とし、ここに記録する。
14. **出力の形は編集の有無で変えない** (§4.3)。`TempEdited` は常に再エンコード、
    `TempOriginal` は常に元バイト列。
15. **追加時に用途 (表示用 / 編集用) を聞かない。** あの 2 択は `for_editing` を立てるための
    もので、フラグを `PayloadPolicy` へ畳んだ時点で「渡すもの」の 1 項目になった。
    アプリを選ぶ前に用途を決めさせる理由が無い。表示用の既定で追加する。

### 決定済み (2026-09-04、利用者判断)

16. **スクリプトは `ShellExecuteExW` の `open` verb で起動する (A 案)。**
    まず従来どおり直接 spawn し、`ERROR_BAD_EXE_FORMAT` (193) のときだけ OS の関連付けへ
    フォールバックする。処理系を自前で解決する B 案は採らない。
17. **起動経路を利用者に選ばせない。** 登録した対象を直接起動できるかどうかで自動的に切り替える。
    選択ダイアログには「すべてのファイル」を足すが、対応拡張子の固定リストは持たない。
18. **「コンソール窓を表示する」をツールごとに持ち、既定は OFF。** 直接起動と関連付け
    フォールバックで同じ意味にし、GUI を自分で開くツールには影響しない。

### 未決

無し。追加の判断が要るものが出たらここへ追記する。
...
1. 元データ (decode したまま)
2. \+ 補正 / 回転 / 隠蔽 / 消しゴム / ローカル調整 / 注釈 / 切り取り ← ページに保存した編集
3. \+ AI (アップスケール / デノイズ)
4. \+ カラー化 / Creative LUT / スマートシャープ / ポストフィルタ ← 表示専用
