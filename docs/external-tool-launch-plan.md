...
`PayloadPolicy` — **2 値に絞る (利用者判断 2026-09-02)。**

| 値 | UI の表示 | 動作 |
| --- | --- | --- |
| `AsDisplayed` (既定) | 表示している内容 | 表示どおりに渡す。ただし**見た目が変わらないなら元ファイルをそのまま渡す** |
| `OriginalFile` | 元のファイル | ディスク上の実ファイルだけを渡す。圧縮ファイル内のページ / PDF ページでは起動しない |

**旧 4 値のうち `Original` / `Container` / `RealFileOnly` と、`for_editing` フラグは廃止する。**
判断の理由は §4.8 に書く。失うものだけ先に書くと:
...
- `Original` (加工を捨てた元バイト列) だけは純粋に失う。ただし**無加工のページは
  `AsDisplayed` でも元バイト列がそのまま出る**ので、実際に消えるのは
  「ZIP 内の**加工済み**ページの、加工前バイト列」だけ。全体に補正プリセットを掛けている
  利用者だけが当たる。必要になったら 3 値目として後から足せる (今は入れない)。
...
8. **`PayloadPolicy` を 2 値 (`AsDisplayed` / `OriginalFile`) にし、`for_editing` を廃止する**
   (§4.3 / §4.8)。「編集に使う」は**目的**の名前で、**契約** (元の実ファイルしか渡さない) が
   読み取れなかった。契約そのものを値の名前にする。
...
10. **`Original` を落とすことで失うのは「ZIP 内の加工済みページの、加工前バイト列」だけ。**
    無加工のページは `AsDisplayed` でも元バイト列がそのまま出る。必要になったら 3 値目として足す。
11. **マイグレーションは書かない。** 外部ツールは v3.4.0 で revert 済みで、`external_tools` テーブルは
