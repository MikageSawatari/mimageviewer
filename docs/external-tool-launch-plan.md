...
**P2b-2 実装済み (2026-09-01)。** ツールバーには既定 OFF の `ExternalTools`
セクションを追加し、お気に入りと同じ展開 / 折りたたみ / プルダウンの 3 表示形式から全登録
ツールを起動できる。「ファイル ▸ 外部ツール」は `show_in_context_menu` に関係なく全登録を
登録順に出す。`OsDefault + Batch` は実際には 1 件ずつ起動する旨を、該当する設定の直下へ表示する。
...
| **P2b-2 (実装済み 2026-09-01)** | ツールバーのセクション、メニューバー「ファイル ▸ 外部ツール」、`OsDefault + Batch` の設定 UI 説明 | S |
