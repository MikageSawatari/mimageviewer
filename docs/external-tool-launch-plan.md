...
**AI アップスケール結果は焼き込まない** (製本と同じ扱い)。表示中に AI 拡大が効いていても渡すのは等倍。
→ §6 で再確認する。
...
- **仮想ページを実体化して渡した場合**: temp を編集しても**元の ZIP / PDF には戻らない**。
  誤解を招くので、焼き込みが起きたツール (`AsDisplayed` で加工あり / `Merged` / 仮想ページ) では
...
- 現在の対象に対して無効なツール (`RealFileOnly` のツールに仮想ページ、など) は
  `MF_GRAYED` で出すか、そもそも積まない。**積まない方を既定にする** (右クリックが伸びるのを避ける)。
...
| **P3** | 一時実体化基盤 (ワーカー + キャンセル + 寿命管理 + 孤児回収) と `PayloadPolicy` | L |
...
1. **PDF ページの解像度**: 製本と同じ長辺 4096 を既定にするか、ツールごとに指定させるか。
2. **AI アップスケール結果を渡すか**: 本書は製本と同じく**焼き込まない**前提 (等倍を渡す)。
   「AI 拡大した見た目のまま渡したい」需要があるなら別途決める。
3. **ポータブル版の temp 置き場**: `%TEMP%` を使うか、`<exe_dir>\data\temp\` に閉じ込めるか
   (持ち出し PC に痕跡を残さない思想なら後者)。
4. **temp への書き戻しを元の ZIP へ反映するか**: 本書では実装しない前提。
