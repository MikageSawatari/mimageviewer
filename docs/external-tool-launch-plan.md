...
| `{dir}` | `{files}` の親フォルダー (まとめて渡すときは先頭の対象) |
...
| `{uri}` | `{files}` を URI エスケープしたもの |

**渡すファイルの綴りは `{files}` 1 つだけ** (2026-09-02 利用者判断)。以前は「1 件ずつ」用の
`{file}` と「まとめて渡す」用の `{files}` に分かれていたが、複数選択を切り替えるたびに引数
テンプレートを書き直す必要があり、片方を書き忘れると黙って 1 件しか渡らない。1 つの意味に
綴りを 2 つ持つ理由が無いので統合した。`{file}` は読み込み時点で `{files}` へ揃える
(開発中に書いたテンプレートが literal `{file}` をツールへ渡さないように) が、設定画面にも
文書にも出さない。

`{page}` は **items 上の位置ではなく本の中のページ位置**で数える (読書履歴と同じ)。
入れ子 ZIP のディレクトリ項目のようにページでない項目が 1 つでも一覧にあるとき、
および検索結果・レーティング一覧のように別の本が混ざる一覧では番号を付けない。
PDF だけはページ番号が対象自身に付いているので一覧に依らない。

- **キーワードを 1 つも含まない引数テンプレートには `{files}` を自動で追加する** (NeeView と同じ事故防止)。
  綴りが 1 つになったので、複数選択の設定によって足すものが変わることは無い。
- **`Batch` なのにテンプレートが `{files}` を含まない場合は起動しない** (2026-08-31 決定)。
  `{container}` などの別のキーワードを書いていると自動追加が働かないため、この状態は今も
  作れる。**5 件選んだのに 1 件だけ開く**という黙った食い違いになるので、`{files}` を
  使うよう促すトーストを出して断る。§4.5 の混在選択と同じ判断 (部分実行より、理由を言って止める)。
