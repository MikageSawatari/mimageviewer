...
mIV 自身への引き継ぎは、曖昧な仮想パスではなく専用の CLI 引数で行う (§4.4 / §6)。
...
([settings.rs:3904](../src/settings.rs:3904))。ネイティブメニューを出せた場合 mIV 独自メニューは表示されない
([context_menu.rs:1232](../src/ui_dialogs/context_menu.rs:1232))。つまり
**独自の「アプリケーションで開く…」は、実質フォールバック時にしか出ない**。
...
pub struct ExternalTool {
    ...
    pub virtual_page: VirtualPagePolicy,  // ZIP/PDF/変換アーカイブ内ページの扱い
    pub video: VideoPolicy,               // 動画の扱い
    pub selection: SelectionPolicy,       // 複数選択の扱い
    pub spread: SpreadPolicy,             // 見開き時の扱い
...
NeeView は 1 軸 4 択だが、mIV は動画があるので**仮想ページ軸**と**動画軸**に分ける。
§2.4 の結論により**仮想パスは選択肢に入れない**。

`VirtualPagePolicy` (ZipImage / PdfPage / 変換アーカイブ内ページが対象のとき)

| 値 | 動作 |
| --- | --- |
| `Extract` (既定) | 一時フォルダーへ実体化して、その実ファイルを渡す |
| `Container` | 書庫 / PDF **本体**のパスを渡す (ページ位置は失われる) |
| `Skip` | このツールは仮想ページを扱わない。メニューで無効表示にする |

`VideoPolicy` (Video が対象のとき)
...
実ファイル (Image / Video / ZipFile / PdfFile / Convertible / Folder) は常にそのまま渡す。
...
| 対象 | 出力 | 決めること |
| --- | --- | --- |
| ZIP エントリ | **元バイト列をそのまま**書き出す (製本 [books.rs:916](../src/books.rs:916) と同じ) | 無し。無劣化 |
| PDF ページ | PNG へレンダリング | **解像度**。既定は製本と同じ長辺 4096、ツール設定で変更可 |
| 動画フレーム | PNG (`capture_frame()` → encode) | 既定 PNG。品質はキャプチャ設定を流用 |
| 見開き合成 | `RenderedSpread` を PNG 化 (エクスポート [export_dialog.rs:129](../src/export_dialog.rs:129) を再利用) | `SpreadPolicy::Merged` のときのみ |

補正・回転・AI アップスケールを適用したものを渡すかは §6 の未決事項。既定は元データを無加工で渡す。
...
- **空に展開されたトークンは引数列から取り除く。** `{page}` を持たない対象に `-page {page}` を書いても
  ...
  そのトークンごと落とす。
- `{miv}` を含むツールを起動するときは、NeeView と同じく先に設定を保存してから spawn する。
...
別の mIV ウィンドウへ引き継ぐ:            {miv} --open-entry "{container}" "{entry}"
...
`SpreadPolicy` (連結読み・見開き表示中のみ)

| 値 | 動作 |
| --- | --- |
| `MainPageOnly` (既定) | 主ページ 1 件 |
| `BothPages` | 左右 2 件を読み順で渡す (`SelectionPolicy` に従って Each / Batch) |
| `Merged` | 合成した 1 枚を渡す (`RenderedSpread` を実体化) |
...
- 置き場所: `%TEMP%\mimageviewer\ext-<pid>\` (ポータブル版での置き場は §6 の未決事項)
...
### 4.8 外部編集からの戻り (round-trip)

- **実ファイルを渡した場合**: ツール定義に「編集用」フラグを持たせ、立っているときだけ対象パスの
  mtime / サイズを監視し、変化したらサムネイル・テクスチャ・カタログを無効化して読み直す。
  Eagle の既知の弱点をここで潰す。
- **仮想ページを実体化して渡した場合**: temp を編集しても**元の ZIP / PDF には戻らない**。
  誤解を招くので、`VirtualPagePolicy::Extract` かつ編集用フラグのツールでは
  「この内容は元のファイルに書き戻されません」と明示する。書き戻し自体は実装しない (§6)。
...
| 場所 | 内容 |
| --- | --- |
| グリッド右クリック | 「外部ツール ▸」サブメニュー。ネイティブメニュー既定 ON との整合が要検討 (§6) |
| フルスクリーン右クリック | 同上 |
| メニューバー | 「ファイル」配下に「外部ツール ▸」 |
| ツールバー | 任意ツールをボタンとして置ける (お気に入り / タグと同じ動的項目パターン) |
| キーボード | §4.10 |

仮想ページはそもそもネイティブメニューの対象外なので、ZIP / PDF ページ上では mIV 独自メニューが出る。
問題は実ファイル上でネイティブメニューが出る場合で、そこへ mIV の項目を差し込めるかは要調査。
差し込めないなら、メニューバー / ツールバー / キーを主導線にする。
...
| **P3** | 一時実体化基盤 (ワーカー + キャンセル + 寿命管理 + 孤児回収) と `VirtualPagePolicy` | L |
| **P4** | `VideoPolicy::CurrentFrame`、`SpreadPolicy`、`{container}`/`{entry}`/`{page}`/`{time}` | M |
| **P5** | round-trip (編集用フラグ + mtime 監視 + 再読み込み) | M |
...
1. **見開き時の既定**: 主ページのみ / 左右 2 件 / 合成 1 枚 のどれを既定にするか
   (案: `MainPageOnly`。ツールごとに選べる形で出す)
2. **PDF ページの解像度**: 製本と同じ長辺 4096 を既定にするか、ツールごとに指定させるか
3. **補正・回転・AI 結果を反映して渡すか**: 既定は無加工。「見えている通りに渡したい」需要があるなら
   `{file}` とは別に `{rendered}` を用意する案
4. **ポータブル版の temp 置き場**: `%TEMP%` を使うか、`<exe_dir>\data\temp\` に閉じ込めるか
   (持ち出し PC に痕跡を残さない思想なら後者)
5. **ネイティブコンテキストメニュー既定 ON との整合**: 実ファイル上の右クリックに mIV の項目を
   差し込めるか。無理なら右クリックは諦めてメニューバー / ツールバー / キーを主導線にするか
6. **`{miv}` 自己引き継ぎをやるか**: やるなら `resolve_openable_path_detailed` の修正
   (または `--open-entry` の新設) が要る (§2.4 の補足)
7. **temp への書き戻しを元の ZIP へ反映するか**: 本書では実装しない前提。
   やるなら ZIP の再構築が要り、破損リスクと引き換えになる
