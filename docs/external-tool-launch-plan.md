...
プレースホルダは**2 群に分かれる**。設定画面でも 2 段に分けて示す。

- **mIV が渡すファイル** — `{files}` / `{dir}` / `{name}` / `{stem}` / `{ext}` / `{uri}`。
  一時ファイルを作って渡す。`PayloadPolicy` に従い、編集を反映したものにできる。
- **対象の場所** — `{container}` / `{entry}` / `{page}` / `{time}` 系。
  ツールに**元のファイルを直接開かせる**ための値。**編集・補正は反映されない。**

場所の値だけを使うと実体化しない (後述)。**両方を使う引数だけは、ツールがどちらを開くかで
編集が反映されたりされなかったりする**ので、設定画面に注意を出す
(`arguments_mix_the_file_and_the_location`、2026-09-02 利用者指摘)。禁止はしない —
「編集した画像を渡しつつ、元の場所もログに残す」のような使い方まで塞ぐ理由が無い。

場所の値が入る条件は対象ごとに違う。

| 値 | 入る条件 |
| --- | --- |
| `{container}` | 対象がある限り常に。実ファイルならそれ自身。合成した見開きだけは対象が実ファイルでないので空 |
| `{entry}` | 書庫内ページのときだけ |
| `{page}` | PDF ページは常に。書庫内ページ / フォルダーの画像は、その本を開いていて一覧が丸ごとページのときだけ |
| `{time}` 系 | フルスクリーンで再生中の動画が、その対象自身のときだけ |
...
- **引数が渡すファイルを 1 つも使わないなら、実体化しない** (2026-09-02 利用者指摘)。
  `-page {page} "{container}"` のように**場所だけ**を伝えて他のビューアへ引き継ぐ使い方
  (SumatraPDF / Acrobat) では、一時ファイルは 1 度も使われない。それでも作っていたので、
  PDF ページを 1 枚レンダリングしては捨てていた。判定は `{files}` / `{dir}` / `{name}` /
  `{stem}` / `{ext}` / `{uri}` の有無 (`arguments_pass_the_materialized_file`)。
  - この経路では **`PayloadPolicy` は意味を持たない**。設定画面では「渡すもの」を
    無効表示にし、理由を出す。
  - **`OriginalFile` の仮想ページ拒否も掛けない。** 元のファイルを渡さないのだから
    §4.8 の約束 (編集用ツールに元ファイルでないものを渡さない) と関係が無い。
    これで「PDF のページを SumatraPDF の該当ページで開く」が「元のファイル」設定でも通る。
  - **`SpreadPolicy::Merged` だけは例外**で、合成した画素を渡すことが目的なので実体化する。
  - 関連付け / OS 既定は引数を持たずファイルそのものを開くので、常に実体化する。
