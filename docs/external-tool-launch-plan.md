...
**v1 は「渡すファイル」から採る値だけにする** (2026-09-02 利用者判断)。
...
**渡すファイルの綴りは `{files}` 1 つだけ。** 以前は「1 件ずつ」用の `{file}` と
「まとめて渡す」用の `{files}` に分かれていたが、複数選択を切り替えるたびに引数
テンプレートを書き直す必要があり、片方を書き忘れると黙って 1 件しか渡らない。
`{file}` は読み込み時点で `{files}` へ揃える (開発中に書いたテンプレートが literal
`{file}` をツールへ渡さないように) が、設定画面にも文書にも出さない。

**対象の場所を渡す `{container}` / `{entry}` / `{page}` / `{time}` は v1 では作らない**
(2026-09-02 利用者判断)。一度実装したうえで取り下げた。理由:

- 値が入る条件が対象の種類ごとに違う (PDF ページ / 書庫内ページ / フォルダーの画像 /
  再生中の動画 / 見開き合成)。**値の無い対象で何が起きるかを利用者が予測できない。**
- 値が無いときにトークンを落とすと、option だけが残らないよう直前の option も落とす
  ことになる。落ちた結果が「意図した動き」である保証は無く、想定外の動作を招く。
  かといって落とさなければゴミ引数が渡る。**どちらも安全でない。**
- 「値が無ければ起動しない」なら安全だが、同じツールが対象によって使えたり使えなかったり
  する。それを設定画面で先に見せるには、対象ごとの値をメニュー構築時に解決する必要があり、
  **需要が分かっていない機能に対して仕組みが重すぎる。**
- 場所を渡す使い方 (別のビューアの該当ページを開く) では、そもそも**編集・補正が
  反映されない**。「渡すもの」の設定と噛み合わず、説明も難しい。

需要が確認できたら、**まず「値が無ければ起動しない」+ メニューでの事前グレーアウト**
から設計し直す。落として黙って動かす方針は採らない。
...
- **値が空になったら起動しない** (拡張子の無いファイルに `{ext}` を書いた等)。
  空の引数を黙って渡すより、理由を出して止める。
...
別名で保存させる         --out "{stem}-edited.png" {files}
