...
> 簡易キーボード図は日本語キーボード風にし、F13〜F24 を F1〜F12 の上段に出し、
> Insert / `^` / `@` / 右下の `\` など現時点で割り当て対象外のキーは disabled 表示にする。
> HOME / BOTTOM 段の先頭は、実キーを描かない代わりに `KeyIndent("Tab")` を置き、描画時に
> 実キーと同じ label-width 関数から幅を導く。`add_space` 自体は `item_spacing` を足さず、
> 後続キー配置が spacing を足すため、Tab の実幅 44px をそのまま代替する。
> Ctrl / Shift / Alt 切替、hover で実割り当て表示、click で選択中コマンドの空きキー欄へ chord 入力する。
> コマンド一覧は短い表示名を使い、hover では詳細説明だけを表示する。英字の stable name は
> ユーザー向け UI には出さず、設定ファイルや開発用ドキュメント上の識別子に留める。
> 一覧の編集ボタンや競合一覧はタブを切り替えず、キー / リング・パッド / マウス進む・戻る /
> マウスジェスチャのタブを持つ割り当て編集ダイアログを重ねて開く。
> ルーペ表示のような ModifierHold 操作は「押して入力」では分かりにくいため、編集ダイアログ内に
> Ctrl / Shift / Alt / 右Ctrl / 右Shift / 右Alt / 割り当て解除の専用ボタンを出し、
> 通常キーの入力欄とは分ける。
> キーボード図のキーをクリックした場合も、割り当て済みコマンドの編集、またはそのキーへ割り当てる
> コマンド選択を同じ編集ダイアログで行う。複数キーは `/` 連結ではなく 1 行ずつ表示する。
>
> **場所系 / ページジャンプ KeyAction 追加スライス実装メモ (2026-06-25, Codex)**:
> リングショートカット側にあったお気に入り前後移動・お気に入り 1〜20・`C:\`〜`Z:\`・
> 場所▼固定項目を Grid 文脈の KeyAction として追加し、既定未割り当てのまま操作カスタマイズから
> キーボードにも割り当てられるようにする。グリッド / 画像フルスクリーン / egui 動画 / native 動画の
> 各入力経路で同じ場所移動を実行し、未割り当て時は `?` ヘルプの未設定欄を圧迫しないよう隠す。
> 画像フルスクリーンには `FsPagePrev/Next` (既定未割り当て) と、`PageUp/PageDown` 既定の
> `FsFixedJumpPrevNoRtl/NextNoRtl` を追加する。従来の `Shift+←/→` は RTL で反転するジャンプ、
> `PageUp/PageDown` は連結モード以外で RTL 反転しないジャンプとして扱う。
> マウス戻る / 進むボタンは右ドラッグから独立した「マウス進む/戻る」設定タブへ分ける。
> コマンド一覧はキー操作と割り当て済みのリング / マウス / パッド操作を同じ表に混ぜ、未割り当ての
> リング候補は各設定タブまたは編集ダイアログで追加する。
> ゲームパッドタブには X+方向リングの割り当てと固定ボタンの役割を確認できる簡易パッド図を出し、
> X+方向セルのクリックで該当方向のリングスロット編集を開く。マウスジェスチャの方向列入力は
> 編集中の無効な途中文字列も一時 state に保持し、追加直後からその場で編集できるようにする。
> active scope 競合警告は実 dispatch に合わせ、通常フルスクリーン共通操作と編集モード専用操作を
> 同時 active とみなさない。
> 実機確認反映として、コマンド一覧の割り当て表示は空の `リング:` / `マウス:` / `パッド:` 行を
> 出さず、全入力未割り当てなら `なし` と表示する。表示順は文脈ごとにまとめる。
> キーボード図からキーをクリックしたときは前回選択コマンドを残さず、そのキーへ割り当てる
> コマンド選択ダイアログを開く。マウスジェスチャ追加は NeeView 風に実際の右ドラッグを
> 記録する小ダイアログにし、重複 / 入力エラーは theme の error 色で表示する。
> `GridColumnCount1..10` は `RingActionId::GridColumnCount1..10` と対応付け、コマンド一覧の
> 同じ行からキー / リング / マウス進む・戻る / X+方向 / マウスジェスチャを編集できる初期例にした。
> 固定ゲームパッドボタン (A/B/X/Y、Select/Start、LB/RB、LT/RT) の単体カスタマイズは
> X+方向リングとの起点衝突や LT/RT の連続入力リスクを避けるため対象外とし、
> ゲームパッド図では既定動作だけを参照表示する。X+方向リングは引き続き
> 統合割り当てエディタから編集できる。
...
> 「設定 → 環境設定…」は設定入口を失わないよう非表示指定を無視し、UI 上でも非表示にできない。
>
