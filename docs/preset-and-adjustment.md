...
再提示して画面にも即時反映し、hidden presenter / 音声のみでは表示復帰時の初回 present へ
反映を遅延する。ページ個別記憶は持たない。
...
ページ単位の見開きでは、読み順先頭を現在ページとして相方が前方 1 枚目に入る。新規設定の
既定は後方 2 / 前方 3 で、表紙なしの通常見開きなら直前・直後の表示単位を両ページとも対象にできる。
v3.7.0 への更新では旧既定の前方 2 だけを一度 3 へ移行し、保存済みの後方値は変更しない。
...
**縮小表示と旧設定の互換**: 手動回避策だった `Downscale2x` / `Downscale4x` は撤去済みで、
保存済み設定に含まれる場合は `PostFilter::None` として読み込む。通常静止画の縮小は最終合成後の
level 0 を表示サイズへ Lanczos3 で直接リサンプルする。フィルタタブの全表示共通設定
「縮小時のなめらかさ」は 0〜100%・10% 刻み（既定 0%）で、Lanczos3 の支持幅を blur
1.00〜1.30 の範囲だけで調整する。値を変えると viewer context の Lanczos 出力 cache は
無効化するが、補正・AI・サムネイル cache や source texture の再 upload は行わない。
v2.10.0 の `image_mipmap_moire_reduction_enabled` / `image_mipmap_lod_bias` は読込時に無視する。
mip chain generator は wipe/diff 比較と 360 度パノラマのために残す。実装詳細は
...
補完ジョブの開始時に元画像を 1 回だけ標本化し、`colorize::mono_tone_axis` で RGB の平均、
主成分軸、軸からの p95 残差を求める。カラー化の近モノクロ判定も同じ要約を使うが、判定に使う
`erase_inpaint_mono_tolerance` は消しゴム専用設定 (1..=64、既定 12) とし、カラー化の
`mono_tolerance` とは連動させない。軸を求められない極小・低分散画像や、残差が許容値を超える
カラー画像では補完色を変更しない。
値は `settings.db` に保存するが、対象画像ごとに調整しやすいよう環境設定ではなく消しゴムツール
パネル上部の「色調の許容」で操作する。

MI-GAN の各段で確定するマスク画素は、推論結果の Rec.709 明度を保ったまま元画像の平均と
主成分軸が作る色の線上へ移す。これによりグレー原稿はグレー、一定の紙色を持つ原稿はその色調の
まま次段の文脈にも渡る。0..255 を超える成分はクランプする。周囲画素の平均で埋める diffusion
fallback はこの処理の対象外とする。

設定変更時は進行中の補完を破棄し、`erase_result_cache` と下流 cache、永続編集プレビューを
無効化する。remote composite / pre-final-AI fingerprint にも、消しゴムマスクがある場合だけ
許容値を含める。この生成規約変更前の焼き込みを一覧へ残さないため、編集プレビューキャッシュは
format v6 で一度再生成する。
...
許容値以下の画素を描画 / 消去モードに従って更新する。「塗る範囲」は全体 / 隣接のみ / 長方形 /
楕円 / 円の 5 択で、図形 3 モードは背景の外部接続を 8 近傍で判定し、region 面積に対する
個々の穴の比率が「隙間の許容」以下のものだけを埋める (0..=50%、既定 10%、0 は穴埋めなし)。
長方形は原寸のクリック位置で断面を 0.5 度刻みに測る。最短断面を基本としつつ、クリック位置が
角寄りでも辺に沿う候補を失わないよう角度幅の局所最小も残し、その幅を固定して両方向へ追跡する。
T 字や交差による幅程度までの短い広がりは通過し、幅より外側への接続が幅程度以上続く場合は
広い接続先の手前で止める。角度候補間では追跡できた長方形の面積が最大のものを選ぶ。
楕円 / 円はクリック位置を含み領域内に収まる最大面積の指定図形へ整形する。楕円モードでは同じ
クリック位置の円候補も原寸で比較し、円モードより小さい面積を返さない。重なり部分のクリックで
複数候補が成立する場合は面積が大きい方を選ぶ。**意図した挙動 (2026-08-21 利用者判断)** として、
楕円 / 円はクリック位置を含み region に収まる最大面積を優先するため、細い部分でつながった先の
広い region まで楕円が伸びる場合がある。狭めたいときは「隙間の許容」と「色差の許容値」を下げる
(両方 0 で意図どおりの範囲になることを確認済み)。図形が退化して成立しない場合はマスクを変えず、整形前の選択範囲を黄色で
約 1 秒表示して漏れ止めまたは色差許容値の見直しを案内する。
漏れ止めは全体以外で有効。「はみ出し」は全 5 モードで 0..=8px (既定 1.0px) を共有し、図形では
確定図形の半径を広げる。全体 / 隣接のみでは漏れ止めまで含めて確定した region から指定距離以内を
色差条件で再制限せず描画 / 消去対象へ加える。0px では距離計算を行わず従来 region と完全に一致する。
適用順は「領域決定 → はみ出し → ビットマップ書き込み」。隙間の許容は図形 3 モードだけで有効。
色サンプル元は消しゴムが `erase_base_cache`、
...
消しゴム / 隠蔽加工 / 補正レイヤーでは、筆系ツールを選びキャンバス上で `Shift+ホイール` を
回すと筆半径を変更する。1 ノッチは約 1.1 倍 / 1.1 分の 1 とし、小さい側でも最低 1px 動かして
各パネルスライダーの範囲へクランプする。補正レイヤーでは通常筆、境界筆、隙間補完が同じ半径を使う。
この入力は筆系ツールかつキャンバス上のときだけ消費し、筆以外やパネル上では従来のホイール経路へ
残す。Ctrl+ドラッグやその他のホイールで筆サイズ・線幅・縦横線の傾きは変えない。
消しゴム / 隠蔽加工の画像上の通常ホイールと `Ctrl+ホイール` は表示ズーム、パネル上の通常ホイールは
パネルスクロールに使う。線のサイズはパネルスライダー、作成後の形状調整は選択ツールのハンドルで行う。
...
縦・横の連結読みでもこの 3 タブを利用できる。補正対象はシークバーとページ番号表示が示す
現在ページを基準に決まり、見開きユニットでは `adjust_spread_target` で選択した左または右の
片側だけを対象にする。左パネルを開いている間は対象ページを枠線で示す。

U / N / T、Ctrl+数字、Ctrl+Alt+数字、Ctrl+Backspace の補正系ショートカットも連結読みで
利用でき、各操作は既存の対象解決規則をそのまま使う。消しゴム、補正レイヤー、隠蔽加工、
切り取り、テキスト注釈、エクスポートの加工機能はページ単位モード専用で、連結読み中は
「ページ単位モードでのみ使用できます」と表示して無効化する。
