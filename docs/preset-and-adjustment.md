...
楕円 / 円の 5 択で、図形 3 モードは 4 近傍の連結領域の穴を埋め、クリック位置を含み領域内に
収まる最大面積の指定図形へ整形する。重なり部分のクリックで複数候補が成立する場合は面積が
大きい方を選ぶ。図形が退化して成立しない場合はマスクを変えず、整形前の選択範囲を黄色で
約 1 秒表示して漏れ止めまたは色差許容値の見直しを案内する。
漏れ止めは全体以外、図形のはみ出し量は図形 3 モードだけで有効。色サンプル元は消しゴムが `erase_base_cache`、
