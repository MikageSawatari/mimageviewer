...
| 消しゴム/分析モードの入出 (`post_filter_bypassed` 切替) | 該当 idx のみクリア (color-only ⇔ post-filtered の切替) | 触らない (サムネは post-filter 非対象) | 残す | 残す |
...
- **ベクタ** (`Vec<LineObject>`): 縦線 / 横線 / 直線はオブジェクトとして保存される。
  中心軸の始点 `p0`・終点 `p1`・太さ `thickness` を持つ矩形として表現され、
  表示・inpaint 時にその場でラスタライズされる。mask_db の `vectors` 列 (JSON) に保存。
...
- ドラッグ: 平行移動 (Pan)
- Ctrl+ドラッグ: 縦成分=回転、横成分=太さ変更 (縦線ツールでのドラッグと同じ軸割当)
- 直線の両端: 端点ドラッグで始点/終点の位置変更
- `Del`: 選択オブジェクトを削除
- `Esc`: 1 回目で選択解除、2 回目で消しゴムモード終了
- 他ツールに切り替えると自動で選択解除

描画系ツール (筆・囲み・縦線・横線・直線) は hit test を行わない — 描画中に近接ベクタを
誤選択しないよう、選択はモーダルに分離した。

描画ツール自身の Ctrl+ドラッグ:

- 筆: ドラッグ起点からの移動量で半径を調整
- 直線: 線からの垂直距離で太さを調整
- 縦線/横線: 向きに沿う軸のドラッグで回転 (傾き)、直交軸のドラッグでパン

消しゴムモード中のすべての修飾キーは **Ctrl に統一**している。Shift は使わない。
