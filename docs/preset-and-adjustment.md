...
消しゴムモード中にベクタ本体をクリックすると選択状態になり:

- ドラッグ: 平行移動 (Pan)
- Shift+ドラッグ: 縦成分=回転、横成分=太さ変更 (縦線ツールでのドラッグと同じ軸割当)
...
- 消しゴムモード内の「ロード」ボタン: 現在のマスクに OR マージ + ベクタ追加 (編集継続用)
- **フルスクリーン表示中の F7/F8**: `apply_slot_in_viewing_mode` — スロットの内容を現ページに
  マージして DB へ保存し、MI-GAN で inpaint → `fs_cache` を差し替え、`invalidate_derived_fs_caches`
  で上位キャッシュを落とす。消しゴムモードに入らずワンキーで適用できる経路。
