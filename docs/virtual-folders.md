...
pool は最初の PDF 操作まで初期化しない。設定数は 3〜10 (既定 5) で、App 起動時に
static snapshot へ固定し、遅延初期化時には設定を読み直さない。起動失敗した worker 枠は
初回を含め最大 3 回だけ再試行し、3 worker 以上を確保できなければ起動済み child も終了して初期化失敗を確定する。
