...
段 1 が入るまでは、段 2 の UI を作っても**操作しても絵が変わらない**。
順序を入れ替えられない。
...
したがってメニューを**高さを変えられるシート**にする (peek / 半分 / 全画面)。
...
容器とジェスチャーは 1 つ。タブによって既定の高さが違うだけにする。
