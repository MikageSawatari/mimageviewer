...
### 4.2（延期分）動画のマウスボタンへ小・中・大シークを割り当てる

- **2026-09-10 更新。** 標準入力の1クリック・連打・長押し反復を利用者確認済み。長押しの解放後停止もログで確認した。修正・独立レビュー・自動回帰を完了し、v3.8.0への統合対象とする。AHK経由の互換は利用者了承により必須対象外。以下の未解決表記は各時点の記録であり、現在の詳細は次版開発台帳を参照。

- **2026-09-09 試用のための再公開依頼。** 利用者が実際にシークを割り当てて確認できるよう、
  次の確認用buildでは6候補と実行の抑止を解除する。下記の原因確定後に再導入する順序は、
  この明示依頼により試用に限って変更する。二重発動・holdの根因は未解決で、出荷検収済みではない。
- **2026-09-09 次版向けに開発再開。** 利用者依頼により入力生成元の調査から進める。
  [次版開発台帳](next-version-development.md)を進捗・検証の正本とし、以下の延期時の根拠は保持する。
- **2026-09-08 利用者了承により、v3.7.0から次版へ延期。** 通常ホイールの音量調整、
  キーボード等の小・中・大シーク秒数設定はv3.7.0に残す。
- 22:08確認用buildでも1クリックから実scan付きVK_BROWSERとscan0合成通知が届き、
  各1回のseek actionを実行した。hold中も両系列がrepeat=falseで繰り返され、左右キーと異なる。
  前回のWndProc既定処理抑止だけでは解消しなかった。詳細は[作業台帳](v3.7.0-priority-work.md)。
- 根因入口は未確定。presenter/HUDのWM_APPCOMMANDとbackdrop合成を区別できるよう、
  source kind・HWND・message・APPCOMMAND device/keystateを限定計測する。
  明示了承済みの隔離実機検証で、単発・連続クリック・hold・releaseを確認してから再導入する。
- v3.7.0では動画マウスボタンの6種seekを候補・実行から外し、保存済み割り当て値は保持する。
  deserialize/sanitize/save/cancelでNoneへ消去しない。利用者指定により、無効項目としても選択肢に残さない。
  試用時の保存値が残る場合、画面上は既存の「なし」表示へ投影し、新しい延期説明UIは追加しない。
  対応完了時に再利用できるよう、キー・ring/gesture等の独立した操作と秒数設定は維持する。
- 完了条件: 1物理クリックにつき1 action、素早い2クリックは2 action、hold/releaseの仕様を明確化し、
  main/F12・presenter/HUD・直接APPCOMMANDのみの機器で所有境界を検証する。
  フォルダ移動側の抑制や時間debounceで二重入力を見えなくする修正は行わない。

### 1.203 画像フルスクリーンの余白背景色を変更できるようにする — >>369 (2026-09-08)

- **2026-09-10 更新。** v3.8.0へ統合予定。画像・映像の余白と静止画サムネイル列の空欄が対象。動画サムネイル・波形ストリップは利用者指定により従来の暗色へ統一する。最新strip描画修正の最終実機確認は未実施。

- **2026-09-09 次版向けに開発再開。** 利用者依頼を受領。
  [次版開発台帳](next-version-development.md)の画像内外の色分離と複数窓の不変条件に従う。
  実装・独立検収・最終全体テストは完了。実機での表示確認とコミットが残る。
- **次版へ延期（2026-09-08 利用者判断）。v3.7.0には含めない。** 黒／灰色／白＋カスタムの案と規模調査を保持し、v3.7.0の後に着手する。優先度P3。
- **現コードに基づく概算: 中、計1.75〜2.75人日（目安2〜3人日）。** 実装1〜1.5人日、
  自動検証0.5〜0.75人日、実機確認0.25〜0.5人日。workerやDB schemaの変更は想定しない。
  早期v3.7.0出荷を優先するなら後版への分離を推奨し、現在の修正と並行して編集は開始しない。
  `FsBgStyle::Default`は「塗らない」でviewportの黒に依存するため、背景fillだけを変更すると
  透過ピクセルにも余白色が混入する。回転した画像quad内の黒／白／市松を明示し、
  active・holdover・passive detached、見開き・連結・比較で余白と分離する必要がある。
  frozen singleの背景所有も整合させるため、detached計画§2の設計合意と§11記録を伴う。
  最小UI案は既定黒のRGB設定1つに、黒／灰色／白ボタンと既存`color_edit_button_srgb`を接続する形。
  プリセット選択前の任意色を別途記憶して「カスタム」へ戻せる仕様まで含める場合は、
  状態設計と検証にさらに0.25〜0.5人日を見込む。
...
### 4.3 表紙あり見開きで、末尾の単独裏表紙と先頭の表紙を組み合わせる — 将来版 (2026-09-08)

- 利用者要望: 表紙と裏表紙が一続きのイラストである本について、表紙ありの見開き設定で
  末尾が裏表紙だけの単ページになる場合、先頭の表紙と一緒に見開き表示したい。
  まず工数を調べ、規模が大きければ v3.7.0 より後へ回すという依頼。
- **中規模と評価し、v3.7.0 には含めず将来版へ登録。未実装。**
  利用者が同日、v3.7.0 での見送りを明示了承した。
  Sol / xhigh のコード調査と独立 Astra / 高の設計確認による静的見積もり。
  概算はページ単位の本体閲覧を先行する場合 **2〜3 人日**、連結読み・Remote まで含める場合
  **合計 3〜5 人日**。仕様整理・実装・自動回帰・レビューを含む親の見積もりであり、
  実機検証枠の待ち時間は含めない。横長境界・一時ずらし・重複表紙への直接シークまで複雑な
  選択規則を求める場合は 5〜7 人日程度へ広がり得る。着手時のソースと確定仕様で再見積もりする。
- 調査で確認した境界:
  - `SpreadDisplayUnit` はページ index 列を持ち、離れた 2 ページを表すこと自体は可能
    (`src/ui_fullscreen.rs` の `SpreadDisplayUnit::spread_pair`)。描画機構の新設は不要と見込む。
  - 表紙が先頭単ページと末尾見開きの両方へ現れるため、ページ identity と表示位置を区別する必要がある。
    `find_spread_display_unit` / `pages_on_screen_with` はページを含む最初の unit を返す。
    末尾の移動・シーク・読書位置は裏表紙側の anchor を維持し、先頭へ吸われない契約が必要。
  - 通常ページ送りの `App::update_prefetch_window` (`src/app.rs`) は現在位置近傍の連続範囲で
    cache を retain してから表示相方を解決する。末尾から離れた表紙は保持範囲外になり得るため、
    描画だけでなく表示 unit 全体の保持・読み込み・先読みへ同じページ集合を渡す必要がある。
  - 共通 builder は Remote と連結読みにも使われる。Remote の `pageGroupIndexIn` と
    `pageRenderContextForEntry` (`crates/remote-web/web/app.js`) は表紙から先頭 unit を逆引きし得る。
    連結読みの VRAM trim は除外 unit のページを keep 集合から remove するため、他の表示 unit が
    同じ表紙を必要としている場合の保持を確認する。いずれも本機能導入時のソース上のリスクであり、
    実行して確認した不具合ではない。
- 実装時の仕様決定: 既定 OFF の追加設定とする案、本単位／共通の保存範囲、左右の綴じ方向、
  横長表紙・横長末尾、1 ページ本、見開き1ページずらし、並べ替え・絞り込み時の本の境界。
  先頭の表紙単独表示は維持し、ファイルの複製・並べ替えや編集データの書き換えで実現しない。
  当初の対象をページ単位の閲覧に絞るか、縦横連結・Remote も含めるかは着手時に決める。
- 完了条件: 通常画像／ZIP／PDF、主窓／F12・複数窓、ページ送り・逆戻り・Home/End・シーク・
  再開、左右別の補正・AI・表示トリム、見開き保存／外部ツールの対象、非同期完了・取消・
  キャッシュ保持の整合を検証する。画像 identity は維持し、表示 unit の解決を利用元へ共通供給する。
  §1.201 の連続 index に対する先読み枚数の改善だけでは、末尾から離れた表紙の保持を保証しない。
  実アプリ検証は事前に明示了承を得た枠で行う。
  detached 経路を変更する際の旧文書の ClaudeCode 検収指定は、本タスクで利用者指定済みの
  Sol 実装／独立 Astra レビューへ移行し、構造的変更の判断と範囲を `detached-rework-plan.md` §11 に残す。

#### 2026-09-10 再検討時の追加要件

- 利用者は、表紙あり見開きで末尾と先頭が単ページの場合だけ、末尾に先頭を添える仕様を希望。
  添えた表紙は追加のシーク・ページ移動対象にせず、末尾の読書位置とページ数を維持する。
  今夜のリリースに間に合わない可能性を考慮し、dupe worktreeでの並行試作を検討する。
- 再調査案: navigation unitの先頭`[0]`・末尾`[last]`を保ち、描画時の構成にだけ表紙を添える。
  page0を末尾のnavigation unitへ重複登録しないことで、同じページからの逆引き・シーク・履歴の
  曖昧さを避ける。表示が必要なページの読み込み・prefetch・PDF promotion・保持は、操作対象と
  分けた役割付き構成から共通に求める。`resolve_spread_pair`だけを非連続Doubleへ変更しない。
- 静的な再見積りはページ単位の本体閲覧（main/F12・画像/ZIP/PDF）で概ね1.5〜3人日。
  連結読み・Remoteを含めると従来の3〜5人日級。実機待ち時間を除く仮見積りであり、着手時に
  設計検収する。横長/回転/見開きずらし、設定の保存範囲、コピー・保存・補正・類似パネル等の
  操作対象は未確定。「シーク・移動対象外」を他の全操作の除外へ自動的に拡張しない。
- dupeは未コミット変更なし、既存branchの全commitがmasterへ統合済みと確認。
  最新master起点の新機能用branchで分離する案。現時点では調査のみで、branch切替・実装未着手。
  現行の役割は親Astra/medium、実装と独立レビューは別のSol/xhigh。上の旧レビュー担当記録は
  当時の履歴として保持し、今回の着手へそのまま適用しない。
