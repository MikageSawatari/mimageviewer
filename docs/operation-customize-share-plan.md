...
> ステータス: **実装済み** (2026-07-12)。バックログ `docs/next-release-backlog.md` §4.6 の正本。
...
- **差分を見る…**: 比較対象 (標準 / 現在 / 前世代) を選べるサブモーダルで §4 の表を表示。基準サイズは 720x540、狭い viewport では余白を残して 560x420 まで縮める。比較元を切り替えても外形は変えず、可変長の差分表だけを縦横に内部スクロールする。本文高もフレームごとの `ui.available_height()` ではなく確定済み modal 高からヘッダ / フッタ予約を引いて決め、初回の前世代ロード開始フレームでも下端を動かさない。前世代の非同期ロード状態は差分ウィンドウ本文だけに表示し、背後の復元ダイアログ末尾へ重複表示しない。
- **書き出す…**: その世代 (現在含む) の操作カスタマイズを `.mivkeys.json` に保存 (`rfd` save)。
  主用途は「現在」行 = 自分の設定を共有。
- **取り込む…**: その世代の操作カスタマイズを現在へ適用 (置換 + viewport 追従サイズの縦横内部スクロール付きプレビュー + ライブ適用 +
...
- **操作カスタマイズを初期値に戻す…**: キー割り当て / リング・マウス /
  メニュー構成だけを確認後に標準値へ戻す。適用前にはファイル取り込みと同じ自動退避を行い、
  成功後は保存済み設定と実行中の keymap / native shortcut / ring 表示へ即時反映する。
...
- 共有ファイルの拡張子は `.mivkeys.json`。二重クリック関連付けは行わない。
- リング / メニュー差分の詳細表示をどこまで作り込むか (初版は件数 + 変更有無から)。
- 編集ダイアログからこのタブへの導線ボタンを付けるか (初版は任意)。
- 自動退避は最新 10 ファイルを保持し、新しい退避の作成直後に古いものを整理する。
