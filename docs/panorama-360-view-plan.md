...
- 一覧は `App::panorama_projection_popup_open` が持ち、**見開き / フィット / スライドショー /
  回転の各ポップアップと同じ guard 群 (上バーの表示維持、タッチ抑止、ホイール / キー /
  右ドラッグの抑止、カーソル自動非表示の抑止) にすべて登録する**。1 つでも登録から漏れると、
  一覧を開いている間の入力が背面のキャンバスへ抜ける。360 を抜けたときは持ち主のボタンごと
  消えるので `toggle_panorama_mode` の OFF 経路で必ず閉じる。
- 一覧から選ぶ経路とキーの順送りは `App::set_panorama_projection` へ合流させる。片方だけ
  画角の丸め (§13.3) や通知を落とさないため。
- **一覧の位置と大きさは実測して画面内へクランプする** (`clamp_bar_popup_rect`)。上バーの
