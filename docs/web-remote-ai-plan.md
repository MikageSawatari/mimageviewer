...
**表示追補 (2026-08-08):** 通常の進捗と完了通知は、下部バーおよびページ境界通知を避けた
左下の小型インジケータに畳む。タップで現在の詳細と実行中ジョブの「取り消す」を展開する。
`Failed` と開始要求失敗は畳まず、従来相当の可読幅で本文を表示する。これは表示だけの変更で、
job の開始・polling・取消・terminal 判定は変更しない。
