...
ステータス: **実装済み。ZIP/PDF 本体と設定連動の画像フォルダ本を追加 (2026-07-20)**。

現在の実ディレクトリ以下にある画像/動画、ZIP/PDF 本体、設定上「本」となる画像フォルダを、その時点のスナップショットとして
...
| 対象 | 通常ファイルシステム上の画像/動画と ZIP/PDF 本体。「画像のみのフォルダを本として扱う」が実効 ON なら画像だけのフォルダは 1 項目。ZIP/PDF/変換アーカイブの中身は対象外 |
| 永続化 | ビュー状態は保存しない。走査深さと走査時フィルタを後方互換なスカラー / 既存 enum の設定として設定DBへ保存する。検索DB / rating DB のスキーマ変更なし |
...
| 走査深さ | OFF 状態の `サブ展開` から確認ダイアログを開き、対象と深さを選んで実行する。ON 状態ではダイアログを出さず即時解除する。既定の「無制限」は従来と同じ実効上限 40 |
| 走査時フィルタ | 実行前モーダルで種類 / サイズ / 更新日を選び、条件外の項目は snapshot へ入れない。既定は条件なし。適用条件は展開後のフォルダバーにも表示する |
...
- フォルダバー右側には幅が変わらない単一の `サブ展開` selectable ボタンを置く。OFF で押すと
  対象と走査深さを確認するダイアログを開き、ON で押すとダイアログを出さず root 通常フォルダへ戻る。
  走査開始から表示準備完了までの進捗と `中止` は中央モーダルに統一し、ツールバーの再レイアウトを避ける。
- 実行前 UI は `egui::Modal` とし、背面操作を遮断する。走査深さと、既存 facet と同じ
  `FacetItemKind` / `FacetSizePreset` / `FacetDatePreset` による種類・サイズ・更新日条件を選び、
  `サブ展開` / `キャンセル` / Escape で決定する。選択値は Settings へ保存し、次回の初期値にする。
  ボタンのツールチップには現在の実効深さ、展開後のフォルダバーには実際に適用した条件を表示する。
- 分割ボタンは採用しない。`Button::selectable` は OFF で枠を描かず枠付き歯車と一体に見えず、
  ポップアップ内 ComboBox は親ポップアップを閉じて選択できないためである。
...
- ZIP/CBZ と PDF は内部ページを列挙せず、本体をそれぞれ 1 項目として snapshot に入れる。
  「画像のみのフォルダを本として扱う」が実効 ON の場合は、通常一覧のページ数判定と同じ共通述語で
  画像だけのフォルダを判定し、個別画像の代わりに `GridItem::Folder` を 1 項目入れる。OFF では
  従来どおり画像を 1 枚ずつ入れる。
...
- サイズ順では走査workerが取得済みの実ファイルサイズを使う。実在する0バイトは0として並べ、
  合成した画像フォルダ本はサイズ不明として昇順・降順とも同じ割当行の末尾へ置く。走査結果の
  snapshotへサイズを保持するため、再ソートや後からのスタック切替でfilesystem I/Oを追加しない。
- 数百万件のソートは事前計算キー + index を固定サイズのチャンクごとに並べ、段階マージする。
  cancel は比較関数内で判定せず、チャンク境界とマージ処理の一定件数ごとに確認することで、
  `sort_unstable_by` の全順序契約と中止への応答性を両立する。キー生成・チャンクソート・マージは
  モーダルの「並び順を計算中」件数へ単調増加で反映する。
...
- サブ展開ビュー上で `スタック` を押すと、保持中のフラット一覧を親フォルダ単位で分類して
  ...
  ファイルシステムを再走査せず元のフラット一覧へ戻す。サイズ順では物理memberと単独mediaに
  snapshotの既知サイズを使い、複数memberを畳んだStackセルは代表画像のサイズを流用しない。
...
- メニュー: `サブフォルダを展開して表示`
- ツールチップ: `現在のフォルダ以下の画像・動画・ZIP・PDFを一覧表示`。画像フォルダの集約は
  既存の「画像のみのフォルダを本として扱う」設定に従うことも併記する。
- ビュー見出し: `サブ展開: <root>`。複数起点の場合は root に加えて `Nフォルダ` を併記する。
  走査時フィルタが有効なら、同じ見出しへ種類 / サイズ / 更新日の条件も併記する。

### 2.2 配置

- フォルダバー右側のフォルダ/表示系の近くに、幅が状態で変わらない単一の `サブ展開` ボタンを置く。
  OFF 状態で押すと、展開対象・走査深さ・走査時フィルタを示すモーダルを開く。モーダルの
  `サブ展開` で開始し、`キャンセル` / Escape で閉じる。ON 状態で押した場合はダイアログを出さず
  即時解除する。
...
- 現在の通常フォルダ一覧は表示したまま、走査進捗を中央モーダルに表示する。見つかった表示項目数、
...
- `GridItem::ZipFile(PathBuf)`（`.zip` / `.cbz` 本体を 1 項目）
- `GridItem::PdfFile(PathBuf)`（PDF 本体を 1 項目）
- `GridItem::Folder(PathBuf)`（画像フォルダ本の設定が実効 ON で、通常一覧と同じ判定を満たす場合だけ）
...
ZIP/PDF/変換アーカイブの中身は展開しない。中身まで展開すると、パスキー、パスワード、変換キャッシュ、ページ列挙、
...
worker:
  ...
  - 画像/動画と ZIP/PDF 本体を SubfolderExpansionEntry にする
  - 画像フォルダ本の設定が実効 ON なら、画像だけのディレクトリは Folder 1 項目に置き換える
...
- walker は各走査起点を depth 0 で積み、`depth > max_depth` のディレクトリを読まない。
  したがって UI の「起点のみ」は `max_depth = 0`、「1 階層」は起点とその直下を読む
  `max_depth = 1` に対応する。UI 選択値から `max_depth` への写像は 1 箇所の純関数に閉じる。
- ダイアログの候補は「起点のみ / 1 階層 / 2 階層 / 3 階層 / 4 階層 / 5 階層 / 10 階層 /
  無制限」。選択値は永続化し、次回ダイアログの初期値と実行時の走査上限に使う。
- 初期版では single-thread worker で走査する。並列スキャンは実測で不足が見えた後に追加する。

### 6.2 走査時フィルタ

種類 / サイズ / 更新日の条件は、表示後の facet ではなく **走査時の除外条件** とする。
条件外の項目を snapshot へ入れないことで、大量件数時のソート、一覧構築、レーティング / タグ /
補正等のメタデータ準備を軽くする。展開後の表示内フィルタとは独立しており、適用した走査条件は
フォルダバーの `サブ展開: <root>` 表示へ残す。

- 種類は `FacetItemKind` のうちサブ展開が生成する Folder / Image / Video / Zip / Pdf だけを扱う。
  空集合は従来互換の「すべて」とする。
- サイズ / 更新日はファイルだけへ適用する。ZIP / PDF 本体はファイルなので対象。画像フォルダ本には
  ファイルサイズや代表更新日時を定義せず、これらの条件では除外しない。
- サイズは既存の 4 つのクイック選択に加え、最小 / 最大を数値と KB / MB / GB の組み合わせで
  指定できる。片側だけの指定を許し、下限を含み上限を含まない。数値と単位は入力どおり保存し、
  判定時だけバイトへ換算する。最小が最大を超える場合は日付範囲と同様に両者を交換する。
- `DirEntry::metadata()` で既に得ている size / mtime をそのまま判定に使い、フィルタ専用の追加 I/O は
  行わない。
- 走査時除外でもディレクトリは読まないと中身を判定できないため、`read_dir` の回数や走査対象
  ディレクトリ数は減らない。ファイルの種類判定と size / mtime の取得も従来どおり行う。
- 走査深さと条件は毎回モーダルに表示してから実行するため、永続化しても隠れ状態にはしない。
- スマートフォルダの走査はこの設定を参照しない。

### 6.3 reparse point / symlink

ディレクトリ symlink / junction は、`search_walker` と同じ方針に揃える。独自の簡易実装を作らない。

- `canonicalize` 由来の visited key でループを防ぐ。
- 「無制限」の depth limit は従来どおり `search_walker` と同じ 40 に揃える。新しい sentinel は
  作らず、reparse point ループ対策の実効上限として扱う。浅い候補はサブ展開だけに適用し、
  共通 walker を使うスマートフォルダの深さは 40 のまま変えない。
...
サブ展開ビューでは、場所ラベルを `subfolder_expansion_root` (= ボタンを押した実フォルダ)
からの相対パスで表示する。チェックした複数フォルダを走査起点にした場合も基準は変えず、
`ID203\thumb` のようにどの起点配下かを残す。root 直下は `(直下)`、相対化できないパスは
従来のフルパス表記へフォールバックする。場所 facet、選択情報、グリッドのツールチップ、
詳細表示の `場所` 列は共通のラベル生成を使う。

詳細表示の `場所` 列は任意表示・ソート可能とし、既定では非表示にする。詳細表示では行の
テキストツールチップを出さず、情報は列または下部情報バーで見る。プレビュー列へホバーした
ときの大きなサムネイル viewport は、列では代替できないため維持する。
...
この機能の価値は `場所` facet と組み合わせると強い。場所 facet 自体は既存の親フォルダ判定を
使い、ラベルだけをサブ展開ビュー限定で root 相対表示にする。他ビューは従来の表記を維持する。
...
- 名前の昇順・降順 / 番号の昇順・降順 / 日付順は通常表示と同じ。番号順は記号・空白などの
  区切りを無視し、降順でも同値は名前昇順で確定する。
...
- 設定 OFF では画像だけのフォルダも画像を 1 枚ずつ拾う。
- 設定 ON では画像だけのフォルダを Folder 1 項目にし、動画・音声・サブフォルダ・ZIP/PDF・有効な変換アーカイブが混ざれば集約しない。
- ZIP/CBZ/PDF は本体をそれぞれ 1 項目だけ拾い、内部ページを列挙しない。
- 同名ファイルフィルタが親フォルダをまたがない。
- symlink / junction / depth limit で無限再帰しない。
- 深いテストツリーで `max_depth = 0 / 1 / 2` ごとの収集件数が変わる。
- UI の深さ選択から walker の `max_depth` への写像で「起点のみ」と「無制限」の境界が保たれる。
- 保存済みの走査深さが設定DBの保存 / 読み込みを往復し、キー欠落時は従来の 40 になる。
- 種類 / サイズ / 更新日の条件外項目が snapshot entries に入らず、条件なしでは従来と同じ結果になる。
  サイズは従来 4 区分に加え、下限のみ / 上限のみ / 両方 / 逆順の範囲と KB / MB / GB 換算を確認する。
- 画像フォルダ本はサイズ / 更新日条件だけでは除外されない。
- 保存済みの走査時フィルタが設定DBを往復し、キー欠落時は条件なしになる。
- cancel が立ったら早期終了する。
...
- 場所ラベルが root 直下 / 深い階層 / 複数走査起点 / root 外フォールバックで正しくなる。
- 詳細表示の場所列が同じラベルを表示・ソートし、旧版向け stash を経て列順 / 幅 / ソートを復元する。
- 詳細表示では行のテキストツールチップを出さず、グリッドと詳細プレビュー viewport は維持する。
...
P2 は概ね採用し、画像色フィルタは確認ゲート付きで解放した。場所 facet の root 相対ラベルは
実機要望を受けて後続対応し、bounded parallel scan は引き続き後続扱いとする。metadata 収集は
初期版では維持し、perf で問題が出た場合に遅延化する。
