...
- **アイコン数の見積り**: 分析ツール −1 / 360 の非表示条件を無効化へ ±0 / 「…」+1。
  **普通の画像では今より 1 個少なくなる。**
- **収録内容 (確定、利用者判断 2026-08-13)**。基準は「**KeyAction はあるのに画面上の
  入口が無い機能**」:
  1. ナビゲータ ON / OFF
...
- **外した候補と理由**:
  - **左右パネルのモード**: タッチには専用ハンドルがあり、モードに関係なくパネルを
    開ける。モードはホバーの挙動を決めるものなので、タッチ操作中に切り替える意味がない。
  - **ルーペ固定** (`FsLoupeLockToggle`、<kbd>M</kbd>): ルーペはカーソル位置に出るもので
    タッチには置き場所が無い。しかも `fs_loupe_locked` は `App` フィールドで毎起動 false に
    戻るため、**タッチだけの利用者は ON にできない**。「OFF にする用途」も成立しない。
