...
- 配線 (2026-09-02): レーン A 取り込み後、`context_menu_model.rs` の checked 分岐へ
  「編集内容をまとめて貼り付け [N件]」を追加し、`request_bulk_paste_page_edit_bundle` へ
  dispatch。入口はモデル 1 か所なので、grid / fullscreen / native の各 renderer へ自動的に
  同じ形で出る。単一貼り付けは従来経路のまま (checked が空のときだけ出るので、同じ項目が
  2 つの意味を持つことはない)。マニュアル (grid.html) 更新済み。
- 残作業: 手動確認後に本項目を削除する。
...
- 配線 (2026-09-02): 単一項目メニューには「編集内容をリセット…」を、checked 分岐には
  「編集内容をリセット… [N件]」を追加し、どちらも `request_bulk_reset_page_edits` へ
  dispatch (単一もチェック複数も同じ engine を通る)。確認ウィンドウの表題は 1 件でも
  読める「編集内容をリセット」にした。マニュアル (grid.html / fullscreen.html) 更新済み。
- 残作業: 手動確認後に本項目を削除する。
