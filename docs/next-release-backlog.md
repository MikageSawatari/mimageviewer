...
### 1.53 ナビゲータの縮小画像がカラー化 / 補正の前の絵になっている

- 出典: v2.12.0 出荷前の実機確認 (2026-08-06)。「Alt のナビゲータが、カラー化がきく前の状態で
  表示されている」。カラー化に限らず、補正・AI・注釈・隠蔽・消しゴムも反映されていない。
- 原因 (調査済み): `draw_fs_navigator` ([ui_fullscreen.rs](../src/ui_fullscreen.rs) の 15421 付近) が
  `self.thumbnails[page_idx]` = **一覧用サムネイル**をそのまま貼っている。表示中の絵は
  `resolve_fs_processed_texture` ([ui_fullscreen.rs:3420](../src/ui_fullscreen.rs)) が返す別物。
- 直し方の骨子: **新しい生成は要らない**。現在ページの processed texture は
  `prepare_fullscreen_state` が毎フレーム解決済みで `FsFrameState.tex` に入っている。それを
  ナビゲータまで引き回す配線が本体。
- 注意点 (ここを外すと悪化する):
  1. `resolve_fs_processed_texture` は `&mut self` で **worker を起こす producer**。
     ナビゲータから直接呼ぶと、見開きの相方ページの合成を余計に走らせる。
     **解決済みの handle を渡す**形にすること
  2. 見開きは 2 ページぶん必要。`draw_fs_spread` が持っている handle を経由させる
  3. 合成が間に合っていないフレームはサムネイルへフォールバックする (黒くしない)
  4. §1.46 の GPU 拡大出力は**可視 source 領域だけ**なので、そちらを掴むと
     ズーム中に切り取られた絵がナビゲータに出る。使うのは拡大前の full-image texture
- 規模 / 優先度: Small〜Medium / P3。新機能ではなく配線だが、表示パイプラインの
  テクスチャ選択に触れ、確認は実機目視が中心。**見た目の問題なので v2.12.0 は見送り**
  (利用者判断 2026-08-06)。
...
- 対応 (2026-08-05):
  ...
  - 第2段階として 360 度パノラマへ対応した。equirect の元画像をサムネイルカタログから全体図として
    ...
    パン、ダブルクリックは中心移動にする。`FOV_MAX` でも全球は見えないため、平面の全体表示時
    自動非表示は適用しない。
- 対象外:
...
### 4.7 タブレット PC のタッチ操作対応 (調査完了・設計検討中 / 未実装)
...
- **利用者判断で確定 (2026-08-06)**: 動画も対応必須 / 左右パネル (AI アップスケール・カラー化・
  ブックマーク・レーティング・タグ) もカバー / 選択済みセルの再タップ open も入れる /
  **タッチ ON-OFF 設定は作らず、マウス無影響でタッチ時だけ別動作**。
  この方針は成立する見込み (egui-winit のイベント列シグネチャで入力源を相関し、
  肯定証拠だけで新経路へ入る fail-closed 設計)。
- 工数目安: **総計 22〜34 人日** (Phase 1 = 入力源分離 + 両 backend 成立確認 7〜10 /
  Phase 2 = 一覧・再タップ open・静止画パネル 7〜11 / Phase 3 = 動画 native 完成 8〜13)。
- **着手前の判断待ち**: ①**ペンの扱い** (winit が指とペンを同じ `WindowEvent::Touch` に潰すため、
  「ペンは従来どおり」を保証するなら下位層に source tag が要る。**最大の後悔要因**)
  ②診断用の強制無効 (`MIV_DISABLE_TOUCH_GESTURES`) を持つか ③動画に AI アップスケール /
  カラー化が元から無い件の整理 (plan §5.14)。
