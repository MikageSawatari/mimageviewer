...
### 1.80 タッチ用パネル (上バーの「…」から下部オーバーレイ) — 設計確定・未実装

- 出典: 利用者との設計相談 2026-08-13。§1.63 でナビゲータをタッチから触れるようにしたが、
  **ヘッダの閉じるボタン (18pt) が指には小さすぎる**という報告。ヘッダの高さが 26pt しか
  無いので、その中に指で押せる大きさ (44pt 前後) は物理的に入らない。隅ボタン 4 つは
  16pt でさらに小さい。**個々のボタンを大きくする方向は行き止まり**と判断した。
- **決まっていること**:
  - **新しいジェスチャーは足さない**。中央タップは既に「chrome の表示 / 非表示」という
    意味を持っており、2 つ目の意味を重ねるとバーを出したいだけの操作の邪魔になる。
  - **上バーに「…」ボタンを 1 つ足し、そこから下部にオーバーレイパネルを出す**。
    上バーは既にタップ位置を最寄りのボタンへ吸着させる仕組み
    (`prepare_bar_button_touch_targets`) と除外矩形を持っており、**タッチで機能に届く
    唯一の場所**。パノラマのアイコンが上バーにあったのが実例。
  - パネルは**下**に出す。上バーが上にいることと、タブレットで親指が届くのが下のため。
  - **1 階層のサブメニュー**を持つ (ナビゲータ位置のような選択肢)。
  - **分析ツールをバーから「…」へ移す**。無条件表示かつ低頻度で、移動候補として適切。
  - **360 ボタンはバーに残す**。使えない場面は非表示ではなくグレーアウトに統一する
    (下記)。
- **HUD バーの規則 (利用者判断 2026-08-13)**: **ページごとにボタン数が変わってはいけない**。
  ホイールで前後に移動したときに押したいボタンの位置がずれるため。
  - **既にこの規則が破れている**: 分析ツールと 360 だけが `!is_spread_double` を条件に
    持つ。`is_spread_double` は「今の位置が見開きペアとして解決したか」なので、見開き
    モードで表紙や横長ページに来ると false に変わり、**ボタンが消えて左側が全部ずれる**。
  - 分析ツールは「…」へ移すことで解消する。360 は `!is_spread_double` と
    `reading_flow.is_paged()` の条件を**非表示ではなく無効化**へ変える。360 は既に
    「360 画像でないときはグレーアウト」を実装しているので、同じ扱いに揃えるだけ。
    ツールチップに理由を出す。
  - 他のボタンの条件は「動画かどうか」「360 モード中か」「ウィンドウモード」など
    **利用者が明示的に切り替えたときだけ変わるもの**で、ページ送りでは動かない (確認済み)。
- **アイコン数の見積り**: 分析ツール −1 / 360 の非表示条件を無効化へ ±0 / 「…」+1。
  **普通の画像では今より 1 個少なくなる。**
- **これはタッチ専用ではない (利用者指摘 2026-08-13)**。入口を中央タップではなく
  **上バー**に置いた結果、マウス利用者も同じものを使う。したがって選定基準は
  「タッチから届かない機能」ではなく、**「使用頻度が低く、画面上の入口が無い機能」**。
  マウスでしか意味を持たない機能も入れてよい。
- **収録内容 (確定、利用者判断 2026-08-13)**:
  1. ナビゲータ ON / OFF
  2. ナビゲータ位置 (サブメニュー、4 隅)
  3. 分析ツール (上バーから移す)
  4. ピクセルグリッド (`FsPixelGrid`、既定 <kbd>G</kbd>。**環境設定に項目が無く**、
     キーとリングからしか触れない。既定 ON で `App` フィールドなので毎起動 ON に戻る)
  5. ルーペ固定 (`FsLoupeLockToggle`、<kbd>M</kbd>)。**マウス向けに残す**。ルーペは
     カーソル位置に出るのでタッチでは使えないが、マウスでは実用で、画面上の入口が無い。
- **外した候補と理由**:
  - **左右パネルのモード**: **上バーの `i` ボタンが既に切り替えている**
    (`fs_info_btn` → `side_panel_mode_pressed`、ツールチップに現在と次のモードを表示)。
    画面上の入口が既にあるので対象外。
  - 編集系 (消しゴム / 隠蔽 / 補正レイヤー) とズームモード: ズームモードはホバー前提、
    編集はタッチ対象外という判断。比較ワイプも Shift+C / X とセットでしか使えない。
- **収録項目は設定で変えられるようにする (利用者判断 2026-08-13)**。将来機能が増える
  ため。既存の `ToolbarSectionId` と同じ作法にする: `#[serde(other)] Unknown` で
  ダウングレード耐性を持たせ、`default_order()` + `ordered_with_fallback()` で
  **保存順に無い新項目を末尾へ自動追加**する。これで新しい機能を足したときに、既存
  利用者の手元にも出る。表示 / 非表示は別 Vec で持つ。
- パネルが入れば、ナビゲータの × と隅ボタンは「マウス用の近道」に格下げできる。
  そこを無理に大きくする必要はなくなる。
- 規模 / 優先度: Medium / P2。新規なのはパネル部品 (行の大きさ・階層・テーマ対応) だけで、
  各項目の配線は小さく、タップの除外とルーティングは既存の再利用。
