...
名前変更と新規フォルダ作成の単一入力欄を Windows 標準の入力画面へ移し、書記素・IME・
クリップボード・Undo の正しさを OS へ委譲した。エクスポート、タグ名、ブックマーク名は
裁定どおり対象外。

**これで直らないもの**: ZWJ 連結絵文字の**表示**。egui は本格的なテキストシェーピングを
行わないため、一覧のファイル名表示では家族絵文字などが結合されない問題が残る。
