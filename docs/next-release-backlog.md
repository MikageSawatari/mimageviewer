...
### 1.270 複数ウィンドウで RAR を開いた画像フルスクリーンだと、ツリー順の前後フォルダ移動が効かない — >>459 (2026-09-23)

- 出典: >>459 (利用者報告、2026-09-23)。「複数ウィンドウモード、RAR だと画像フルスクリーンでツリー順、前/次フォルダの
  ような移動が機能しない。ZIP は問題なさそう」。開発者が >>461 で「こちらでも再現を確認して、原因を調べてみます」と返信済み。
- **エージェントの観測は無い** (製品バイナリを起動しない)。再現の確認は利用者の実機で行う。
- 見立て (未確認): 複数ウィンドウの別窓は、RAR (直接読む非ソリッド RAR / 変換 ZIP) と ZIP で開く経路が違い、
  別窓の Ctrl+↑↓ (ツリー順の外側移動) が RAR の context で物理フォルダの位置を解決できていない可能性。
  §1.267 で追加した collection-root 以外の別窓 Ctrl 経路 (`handle_fullscreen_ctrl_nav_context` の
  `detached_independent_session_blocks_folder_nav`) と、変換 archive の `archive_source_override` を先に読む。
- 進め方: 第 1 層に「複数ウィンドウで ZIP と RAR を同じ位置から開き Ctrl+↑↓」のシナリオを作り、修正前に RAR 側が失敗することを
  確かめてから直す。RAR の試験素材は、リポジトリの既存 fixture を先に探す (RAR の作成ツールはライセンス上の制約がある)。
- 規模 / 優先度: 不明 (原因特定から) / P1 (利用者報告、v4.1.0 で修正予定)。
