...
### 1.254 「この本と重なる本」が大きな画像フォルダで終わらない / 更新中表示でパネルが上下に動く — 方針合意済み・未実装 (2026-09-19)

- 出典: 利用者の報告 (2026-09-19)。約 8,700 枚の画像フォルダで類似タブを開くと「調べています」のまま。
  同じフォルダでちらつく印象もあった (利用者の記憶では上下にずれていただけかもしれない。現在は再現しない)。
- 正本: [similar-book-query-limit-plan.md](similar-book-query-limit-plan.md)。観測 (perf ログ・通常ログ・
  索引 DB・読み取り専用ベンチ) と決定事項はそちら。
- 分かっていること: perf ログでは照会が 91 秒 / 197 秒 CPU を使っても完了せず、毎回フルスクリーンを
  閉じる操作でキャンセルされていた。ベンチでは同じフォルダが 591 秒・618 冊ヒット。所要時間は
  origin のページ数ではなく候補の本の総ページ数で決まる (980 ページ・7 冊で 13 秒、1 万ページ・8 冊で 178 秒)。
- 残り: 作業 A (「索引を更新中です」行と「(更新中)」の削除)、作業 B (照会の内訳の計装) は着手可。
  作業 C (内訳の実測としきい値決定、ClaudeCode) の後に作業 D (対称のページ数上限 + 仕事量の予算)。
- 規模 / 優先度: A・B は Small、D は Medium / P2。照会の並列化は対象外 (plan §6)。
...
- **2026-09-16 追加修正 (>>415、v3.10.0 への報告): 実装済み・未リリース** (`91441ce43`)。横長判定で単独になったページは、
  先頭・末尾にあっても、回転後に横長となる場合も中央表示する。片側配置は「相方が無い端の単ページ」
  (`SpreadDisplaySingletonCause::UnpairedSlot`) だけ。判定は見開き構成の共通 builder にあり、ページ表示・縦横連結・保持画像・
  F12 / 複数ウィンドウ・Remote が同じ結果を使う。設計記録の旧「endpoint landscape」記述も同 commit で訂正済み。
  マニュアル (`fullscreen.html` / `settings.html`) に残っていた「途中の横長だけ中央」という記述は 2026-09-17 に訂正した。
- 残り:
  - 次版の README 更新履歴へ、この追加修正を 1 項目として載せる (リリース手順 Phase 0)。
  - 追加修正の実機確認の記録が無い (自動テスト・静的検査・`build-dev.ps1` まで)。報告者 (>>415) の再確認か、公開前 smoke で見る。
- 規模 / 優先度: 記載と確認のみ / P2。
...
- **対応中（2026-09-14）**: 前段の§2.25/§2.24が独立検収・確認ビルドまで完了し、利用者の指示により具体設計と基盤実装へ移行。確定仕様は下記仕様案を参照。実機確認は帰宅後とし、当面は非対話検証を進める。

- **2026-09-14仕様検討開始**: [仕様案](collection-spec-proposal.md)を利用者と検討し、その間は別機能を直列実装する。コレクション実装は最後。
- **2026-09-14確定事項**: 名前付きで複数管理し、BGM用・静止画スライドショー用など用途別に使う。入口は本棚同様の専用コレクションツールバー。手動順を既定にし、通常ソートに切り替えても手動順を保存する。PCは作成・編集、Remoteは一覧・閲覧に対応する。Ctrl+上下は登録されたフォルダ/本の最新の有効順で横断する。再生中の編集は次の対象選択へ反映し、最新一覧で現在のファイル参照を探してその次へ進む。現在の参照自体が無い場合は先頭から選ぶ。詳細と独立設計照合は仕様案を正本とする。
