...
### 1.3 アニメーション WebP の再生対応

- 背景: mImageViewer 専用スレ 3 (総合スレ 864 への返信)。「webp アニメの再生に非対応」という
  報告 / 要望。
- 現状:
  - GIF / APNG は `fs_animation.rs` でフルスクリーンロード時にアニメーション展開する。
  - WebP は静止画、サムネイルキャッシュ、XMP タグ読み書きなどでは対応済みだが、
    animated WebP は現状では再生対象として扱っていない。
- 方針:
  - 初期版は画像フルスクリーンでの animated WebP 再生を対象にする。
  - 通常画像ファイルだけでなく、ZIP / 変換済みアーカイブ内の WebP 画像でも同じ経路で扱えるようにする。
  - サムネイルと詳細表示は、まずは先頭フレーム表示のままでよい。サムネイル上のアニメ再生や
    動画扱いのメタデータ表示は後続判断。
  - デコードは UI スレッドで行わず、既存のフルスクリーンロード worker / `fs_animation` の考え方に寄せる。
  - ライセンスと配布形態に合う decoder を選ぶ。候補調査時は、frame delay、loop count、
    alpha、ICC / orientation、巨大画像時のメモリ使用量、ZIP 内 bytes 入力の可否を確認する。
  - 再生に失敗した場合は先頭フレームの静止画表示へフォールバックし、通常画像閲覧を壊さない。
- 確認:
  - animated WebP がフルスクリーンで再生され、フレーム間隔とループが大きく崩れない。
  - 通常 WebP / JPEG / PNG / GIF / APNG の既存表示に退行がない。
  - ZIP 内 animated WebP、透過 animated WebP、大きめ animated WebP で UI が固まらない。
- 優先度: P2 candidate。キー / マウスカスタマイズの大きなリリース後に実装候補。
