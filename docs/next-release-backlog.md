...
### 4.2 ファイルシステム名の入力だけ Windows 標準ダイアログへ寄せる (対応済み)

名前変更と新規フォルダ作成の単一入力欄を Windows 標準の入力画面へ移した。エクスポート、
タグ名、ブックマーク名は裁定どおり対象外。得られたのは IME・クリップボード・Undo・右クリック
メニューと、**エクスプローラーの名前変更欄と同一の編集挙動**。

**当初の前提が 1 点外れていた (実機 2026-07-31)**: EDIT コントロールは書記素クラスタ単位では
削除しない。egui と同じく **コードポイント 1 個ずつ**消すため、👨‍👩‍👧‍👦 は Backspace 1 回ごとに
1 人ずつ減り、❤️ は異体字セレクタだけ落ちて白黒になる。エクスプローラーの名前変更欄も同じ。
つまり「EDIT は OS のテキストスタックそのものだから書記素も正しい」という §4.2 着手時の
推測は誤りだった。

**書記素単位の削除は入れない (裁定 2026-07-31)**: EDIT をサブクラス化して Backspace /
Delete の選択範囲をクラスタ境界へ広げる実装は一度作ったが撤回した。この変更の目的は
**OS の挙動に合わせること**であって OS を改善することではない。OS より「良い」独自挙動を
足すと、それ自体が意図しない動作の持ち込みになる。将来また同じ提案が出たときのために
理由をここに残す。

**これで直らないもの**:
- 上記の書記素単位削除 (意図的に見送り)
- ZWJ 連結絵文字の**表示**。egui は本格的なテキストシェーピングを行わないため、一覧の
  ファイル名表示では家族絵文字などが結合されない問題が残る

**着手時に見落としていた実装上の要点**: `DialogBoxIndirectParamW` はオーナー 1 つしか
無効化しない。mIV はフルスクリーンビューポートとネイティブ動画ウィンドウを別トップレベルとして
持つので、モーダルを明示的に前面へ出さないと「動画の左パネルだけ反応してアプリが固まって
見える」状態になる (実機で再発、Alt+Tab でしか到達できなかった)。`WM_INITDIALOG` で
`SetWindowPos(HWND_TOP)` + `SetForegroundWindow` し、中央寄せ位置はオーナーのモニター
作業領域へクランプしている。
