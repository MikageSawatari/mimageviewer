...
- 現状: ナビゲータの表示保持は <kbd>Alt</kbd>+<kbd>N</kbd> のトグルだけで解除する作り。
  タッチ対応 (§4.7、v2.13.0) を入れたときに、この経路をタッチから触れるようにしていない。
  **キーボードのない端末では、一度出すと消せない**ことになる。
- 検討する形 (どれが素直かは着手時に決める):
  - ナビゲータ自体にタッチ用の閉じる当たり判定を持たせる
  - 中央タップの chrome トグル (上部バー / 下部バー / 左右ハンドル) の対象にナビゲータを含める
  - タッチで開いた左右パネルと同じく、パネル外タップで閉じる扱いにする
- **同型の確認**: タッチで触れない「キーボード専用の保持状態」が他にないかを洗う。
  タッチ対応は後付けなので、同じ抜けが他の保持系トグルにもあり得る。
- 規模 / 優先度: Small / P2。
...
### 1.64 ズームを保ったままページを送る (ズーム固定モード) — 利用者要望
...
- 仕様として決めること:
  1. **何を保つか**。(A) 画像に対する**表示範囲の割合**を保つ = 解像度が違ってもほぼ同じ構図になる。
     (B) 表示倍率 (画像 1 画素の見かけの大きさ) を保つ = 画素検分向け。**用途からは (A) が既定**、
     (B) は将来オプション。位置は px ではなく**画像に対する正規化座標**で持つ (利用者提案どおり。
     サイズが違う画像でも同じ場所に着地する)。
  2. **解除の条件**。フォルダ移動 / フルスクリーン終了 / グリッド復帰で解除する (`Z` モードと
     同じ規則へ揃える)。永続化はしない。
  3. **固定中であることの表示**。出さないと「なぜズームが残るのか」が分からず不具合に見える。
  4. **他モードとの排他**。`Z` の全画面ズーム、見開き、連結読み、比較表示、パノラマ (360) での扱い。
- **トグルの入力は `KeyAction` を新設する。修飾キー + ホイールは使わない**:
