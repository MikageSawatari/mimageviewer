...
- Status: 対応済み。`rebuild_visible_indices` / `zip_nav_show_current_level` で未選択時は先頭の選択可能ページへカーソルを置く。通常 ZIP/PDF をページ一覧として開くケースでは表示される。
...
- Request 3: allow AI upscale on larger joined landscape scans.
  ...
  - Status: 対応済み (v1.6.0). `AI_SIZE_LIMIT_OPTIONS` に `6144x4096` / `8192x4096` / `8192x8192` の高負荷オプションを追加 (長辺は `MAX_TEXTURE_DIM`=8192 以下に制約、テスト `ai_size_limit_presets_stay_within_gpu_texture_limit` で保証)。メモリ警告文を強化し、smart-sharpen の `output_is_ai_upscaled` 不変条件 (= 入力長辺 < 8192 なら clamp 後出力 > 入力) のコメントを更新。
  - **未対応 (次版検討) — render-to-target 最適化**: 現方式は入力をフル 4× バッファに組み立ててから 8192 へ縮小するため、8192 入力で一時的に ~4.3 GB (32768²×4B) を確保する。最終クランプ後サイズ (≤8192²≈256 MB) へタイル単位で縮小しながら組み立てれば中間メモリを劇的に削減できる (推論時間は固定 4× モデルのため不変)。実装にはタイル overlap blend を最終解像度で行う必要があり (ストリップ方式で既存 blend を流用するのが低リスク)、実効倍率が 1× になり得るため `output_is_ai_upscaled` を `used_upscale` フラグ方式へ置き換える必要がある (上記コメント参照)。シーム/blend の回帰リスクが高いため v1.6.0 では見送り。優先度: Medium。
