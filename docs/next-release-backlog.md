...
### 1.81 360 度ビューで高画質へ切り替わると消しゴムの効果が消える — 利用者報告
...
### 1.83 ニアレストが効く経路と効かない経路がある — 利用者報告
...
- 追記: v3.0.0 では対応が漏れ、専用スレ #230 で再指摘を受けた。#210 で
  「次のバージョンで追加する」と回答済みだったため、次バージョンで対応する。
- 要望:
...
### 1.90 リモートのサムネイル一覧を指でゆっくり動かすと上下に振動する — 利用者報告
