...
### 1.248 固定の Esc・矢印キーと操作カスタマイズの競合警告を分かりやすくする — >>423-427 (2026-09-16)

- 出典: >>423。操作カスタマイズで Left を割り当てたところ、競合相手として
  `plain arrow navigation` が表示されたが、どこで解除できるか分からないとの報告。
- 現状、Esc と修飾キーなしの矢印ナビゲーションは最低限の終了・移動操作として固定され、
  keymap から解除できない。一方、固定キーと重なる利用者割り当ては拒否せず、競合警告を出したうえで
  読み込む仕様になっている。競合時の結果は dispatch 経路の処理順で決まり、consume 経路では先に
  処理した側、grid の非消費経路では両方が発火する場合がある。
- 当初は修飾キーなしの Esc・矢印を割り当て先として選べなくする案を回答したが、>>426-427 で
  **固定の見開き移動と、割り当てた1ページずらしを同じ矢印キーで使い分けている既存利用例**が
  確認されたため、この変更は取りやめる。固定キーと重なる割り当ても従来どおり許可し、現在の
  dispatch 順も変更しない。競合状態の優先関係は画面・処理経路により異なり、将来まで同じ結果を
  保証する仕様にはしない。
- 修正内容:
  - 英語の `plain arrow navigation` を
    `修飾なし矢印の固定ナビゲーション（解除不可）` のような日本語表示へ変更する。
  - 固定側は解除できないこと、不要な競合を解消する場合は利用者が追加した側を変更・解除すること、
    競合したまま使う場合は処理順によって動作が決まることを警告内で明示する。
  - 予約キーの説明に残っている `Enter` は固定キーではないため削除する。Esc と修飾キーなし矢印だけを
    現行仕様どおり案内する。
- 回帰確認: Esc / Left / Right / Up / Down との競合表示、Ctrl / Shift / Alt 付き矢印は通常の
  割り当てとして扱われること、既存の競合割り当てを保存・読込できること、見開き1ページずらしと
  固定ナビゲーションの現行 dispatch 順が変わらないこと。UI snapshot と conflict model の単体テストを更新する。
- 規模 / 優先度: Small / P2。表示と警告だけを直し、入力 routing は変更しない。
