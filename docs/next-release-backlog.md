...
### 1.203 画像フルスクリーンの余白背景色を変更できるようにする — >>369 (2026-09-08)

- **利用者要望。対応時期未指定。規模 / 優先度: 小〜中 / P3。**
- 症状 / 用途: 画像・本をフルスクリーン表示した際、画像の外側にある黒い余白へ、
  グレア液晶では周囲や利用者自身が映り込みやすい。余白の背景色を変更したい。
- 対応方針:
  - 画像・本のフルスクリーンにおける、画像表示領域の外側を塗る背景色を設定可能にする。
    プリセットと任意色のどちらを提供するか、設定場所を含めて実装時に現行 UI と照合して決める。
  - `FsBgCycle` の透過画像背景（黒 / 白 / 市松）とは別の設定として扱う。余白色を変更しても、
    透過ピクセルの合成、AI 処理、編集・書き出し結果を意図せず変更しない。
  - 単ページ / 見開き / 連結読み、回転・トリム、固定 HUD / 固定パネル、比較表示で、
    画像外の領域へ一貫して適用する。通常ウィンドウと F12 / 複数ウィンドウで設定を共有する。
- 回帰確認: 黒以外の余白色を選んだ状態で各閲覧方式の画像外だけが指定色になり、
  透過画像の背景設定、画像本体、コピー・書き出し結果へ混入しないことを snapshot 等で確認する。
