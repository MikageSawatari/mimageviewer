...
### 1.0m Ctrl+F の AI プロンプト検索が遅いのは「必要量の数百倍を読んでいる」ため (2026-09-05 調査)

**発端**: 利用者から「索引と同じように Ctrl+F も並列化すれば速くなるか。AI プロンプト
検索はそこそこ待たされる」。

**並列化は効くが、それより大きい原因が別にある。** 先に後者を直す。

#### 何が起きているか (コード確認)

1. **ファイル全体を読んでいる**。[png_metadata.rs](../src/png_metadata.rs) の
   `std::fs::read(path)` で PNG を丸ごとメモリへ載せてから走査する。
   JPEG 側も [exif_reader.rs](../src/exif_reader.rs) の `rexif::parse_file` で同様
   (同じクレートに `parse_buffer` があるので先頭だけ渡せる)。
2. **逐次である**。[metadata_ops.rs](../src/app/metadata_ops.rs) `run_metadata_search` は
   `for (idx, item) in items.iter().enumerate()` を 1 本のワーカースレッドで回す。
   UI は止めないが、1 ファイルずつ読んで待つ構造は索引と同じだった。

#### 実測 (利用者の実コレクション、PNG の text チャンク終端位置)

| | G:\home\imagedl\images | H:\home\eagle |
| --- | --- | --- |
| ファイルサイズ 中央値 | **3.31 MB** | 1.12 MB |
| メタデータが読み終わる位置 中央値 | **0.1 KB** | 2.9 KB |
| 同 最大 | **6.3 KB** | 4.9 KB |
| 256 KB 先頭で足りる割合 | **100%** (192/192) | **100%** (241/241) |
| そのとき読むバイト量 | **5.3%** | 23.1% |

**3.31 MB 読んで使うのは先頭 3 KB。** G: では必要量の約 500 倍を読んでいる。

#### 対策と順序

| | 見込み | 難度 |
| --- | --- | --- |
| **① 全体読みをやめる** | I/O が **5〜25%** に (= 4〜20 倍) | 低 |
| ② 並列化 | さらに **3〜10 倍** (§20 のドライブ実測) | 中 |

**① を先にやる。** 効果が大きく、同時実行の危険がない。

①は**先頭 N KB 固定にしないこと**。チャンクを順に読み、必要な物が揃った時点で打ち切る
形にする。ツールによっては IDAT の後ろにメタデータを書く場合があり、固定長では取りこぼす
(今回のサンプルには 1 件も無かったが、無いことの証明にはならない)。

#### 着手時の罠

**`ActivityGate` を流用しないこと。** 別バージョン索引は「利用者に譲る」ため閲覧中は
1/1 に落とすが、**Ctrl+F は利用者自身が待っている前面処理**である。同じ仕組みを付けると、
検索中に操作した瞬間に検索が 1 並列へ落ちるという逆の動作になる。

並列度は [duplicate-detection-plan.md](duplicate-detection-plan.md) §20 の実測が使える
(ボリュームごと 8 / 全体 16、遅い SSD は 8 を超えると劣化)。

#### 未計測

**Ctrl+F の実所要時間そのものは測っていない。** 上記は「読んでいるバイト量」と
「並列化した場合のドライブ性能」からの推論。着手時はまず現状を実測し、① の後・② の後で
比較すること。

- 規模 / 優先度: ① 小 / P2、② 中 / P3。
- 関連: `docs/search-architecture.md` §5.2、`docs/ui-responsiveness.md`。
