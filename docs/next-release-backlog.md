...
### 1.14 本棚追加時のフル composite 焼き込み (消しゴム再推論) — v2.4.0 送り

- 背景: ユーザー実機 FB「隠蔽加工した画像を本棚に右クリック追加すると加工が消える」
  (SNS 直アップ用途)。現状の焼き込みは `AdjustedFile` = 補正 + 非破壊回転のみで、
  隠蔽 / 消しゴム / 補正レイヤー / テキストを含まない。確定仕様 = 「編集がある時だけ
  フル最終 composite を焼く / 無編集は byte コピー (原本・AI メタ・アニメ保持)」。
- 実現可能性調査 (2026-07-11 第 14 弾、詳細は [compile-book-plan.md](compile-book-plan.md)
  「実現可能性調査」節): **ケース C**。隠蔽 / 補正 / 回転 / 補正レイヤー / テキストは
  headless (非表示アイテムでも) CPU 合成可能だが、**消しゴム (MI-GAN) の確定画素は
  `erase_result_cache` (メモリ) にしか無く DB/ファイルに永続化されていない**ため、
  任意アイテムを焼くには製本 worker から AI ランタイム + MI-GAN を再推論する経路が要る。
- **ユーザー裁定 = 案 2 (製本 worker で保存マスクから MI-GAN 再推論)**。案 3 (hybrid =
  消しゴム付きは表示後追加) は「任意本棚に追加する UI が無く操作しづらい」ため不採用。
  案 1 (消しゴム結果の永続化) は保存容量・失効管理の追加設計が要るため見送り。
- **実装規模 = 中〜大**。非消しゴム層 (2〜4 日) + 表示パイプラインに密結合した MI-GAN
  推論経路の headless 切り出し + AI ランタイム worker 配線 + 確定時との一致検証。
  v2.3.0 の収束を優先し次版送り。
- 実装時の必須: `BookPageSource::CompositedFile` 等の追加、焼き判定を全編集レイヤーへ
  拡張、full-composite worker、リリース前レビュー + 実機 smoke
  (「隠蔽加工を本ファイルへ焼き込み」「無編集は byte copy で AI メタ保持」)。
- **v2.3.0 の暫定対応 (ユーザー裁定済み)**: 焼かれない編集 (隠蔽/消しゴム/補正レイヤー/
  テキスト) が乗ったアイテムを本棚追加したとき警告 toast を出す (第 15 弾で実装)。
  「無言で編集が消える」戸惑いを防ぐ。**メタデータ・コピー方式 (編集メタを新ページ
  path キーへコピーして mIV 表示上は再現) は不採用**: 生ファイルは無編集のままなので
  直アップ安全にならず、隠蔽で「mIV では隠れて見えるのに生ファイルは無修正」という
  誤アップロード (流出) を誘発するリスクがあるため (ユーザー裁定 2026-07-11)。
