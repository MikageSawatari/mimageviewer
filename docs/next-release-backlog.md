...
- **実装済み (2026-08-23、レーン C、branch `panorama-projection`)**。着手可否を利用者へ再確認し、
...
- **第 3 段 (入力) — レーン B との衝突は「予告ほどではない」(2026-08-24 実測で訂正)**。
...
- **入力設計の決定** (2026-08-24、Codex Sol と相談のうえ確定):
...
- **未決**: **360 ON をファイルをまたいで保持するか**。Codex は source-scoped (別ファイルへ
  移ったら改めて明示 ON) を勧める。理由は、Hint (2:1 のみ) の通常動画へ ↑/↓ で移ったときに
  「明示 ON が要る」という条件を迂回して自動投影してしまうため。
  ⚠ ただし**静止画側は既にまたいで保持する実装で出荷済み**
  (`is_panorama_mode_active` = `panorama_state.is_some() && detect_panorama(idx).is_some()`、
  detect には Hint も含む)。source-scoped にすると静止画と動画で挙動が分かれる。
  **一貫性を取るか、誤爆を防ぐかの判断**なので、実装前に利用者へ確認する。
