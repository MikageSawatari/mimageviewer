...
- 出典: X の利用者要望 (2026-09-04)。外部ツールへ `.py` や `.ahk` も直接登録したい、
  という要望。
- **利用者判断 (2026-09-04)**:
  - 起動は A 案を採用した。まず従来どおり `Command::spawn` を試し、Windows の
    `ERROR_BAD_EXE_FORMAT` (193) のときだけ `ShellExecuteExW` の `open` verb へ
    フォールバックして、関連付けを OS に解決させる。自前の `ftype` 相当解決は持たない。
  - 起動経路は利用者に選ばせず、登録したものを直接起動できるかどうかで自動的に切り替える。
  - ツールごとの「コンソール窓を表示する」を追加し、既定は OFF とした。直接起動と
    関連付けフォールバックの両方で同じ意味にする。
- 選択ダイアログは「実行ファイル (*.exe)」を既定に保ち、「すべてのファイル (*.*)」を追加した。
  スクリプト拡張子の固定リストは持たず、起動できる範囲の正本を spawn と OS の関連付けに一本化した。
- Shell へ渡す引数は CreateProcess 互換の quoting で連結する。実際に quote する関数を正本とし、
  直接起動のコマンドライン長判定も同じ生成結果から求める。
- `external_tools` は v3.5.0 で出荷済みのため、開発中の shape marker による
  `DROP TABLE` 経路を撤去した。`show_console` は
  `ALTER TABLE ... ADD COLUMN ... DEFAULT 0` で追加し、既存登録を保持する。
- 回帰テストは quoting (空文字 / 空白 / quote / 末尾・連続 backslash)、193 の判定、
  v3.5.0 相当 DB を開いたときの列追加と登録保持を固定する。
