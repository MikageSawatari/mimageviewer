...
> 省略の述語は `設定 ON && DeleteConfirmKind::RecycleBin && DeleteTargetScope::ListedFilesOnly`。
> **フォルダと ZIP / PDF / 対応アーカイブは、設定 ON でも確認を残す** — タイル 1 つが一覧に
> 見えていない多数の内容を持ち、既存の確認が削除範囲を見直せる最後の機会だから。
