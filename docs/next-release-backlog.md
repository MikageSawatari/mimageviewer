...
### 1.125 見開き表示のまま Ctrl+G で補正レイヤーへ入ると、左右の切り替えもマスク編集も効かない — 実機報告 ✅ 修正済み (2026-08-26、実機確認待ち)
...
- **残り**: テキスト注釈には L/R セレクタ自体が無い。パネルへ追加する必要があり、ページ
  切り替え時に `comic_docs` を読み直す経路 (`switch_erase_target_in_spread` 相当) も要る。

### 1.126 フォルダ切替でサイドカーの読み書きが UI スレッドを 8 秒止める — perf-log で実測
...
- **再現条件の見立て**: 直前まで消しゴム / 隠蔽 / 注釈 / 補正レイヤーで編集していた。dirty な
  サイドカーが溜まっているほど flush が伸びると考えられる。**未検証** — 着手時に、編集した
  ページ数とサイドカーの件数・サイズを計装して確かめること (件数なのか 1 件あたりのサイズ
  なのかで対策が変わる)。
