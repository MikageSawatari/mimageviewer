...
- **進捗 (2026-09-25)**: 評価の 1 段階増減 (項目 / 入れ物の上げる・下げる、操作カスタマイズ、既定キーなし) は実装済み (`8878aca2b`、v4.1.0)。
  **レーティング昇順 / 降順と未評価の比較位置は v4.1.0 に含めず次版以降** (利用者決定。規模は `docs/rating-snapshot-order-plan.md` の見積もりで約 3.5〜6 開発日・16〜22 ファイル、設計は独立レビュー前)。
