...
- **見込みは軽い (2026-08-13 調査)**: 判定の本体は既に純関数として存在し、テストも付いている。
  ...
  - 作業の本体は**プルダウン側のクリックを同じ op 解決へ通すこと**で、判定ロジックの
    新規実装は要らない。
- 着手時に確認すること:
  - プルダウンは checkbox の集合なので、修飾クリックの当たり判定をどう取るか
    (checkbox の `response` から修飾を読む形になるはず)。
  - 修飾の対応表をツールバーと**完全に一致**させること。片方だけ違うと、同じ★を
    押しているのに結果が変わって見える。
  - 修飾が使えることをどう伝えるか (ツールチップに併記するのが既存の作法)。
- 規模 / 優先度: Small / P2。**軽微に収まれば次リリースに入れたい** (利用者判断)。
