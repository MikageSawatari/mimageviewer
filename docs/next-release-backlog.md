...
### 3.4 バケツを「全体 / 隣接のみ / 長方形」の 3 タイプにする — 調査済み / 未実装

- 出典: 利用者要望 2026-08-20 (調査のみ、実装は未着手)。目的は**付箋が貼られている / 黒塗りで文字が
  消されている箇所の隠蔽の見栄えを良くすること**。バケツで塗った領域を長方形に整形し、領域の欠け
  (中の文字・角の丸み) は埋め、はみ出しは切る。長方形は傾いていることがある。
- **対象は「中身が一様な長方形」に限定する** (利用者判断)。写真やテクスチャ入りの矩形を画面から
  検出する用途は外すので、エッジ検出は要らず **flood fill の結果を整形するだけ**で足りる。
- 決まっていること:
  1. UI は排他 3 択「全体 / 隣接のみ / 長方形」。長方形は隣接のみの後段処理なので制約が段階的に
     強くなる並びになり、「全体 × 長方形」(= 画面中に散った同色画素の外接矩形 ≒ 画像全体) という
     無意味な組み合わせも自然に塞げる。「漏れ止め」は隣接のみ / 長方形の両方で出す。
  2. **出力はビットマップ** (`Shape::Rect` にしない)。Shape だと自動生成物が増え続け、後から直したい
     ときにハンドル操作のほうが煩わしい。加えて消去モードの Shape は `ShapeOp::Subtract` になり、
     その矩形の範囲で下地のビットマップも削り続ける (既存の矩形ツールと同じ挙動) ため、「バケツで
     消す」の感覚から離れる。ビットマップなら矩形の画素を書くだけで済む。
- 現状の構造 (source inspection 2026-08-20):
  ...
  - `BucketFill.connected: bool` → `region: BucketRegion { Whole, Connected, Rect }` への置き換えは、
    **`BucketFill` が実行時の値で永続化されていない** (App のセッション内保持のみ、settings.db にも
    無い) ため**移行コード不要**。
  - 矩形のラスタライズは既存部品で足りる: [mask_db.rs:453](../src/mask_db.rs:453) `rect_corners` +
    [mask_db.rs:1137](../src/mask_db.rs:1137) `scanline_fill_polygon`。Undo はビットマップと Shape の
    両方を snapshot 済み ([ui_conceal.rs:332](../src/ui_conceal.rs:332))。
  - 凸包 / rotating calipers 相当のヘルパーは**リポジトリに無い**ので新規。純関数モジュールに置けば
    合成ビットマップ (既知の角度・サイズの矩形を描いて復元誤差を見る、漏れの尻尾を付けて拒否される
    ことを見る) で単体テストが書ける。
- 当てはめの段取り:
  1. 連結領域を求める (既存 flood fill、漏れ止めもそのまま有効)
  2. 境界画素 → 凸包 → rotating calipers で最小面積外接矩形 (最小面積矩形の 1 辺は凸包の 1 辺と平行
     なので、走査は凸包の辺数ぶんで済む)
  3. **被覆率ガード**: `領域の画素数 / 矩形の面積`。付箋・黒塗りなら 0.95 以上になるはずで、隙間から
     地の紙へ漏れると矩形が伸びて比率が落ちる。しきい値未満なら**塗らずに理由を返す**
     (`BucketFillOutcome` に variant を追加。既存の `SeedTooThin` と同じ流儀)
  4. **角度スナップ**: 0°/90° から 1〜2° 以内なら 0 に丸める (スキャンのわずかな傾きで 0.4° 回転した
     矩形が入るとかえって汚く見える)
  5. **はみ出し (px)**: 付箋・黒塗りの縁は JPEG のにじみ / アンチエイリアスで許容差から外れ、1〜2px
     取り残されて隠蔽後に輪郭として見える。矩形なら `half_w + m` で厳密に効く
- **追補 (2026-08-20): 楕円 / 円も同じ枠組みで足せる**。長方形より軽い。
  - ラスタライズは既存の [mask_db.rs:478](../src/mask_db.rs:478) `scanline_fill_ellipse` (回転対応、
    ビットマップへ値を直接書く) がそのまま使える。円は `rx == ry` の楕円。**出力側は新規コード 0 行**。
  - 当てはめは凸包が要らず、**2 次モーメント 1 パスで解析的に解ける**: 領域画素の重心と共分散
    `σxx, σyy, σxy` を累積し、2×2 の固有分解を閉形式 (`θ = 0.5·atan2(2σxy, σxx − σyy)`) で解く。
    一様な楕円板は軸方向の分散が `(半径)² / 4` なので **半径 = 2√λ**。真円 / 楕円の領域なら角度込みで
    元の楕円が復元される。円モードは等方化するだけ (`r = √(a·b)` または重心からの距離のパーセンタイル)。
  - 被覆率ガード (`領域の画素数 / (π·a·b)`)、漏れ対策のパーセンタイル (重心からのマハラノビス距離)、
    はみ出し px (`rx + m, ry + m`) は長方形と同じものを流用できる。
  - **楕円固有の注意**: 軸比が 1 に近いと固有ベクトルの向きが数値的に不安定になるので、`a/b < 1.05`
    程度なら**円として扱い回転 0 に固定する**。入れないとほぼ円の領域でクリックのたびに角度が跳ねる。
  - 追加コストは長方形の作業に対して **+50 行程度**。テストは既知の角度・軸長の楕円を描いて復元誤差を
    見るだけで書ける。
  - UI は 3 択が 5 択 (全体 / 隣接のみ / 長方形 / 楕円 / 円) になる。軸としては「範囲の決め方」と
    「整形」の 2 軸だが、整形は連結領域にしか意味がないのでフラットな 5 択で破綻しない。
    「**自動**」(長方形 / 楕円 / 円のうち被覆率が最大のものを採る。領域抽出は共通なので追加コストは
    わずか) も成立するが、利用者が形を決めている場面で別の形が選ばれると事故になるので**既定にはせず、
    明示 5 択に加える形**にする。用途は顔の隠蔽 (楕円) / 丸い判子・ロゴ (円)。
    [conceal-feature-plan.md](conceal-feature-plan.md) の用途表でも「顔 = 楕円」が挙がっている。
- 着手前に決めること:
  - ローカル調整にも長方形モードを出すか (enum 化すれば判定は数行だが、UI ブロックは別途必要)
...
- 着手時に読む: [conceal-feature-plan.md](conceal-feature-plan.md) のツール表 (バケツの行)。実装したら
  ...
  `KeyAction` 追加は不要。
- 規模 / 優先度: 小〜中 (整形の純関数 150〜200 行 + 3 系統の enum 化と UI) / P3 (新機能)。
