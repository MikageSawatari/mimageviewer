...
### 1.0j 貼り付けの残りが遅れて届いたときに拾えない — R-09 (2026-08-30)
...
- **Shell の完了通知**: `IFileOperation` の進捗を受ける。貼り付けは Shell が
  所有しているので、こちらが数えるのではなく**向こうに聞く**のが筋。ただし現在は
  `invoke_shell_folder_background_verb` で verb を投げるだけなので、経路が無い
- **同じ集合を再適用しない state**: レビューの提案。要求を保持しつつ、既に選んだ
  ものを覚えて差分だけ足す。完了判定は要らないが、要求がいつまで残るかは別途要る

**実害の大きさ**: 選択がずれるだけでデータ損失は無い。時間窓で誤魔化すより、
Shell に聞く経路を作るかどうかの判断を先にする。
...
| R-08 / R-09 | paste 後の選択 request の lifecycle | いいえ | 選択がずれる / 不完全。データ損失なし。**R-08 は v3.3.1 で対応済み** (Shell 呼び出しが即失敗したら要求を取り下げる)。**R-09 は未対応** — 遅れて届く残りのコピーを拾うには「いつ完了か」の方針が要る (下記) |
| R-11 | トレイ退避が sidecar 書き込み完了を UI で待つ | いいえ | `df245720` で writer 側の複製 1 回分は減った。待つこと自体は残る |
...
**(b) と (c) に着手するときの注意** (見積もりを 1 度外している):

- (c) は「複製をやめて直接書き換える」だけでは済まない。塗りの closure には
  `create=true` でマスクを**作ってから false を返す**経路があり、直接書き換えると
  空マスクが残って「編集していないのに編集済み」になる (R-05 と同型)。マスクの
  実体化を closure の外へ出すこと。塗り本体 `paint_local_adjust_alpha_line` は
  **変えたときだけ書く**ことを確認済みなので、外へ出せば厳密に等価にできる。
