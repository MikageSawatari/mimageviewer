...
### 2.9 一覧のダブルクリック間隔を Windows 設定へ合わせる — 専用スレ >>253-254
...
- 規模 / 優先度: Small / P2。次バージョン以降で対応すると回答済み。
...
- 原因 (特定済み): [displayed_image_transform.rs:801](../src/displayed_image_transform.rs:801) の
  `z_cursor_image_px` が、カーソルを **`pan_band` 内の割合**で画像座標へ写している。カーソルの
  真下にある画像上の点を求めているわけではない。
  1. `pan_band` は上部バー / 下部シークバーの高さぶん**上下だけ内側に詰めてある**
     ([ui_fullscreen.rs:6398 付近](../src/ui_fullscreen.rs:6398)、「カーソルがバーの反応領域へ入る前に
     画像の上端・下端へ到達できるように」実機 FB 2026-06-21)。狭い帯を画像全体へ引き伸ばすため、
     **縦は手の移動よりも枠が速く動く**。
  2. 画像がレターボックス表示のときは、左右の余白ぶんも画像内へ写り込むので横にも同じずれが出る。
  3. さらに `z_visible_source` が拡大窓の中心を画像内へクランプする (余白を見せない設計)。
     **画像の端では原理的に中心をカーソル位置へ置けない**。これは仕様として残る。
- **方針 (利用者判断 2026-08-19): 照準中 (`fs_zoom_aiming`) はマウスカーソルを隠す。
  マッピングは変えない。**
  - 照準枠 `z_aim_frame_rect` と確定後の可視範囲は **同じ `cursor_image` と同じ `z_visible_source`**
    を通っているので、「枠に見えている範囲＝離した後に出る範囲」は既に一致している
    ([displayed_image_transform.rs:895](../src/displayed_image_transform.rs:895))。知覚されている
    問題は「枠がカーソルの真下にない」の一点だけなので、基準であるカーソルを消せば解消する。
  - マッピングを変えないので、**Z を離した瞬間のジャンプが発生しない**。1:1 化も HUD 抑止も
    ラッチも要らない。
  - 実装は既存の仕組みに乗る。フルスクリーンの自動非表示が使っている
    `ctx.set_cursor_icon(CursorIcon::None)` + `ViewportCommand::CursorVisible(false)`
    ([ui_fullscreen.rs:17069](../src/ui_fullscreen.rs:17069)) を、照準中だけ立てる。
  - 残る差は**ゲインの非対称** (帯が上下だけ狭いので縦が敏感)。カーソルという基準が無ければ
    「縦が少し敏感」程度の感触に留まる見込み。実機で気になるようなら次段階へ。
- 採らなかった案と理由:
  - **照準中だけ 1:1 マッピング + HUD 抑止**: 離した瞬間に帯マッピングへ戻るので枠と着地がずれる。
    防ぐにはラッチが要るが、ラッチはポインタを動かした瞬間にジャンプするので不満が移動するだけ。
  - **確定後も帯を撤廃する**: 中心が画像内へクランプされる関係で、カーソルが到達すべき範囲は
    倍率 f のとき画像の中央 (1 − 1/f)。f=2 で中央 50%、f=4 で 75%、f=8 で 87.5% と、
    **高倍率ほど端に寄る必要がある**ため HUD のホバー領域と衝突する。確定後の帯は必要。
  - **`SetCursorPos` でカーソルを動かす**: mIV には現在カーソル位置を動かす経路が無い
    (制御は「隠す」側だけ)。物理ピクセルと DPI の換算、合成 `WM_MOUSEMOVE`、手と画面の位置ずれが
    付いてくる。上記で解決しなかったときの最終手段。
- 確認項目: 離したらカーソルが戻る / Z を押したまま対象外モードへ抜けたときも戻る
  (`update_fs_zoom_mode_keys` のリセット経路) / フォーカス喪失時 / 既存のカーソル自動非表示と
  二重管理にならないこと。
- 規模 / 優先度: Small / P2。
