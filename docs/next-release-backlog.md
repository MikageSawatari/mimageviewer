...
- **残り**: 倒した後は `resolve_spread_pair` が Single を返すため、補正レイヤーパネルの
  L/R セレクタが出なくなる。消しゴムの `switch_erase_target_in_spread` に相当する「退避した
  ペアを使う切り替え」が補正レイヤーには無いので、左ページは編集できるが**右ページへ切り替える
  手段が無い**。実機確認で必要と判断されたら追加する。
