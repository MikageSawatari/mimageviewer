...
### 1.257 スマートフォルダの PDF が開けない・子フォルダから戻ると一覧の先頭へ戻る — 利用者報告 (2026-09-20)

- 対応中（根因調査）。スマートフォルダに表示した PDF を開こうとすると、PDF ではなく
  スマートフォルダ一覧が再表示される。実フォルダは中へ入れるが、Backspace で戻ると
  一覧の先頭へ戻される。利用者によるとコレクションでは同じ問題は起きない。
- 期待: PDF は通常の閲覧経路で開き、親へ戻ると元のスマートフォルダの選択・表示位置を
