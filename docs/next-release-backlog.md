...
1. **エクスプローラーでサムネイルが出るか利用者に聞く。** mIV は Shell へ丸投げなので、
...
3. 1 と 2 の結果で、**3gp だけ足す / 全部足す / 見送る**を決める。
...
- 規模 / 優先度: 拡張子追加のみなら Small / P3。**判断が先。**
...
### 1.233 サイドカー復元待機で画像フルスクリーンのフォルダ移動がちらつく (2026-09-13)

- **P1・対応中、公開保留の先行修正**。Ctrl+上下で次のフォルダへ移る際、前の画像が消え、一覧や黒い読み込み表示を挟む。v3.9.0からの退行。
- §1.209の非同期化で再openをsidecar ownerへ預けた一方、navigationの表示保持側はその待機を認識していなかった。コード・ログ・独立担当とClaudeCodeの分析が一致。
- exact context/generation/itemの再open待機を既存の表示継続へ統合する。非同期復元・初回の補正反映・入力gate・データ保護は維持する。
- [調査と修正記録](fullscreen-folder-sidecar-transition-investigation-20260913.md)。通常一覧・別context・cancel/errorへの回帰、自動検証、独立レビュー、確認buildと利用者確認が残る。
