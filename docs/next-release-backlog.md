...
- **`render_page_async` はフォールバックではない**。`worker_count` を見ずに常に
  ...
  では**意図的に残した**。
- 移すかどうかの論点:
  - 移す利点: 先に作った Critical 予約・レーン分割・昇格・epoch 間引きが再レンダリングにも効く。
    PDFium がメインプロセスから消える。同じ viewer が経路を 2 本持つ状態が解消する。
  - 移すリスク: 今は再レンダリング専用の資源 (サムネイルと競合しない) なのが、共有プールに
    なる。**Critical 予約は 1 ワーカーを無条件で確保する** (`CRITICAL_RESERVATION_ACTIVE` は
    v1.0.0 から常時 ON) ので理屈上は待ちの最悪形が変わらないが、**未実測**。
  - 現状の in-process 側の弱点: 1 スレッド逐次なので、現ページ・見開き相方・AI 先読みが
    互いに待つ。priority チャネルはあるが実行中の render は横取りできない。
- **先に実測する**: どちらが速いかは測れる (再レンダリング要求から完了までの latency を、
  サムネイル先読みが走っている状態と静止状態で比較)。§2.13 の計装と同時に入れると安い。
