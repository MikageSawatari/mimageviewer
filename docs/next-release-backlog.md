...
| R-14 | ✅ (残存) doc comment に明記済みの設計上のトレードオフ | 中 | **R-07 と 1 件として扱い、対応済み** (`a6367fb1` / `c093f0d7`) |
| R-07 | ✅ 既知 | 中 | **(b)(c)(d) すべて対応済み** (`116ca8cd` / `a9f69d14` / `976aef4a` / `d6a591b2` / `368499d9`) |
...
- **R-14 と R-07 は同じ構造問題** (編集文書という大きな値を UI スレッドが所有して複製する)。
  ...
  → **その通りに実施した** (2026-08-31)。文書を `Arc<Vec<LocalAdjustmentLayer>>` の
  共有所有にし、保存 worker が最新 generation だけを書き、`Committed` のときだけ
  完了側が sidecar ミラーを書く。経緯と判断は
...
| ID | 内容 | 退行か | 実測 / 根拠 |
| --- | --- | --- | --- |
| R-07 (d) | 補正マスクの圧縮・DB 保存が UI スレッド | v3.2.0 以前から。**v3.3.0 が 5.8 倍速く**した後 | 24MP で 70.6ms。**ストローク単位ではなくフレーム単位だった** (`Slider::changed()` はドラッグ中毎フレーム真)。**対応済み** (`976aef4a`) |
| (b) | 図形のキー移動 / 回転がキーリピートごとに全文書を保存 | v3.2.0 以前から | リピート 1 回ごとに 70ms + undo 2 文書。**対応済み** (`368499d9`) |
...
**(c) 対応済み** (`116ca8cd` / `a9f69d14`)。以下は着手前の調査記録で、**契約を変えた理由**
としてそのまま有効。実際に何をしたかは
[briefs/local-adjust-ownership-progress.md](briefs/local-adjust-ownership-progress.md)。
...
→ **対応済み** (`976aef4a`)。判断を `App::apply_local_adjust_write_completion` の 1 か所へ
集約し、`Committed` のときだけ sidecar ミラーを書く形で境界を保った。非同期化で新しく
必要になった 3 点 (ミラー先座標を積む時点で固定 / worker を通さない key 付け替えの前に
drain / 終了時に在庫を書き切ってから flush) は
[async-architecture.md §5.7](async-architecture.md) に一般則として書いた。
**同じ形の保存を非同期にするときは 3 点とも要る。**

**(b) の結果** (`368499d9`。着手前の見積もりを 2 点外していた):

- 畳む箇所は 14 ではなく **8** (`local_adjust_mode = false` を書く箇所)。ただし内訳は
  想定より悪く、**5 箇所が破棄・3 箇所は何もしていなかった**。
- **ブラシも破棄してはいけなかった。**塗った画素はメモリの文書に入っているだけで、
  保存は `persist_*` でしか行われない。破棄すると保存されず、
  `start_loading_items_inner` はそのあと文書ごと消す。4 種類とも確定する形にした。

### 残っている 1 件 — 補正パネル編集器の毎フレーム複製
...
複製する (24MP で最大 3 枚 = 288MB/frame)。(c) と同じ「複製 → 貸す → 変わっていたら
publish」の契約。

**(b) で畳むつもりだったが別物だった。**(b) の owner は「保留中のジェスチャ」を確定する
もので、編集器の作業用コピーとは扱う対象が違う。

有望な形は `RasterVectorMask::alpha` を `Arc<Vec<f32>>` にすること。編集器はマスクの
画素を書かないので、共有すれば `layer.clone()` が参照カウントの複製になる。フレーム内の
順序が塗り → パネル描画なので、塗りの `Arc::make_mut` は refcount 1 で無料になる。
検討して効かないと分かった案 3 つを含め、詳細は
[briefs/local-adjust-ownership-progress.md](briefs/local-adjust-ownership-progress.md)。
...
**教訓**: 狭いテストで 3 度続けて取りこぼした。抜けていた次元は順に、ページ幅 → **表示設定**
(トリム) → **表示中の解像度**。どれも「利用者の環境で何が起きているか」を先に聞けば
分かったもので、コードだけ見て走査を組んだのが誤りだった。利用者の実ファイル・実設定・
実際の遷移を最初に確認する。
