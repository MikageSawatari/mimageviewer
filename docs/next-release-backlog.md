...
### 1.1 ダウンロードページに更新日付と更新履歴を表示

- 背景: 5ch レス 783。初見ユーザーにはダウンロードページだけだと、現在も更新されているソフトか判断しにくい。
- 要望:
  - ダウンロードページのバージョン表記に更新日付を併記する。
    - 例: `mImageViewer v1.6.1 (2026/06/15)`。
  - ダウンロードページから更新履歴を確認できるようにする。
- 方針:
  - 次回リリース時の `htdocs/mimageviewer/index.html` 更新に含める。
  - 更新履歴は README の履歴と二重管理になりすぎない形を優先する。必要なら README の該当セクションへ誘導、または直近数件だけをサイト側に掲載する。
- 優先度: P2。次回リリース作業で対応。
