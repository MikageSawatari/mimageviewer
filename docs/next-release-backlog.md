...
#### 別件として切り離すもの — os error 32 (**未再現**)

利用者報告にあった「**別のプロセスが使用中です (os error 32)**」は、**上とは別の失敗**。
**2026-09-13 時点で再現できていない。** 観測されているのは報告者の 1 回だけで、
そのときの操作 (どのボタンを押したか) も分かっていない。**開発機で再現できたのは上の
vunknown の検証失敗のみで、それは os error 32 とは別の症状。**両者を同じ節にまとめない。

到達経路が分かっていない。**検証段階ではファイルに触れないので共有違反は出ない**
...
`full_reset` の `remove_file`)。しかも**ダウングレード直後は全世代が NewerVersion で
`validate_backup` に落ちる**ので、「この時点に戻す」からは file 操作へ到達しないはず。
**残る候補は「初期状態に戻す」(`full_reset`) 側**だが、報告からは判別できない。
**まずどちらのボタンだったかを確認する。**

静的読みで**排除できたもの** (ハンドルが残る経路として考えたもの):
...
**「旧版のプロセスが残っていた」を説明として採らない。** 復元はダイアログから行う以上、
**動作中のプロセス自身が自分の `settings.db` を差し替える**のが正規の流れで、それは設計どおり
(ダイアログにも「復元すると現在の設定は上書きされ、アプリは自動で終了します」と出る)。
「プロセスが動いていた」こと自体は異常ではないので、原因の説明にならない。
**ただし否定もできていない** (ポータブル版は mutex が分かれるので同時起動は可能)。
**確かめずにどちらへも倒さない。**

**次にやること。** 推測ではなく観測から始める。**失敗した操作と対象パスをエラーメッセージへ
出す** (`RestoreError::Io(e)` は今どのファイルの話か言わない)。報告者に押したボタンを聞く。
どの段で落ちたかが確定してから直す。
