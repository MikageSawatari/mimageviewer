...
- 原因: [ui_fullscreen.rs:21538 付近](../src/ui_fullscreen.rs:21538) で、サンプルする uv 矩形の
  ...
  引き伸ばすため像が歪む。
- 方針 (どれを採るか決める):
  - (a) uv 窓を**クランプせずスライド**させ、サンプルサイズを一定に保つ。歪まないが、端では
    ルーペ中心とカーソル位置がずれる (画素検分用途では嘘になる)
  - (b) uv のクランプは残し、**描画先を同じ比率で縮める** (レターボックス)。実装が最も小さい
  - (c) 倍率を一定に保ったまま、画像の外側は背景色で埋める。位置は正しく、歪まない
  - 現状の用途 (画素検分 + 端の確認) なら **(c) を第一候補**、簡易に済ませるなら (b)。
- 規模 / 優先度: Small / P2。
