...
| R-07 | ✅ 既知 | 中 | **(c)(d) 対応済み** (`116ca8cd` / `a9f69d14` / `976aef4a`)。**(b) は未着手** |
...
| (b) | 図形のキー移動 / 回転がキーリピートごとに全文書を保存 | v3.2.0 以前から | リピート 1 回ごとに 70ms + undo 2 文書。**未着手** (下の注意を読むこと) |
...
- **畳む箇所は 14 か所**あり (`src/app.rs` 4 / `src/ui_adjustment_panel.rs` 9 /
  `src/ui_fullscreen.rs` 1)、**その多くが `= None` で破棄している**。上の警告どおりの
  形になっている。
- **同じ段で直すものがもう 1 件ある。**`draw_selected_local_adjust_layer_editor`
  ([ui_adjustment_panel.rs](../src/ui_adjustment_panel.rs) `3566` 付近) の
  `let mut edited = layer.clone();` が、補正パネルを開いている間**毎フレーム** 1 レイヤーを
  複製する (24MP で最大 3 枚 = 288MB/frame)。(c) と同じ「複製 → 貸す → 変わっていたら
  publish」の契約で、しかも**編集器はマスクの画素を書かない**ので複製の大部分は無駄。
  `Arc` 化では直らない (egui のウィジェットが毎フレーム `&mut` を要求するため)。
  正解は「編集中のレイヤー」を単一の state owner に持たせることで、**(b) が作ろうと
  している owner と同じもの**。調査の詳細は
  [briefs/local-adjust-ownership-progress.md](briefs/local-adjust-ownership-progress.md)。
