...
- 対応 (2026-08-05): 利用者判断どおり **(A) と (B) の両方**を実装した。
  ...
  主数字は同名設定で畳んだ数 + 隠し項目数とし、対象外拡張子はクリック時の内訳だけに出す。
  絞り込みバーを隠した場合はアドレスバーへ移し、サブ展開 / スマートフォルダ / 検索結果 /
  スタック表示は後追い範囲として表示しない。件数取得専用の I/O は追加していない。
...
- 採用方針:
  ...
  - **(C) 一覧末尾に「他 N 件非表示」セル (利用者案) — 採らない**。仮想スクロールで末尾へ
    ...
    情報量に対して影響範囲が大きい。
- 主数字: 「対象外の拡張子」まで数えると `Thumbs.db` 等でノイズになるため、内訳では分けて
  数え、チップの主数字は正規化 + 隠しに限る方針を採用した。
