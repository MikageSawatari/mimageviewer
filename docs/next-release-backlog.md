...
### 1.146 波形ストリップが消えた後、再表示に時間がかかる — 利用者報告、再現条件の確認待ち

- 出典: 利用者報告 (2026-08-31)。「一度音声波形ストリップの表示範囲から外れると、また
  音声波形を ON させないといけない。もう一度 ON すると音声データのロードから始まり、
  表示までに時間がかかる」。
- **前提の訂正**: 波形ストリップは音楽ビューの `TimelineAnalysis` とは**別の仕組み**。
  専用ワーカー ([seek_strip_wave.rs](../src/video/seek_strip_wave.rs)) が
  `AudioRangeDecoder` でファイルを 1 回だけ開き、要求ごとに seek して**必要な範囲だけ**
  デコードする ([audio_decode.rs:153](../src/audio_decode.rs:153))。全尺デコードではない。
- 表示状態は既に設定へ永続化されている (`Settings::video_seek_strip_state` /
  `video_seek_strip_last_choice`)。したがって「ON 状態が消える」のが何を指すかが不明。
- **報告者へ確認する 4 点**:
  1. 何が消えるか — ストリップごと消えるのか、サムネイル列に戻るのか、
     `Shift+S` の状態が「なし」に戻るのか
  2. 「表示範囲から外れる」の意味 — マウスが HUD から離れた / シークで波形の描画済み範囲の
     外へ出た / 別の動画へ移った、のどれか
  3. 同じ動画のままか — 動画を変えずに再 ON したときも遅いのか
  4. 待ち時間 — 初回表示までと再 ON の表示までが、それぞれ何秒くらいか
- 回答が来るまで着手しない。**推測で worker のライフサイクルを触らない**。
- 規模 / 優先度: 不明 / P2。
