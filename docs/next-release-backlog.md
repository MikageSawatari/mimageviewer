...
**残り (Sol のテスト提案、未実施)**: production 配線を通した回帰 (描画 →
`FullscreenPageLayout` → 範囲キャプチャ)、rotation / Original / 非ゼロ viewport 原点、
左右で違うテクスチャ倍率、片側だけの trim。detached continuous frozen も同型
(paged frozen と同じく clip の所有が別なので、そちらの契約変更と一緒に扱う)。
