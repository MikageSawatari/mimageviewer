...
- リリース目安: この対応だけなら小規模な機能補完として v1.8.1 候補。他の修正 / 機能追加が増える場合は
  v1.9.0 に束ねる。
- 目標:
...
- 画像側の仕様:
  ...
  - 見開きで左右が同じコンテナに属する場合 (ZIP/PDF/変換アーカイブ): 1 つのタグセクションだけを表示し、
    その本体へ付与 / 解除する。
  - 画像フォルダの通常画像を見ている場合だけ、右パネルのタグ欄を 2 段にする。
    - `フォルダ`: 現在の画像フォルダ本体へタグを付ける。作品 / 作者 / ジャンル / 未読など、
      そのフォルダ全体を表すタグを想定する。
    - `ページ`: 表示中の画像ファイルへタグを付ける。単ページではその 1 ファイル、見開きでは
      左右 2 ファイルを同時に対象にする。片側だけ選ぶ UI は初期スコープに入れない。
  - 画像フォルダ見開きの `ページ` 行では、左右どちらか片方だけに付いているタグを mixed 状態として
    表示する。mixed をクリックした場合は左右両方へ付与する。削除は、両方 ON の状態でクリックすると
    両方から削除する。必要なら右クリックメニューで「左右両方から外す」を追加する。
...
- 実装メモ:
  - 静止画側の `ui_metadata_panel.rs` にはすでに `current_tag_target_path` があり、
    ...
    同じ rect で更新する。
  - `tag_ops.rs` のフルスクリーン対象解決は現在 `fullscreen_idx` 1 件のみ。画像フォルダ見開きの
    `ページ` 行では右パネル専用に左右 2 ファイルの target path 群を渡す経路を追加し、
    古いグリッド選択や checked に影響されないようにする。`フォルダ` 行は現在フォルダ path を
    target にする。
...
- 確認項目:
  ...
  - 画像フォルダでは `フォルダ` 行がフォルダ本体、`ページ` 行が単ページまたは左右 2 ファイルに
    付くこと。見開きで片側だけタグ付きの場合は mixed 表示になり、クリックで両方 ON になること。
  - 右パネル上のホイール / クリックがページ送りや動画操作へ漏れないこと。
  - Ctrl+Z / Ctrl+Y でタグ変更を戻せること。
- 優先度: P2 / v1.8.1 または v1.9.0 候補。
