...
### 1.230 Ctrl+G の案内が、存在しない設定名を指している (2026-09-12)

- 出典: §1.229 と同じ利用者質問 (2026-09-12) の調査中に見つけた。
- Ctrl+G のツールチップは「お気に入り編集で『**アイテムを索引化する**』を有効にしてください」と
  案内する ([global_search_ui.rs:2260](../src/global_search_ui.rs:2260))。**この名前の項目は無い。**
- 実際の表記は、お気に入り編集ダイアログの列見出し「**アイテム索引 (Ctrl+G)**」
  ([favorites_editor.rs:479](../src/ui_dialogs/favorites_editor.rs:479))。チェックボックス自身の
  ラベルは空文字で、名前を持っているのは列見出しだけ
  ([favorites_editor.rs:591](../src/ui_dialogs/favorites_editor.rs:591))。
- この索引は既定 OFF ([settings.rs:336](../src/settings.rs:336)) なので、**Ctrl+G を初めて使う人は
  必ずこの案内を読む**。そこに書かれた名前がダイアログに無いと、探しても見つからない。
- 対応: ツールチップの文言を列見出しに合わせる。同じダイアログの「コンテナ索引 (Ctrl+S)」
  「別バージョン索引」とも表記を揃える。
- 規模 / 優先度: Trivial / P3。
...
- 出典: 利用者質問 (2026-09-12)。「Ctrl+G で EXIF のカメラ機種名を検索したいが出てこない」。
  回答のためにコードを確認して見つけた。**利用者はまだこの経路が原因だと確定していない**
  (索引が未作成の可能性もある) が、下記はコードから確定した独立の欠陥。
...
1. **1 文字語**: bigram を作れないので構造的に不可。語ごとの下限として残す。
2. **2 文字の英数字語**: AND モードでは「長さを満たす語が 1 つ以上あれば通す」に変えるか。
   OR モードは 1 語が候補を広げる側なので、語ごとの検査を残すのが素直。
   **この 1 と 2 は別の問いなので、同じしきい値で表さない。**
3. **メッセージ**: 短かった語を出す (例: 「"r5" が短すぎます」)。1 / 2 をどう決めても要る。
...
- 1 文字語を含むクエリは拒否される (`build_bigram_and_query` の `None` へ落ちない)。
...
2026-09-12設計検収: 対象を起動時の`FullReason::Initial`に限定する。全体確認開始時に既に
取得しているinventoryを使い、コンテナごとの追加DB読取はしない。保存ページ数とmember件数、
kind、完全性も事前判定に含め、不整合なら従来の列挙経路へ戻す。パスワード変更を含む
Reconfigure、修復、手動Full、Deltaは従来どおり開く。これにより再認証や修復の意味を維持する。
...
- 規模 / 優先度: Small〜Medium / **P2**。v3.9.1の次リリースで最優先 (利用者指定)。
  実装・全体テスト・独立レビュー・確認build完了。9月13日の利用者起動ログではInitial Full約42秒（旧観測391秒）、後続Deltaは6～208ms。条件固定のA/B計測ではない。
  [設計と検証記録](section228-similar-container-preopen-plan.md)を参照。
...
- 2026-09-13実機確認後の追加指定: 下記の初回hover後に消す案を撤回し、境界を常時表示する。
  ドラッグ中・非ドラッグ中とも、既定Ctrlの非表示操作を押している間だけ隠す。
...
- 改善案: ワイプ比較へ入ったときは境界線・操作ハンドルを表示し、境界へ初めて意図的に
  ...
  明暗の境界線とハンドルを採用し、時間による消去はしない。
- 表示中の画像全体へのホバーだけでは案内を消さない。開始時に偶然カーソルが境界上にある場合も、
  最初から案内が見えなくならないよう扱う。ドラッグ中の線表示とCtrl併用時の線非表示は維持する。
- 初期案内は既存の比較セッションと同じ所有境界に置き、比較を終了して入り直したときは再表示する。
  2026-09-12コード照合: 現比較はApp-globalの単一foreground sessionで、detached park時に終了し、
  passiveはfrozen表示、temporary context mountでは入力しない。この契約を維持し、案内だけを別contextへ
  分離しない。ウィンドウごとに比較を保持・再開する機能は今回追加しない。
  Wipe開始は既存`FsCompareWipe`のhelperへ統一し、存在しないメニュー入口は新設しない。
- 既存の画像合成、比較対象、境界位置、ドラッグの入力判定は変えない。
  境界線判定は `compare_wipe_affordance`（`src/ui_fullscreen.rs`）に統一した。
- 確認: 比較開始時の発見しやすさ、初回ホバー/ドラッグ後の消去、再入場、複数窓、
  明暗画像での視認性、通常の比較操作の維持。
- 規模 / 優先度: Small想定 / P2。後続の利用者指定で次版の直列実装対象となった。
- 2026-09-13: 実装・対象回帰・全体テスト完了（main 8307 passed / 0 failed / 45 ignored）。
  独立レビュー承認・確認build完了。実機は未確認。
  [設計と検証記録](section223-compare-wipe-guidance.md)を参照。
...
- 2026-09-13追加指定への対応: 表示場面の例を既存builderへ通した結果を表示/非表示で示し、対象外の項目は表示可能な場面も確認できるようにした。通常メニューと右クリックは別設定ページへ分割。内部の既定継承と旧保存互換を維持し、閲覧・場面切替は保存変更しない。実装・独立レビュー・全体gate（8382/0/45、snapshot 50/50）完了。利用者の終了連絡後、20:18の確認buildが成功。2026-09-13、追加指定分も利用者から動作問題なしの確認済み。[記録](section221-context-menu-layout.md)。
- 2026-09-13実機追補: Open Withの不要な上下ボタン撤去・列位置共有、項目の前の区切り線（標準/表示/非表示）を実装。明示指定は並べ替えた項目へ追従し、標準配置・動的固定枠は維持。独立レビュー・combined全体gate（8378/0/45）・18:36確認build完了。利用者の再確認待ち。
- 2026-09-13: 実装・独立レビュー・全体テスト（main 8349/0/45）・確認build完了。
  実機操作は未確認。[設計と検証記録](section221-context-menu-layout.md)。
...
- 2026-09-13: 追補build案内後に利用者から「切り取り表示は良さそう」との確認を受け、対応完了。以下の再確認待ちは確認前の経過記録。
- 2026-09-13実機追補: mIV内Cutの半透明表示は確認済み。Explorer Cut非反映は独自format不在の`DV_E_CLIPFORMAT`とShellの`S_FALSE`を一時失敗と誤分類したためで、修正・実Shell IDataObject読取回帰を完了。中央ハサミも追加し選択チェックは維持。独立レビュー・combined全体gate（8378/0/45）・18:36確認build完了。実Explorer再確認とcommitは保留。
- 2026-09-13: 実装・独立レビュー・全体テスト（main 8373/0/45、snapshot 50/50）・確認build完了。実クリップボードでの切り取り・貼り付け確認は未実施で、利用者確認後にcommitする。[設計・検証記録](section220-cut-item-appearance.md)。
...
- 2026-09-13: [設計と検証記録](section218-singleton-spread-placement.md)。実装・全体テスト（main 8323/0/45）・監査・独立レビュー・確認build完了。本体・Remoteとも利用者確認済み。
  先頭側は表紙位相も含む既存見開き規則に従う。実ページ1件のまま、自己寸法2枚分の
  表示キャンバスと片側配置を本体・連結・Remote・保持画像へ共通に運ぶ。設定既定OFF。
...
- 2026-09-13実機確認: 縮小動作は確認できたが中心固定によりドラッグ不可。利用者指定により、
  Vモード中は縮小状態でもドラッグ移動できるよう追加修正する。
- 同日追補: ドラッグ中に左右パネルが開くため、利用者承認により動画360と同様に
  Vモード中の左右パネル・呼び出し入口を抑止する。上下HUDは維持し、パネルのopen/固定設定は
  消さずV終了後に復帰させる。

- **2026-09-13更新**: 利用者の後続実装指定により、下記の見送り判断を変更して順次実装中。
  下限は静止画通常ズームと同じfit比0.1を基準とし、初期/resetのfit1.0と分離する。
  以下は当時の調査記録として残す。現codeではDefaultも下限定数を参照するため、定数だけの変更では不十分。
- 同日: MIN0.1/FIT1.0分離を実装し、狭域検証・独立レビュー・確認build完了。
  §218の全体gateは未変更範囲へ再利用。native実機確認・commit待ち。
  証跡は`target/section212-video-subfit-zoom-20260913/`、設計・検証記録は`video-zoom-pan-plan.md`。
...
- **2026-09-13設計合意**: Spaceパンを優先し、開始時に選んだ操作を同じドラッグの終わりまで維持する。
  ...
  独立レビュー・確認build完了。2026-09-13、利用者から§1.164の実機確認済み・動作問題なしの連絡を受け、対応完了。
- 規模 / 優先度: Small / P2。
