...
- **アイコン数の見積り**: 分析ツール −1 / 360 の非表示条件を無効化へ ±0 / 「…」+1。
  **普通の画像では今より 1 個少なくなる。**
- 収録内容は着手時に確定する。基準は「**KeyAction はあるのに画面上の入口が無い機能**」。
  候補: ナビゲータ ON/OFF、ナビゲータ位置 (サブメニュー)、分析ツール、ピクセルグリッド、
  ルーペ固定、左右パネルのモード。編集系 (消しゴム / 隠蔽 / 補正レイヤー) とズームモードは
  **タッチ対象外**の判断 (利用者判断 2026-08-13: ズームモードはホバー前提、編集はタッチ
  対象外)。比較ワイプも Shift+C / X とセットでしか使えないので対象外。
