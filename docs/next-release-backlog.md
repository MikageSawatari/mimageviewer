...
- 出典: 5ch 不具合報告 (2026-07-15、調査 2026-07-16)。前方から読み進めて最後の 2 枚を
  見開き表示した状態で ↓ を押しても、画面も案内も変化しない。もう一度 ↓ を押すと、
  本来 1 回目に出るべき「最後の項目です」が表示される。
- 期待する不変条件: 見開きの末尾 2 ページは 1 つの表示単位である。最後の表示単位から
  次へ進む操作は、内部ページ index を同じ見開き内で移動させず、その 1 回目の操作で
  末尾境界を通知する。先頭から前へ進む場合も対称に扱う。
- 原因 (確認済み): `spread_nav_delta()` は最終見開きの先頭ページから次へ進む際、表示単位の
  終端までの raw page delta (`+2`) を返す。一方、受け側の `adjacent_navigable_idx()` は
  page index に delta を足して一覧末尾へ clamp するため、同じ見開きの 2 枚目へ内部 index
  だけが移動する。表示内容は変わらず、2 回目で初めて `None` となって境界ヒントが出る。
  直前のシークバー修正は見開き単位の表示・位置を直したが、このナビ契約は page delta のまま残った。
- 根本修正方針: 見開きナビの結果を raw page delta ではなく、表示単位での
  `Target(index)` / `Boundary` のような明示結果にする。ページ単位の他用途もある
  `adjacent_navigable_idx()` の clamp 挙動を全体変更せず、誤った状態遷移を作る
  `spread_nav_delta()` とハンドラの境界で修正する。キー、ホイール、クリック、スライドショーの
  各呼出元で同じ表示単位契約を維持できるかも確認する。
- 回帰テスト: 最終見開き + ↓ 1 回で `FsBoundaryHint::Edge { at_end: true }` かつ index 不変、
  先頭見開き + ↑ の対称ケース、中間見開きへの正常遷移をハンドラ / 状態遷移レベルで確認する。
  LTR / RTL、表紙単独、奇数ページ、横長単独ページ、`spread_shift_anchor_idx` も組み合わせる。
- 着手条件 / 優先度: DetachedWindowManager 抽出と実機確認が完了したため着手可。P3
  (実害は小さいが、表示単位とナビ状態の不整合が原因)。
