...
- 再現: `ai_backend=tensorrt` の隔離プロファイルで、TensorRT パックの `onnxruntime.dll` を同じ長さのまま壊して起動し、
  AI アップスケール対象の画像を全画面で開いて送る。
- 観測: 1 回目は起動試行から 31 ms で `TRT worker spawn failed (RuntimeInit)` になり、DirectML で処理が完了し、
...
- 構造 (修正前コードから): `App::maybe_start_trt_worker_pool_for_ai_use` ([app.rs](../src/app.rs)) は
  ...
  遅延起動の入口がそれと別経路で再試行している。
- 守るべき不変条件: `RuntimeInit` / `CommandRejected` / `Protocol` / `ParentSetup` の失敗後は、利用者が
  「ワーカーを再起動」を押すか、TensorRT の設定・パックが変わるかアプリを再起動するまで、遅延起動しない。
  一時的な失敗 (`ProcessSpawn` / `Transport` / `Timeout`) の 1 回再試行は現状どおり。
- 直し方の方向: `has_worker_pool` + `trt_restart_in_flight` + notice に分かれている状態を、TensorRT ワーカープールの
  単一の型付き状態 (未起動 / 起動中 / 接続済み / 失敗(種別)) にまとめ、遅延起動・自動再試行・手動再起動・パック導入が
  同じ状態を見るようにする。遅延起動の入口に失敗フラグを足すだけの対処は避ける (CLAUDE.md「バグ修正の一般原則」)。
...
- 正本: [調査記録と修正候補](msstore-startup-hang-vcrt-investigation-20260915.md)。ここは要点と残りだけ。
- 出典: Microsoft Store 審査 (v3.6.0 却下、「起動画面から進まない」とスクリーンショット)。
  **ClaudeCode が Windows Sandbox (VC++ ランタイム無し、Defender 無効) を Computer Use で操作し、
  v3.5.0 / v3.6.0 / v3.10.0 とも「応答なし」+「起動中…」で止まるのを観測。**
- 原因: `onnxruntime.dll` が MSVCP140 / MSVCP140_1 / VCRUNTIME140 / VCRUNTIME140_1 に依存し読み込めない
  (win32error=126)。`ort` 2.0.0-rc.12 がそのエラーを作る途中で自分の OnceLock に再入してデッドロックし、
  `ensure_ai_runtime()` の「失敗したら AI なしで続行」へ戻らない。初回フレームの UI スレッドで起きる
  (v3.10.0 のダンプ + PDB で主スレッドを確定)。2026-04-21 `b27e7f9ef` 以降、全配布形態、AI 設定に関係なく毎回。
- 陽性対照 (同じく Sandbox で観測): VC++ 4 DLL を **core exe の隣**に置くと起動し AI 初期化も成功、
  **`onnxruntime.dll` の隣だけ**では止まる。
- 直す範囲 (判断は開発担当): ①`ort` の失敗経路がハングしないようにする (上流更新 / 事前ロード確認 / patch)、
  ②VC++ ランタイムを exe 隣へ同梱して AI を使えるようにする、③AI 初期化を UI スレッドから外す (①とセット)。
  CLAUDE.md の「VC++ 不要」の記述と、リリース手順 Phase 3 step 12 の `dumpbin` 対象 (本体 exe のみ → 同梱 PE 全部) も同時に直す。
- 追加ダウンロード (開発機のパックを dumpbin): **TensorRT パック**は ORT GPU 版の 4 DLL が同じく VC++ ランタイム依存
  (NVIDIA の 13 DLL は不要)。子プロセスで読むので固まらず、45 秒 timeout×2 の後に原因と違う通知になる (コードから推定)。
  子プロセスは本体と同じ exe なので、exe 隣への同梱で一緒に解決する見込み。**編集用パック**はフォントと ONNX モデルだけで
  DLL を含まず、本体の修正以外の対応は不要。詳細は正本の「追加ダウンロード」。
- 回帰確認: Sandbox で配布 3 形態の初回起動と 2 回目起動、VC++ ランタイムのある PC で AI 機能と
  TensorRT パック・編集用パックの被写体分離、DLL を読めない場合に AI なしで起動が続くこと
  (TensorRT の子プロセスも timeout ではなく正しい理由で失敗を返すこと)。Store 再申請は修正版の正式リリース後。
- 規模 / 優先度: Medium / **P0** (該当環境では起動できない。Store の再申請を止めている)。

### 1.238 タグビューの「フォルダに移動」が動作しない — >>397 (2026-09-14)

- 出典: >>397。タグビューの項目を右クリックし、「フォルダに移動」を実行しても移動しない。
  開発環境でも再現済み。
- タグビューでもこの項目は利用可能として表示しており、実ファイルの親フォルダへ移動し、
  タグビューを閉じて対象項目へカーソルを合わせるのが期待動作。仕様変更ではなく不具合修正とする。
- `JumpToFolder` の対象解決、タグビュー終了時の `saved_folder` と一覧復元、実フォルダへの遷移要求の
  発行順を確認し、途中で要求を破棄・上書きしている所有境界を直す。待ち時間や再試行による回避はしない。
- 回帰確認: 通常フォルダの実画像 / 動画、検索結果からの同操作、タグビューへの戻る操作、
  対象消失時のエラー、ソート・カーソル位置。handler-level testでタグビュー終了と移動先を同時に検証する。
- 規模 / 優先度: Small-Medium / P1。
...
### 1.234 `settings.db-shm` を削除できず、設定の復元と完全リセットが両方失敗する (2026-09-13)

**2026-09-14 朝の検収更新**: 所有境界の修正・独立レビュー・full gate・確認ビルドに加え、使い捨てportableで復元と完全リセットが成功し、終了後の退避/DB/WAL/SHMも検査済み。§1.231と合わせた最後の初回起動liveだけは利用者の外出により後日確認へ残す。詳細と検証証拠は[設定復旧の設計](settings-recovery-234-231-plan.md)。以下は原因調査の履歴。

**2026-09-14 開発調査による方針更新**: 以下の初期調査・案は履歴として保持する。コード照合では、常設 Remote IPC の `LiveFavorites` が持つ `SettingsFavoritesReader` が復元中も接続を保持する。一覧生成の `read_summary` は一時接続で、常設の保持者ではない。使い捨て Windows WAL DB の試験では、読み取り接続がある間は SHM と WAL の両方が共有違反になり、閉じると両方削除できた（利用者の失敗時にハンドルを採取した結果とは区別する）。SHM の失敗を無視する初期案は採用せず、設定を読む処理の完了と接続の解放を確認してから復元する。Remote の短期設定読み取りも含む所有境界を独立レビュー中。規模は Small ではなく中規模に改める。詳細は [設定復旧の設計](settings-recovery-234-231-plan.md)。

**P1。ダウングレード後、設定を戻す手段が全部塞がる。** 出典: 利用者報告 (2026-09-12) →
**開発機で 2 通りとも再現 (2026-09-13、開発者本人が操作)**。

#### 症状は 2 つ、原因は 1 つ

| 操作 | 表示 | 実際に止まった場所 |
| --- | --- | --- |
| 「この時点に戻す」 | 「復元 に失敗しました … os error 32」「設定ファイルは変更されていません」 | `restore_from` step 6 |
| 「完全リセット」 | 「**致命的なエラー**が発生しました … os error 32」「不整合な状態になっている可能性があります」 | `full_reset` の削除ループ |

**どちらも `settings.db-shm` の `remove_file` で止まっている。**

#### 残存ファイルから確定した (2 つの独立した再現フォルダ)

**A: 3.9.1 の folder を 3.9.0 で起動 → 完全リセット**

`bak1` / `bak2` は消え、`-shm` から先 (`-shm`, `-wal`, `settings.db`) が残っていた。
削除順は `bak10..bak1 -> -shm -> -wal -> settings.db` で最初の失敗で return するので、
**`-shm` で止まったことが確定**。

**B: 3.9.0 -> 3.9.1 -> 3.9.0 の folder で「バックアップ 3」(3.9.0、復元候補) を復元**

`.before-restore-*` の退避一式は作られ (step 3 成功)、**`-shm` と `-wal` は両方残り**、
`settings.db` も bak も preupgrade も**すべて無傷**。step 6
[`drop_wal_sidecars_strict`](../src/settings_restore.rs:700) は `-shm` を**先に**消すので、
**`-shm` で止まって `-wal` の削除も rename も走っていない**ことが確定。

#### なぜ全部塞がるか

- step 6 は **`source.is_current()` に関係なく必ず通る**。どの世代を選んでも同じ場所で落ちる。
- 「使えるバックアップが無い場合」の逃げ道である完全リセットも、同じ `-shm` で落ちる。
- **B の画面では `3.9.0` の復元候補が 4 行あり、どれも正当に押せる状態だった。**
  それでも 1 つも復元できない。**安全網が完全に閉じている。**

#### 要点 — データを持たないファイルが、持つファイルを人質にしている

- **`-shm` は派生物**。SQLite がロック調整に使う共有メモリの実体で、次に開けば作り直される。
  Windows ではメモリマップされるため、マッピングが外れるまで `DeleteFile` が
  `ERROR_SHARING_VIOLATION` (os error 32) になる。
- strict にした根拠は「stale な `-wal` が再適用されて復元が部分的に巻き戻る」ことで、
  **それは `-wal` の性質。`-shm` には当てはまらない。**
- しかも `-shm` が**先**なので、**根拠のある `-wal` の削除が、根拠のない `-shm` の失敗で
  実行されない。** 順序と強度の組が逆になっている。
- 完全リセット側では、**実際の被害は世代バックアップの消失だけ**なのに
  「不整合な状態になっている可能性があります」という最も強い警告が出る。

#### 分かっていないこと

- **誰が `-shm` を掴んでいるのか。** ダウングレード起動では boot が `db: None` を返すので
  `GLOBAL_DB` は空で、`checkpoint_global_db` も
  ([settings_db.rs:3876](../src/settings_db.rs:3876)) `None` なら何もしない。
  **`SettingsDb` ではない。**
- **保持者はプロセス内**。失敗後に mIV を終了した状態ではプロセスが 1 つも残っていないので、
  ウイルス対策や検索インデクサのような外部要因ではない。
- 符合する事実: 復元ダイアログの一覧生成 [`read_summary`](../src/settings_restore.rs:225) が
  「現在の設定」行のために `settings.db` を read-only で開く。WAL DB への read-only 接続も
  `-shm` を作ってマップする。実際 `-shm` の mtime は操作時刻と一致していた。**未確認。**
- **訂正**: 「`-wal` が 0 バイトだからチェックポイントが走った」は誤り。
  ダウングレード起動は save 抑止なので、**そもそも WAL へ一度も書いていない**。

#### 対応方針

1. **`-shm` の削除失敗で操作を失敗させない。** 残っても次回起動で作り直される。
   復元・完全リセットの両方で緩める。
2. **`-wal` は strict のままにする。** 根拠があるのはこちらだけ。順序も `-wal` を先に
   処理できるようにして、`-shm` に人質を取らせない。
3. **完全リセットの失敗文言を、実際の被害に合わせる。** 「bak は消えたが現在の設定は無事」と
   「設定が壊れたかもしれない」を同じ文面で出さない。
4. 保持者の特定は**別途**進める。1 を入れれば症状は消えるが、掴んだままの接続が居ること自体は
   残るので、`RestoreError::Io(e)` に**失敗した操作と対象パス**を載せたうえで、
   エラー表示中 (プロセスは生きている) に handle 列挙で追う。
   **1 を「原因を直した」と書かない。**

#### 回帰確認

- `-shm` を掴んだ状態で「この時点に戻す」→ **復元が成功する**
- 同じ状態で「完全リセット」→ **致命的エラーにならず**、family が消える
- `-wal` の削除失敗は従来どおり失敗として扱われる
- 成功時の挙動 (WAL 破棄、rename、次回起動) が変わらない
- 失敗時も `before-restore-*` の退避が揃っている
- 規模 / 優先度: Small / **P1**。
...
### 1.231 設定の復元で、来歴用のファイル名 version が「開けるか」の判定を上書きする (2026-09-13)

- 出典: 利用者報告 (2026-09-12) と開発機での再現 (2026-09-13)。
- 再現: ポータブル版 3.9.1 で起動 → exe 一式を 3.9.0 で上書き → 起動 → 設定の復元 →
  **「アップグレード前 (vunknown)」** を選ぶ。
- 症状: 他の 3 行 (現在の設定 / バックアップ 1 / バックアップ 2) は「**利用不可（新しい版）**」で
  ボタンが無効なのに、**この行だけ「要確認」でボタンが押せる**。押すと検証段階で失敗する。

  > バックアップ内容を検証できませんでした: open 失敗: settings.db is newer than this build:
  > saved by mImageViewer 3.9.1; current build is 3.9.0

  ファイルは変更されない (`Recoverable`) ので実害は「進めない」ことだが、
  **ダウングレード後に押せる行がこれだけなので、復旧の出口がここしか見えない。**

#### 原因 (コードから確定)

[`read_summary`](../src/settings_restore.rs:281) は **DB の中身から版を読んでいる**。

```rust
summary.app_version = conn
    .query_row("SELECT value FROM schema_meta WHERE key = 'app_version'", ...)
    .ok();
// 既存 preupgrade の … ファイル名の方が正確。新実装では両者が一致する。
if let BackupSource::PreUpgrade(version) = &summary.source {
    summary.app_version = Some(version.clone());   // 無条件に上書き
```

ファイル名が `settings.db.preupgrade-vunknown` なので、**読めていた `3.9.1` が捨てられ、
文字列 `"unknown"` に置き換わる**。すると
[`backup_compatibility`](../src/settings_restore.rs:298) の
`semver::Version::parse("unknown")` が失敗して `BackupCompatibility::Unknown` になり、
UI は「要確認」を出して**ボタンを有効のまま残す**
...
#### 通常のアップグレードは壊れていない (実インストールで確認、2026-09-13)

`%APPDATA%\mimageviewer` の実データを read-only で読み、**実際の版跨ぎで作られた snapshot は
ファイル名と中身が一致している**ことを確認した。

| ファイル | 中身の `app_version` |
| --- | --- |
| `settings.db.preupgrade-v3.8.0` | `3.8.0` |
| `settings.db.preupgrade-v3.9.0` | `3.9.0` |
| `settings.db.preupgrade-v3.9.1` | `3.9.1` |

いずれも `favorites=40` の実データ。**この実インストールに `settings.db.preupgrade-vunknown`
は存在しない** (あるのは JSON 時代の `settings.json.preupgrade-vunknown` だけ)。
**偽ファイルが出るのはデータフォルダが新規に作られたときだけ** = 新規利用者と新しい
ポータブルフォルダ。**動いている経路を巻き込んで直さないこと。**

#### より重い出方 (**未実行・コードからの推論**)

**今回のダウングレードは穏やかな方**で、派手に失敗してファイルには触れない。
**版が一致しているとき**、同じ行は次のようになるはず。

1. `backup_compatibility` が `Unknown` を返すので `newer` は false → **ボタンは有効**
   ([ui_dialogs/settings_restore.rs:639](../src/ui_dialogs/settings_restore.rs:639) の
   `add_enabled(!newer, ...)`)
2. `validate_backup` は 3.9.1 対 3.9.1 で**新版ガードを通過する**
3. 復元が**成功し**、`settings.db` が**初回起動時のスナップショット**に置き換わる

ラベルは「アップグレード前」だが中身は初回起動時の状態なので、**使い込んだ後に押した利用者は
お気に入りも設定も初回状態へ戻る**。失敗ではなく成功するため警告も出ない (互換性欄は
「要確認」という弱い表示のみ)。`before-restore-*` の退避は残るので取り返しはつくが、
それを知らなければ「消えた」と見える。**この経路は実行して確かめていない。**
...
「別のプロセスが使用中です (os error 32)」は**別の経路の別の欠陥**で、**§1.234 へ分離した**。
この節 (§1.231) は復元一覧の互換性判定だけを扱う。
...
- 規模 / 優先度: Small / P2。版が一致する環境では設定が初回状態へ戻る経路が開いているが、
  **一覧の「お気に入り / タグ / 動画再開」列が 0 と表示される**ので、完全に無言ではない
  (2026-09-13 の再現画面でも、偽の行だけ動画再開が 0 だった)。復旧も `before-restore-*` で効く。
...
### 2.24 通常のサムネイル一覧をファイルサイズ順で並べる — 専用スレ >>392 (2026-09-14)

- 要望: 詳細表示だけでなく、通常のサムネイル一覧でもファイルサイズの昇順 / 降順を選びたい。
- 現状:
  - 詳細表示は `DetailsSortKey::Size` でサイズ列を並べ替えられる。
  - 一覧ツールバーの `SortOrder` はファイル名 / 番号 / 日付だけで、サムネイル表示にはサイズ順がない。
  - 左側のフォルダツリーも同じ `Settings.sort_order` を参照しているため、ここへサイズ順をそのまま
    足すと、サイズを持たないツリー側まで新しい値が流れ込む。
- 方針:
  - 一覧のソート候補へ「サイズ順（小さい順）」「サイズ順（大きい順）」を追加し、サムネイル表示で
    選べるようにする。詳細表示の列ヘッダによるサイズソートは現行どおり維持する。
  - フォルダ、仮想項目、サイズ取得失敗など値を持たない項目の配置規則を明示し、同値時は名前順で
    tie-break して列挙順に依存させない。
  - ZIP / PDF 等の中を本として見ているときのページ順は変えず、ファイル選択一覧のソートとして扱う。
  - 設定の保存 / 復元、お気に入りごとの表示状態、カテゴリ別配置との組み合わせを回帰確認する。
- 依存: §2.25 と同時、または §2.25 を先に実装する。ツリーと共有した `SortOrder` へサイズ値だけを
  先行追加しない。
- 規模 / 優先度: Small〜Medium / P2。

### 2.25 一覧と左側フォルダツリーのソート順を分ける — 専用スレ >>392 (2026-09-14)

- 要望: 一覧を日付順やサイズ順にしても、左側のフォルダツリーは名前順など別の並びにしたい。
- 現状: `FolderTreeOptions::from_settings` とフォルダペインの再読み込み / キー操作は、一覧と同じ
  `Settings.sort_order` を使っている。ツリー上部はドライブ選択と再読み込みだけで、専用のソート UI はない。
- 方針:
  - フォルダツリー専用の保存設定を追加し、初期値はファイル名順とする。一覧側のソート変更、
    お気に入りごとの表示状態の復元、本のページ順から影響を受けない所有に分ける。
  - ツリー上部へ横幅を取りすぎないソート順のプルダウンを追加する。候補はツリーで意味のある
    ファイル名 / 番号 / 日付の昇順・降順とし、一覧用のサイズ順は含めない。
  - 変更時は展開状態と現在位置をできる限り維持したまま、既存の scan worker 経由で並べ直す。
    UI スレッドでフォルダ走査や同期 I/O を行わない。
- 回帰確認: 起動時の復元、ドライブ切り替え、再読み込み、隠し項目切り替え、キーボード移動、
  お気に入り配下への出入りで、一覧とツリーのソート設定が互いに書き換わらないこと。
- 規模 / 優先度: Small〜Medium / P2。
