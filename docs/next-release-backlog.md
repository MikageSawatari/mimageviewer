...
## 1. 優先候補
### 1.219 類似本の対応ページ帯を押しても一時表示されない — 利用者報告 (2026-09-11)

- **2026-09-12更新: `dbd7b4efc`で製品修正・独立レビュー・自動回帰・全体gate・確認buildまで完了。
  利用者実機確認は未実施。** 見開き中、「この本と重なる本」の□を押している間だけ候補1ページを
  表示し、releaseで元の見開きへ戻す。読書位置・見開き設定は変更しない。
- 根因: 圧縮帯では候補の実描画列が1ptでも最低3ptの見える枠を描く一方、入力はraw columnだけを
  参照していた。見えている□の内側でも隣のtarget無しcolumnを押せたため、candidateが開始しなかった。
  pageと実際に見せる3ptのhit/outline rectを一度だけ解決し、hover、thumbnail、caption、pressで
  同じownerを使うよう修正した。
- 検証: 実locked panelの帯をhover・押下し、production dispatcher、候補完了、単ページpreview、
  releaseによる元見開き復帰までLTR/RTL × 広幅/圧縮帯の4ケースで成功。通常の「長押し表示」、
  worker失敗、source失効、context切替・遅着取消も維持した。全gateはmain 8294成功/0失敗/
  45 ignored、workspace/integration/doc/vendorも成功し、`build-dev.ps1 -PreserveRuntime`も成功。
  残る確認は利用者実機での表示と押下感触であり、新しい製品実装ではない。
...
その後、利用者から右クリックメニューの再出現が解消し、F12も動くとの確認を得た。
別窓での長押しを含む全組合せを確認したとの報告ではなく、その範囲は未確認として残す。
...
**進捗 (2026-09-11): 実装・自動検証済み、VST未設定portableで利用者実機確認済み。** 上記は調査時の経路。
...
その後、VST未設定の通常portable確認版で利用者から「♪はうまく動くようになりました。OK」
との確認を得た。VSTありの全復帰経路の実機確認を代替するものではない。
証跡の所在は [優先作業台帳](post-v3.8.0-priority-work.md) を参照。
...
### 1.205 動画シークバーのホバーサムネイルが表示されない — >>375 (2026-09-09)

**2026-09-10更新：修正・独立レビュー・自動回帰に加え、利用者がプレビューの表示復旧を確認済み。** 以下は調査時の記録。

**P1・対応中。利用者指定により次リリースの優先修正対象。** 表示をオフにしていないのに、
シークバーへカーソルを乗せてもサムネイルが出ない。利用者の確認では、下部の
サムネイルストリップの表示・非表示のどちらでも発生する。

設定DBの値は `always` で、読み込み・プレイヤーへの反映も成立していた。
v3.7.0 で下HUDの painter に付けた clip が、HUDの上に描くプレビューにも適用され、
ポップアップ全体を切り落とす経路を確認した。HUD本体の clip を維持し、
プレビュー用の描画範囲を分離する。実アプリの起動・入力は事前了承の検証枠まで行わない。
実装・独立検収・最終全体テストは完了。実機での表示確認とコミットが残る。
進行と検証記録は [次版開発台帳](next-version-development.md) を参照。

### 1.204 見開きの寸法判明後にページ送りが「読み込み中」のまま停止する (2026-09-09)

**2026-09-10更新：修正・独立レビュー・自動回帰を完了。利用者の継続使用では問題なし。ただし低再現性のため、元の実機症状の完全な解消確認とは区別する。** 以下は調査時の記録。

**P1・対応中。利用者指定により次リリース対象。** v3.7.0 の使用中に発生し、
ホイール移動だけが止まる一方で HUD / パネルのホバーと別窓の動画再生は続く。
画像のデコード完了後、寸法未判明時の移動先ページ集合と、横長判明後の描画単位が
食い違い、navigation sequence が完了しない経路を確認した。

表示単位と待機完了判定を同じ所有境界で整合させる。タイムアウトや強制解除にはしない。
実装・独立検収・最終全体テストは完了。実機でのページ送り確認とコミットが残る。
調査・担当・検証状況は [次版開発台帳](next-version-development.md) を参照。
...
### 1.200 詳細表示からの静止画シーク・画質設定の不具合 2 件と、サムネイル列「最大」 — >>367 (2026-09-08)
...
- **実装修正済み (`052bd6339`)・自動検証済み、実アプリ検証全体は未合格。**
  実worker回帰、旧predicateでの失敗確認、取消・失敗・backlog・世代・contextの回帰、
  独立Astraレビュー、`test-full.ps1`（本体7495成功/33既存ignore、vendor含む）を通過した。
  9/8の実ログでは374px要求と拡大後に各素材が収束し、各約29秒の追加生成は0だった。
  前面条件と外部marker観測が不成立のため全体は環境失敗であり、詳細は作業台帳に記録する。
  測定runnerの未完了を解消し、了承済みのリリース前検証で再確認する。
- 発見: v3.6.0 出荷前の `check-idle-health.ps1`。**進捗バーが 112/116 と 113/116 を往復**し、
  静止しているのに収束しない、という実機報告から。
- **2026-09-07 再調査**: 初稿の「セル幅で頭打ち」「結果が捨てられる」は反証された。
  ...
  idle 判定の品質契約の不一致が根因。セル上限を追加する旧提案は採用しない。
  実ログの要求374pxに対し、比率精度を優先する既存選択器が247×124を返し、
  `247 * 5 < 374 * 4` が毎回再成立することを確認した。
  経過・反証・設計判断は [v3.7.0 作業台帳](v3.7.0-priority-work.md) に記録する。
...
- 対象ファイルはすべて **884x444 (2:1 の横長)**、`h:\home\mimageviewer_old	estimage`。
  比率設定は「自動 (3:4)」、7 列。
- 根因: idle側は生成実寸だけで解像度不足を判定するが、生成側は0.05%の比率誤差を
  満たす整数寸法を選ぶため、要求長辺に近い結果を常に返すとは限らない。
  完了したSource要求サイズをLoadedへ保持していないため、同一要求の完了を認識できない。
  374px要求では247×124、400px要求では400×201となり、サイズ帯によって発生する。
...
- 採用設計 (親Astra / 独立Astra合意、Sol実装前確認): Sourceの生成に使った要求サイズを
  画像のtyped provenanceとしてLoadedまで保持する。同一サイズでの完了後は再投入せず、
  実寸不足かつ要求サイズが増加した場合に再評価する。実寸と要求サイズは混同しない。
  cancel/error/finalizeだけでは完了を公開せず、backlog・items世代・contextと共に所有する。
  既存の比率精度・生成画素は変えない。大きなcacheから小さなSourceへの置換が起き得る
  既存の品質仕様は別判断として残し、「十分な実解像度になった」とは扱わない。
- 回帰テスト: 実worker→poll→idle判定で**同一要求の再投入が止まる**こと、374→400pxで
  再生成できること、cancel/error/backlog/別context・画像種別の兄弟経路を固定する。
...
- **2026-09-08 追加の利用者判断:** §1.200 など通常閲覧の修正を早く届けるため、
