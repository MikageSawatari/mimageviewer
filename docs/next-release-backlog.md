...
- 修正・独立レビュー・自動検証・利用者実機確認済み（2026-09-20）。スマートフォルダに表示した PDF を開こうとすると、PDF ではなく
...
- 2026-09-20 の保存済みログでは、PDF 開始直後に同じ 321 件のスマート一覧を再設置し、
  ...
  切替が frame-end reconcile から root 再 prepare を起こす連鎖を、追加した handler 回帰 2 件の
  失敗で再現した。PDF の開始後と Folder の位置復元後に、いずれも root 再 prepare が立つ。
  原因と設計レビューの正本は [スマートフォルダのコンテナ閲覧と復帰](section257-smart-folder-navigation.md)。
  調査証跡: `target/section257-log-triage-20260920.txt`。
- 初期修正の独立レビューで、取消時に元の Collection/検索 owner や旧 PDF/ZIP worker を
  保持できない問題が判明したため、その案を置き換えた。共通 loader の準備と可視採用を分離し、
  成功するまで元表示を維持する typed transition へ統一した。履歴、検索復帰、Ctrl 横断、
  変換書庫、取消・遅着結果も同じ要求所有者で扱い、元一覧の選択・位置・比率を復元する。
- 初回確認ビルドの関連回帰は 148 件成功（既存 ignore 1 件）。全体検証の初回は UI snapshot
  実行プロセスのアクセス違反で終了したが、同一ソースで直列・通常並列とも 53/53 成功し、
  他 target と未到達 vendor test も成功した。初回異常の原因は未特定で、詳細は上記文書へ記録。
  エージェントによる GUI／実データの操作は行っていない。
- 利用者は PDF open、Folder→Backspace の位置復元、画像フォルダの Ctrl+上下を確認済み。
  一方、root 一覧から直接開いた PDF の Ctrl+上下が次のファイルへ移動しない。
  また新 transition の進捗を左下表示にした際に旧中央モーダルと入力ブロックが欠け、
  準備中の別操作後に確認ダイアログや遅い画面切替が起こる。モーダル解除は未承認の動作変更であり、
  従来の操作ブロックを保持して是正する。前述の合格結果は追加修正前の証跡として扱う。
- root PDF の Ctrl+上下は修正前から Folder-only 横断の対象外だった。利用者は今回、
  **フォルダ・PDF・ZIP 等の本を root 一覧順で共通に横断する**仕様へ変更することを承認。
  フォルダ内の既存 DFS は維持し、端で次の root フォルダ／本へ進む。画像・動画単体は横断列へ加えない。
- 上記追加修正は実装・限定独立レビュー済み。中央モーダル、共通入力ゲート、要求 ID を照合した中止を
  復元した。単一の型付き一覧でフォルダ／本を横断し、空フォルダ・有効な子を持つフォルダも区別する。
  関連回帰 **155 pass / 0 fail / 1 ignored**、モーダル実クリック回帰、静的監査が成功。
  追加修正後の `test-full.ps1` は **初回 PASS**（UI snapshot 53/53、vendor 含む）。
  追加修正の確認ビルドについても、利用者から「動作大丈夫そうです」と確認を受けた。
