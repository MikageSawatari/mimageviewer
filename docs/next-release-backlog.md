...
- 済み (2026-09-19): 作業 A (「索引を更新中です」行と「(更新中)」の削除、`040a6eb34`)、作業 B
  (照会の内訳の計装、`57456b8b3`)。どちらも自動検証・レビュー済みで、利用者の実機確認は未了。
  作業 C (内訳の実測) で、近傍検索 1 回約 2.3 ms・所要は検索回数に比例、と確認。
- 済み (同日): 作業 D。ページ数上限 3,000 (調べる側・相手の両方) と、候補の総ページ数の予算
  10,000 (最悪でも約 30 秒、しきい値は利用者が選択)。予算超過は origin 段の途中でも打ち切る。
  ベンチでは報告のフォルダが 0.005 秒で「対象外」、上限以下の本は結果ハッシュが従来と一致。
  独立レビューは P1 / P2 なし。
- 残り: **利用者の実機確認** (A・B・D まとめて。手順は plan §5 の実機確認の項)。確認が取れたら
  この節を削除する。照会の並列化と、予算超過時の部分的な結果は対象外 (plan §6)。
- 規模 / 優先度: 残りは確認のみ / P2。
