...
§1.115 → PDFium/FFmpeg → §1.223 → §1.222 → §1.218 → §1.212 → §1.164 → §1.221 → §1.220。
以下の節の配置順は着手順ではない。
...
- **2026-09-12 再確認後: 今回完了**。利用者の最大化Enter開閉では旧連続ちらつきは再現せず、
  スムーズな移動を確認。最大化F12時のメイン窓の一瞬の前面化は、単発の表示移譲として利用者が許容した。
  現行backendの正常描画経路はhidden生成→submit/present→maximize/visibleの順であり、
  独立レビューも追加host再利用実装なしでの完了に合意。製品コード変更なし。
  [再確認・根拠・残る独立課題](section115-revalidation-plan.md)を参照。使用バイナリのexact SHAは未採取。
  placementモデルとsurface取得失敗時の可視化強化は別の構造課題として保持し、今回の合格を流用しない。
- **2026-09-12 (v3.9.0 公開後、再確認前) の判断〔上記で更新〕**: 原因 C (font atlas の 5 フレーム破棄) は option (d) で
  解消済みだが、close ごとの host 再生成と placement A/B が残るため、完了扱いにせず残す。
  **次バージョンの開発では、まずこの再現確認から入る。**
