...
- 2026-09-13追加指定への対応: 表示場面の例を既存builderへ通した結果を表示/非表示で示し、対象外の項目は表示可能な場面も確認できるようにした。通常メニューと右クリックは別設定ページへ分割。内部の既定継承と旧保存互換を維持し、閲覧・場面切替は保存変更しない。実装・独立レビュー・全体gate（8382/0/45、snapshot 50/50）完了。利用者の終了連絡後、20:18の確認buildが成功。追加指定分は利用者確認待ち。[記録](section221-context-menu-layout.md)。
