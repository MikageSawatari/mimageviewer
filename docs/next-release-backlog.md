...
- 実装点: `MenuCommand` にファイル切り取り / コピーを追加し、`build_context_menu` の最上位へ
  ...
  同じメニューモデルを使う。
- 回帰確認: 実ファイル / 実フォルダの単一対象、実項目の複数選択、仮想項目単独、実 / 仮想の
  混在選択、空白右クリックの既存「貼り付け」を model test で固定する。実 dispatch は
  <kbd>Ctrl</kbd>+<kbd>X</kbd> / <kbd>Ctrl</kbd>+<kbd>C</kbd> と同じパス集合と Shell verb を渡すことを
  確認する。
- 規模 / 優先度: Small / P2。
...
- **正本は [archive-page-load-scheduler-plan.md](archive-page-load-scheduler-plan.md)** (2026-09-03 に設計、v3.6.0 対象)。
  以下は調査記録。着手時は計画書のほうを読む。
- **S1 (実行上限・優先度・取消境界) は実装と自動テストまで完了。**
  実機 smoke と perf 再計測は未実施。S2 (ZIP 目次再利用)、S3 (アニメーション通知)、
  S4 (目盛りテスト) は未着手で、v3.5.0 には含めない。
...
#### 実測 (2026-09-03、改善方針 5 の先行実施)

計測は `cargo run --release --features dev-tools --bin bench_zip_seek -- <zip>`
([bench_zip_seek.rs](../src/bin/bench_zip_seek.rs))。ZIP open / 中央目次解析 /
エントリ読み出し / 画像デコードを分けて測り、同じ経路を N 本同時にも走らせる。
いずれも OS のファイルキャッシュが温まった状態。**冷えた初回の目次解析は 1,127ms** で、
以下の温かい値とは別物。

1 本だけ走らせたときの 1 ページあたりの内訳:

| 書庫 | エントリ数 | 目次解析 | 読み出し | デコード | 合計 |
| --- | ---: | ---: | ---: | ---: | ---: |
| `text2img_flat_10000_stored.zip` (42.29 GiB) | 10,000 | 56.2ms | 2.8ms | 25.0ms | 84ms |
| `原神.zip` (2.29 GiB) | 669 | 4.3ms | 3.7ms | 23.6ms | 34ms |
| `36e09bc4_…_特典用.zip` (0.02 GiB) | 27 | 0.15ms | 2.8ms | — | 3ms |

**目次解析だけがエントリ数に比例し、1 ページごとに毎回払い直している。**
10,000 エントリでは 1 ページの 67% がこれで、読み出しとデコードの合計より大きい。

同じ経路を N 本同時に走らせたときの 1 本あたり p50:

| 同時数 | 10,000 エントリ: 目次 / 合計 | 669 エントリ: 目次 / 合計 |
| ---: | ---: | ---: |
| 1 | 56ms / 83ms | 4ms / 34ms |
| 8 | 152ms / 179ms | 11ms / 42ms |
| 16 | 285ms / 314ms | 24ms / 59ms |
| 32 | 599ms / 626ms | 49ms / 88ms |
| 51 | **998ms / 1,033ms** | 74ms / 114ms |

**目次解析は同時数にほぼ比例して伸びる** (実質直列化する)。読み出しとデコードは
ほぼ横ばいなので、同時数の増加ぶんはすべてこの再解析に乗る。

これがログの数字と一致する。同スナップショットを再集計すると、意図した先読み窓は
**17** (`prefetch_back`=4 + `prefetch_forward`=12 + 現在ページ) なのに、
**実際の同時実行は最大 51 (3 倍)** で、35.1 秒の区間の **29%** で 17 を超えていた。
上表の 51 本同時での 1,033ms は、スナップショットの `cancel_after_decode`
中央値 1,152ms とほぼ一致する。438 件中 **349 件 (80%) が、全額を払い切った後に捨てられている**。

つまり因果は閉じている。**現在ページの worker も 51 本のうちの 1 本なので、
利用者が見ているページ自身が約 1 秒待たされる。** その間に来たホイールが worker を
足すので窓は空かない。取消は出ているが、`cancel` の確認がデコード後にしかないため、
不要と判明した処理も最後まで CPU と I/O を使い切る。

**方針 1 (受付の上限) はどの書庫にも効き、方針 4 (目次の再利用) は大きい書庫で効く**
という切り分けになる。27〜669 エントリの通常の書庫では目次解析は 0.15〜4ms しかないので、
そちらの主因は同時数のほうである。

`zip` 2.4.2 の `ZipArchive<R>` は `Clone` で、解析結果を `shared: Arc<Shared>` として
持つ。**reader 側を clone 可能にすれば、目次を再解析せずに読み出し用ハンドルを増やせる**
(位置指定読みで seek 位置を独立させる、あるいはハンドルを有界プールで使い回す)。
どちらを採るかは方針 1 の実行枠数と揃えて決める。
...
- 動画と静止画は利用目的と描画経路が異なるため、次の表示設定を**それぞれ独立して**持つ。
  既存利用者の表示を変えないよう、既定はどちらも従来どおり表示する。
...
計測は利用者実機の `--perf-log` (140 秒、`C:/tmp/miv-spread-test/` の大きめ画像)。

- **フレーム全体は健全**。13,031 フレームで間隔 p50 **8.0ms** / p95 18.0ms。
- **60ms を超えたのは 232 フレーム (1.8%)** = 約 1.7 回/秒。**報告の頻度と一致する。**
- そのうち **179 件は直前が `request_repaint`** = アプリが「すぐ描きたい」と言った直後に
  遅れており、就寝ではなく本当の遅延。**大きさは p50 80ms。**
- **サムネイルはもう原因ではない**。これらのフレームは `thumbnail_work_active=false`、
  `texture_backlog_len=0`、`requested_len=0` (§1.181 の修正前は 12〜118 積まれていた)。
- 遅いフレームは **`fs.ready` (ページ完成) と強く相関**する (240 件中 117 件が含む)。
  速いフレームにはほとんど現れない。

#### 計測できている UI スレッドの費用では説明がつかない

| 計測項目 | p50 | max |
| --- | ---: | ---: |
| `fs.ready.upload_ms` (テクスチャ転送) | 8.24ms | 21.8ms |
| `edit_materialize.upload.ms` | 7.55ms | 27.7ms |
| `gpu.lanczos_regenerate.encode_submit_cpu_ms` | 0.90ms | 990ms (初回のみ、§1.87) |

**これらを全部足しても、実際の遅延の 2% にしかならない。** 残り約 70ms/回の行き先が
計測されていない。

#### 外した推測 (記録)

`fs.final_effect_worker` の `ms` が p95 83.7ms / max 186.8ms (`colorize_apply_ms` が支配的)
だったので、一度「カラー化が UI スレッドで走っている」と判断しかけた。**誤り。**
`spawn_final_effect_job` は `miv-final-effect-*` という専用スレッドを立てており、
`worker_ms` はその worker 内の時間である。UI スレッドで発行される perf イベントの
`tid` を見て「UI スレッドの費用」と読むと必ず間違える。**同イベントのうち UI スレッドの
費用は `upload_ms` / `load_texture_ms` / `clamp_ms` だけで、いずれも p95 0.01ms。**

#### 外した推測 その 2 と 3 (2026-09-04、記録)

**② GPU の Lanczos が原因だと読んだ。相関は本物だが因果ではなかった。**
Lanczos の submit を含むフレームは p50 **60.0ms**、含まないフレームは **8.0ms**。
遅くなる確率は **22.7 倍**で、次のフレームも p50 61.0ms。頻度も約 1.3 回/秒で報告と一致する。
CPU の submit は 0.9ms しかないので「GPU 実行が重い」と考えた。

**しかし `texture_fetches` とフレーム時間の相関が逆だった**:

| フェッチ数 | n | フレーム + 次フレーム p50 |
| --- | ---: | ---: |
| 1000万〜5000万 | 161 | **148ms** |
| 1.5 億超 | 20 | **21ms** |

4 億フェッチ (6372×8192 → 995×1279) のフレームが 28〜42ms で済んでいる。
**GPU の仕事量では説明できない。** Lanczos はページ完成と同じフレームに居合わせるだけで、
費用はそこではない。シェーダは既に 2 パスの分離実装で、素朴な実装でもない。

**③ フルスクリーン描画が原因だと読んだ。これも違う。**
`ui.fs_viewport_breakdown` は `fs_viewport_ms > 8.0` のときだけ出る。140 秒で **81 件**しかなく、
60ms 超のフレームは 232 件ある。**つまり遅いフレームの大半で、フルスクリーン描画は 8ms 未満。**
内訳は完全に説明できており (`central_unaccounted_ms` p95 0.11ms)、40ms を超えた 3 件では
`panels_ms` が支配的 (54.8ms / 39.7ms) だが、件数が少なく全体の説明にはならない。

#### 入れた計装 (2026-09-04)

`frame.begin` に **`prev_update_ms`** と **`prev_outside_ms`** を足した。前者は
`App::update` が使った時間、後者は update が返ってから次フレーム開始までの時間
(= eframe の描画と present)。**フレーム間隔だけでは「自分のコードか、その後の描画か」が
分からず、それが 3 回の誤読の原因だった。**

次の実機ログで、まずこの 2 つを見る。`prev_outside_ms` が支配的なら GPU / present 側、
`prev_update_ms` が支配的なら `App::update` の中を更に割る。
...
### 1.185 全体表示のストリップが埋まらない — run をまとめる前提が全体表示で逆転する
...
### 1.188 ストリップ表示中のメモリ増加 — 調査済み、仕様と判断 (2026-09-05)

**再調査しないための記録。** 利用者報告 (2026-08-31)「全域ストリップでサムネイル表示を
選ぶと、動画のサイズによりかなりのメモリを使う (5760x2880 / 4.5GB の 360 動画)」。

#### 実測 (同型のテスト動画 5760x2880 HEVC 4.4GB、`scripts/make-seek-strip-test-videos.ps1`)

| 状態 | 常駐 | コミット |
| --- | --- | --- |
| 起動直後 | 708 MB | 1,133 MB |
| 再生 + サムネイル全体表示 (ピーク) | 1,546 MB | 3,488 MB |
| 動画を閉じた後 | **774 MB** | 2,326 MB |
| もう一度開いて閉じた後 | **774 MB** | 2,324 MB |

- **常駐はほぼ基準へ戻る** (+66 MB)。ピークの +840 MB は動画を閉じた時点で解放される
- **4 と 5 が一致 = 動画をまたいで積み上がらない**。コミットが基準へ戻らない +1.2 GB は
  初回再生時の一度きりの確保 (D3D11 ビデオデバイス / presenter / FFmpeg DLL) と、
  CRT ヒープが返していないぶん。2 周目で増えないので再利用されている

#### 内訳

- セル画像は解像度に依存しない (`STRIP_THUMB_EXTRACT_WIDTH/HEIGHT = 320`)。保持上限も
  `MAX_RETAINED_CELL_BYTES = 128 MiB` で固定
- 解像度に比例するのは**復号側**。ストリップは再生用とは別に自前のデコーダーを開き、
  `runtime.decoder` を手放すのは再試行の 2 か所だけなので、**セッション中は保持し続ける**
- ログで `decode_path=hw_d3d11va` を確認済み。抱えているのは D3D11 のデコーダーサーフェス

#### 判断: デコーダーを idle で手放す設計は採らない (2026-09-05)

- 得るものは常駐 +840 MB の一時的な削減だけ。閉じれば返るので蓄積しない
- 代償が大きい: 新しい範囲へドラッグするたびの開き直しに加え、**hw 復号なので D3D11
  デコーダーリソースの取り直し**になる。過去に不安定さを出してきた層
  ([decoder.rs](../src/video/decoder.rs) 冒頭の stuck thread 注記を参照)
- **利用者の報告値がこの桁と合うかは確認中。** 10 GB 級だったり閉じても戻らない場合は
  別の要因なので、この判断ごと見直す
