...
#### 追加項目: 取り消した編集が復元元として残り続ける (2026-08-25、利用者報告)
...
- **直す方向**: 編集が破棄されたときに flag を下げる経路を作る。ただし単調遷移にした理由
  (競合する更新の順序) を壊さないこと。破棄を「遅れて届いた 0」と区別できる形が要る。
...
- **未実装**: 再起動上限に達したときの利用者通知 (現状はログのみ)。診断画面への
  再起動回数・最終失敗理由の表示も残っている。

### 2.21 フルスクリーンが Susie プラグインの拡張子を画像として扱わない — §1.120 の検証中に発見
...
- 出典: 2026-08-25。クラッシュプラグインの検証中、サムネイルは Susie 経路を通るのに、
  ...
  で落ちた。フルスクリーンは Susie に到達していない。
- 未確認: 本物の Susie 形式 (PI / MAG など) は v0.7.0 から動いているはずなので、
  **拡張子の伝わり方に条件がある**とみられる。テスト用拡張子だけの問題か、
  ハンドシェイクで得た拡張子全般がフルスクリーン経路へ渡っていないのかを切り分ける。
  後者なら実害があるが、その場合は既に報告が出ているはずで、前者の可能性が高い。
- 規模 / 優先度: 未見積 / P3 (切り分けが先)。
