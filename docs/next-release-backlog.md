...
- 仕様の論点: 動画の上部バーには既に動画専用ボタン (タイル一覧 / Perf グラフ 等) が並ぶ。
  「…」に何を入れるかを決める必要がある。少なくとも 1.104 の「ズレ許容」を
  フルスクリーン中から変えられると、シークの体感を確かめながら調整できる。
- 規模 / 優先度: Small〜Medium / P2。1.104 とまとめて出すと説明が 1 回で済む。
...
- 実装の見通しは良い。取り消し自体は既にあり ([ui_main.rs](../src/ui_main.rs) の
  ...
  足すのは発火条件だけ。
- 確認すべき点:
  - 発火面が 3 つある。メイン一覧 ([ui_main.rs](../src/ui_main.rs))、フルスクリーン /
    native 動画 ([ui_fullscreen.rs](../src/ui_fullscreen.rs))、タッチ経路。
  ...
  - 取り消しに使った左クリックが、離した時点で通常のクリック動作 (選択 / 開く) として
    ...
    両方消費する必要がある。
  - タッチでの右ドラッグ相当 (長押し等) がある場合の対応も決める。
- 規模 / 優先度: Small / P2。
