...
## 1. 優先候補
### 1.217 サムネイル画質設定ダイアログが画面下端まで自己拡張する — >>382 (2026-09-11)

- **再現・原因特定済み。** 設定 → サムネイル画質…を開くと、ダイアログが短時間で
  下方向へ伸び、操作項目の下に大きな空白ができる。ZIP版固有ではなく共通UIの問題。
- 根因: `show_thumb_quality_dialog_window` が、そのフレームの `ui.available_size()` を
  基準に本文の固定領域を確保した後、同じUIへ区切り線と固定フッターを追加している。
  eguiのresizable Windowは前フレームの内容寸法が現在値を超えると自動拡張するため、
  次フレームでは拡張後の高さを本文が再び使い、最大高まで増え続ける循環になっている。
- 導入履歴: 原因箇所は `6bda865f1` の狭幅・詳細表示対応で入り、v3.7.0にも含まれる。
  `src/ui_dialogs/thumb_quality.rs` はv3.7.0とv3.8.0で同一。v3.8.0でローカル化した
  eguiもWindow / Resize / ScrollAreaの実装はregistry版0.33.3と同一なので、
  v3.8.0の変更が直接の原因ではない。
- 修正方針: 区切り線とフッター領域を先に確保し、実際に残った矩形だけを本文ScrollAreaへ
  渡す。差分の固定値を引いて帳尻を合わせる方式や、リサイズ自体の無効化にはしない。
  A/B横並び・狭幅での縦並び、手動リサイズ、下部操作の固定、全画面プレビューを維持する。
- 回帰確認: 大きいviewportのサムネイル表示・詳細表示で20フレーム以上描画し、初期整定後に
  外枠の高さが増えず、計算した初期高の近くに留まることを検査する。狭幅でのスクロールと
  B適用・閉じる操作も既存テストで維持する。現行テストは「viewport内に収まる」ことしか
  見ておらず、最大高まで伸びても成功するため、サイズ安定性を明示的に追加する。
- 規模 / 優先度: Small / P2。次バージョン候補。

### 1.215 同じピン留めタグの再クリックでタグビューを閉じる — >>380 (2026-09-11)

- 出典: >>378 の「ピン留めしたタグを押す度に ON/OFF」という要望について、
  >>380 で「タグビューを開いた後、×を押さないと元の画面へ戻れないのが不便」と
  意図が確定した。タグの付与 / 解除を左クリックに入れ替える要望ではない。
- ツールバーのピン留めタグの左クリックを次のトグルにする。
  - タグビュー外: 指定タグでタグビューを開く。
  - 同じタグのタグビュー表示中: `close_tag_view()` で閉じ、保存済みの
    `TopLevelGridRestore` を使って開く前の一覧へ戻る。
  - 別のタグのタグビュー表示中: 閉じずに指定タグへ切り替える。
- 展開表示の `#タグ` ボタンとプルダウン表示の「タグビュー」は、どちらも
  `toolbar_tag_search` の共通経路で同じ判定を使う。タグ画面内の他の検索導線まで
  「同じタグなら閉じる」に変えない。
- 右クリックの「選択項目へタグを付与 / 解除」と Shift+右クリックのコンテナ付与は
  現行のまま。「左で参照、右で更新」のツールバー共通操作は維持し、
  左右を入れ替える設定は追加しない。
- 回帰確認: 通常フォルダ / スマートフォルダ等から開いた同一タグの再クリックで
  元の一覧へ戻ること、別タグでは戻らず検索結果が切り替わること、右クリックの
  付与 / 解除が変わらないこと。
- 規模 / 優先度: Small / P3。
