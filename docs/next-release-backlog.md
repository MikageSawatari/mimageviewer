...
### 1.81 360 度ビューで高画質へ切り替わると消しゴムの効果が消える — 利用者報告 (完了)

- **完了 (2026-08-13、`acd2cc5e` + `5f12408a` + `e7816aeb`)**。高画質経路が再現できる編集
  だけを 360 ビューへ通し、落とす編集があることを画面に出す形にした。

- 出典: 利用者実機確認 2026-08-13。360 度ビューで高画質が ON のとき、**ドラッグ中の
  低解像度表示では消しゴムが効いているのに、高解像度へ切り替わった瞬間に効果が消える**。
- **後回しで合意済み** (利用者判断 2026-08-13: 「360 度で消しゴムは需要が低いため」)。
- 着手するときの見立て: 360 の高解像度差し替え (`poll_pano_high_res` 一帯) が、
  消しゴムの合成結果ではなく元のピクセルを載せている疑い。低解像度側だけ合成が
  効いているなら、差し替え経路が編集後のラスタを参照していない。
- 規模 / 優先度: Small〜Medium / P3。
- **方針確定 (2026-08-13、利用者判断)**: 食い違いは仕様として受け入れ、画素編集があり
  高画質化が実際に動くときだけ、画質ステータスの下へ
  「高画質化には編集は反映されません」と表示する。
  - 原因は、8K base が `final_composite_cache` の消しゴム・隠蔽加工・補正レイヤーを含む一方、
    高画質表示は原本ファイルを再デコードするため、この 3 種を再現できないことだった。
  - `acd2cc5e` では画素編集を 360 へ反映しない仕様にし、`5f12408a` では AI / ポストフィルタ等を
    base へ戻しつつ画素編集だけ final composite から除外した。しかし、AI アップスケールの実結果は
    final composite にしか存在しない。`ai_upscale_cache` を埋める `prefetch_ai_upscale` は
    Pipeline P1 リファクタ後に dead code 化しており、通常経路では cache が埋まらない。
    そのため final composite の除外は AI とポストフィルタまで道連れにし、実機で AI が消えて
    「高画質 OFF (ポストフィルタ適用中)」のままになる退行を生んだ。
  - 以上から、ソース選択は従来の final composite → adjustment → AI → raw へ戻し、
    画素編集だけを理由に高画質化を止めない。入場時トーストと adjustment cache の同期 backfill は
    撤去し、注意書きは実際に高画質差し替えが起こる状態だけに限定する。
  - 独立して正しい修正は維持する。対象ページごとの AI 実効判定、Disabled 理由の型保持、
    `fullscreen_edit_mode_entry_allowed` の共通入口である。従来の手書き抑止列挙では
    **Ctrl+G（補正レイヤー）/ Ctrl+M（隠蔽加工）/ Ctrl+T（テキスト注釈）**が漏れていた。

### 1.82 絞り込みバーの ★ プルダウンでも修飾クリックの選び方を使いたい — 利用者要望 (完了)

- **完了 (2026-08-13、`10d3e930`)**。ツールバーと絞り込みバーの両方が
  `rating_filter_op_for_click` を呼ぶ形に統一した。

- 出典: 利用者要望 2026-08-13。「★3 だけ見たい」をやろうとすると、**それ以外を全部
  チェック外しする**ことになり使いづらい。★のツールバーには Ctrl / Shift 修飾で
  特別な選び方があるので、絞り込みバーの ★ プルダウンでも同じことをしたい。
- **見込みは軽い (2026-08-13 調査)**: 判定の本体は既に純関数として存在し、テストも付いている。
  - `RatingFilterOp` ([ui_main.rs:229](../src/ui_main.rs:229)): `Toggle` / `Solo` /
    `SoloWithUnrated`(★N + 未評価) / `AllOn`
  - `apply_rating_filter_op(&mut [bool; 6], op, idx)` が適用、
    `is_rating_solo_with_unrated` が「2 回目の修飾クリックで全 ON へ戻す」ための述語
  - 作業の本体は**プルダウン側のクリックを同じ op 解決へ通すこと**で、判定ロジックの
    新規実装は要らない。
- **対応した (2026-08-13)**。対応表を `rating_filter_op_for_click(rf, idx, mods)` に切り出し、
  ★ツールバーと絞り込みバーの ★ プルダウンの**両方をそこへ通した**。修飾表を 2 つ持たない。
  - プルダウンは checkbox の `bool` 反転を**使わず捨てる**。単純な toggle では Ctrl / Shift の
    意味 (これのみ / ★N 以上) を表せないため、`clicked()` を見て op を解決し
    `settings.rating_filter` 全体へ適用する。反転した値は書き戻さないので、次フレームは
    実値で描き直される。
  - ツールチップは元から 4 通りを併記しており (`rating_tooltip`)、プルダウンも同じものを
    出しているので追加の告知は不要だった。
  - 修飾表のテストは**表を書き写す形をやめ**、実際に UI が通る
    `rating_filter_op_for_click` を呼ぶ形に直した (書き写すと実装だけ変わっても
    テストが古い表を守り続ける)。
- 規模 / 優先度: Small / P2。**次リリースに入れる**。

### 1.83 ニアレストが効く経路と効かない経路がある — 利用者報告 (完了)

- **完了 (2026-08-13、`15b33dd1` + `1bc45c74`)**。`App::display_texture_options` を新設し、
  表示テクスチャのアップロード 7 箇所をそこへ通した。

- 出典: 利用者報告 2026-08-13。ピクセルグリッドの確認中、**ニアレストにしてもドットが
  くっきりしない**画像があった。他の画像ではくっきり出る。
- **原因 (2026-08-13 調査、経路の特定は未了)**: `PostFilter::Nearest` は**画素を加工しない**
  ([post_filter.rs](../src/post_filter.rs))。`src.clone()` を返すだけで、実体は
  「**描画時のサンプラを NEAREST にする**」という指定。
  そして**その判定がテクスチャのアップロード箇所ごとに書かれている**:
  - `needs_nearest_sampler()` を見て NEAREST にする箇所: **4**
    (final composite / final effect ほか)
  - 無条件に `DISPLAY_IMAGE_TEXTURE_OPTIONS` (= LINEAR + mipmap) の箇所: **8 以上**
    — AI 結果 `ai_fs_*`、補正レイヤー `local_adjust_*`、消しゴム下地
    `erase_base_display_*`、注釈 `comic_*` など
  - つまり**どの加工が効いているかで通る経路が変わり、ニアレストが効いたり効かなかったり
    する**。利用者の該当画像は補正レイヤー / 消しゴム / 注釈が全部入っており、
    どれも LINEAR 側にある。
- **グリッドの目盛り自体は正しい** (確認済み)。`draw_fs_pixel_grid` は表示テクスチャの
  実サイズ `tex_size` から引いているので、1 マス = 表示テクスチャの 1 texel。
- 直し方: **サンプラの選択を 1 か所に集める**。表示用テクスチャのアップロードが同じ
  helper を通るようにし、そこでニアレストかどうかを決める。今日の `FsZoomOwner` /
  `fs_touch_tap_zones_enabled` と同じ形 (1 つの判断が複数箇所に散っていて、一部だけが
  新しい事実を知らない)。
...
### 2.3 サブ展開のオプション (走査階層 / 走査時フィルタ / 相対パス表示) — 利用者要望 (完了)
...
### 3.3 手動ビットマップマスクの共通強化 (バケツ / 1px 拡張・縮小) — 利用者要望 (完了)

- **完了 (2026-08-13、第 1 段 `01038cf7` / 第 2 段 `365ddc0a` + `b24dda59`)**。
...
### 1.89 フルスクリーンの先頭・末尾通知と先読み状況を非表示にできるようにする — 利用者要望 (完了)

- **完了 (v3.1.0、2026-08-15〜16、`095a0fd1` + `f4c672ea` + `01656536` + `9d4eaa35` + `8b7c3e35`)**。
  3 つの表示設定に加えて、先読み状況を現在ページの前後へ分ける表示も入れた。

- 出典: 5ch 専用スレ >>222 / >>225 (2026-08-15)。ページ送りの先頭・末尾で出る
  「最初の項目です」「最後の項目です」が目立つため、非表示にできないかという要望。
- 表示文言も、内部の `item` を直訳した「項目」ではビューア上の案内として抽象的なため、
  **「先頭です」「末尾です」へ変更**する。
  - 静止画 / 本のフルスクリーン (`ui_fullscreen`) と native 動画 (`native_video`) で
    表現が分かれないよう、ユーザー表示箇所を棚卸しして揃える。
- 環境設定の「表示 → 閲覧表示」に、少なくとも次の 2 項目を個別に追加する案:
  1. `先頭/末尾の案内を表示` (既定 ON)
  2. `先読み状況を表示` (既定 ON。フルスクリーン左下の先読み進捗表示が対象)
  - 現行利用者の表示を勝手に変えないため、どちらも既定は ON とする。
  - 一括の「案内表示」スイッチにはせず、境界通知だけ残したい場合と、先読み状況だけ
    確認したい場合の両方を許可する。
- 実装時の確認:
  - 設定 OFF は表示だけを抑止し、ページ移動、境界判定、先読み処理そのものには影響させない。
  - 静止画 / 本、detached、native 動画で設定が同じ意味になることを確認する。
  - 設定の既定値 / 保存復元テストと、環境設定検索の索引を追加する。
- 規模 / 優先度: 小〜中 / P2。v3.0.0 の次版で対応予定。

### 1.90 リモートのサムネイル一覧を指でゆっくり動かすと上下に振動する — 利用者報告 (完了)

- **完了 (v3.1.0、2026-08-15、`1f91f472`)**。`pointercancel` ではなく touch 接点で
  指の有無を判定するようにした。

- 出典: 2026-08-15、利用者の実機 (mIV Remote をスマホのブラウザで操作)。指を置いたまま
  ゆっくりスクロールさせると、**行スナップが走る → 指の位置とずれる → また走る**、を繰り返して
  一覧が上下に震える。
- 経路 (`crates/remote-web/web/app.js`):
  - `onScroll` が毎回 `scheduleRowSnap()` を呼び、**140ms 後に `snapToRow()`** で
    `scroller.scrollTop` を行境界へ代入する (7315-7318 / 7693-7712)。
  - `scheduleRowSnap` / `snapToRow` は `activePointers.size > 0` なら降りる。**この guard が
    指の接地を表していない**のが原因: `pointercancel` も `handlePointerEnd` に配線されており
    (7354)、**ブラウザがスクロールを引き取った瞬間に activePointers が空になる** (7563-7570)。
    指はまだ画面に乗っているのに guard が外れ、ドラッグ中にスナップが代入される。
  - ゆっくり動かすと 140ms あたりの移動量が半行未満なので、毎回**同じ行へ引き戻される**。
    速く弾くと 1 行以上進むため目立たない (報告が「ゆっくり」なのはこのため)。
- `scrollend` は既に配線済み (`onScrollEnd` → `snapToRow`、7341)。**本来はこれだけで足りる**はずで、
  140ms タイマーがジェスチャ中に割り込む余分な発火源になっている。
- 直し方の候補 (どれか、または組み合わせ):
  1. 接地状態を `pointercancel` で消えない形で持つ (`touchstart` / `touchend` / `touchcancel` の
     数え上げ)。guard の意味を「ブラウザがイベントを送っているか」から「指が乗っているか」へ戻す。
  2. `scrollend` が使える環境ではタイマーを使わない (未対応ブラウザ用の fallback としてのみ残す)。
     **iOS Safari の対応状況を確認してから決める** (このUIは電話が主な入口)。
  3. CSS の `scroll-snap-type` / `scroll-snap-align` に寄せ、JS で `scrollTop` を書かない。
     ブラウザ側がジェスチャと調停するので競合しない。
- 本体側の一覧は「canonical offset は常に行境界に保ち、1 行未満の端数だけ viewport の一時データへ
  分離する」形で同種の問題を解いている (CLAUDE.md「UI / スクロール」節)。**リモートは別実装**なので
  同じ不変条件を持てるかは要検討。
- テスト: `snappedGridOffset` 自体の単体テストは `command-core.test.mjs` にある。今回の問題は
  **呼ばれる条件**なので、`app-runtime.test.mjs` 側に「pointercancel 後も接地中はスナップしない」
  相当の回帰を置く。
- 確認は実機のタッチが要る。DevTools のタッチエミュレーションは `pointercancel` の発火タイミングが
  実機と揃わない可能性があるため、再現しなくても直ったとは判断しない。
- 規模 / 優先度: 小 / P2。
