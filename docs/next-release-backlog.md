...
- **実測 (Rust の `Command::new` を素で叩いて確認)**: `.py` / `.ahk` / `.ps1` はいずれも
...
- 実装案 (決めるのはこれから):
  ...
  - **C. フィルタだけ緩めて、起動できない拡張子は登録時に断る**。要望には応えられない。
- 論点:
  - **コンソールが出る**。現在は `CREATE_NO_WINDOW` を付けているが、A ではこの指定が効かない。
    python スクリプトを起動するたび黒い窓が出るのは体験として悪い。
  - **引数が落ちる場合がある**。関連付けのコマンドが `"%1"` だけで `%*` を持たない処理系では、
    `{files}` 以降が渡らない。「まとめて渡す」との組み合わせも確認が要る。
