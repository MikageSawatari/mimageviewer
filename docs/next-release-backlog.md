...
- **直す対象は「閲覧が閉じること」だけ** (利用者判断 2026-09-05)。一覧で Ctrl+E の進行状況
  ダイアログを見ている間に背後の一覧が 1 枚ずつ増えるのは、処理中であることが分かるだけで
  不便ではない。**一覧側の再読み込みは今のままでよい。**
- 方向: 監視停止や debounce 延長ではなく、フルスクリーンが開いている間は
  `consume_export_folder_refresh_pending` と**同じ先送り**へ乗せる。この仕組みは既に
  「フルスクリーン中は適用せず、一覧へ戻ったときに 1 回だけ読み直す」を持っており、
  監視由来の再走査だけがそれを迂回している。先送りの持ち主を 1 つにする。
- v3.5.0 以前からある不具合。**未着手。**
