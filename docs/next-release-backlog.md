...
### 2.2 ZIP/RAR などのサムネイル上アイコン / バッジのサイズ調整

- 出典: mImageViewer 専用スレ レス 112 (2026-07-24)。「ZIPとかRARの表示アイコンが
  大きすぎるのでサイズ変更、表示/非表示の切り替え」要望。
- 現状整理:
  - 通常フォルダ一覧の ZIP/PDF/RAR/7z/LZH 等は、代表サムネイルが無い場合に中央の
    プレースホルダーアイコンを表示し、左下に `ZIP` / `RAR` などの形式バッジを重ねる。
  - ユーザーが大きいと感じている対象が、中央プレースホルダーアイコンなのか左下形式バッジなのかは
    まだ未確定。比較画像
    `C:\Users\mikag\AppData\Local\Temp\miv_icon_badge_vertical_compare_v2.png`
    (元 / 中央アイコン縮小 / 左下バッジ縮小) で確認中。
- 方針:
  - 対象が左下形式バッジの場合: 既定サイズを見直すか、サムネイル設定に形式バッジの
    表示 / 非表示とサイズを追加する。`ZIP` / `RAR` / `PDF` など短いラベルの可読性を保つ。
  - 対象が中央プレースホルダーアイコンの場合: 代表サムネイル未生成 / 未変換 / 読み込み中の
    プレースホルダーアイコンサイズを調整し、必要なら表示 / 非表示を設定化する。
  - どちらも通常サムネイル、詳細表示のプレビュー列、フォルダ / ZIP / PDF / 変換アーカイブで
    見た目の一貫性を確認する。
- 非対象: 代表サムネイルとして実画像が出ているセルの画像自体は縮小しない。アイコン / バッジだけを扱う。
- 規模 / 優先度: Small〜Medium / P3 candidate (対象確認後に決定)。
...
### 4.2 一覧のクリック選択方式を「チェック方式 / エクスプローラ方式」から選択可能にする

- 出典: mImageViewer 専用スレ レス 112 (2026-07-24)。「マウスでのファイル選択が使いにくい。
  ドラッグ、Ctrl、ShiftキーなどWindowsエクスプローラの標準の動作が良い」要望。
- 背景:
  - 現状は、複数項目をチェックしたまま整理・タグ付け・コピー等を行う使い方を想定し、
    通常クリックや空白クリックでチェック状態が消えにくい動作にしている。
  - `Ctrl` / `Shift` クリック自体は既に近い動作を持つが、`Shift` が既存選択へ範囲追加し、
    通常クリックや空白クリックでチェックを解除しない点がエクスプローラと異なる。
- 追加仕様:
  - 環境設定 (表示 → サムネイル、または操作カスタマイズ配下を検討) にクリック選択方式を追加する。
  - **チェック方式** (既定、現状に近い):
    - 空白クリックで選択 / チェックを解除しない。
    - 通常クリックはカーソル移動中心で、既存チェックを維持する。
    - `Shift` 選択は既存チェックに新しい範囲を追加する。
    - 誤操作で仕分け中のチェックが消えにくいことを重視する。
  - **エクスプローラ方式**:
    - 空白クリックで選択 / チェックを解除する。
    - 通常クリックはその項目だけを選択し、既存チェックを解除する。
    - `Ctrl` クリックは個別トグル、`Shift` クリックは最後に選んだアンカーから現在項目までの
      範囲だけを選ぶ (既存チェックへの追加ではなく置換)。
    - サムネイル表示と詳細表示で同じ選択規則を使う。
- 実装メモ:
  - `handle_cell_interaction` 近辺へ「選択アンカー」と「クリック方式」の分岐を集約する。
    詳細表示 / サムネイル表示 / 仮想フォルダ / 検索結果で同じ helper を通るよう維持する。
  - 既存のファイル D&D 送出 (`docs/file-drag-drop-design.md`) と競合しないよう、クリック選択だけを
    先に扱う。左ドラッグによる範囲選択は今回のスコープ外。
- ドラッグ範囲選択の扱い:
  - mIV の一覧は仮想スクロール + 自前描画で、さらに実ファイル D&D 送出とドラッグ開始領域が競合する。
    使いやすい範囲選択にはドラッグ中の自動スクロール、セル / 余白 / ファイル名領域の hit-test 分離、
    詳細表示とサムネイル表示の両対応が必要で、修正範囲が大きい。
  - そのため、次に実装する場合も別設計として扱い、今回のクリック方式追加には含めない。
- 回帰確認:
  - 単一クリック、空白クリック、`Ctrl` クリック、`Shift` クリック、`Ctrl+A` / 選択解除、
    右クリックメニュー、ファイル D&D 送出、詳細表示の行クリックを確認する。
