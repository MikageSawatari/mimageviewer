...
### 1.232 更新後の「重要な変更点」が、ビューアの裏に隠れたまま入力を止める — 利用者が v3.10.0 で再現手順を確定 (2026-09-13)
...
- **ビューアの 3 つの表示モードすべてで、更新ウィンドウがビューアより手前に見える**ようにする。
  描く場所を分ける案は全画面・別ウィンドウにしか効かないので、単独では足りない。
  ビューアを開いている間は表示を遅らせる案なら 3 モードに同じく効く (要検討)。
- **不変条件は「見えないのに入力を止める状態を作らない」。** 時間任せの抑止や、モーダル判定
  からの除外だけで済ませない (後者は見えないダイアログを操作不能のまま残す)。
- 回帰確認: 通常起動での表示、引数起動での表示、フルスクリーン中の表示、detached 中の表示、
  「すべての変更を見る」からの更新履歴遷移。
