...
### 1.15 フルスクリーン左右パネルの表示モード (3 面統一)

- **正本: `docs/fullscreen-side-panel-mode-plan.md`** (設計確定 2026-07-12)。以下は背景要約。
  設計確定事項: **2 モード** `FsSidePanelMode { Hover / ClickToShow }` (既定 Hover、migration 不要)
  を `i` ボタン / I / Tab / パッドでトグル。決定 A = 通常ホバーも辺ごとに分離 (左端=補正/右端=情報/
  上端=上バー、右端で編集パネルが出る結合を解消)。決定 B = クリック表示で開いたパネルは明示
  クローズまで維持。決定 C = Pinned 廃止 (ClickToShow の右情報パネル開状態を `fullscreen_click_info_open`
  で永続化して旧 Pinned の「常時表示」を吸収。左編集パネルはセッション限定)。**スコープ = 静止画 +
  動画 + 音楽の 3 面統一** (ユーザー要望、最初から統一した操作感)。動画・音楽は現状 `i` ボタン無し
  なので上 HUD に追加。detached 固有には触れない。**未実装** (実装は Codex Sol へ発注予定)。
...
- 状態: **実装済み**。共有ファイル、差分表示、世代 / ファイル取り込み、操作カスタマイズだけの初期化、ライブ適用、自動退避、マニュアル更新まで完了。差分 / 取り込みモーダルは viewport 追従の固定外形とし、本文高も確定済み modal 高から算出する。前世代ロードの進捗は差分モーダル内だけに表示し、初回の比較元切替で背後の復元ダイアログ下部をちらつかせない。
...
- 状態: **実装済み**。表示方式の設定、詳細列設定を共有する下部 2 行（ヘッダ + カーソル行）バー、遅延列ロード、マニュアル更新まで完了。バー矩形上の右クリックは背面グリッドへ通さず、データ行は何のメニューも出さない表示専用、ヘッダ右クリックは詳細一覧と共通の列カスタマイズメニューだけを開く（左クリックソートなし）。
