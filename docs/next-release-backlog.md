...
### 1.150 編集内容を複数画像へまとめて貼り付ける — 本体実装済み / context menu 配線待ち

- 出典: 利用者要望 (2026-08-31)。
- 実装状況 (2026-09-01): `edit_bundle_bulk.rs` に、チェックがあればチェック項目、無ければ
  カーソル項目を対象とする共通 engine、対象外件数の明示、上書き件数の一括確認、
  進捗 / キャンセル、1 worker による対象順の逐次適用、成功 / 失敗 / 対象外の集計を実装済み。
  各対象は 6 DB の 1 transaction とし、途中失敗時は成功分を保持する。操作自身は Undo に
  積まず、成功前の対象ページ編集履歴だけを無効化して ★ / タグと別ページの履歴は保持する。
- 残作業: 並行改修中の `context_menu.rs` を取り込んだ後、一覧 / フルスクリーンにある全入口を
  `request_bulk_paste_page_edit_bundle` へ配線する。未配線中の UI から到達できるのは従来の
  単一貼り付けだけ。配線、公開文書更新、手動確認後に本項目を削除する。

### 1.151 編集内容をリセットする機能 — 本体実装済み / context menu 配線待ち

- 出典: 利用者要望 (2026-08-31)。
- 実装状況 (2026-09-01): 1.150 と同じ対象 engine 上に必須モーダル確認を実装済み。
  補正 / 消しゴム / モザイク / 補正レイヤー / 注釈 / 切り取り / 回転の 7 種を件数付きで
  表示し、0 件の種類は無効・未選択、それ以外は既定で選択する。6 種の保存内容は既存
  bundle の空フィールドを同じ atomic apply 経路へ渡して削除し、回転は UI thread で既存の
  `set_image_rotation(..., Rotation::None)` を使う。回転だけなら bundle worker は起動しない。
  ★とタグは変更せず、この操作自身を Undo には積まない。
- 残作業: `context_menu.rs` 取り込み後に一覧 / フルスクリーンの各入口へ
  `request_bulk_reset_page_edits` を配線する。配線、公開文書更新、手動確認後に本項目を削除する。
...
### 1.154 見開きの継ぎ目合わせが detached の凍結スナップショットに入っていない

- v3.4.0 で live 経路 (通常見開き / active detached / 連結読みの見開き unit) は
  「寄せ終わってから 1 つの所有者が内側の 2 辺を置く」形に直り、実機確認も済んでいる。
  **残っているのは `detached_spread_frozen_pages_for_snapshot` と
  `detached_continuous_frozen_pages_for_snapshot` の 2 つ**で、どちらもページごとに
  独立して `from_resolved_rect` を呼ぶ同じ二段丸めを持つ。
- **同じ helper をそのまま当てるのは誤り** (2026-08-31、Codex が不同意。凍結ルールの
  合意手続きで判明)。frozen 経路は transform を `content_bbox: None` で組み、可視範囲を
  別途 `left_clip_rect` / `right_clip_rect` として持つ。この経路の `paint_rect` は
  **見える端ではなくフルページ矩形**なので `align_spread_pages_for_gap` が測る所が違う。
  移動するのは transform だけで clip は旧 `rects` 由来のまま残るため、最終的な可視間隔を
  helper と旧 layout の 2 者が決めることになり、直そうとした二段構造がそのまま残る。
- 既存テスト `detached_spread_snapshot_preserves_trim_uv_and_background`
  ...
  **フル矩形が重なり、clip は重ならない**ことを要求しており、gap 0 でフル矩形同士を
  合わせる当て方と正面から矛盾する。
- **正しい直し方は、clip / UV の導出を配置と同じ所有者へ寄せること**で、この関数の
  契約変更になる。現状を固定するテストは足さない (直っていない挙動を仕様にしてしまう)。
- 関連する同型の残り: [1.159](#1159) (連結読みの unit **間**の間隔)、
  [1.161](#1161) (端数倍率の拡大)。**v3.4.0 の更新履歴と既知の問題ページに
  「一部の条件で継ぎ目の太さがわずかにずれる」と明記済み**なので、直したら両方から消す。
- 規模 / 優先度: 中 / P2。detached の凍結表示に限るため実害は live より小さい。

### 1.162 右クリックでメインウィンドウが前に出る — ネイティブメニューのオーナー窓 (2026-09-02)

**症状**: F12 で切り離した窓や、専用フルスクリーン窓で右クリックすると、**メインウィンドウが
前面に出てくる**。フルスクリーン中に右クリックがうまく使えないケースがある (利用者報告)。

**原因は特定済み**。ネイティブコンテキストメニューのオーナー窓が、右クリックされた面に
かかわらず**常にメインウィンドウ**になっている:

```rust
// src/ui_dialogs/context_menu.rs
let Some(hwnd) = self.main_hwnd else { ... };   // ← ここ
...
TrackPopupMenuEx(menu.handle(), ..., hwnd, None)
```

`TrackPopupMenuEx` はオーナー窓をフォアグラウンドにするので、別窓で出したメニューが
メインを引き上げる。**master にも同じコードがあり、外部ツール対応で入ったものではない**
(2026-09-02 に master 側を確認済み)。フルスクリーン右クリックにツール項目が増えて
右クリックの機会が増えたため、目に付くようになったと思われる。

**直し方**: 右クリックされた面の HWND を渡す。取得手段は既にある
(`detached_window_hwnd_raw_for_window_id`)。`NativeContextMenuRequest.hwnd` の供給元を
「メイン固定」から「その面」へ変える。

**⚠ 着手手順**: detached viewer は**リワーク中で凍結対象**
([docs/detached-rework-plan.md](detached-rework-plan.md))。CLAUDE.md の凍結ルールに従い、

1. 着手前にプラン §2 (憲法) を読む
2. **ClaudeCode と Codex の双方で「これは症状パッチではなく構造的修正である」ことに合意する**
3. 触れた範囲と判断理由を `detached-rework-plan.md` に記録する

- 規模 / 優先度: 小 / **P2** (通常フルスクリーンでも右クリックが使いづらくなる)。
- 出所: 外部ツール対応の実機確認中に発見 (2026-09-02)。切り分けて別作業にした
  (利用者判断)。
