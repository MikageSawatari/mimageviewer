...
**フルスクリーンの描画経路に、`ui.pre_grid_breakdown` と同じ形の内訳イベントを足す。**
一覧側は p95 0.20ms まで内訳が取れていて健全と即断できるのに、フルスクリーン側は
同等の計装が無いため「`fs.ready` と相関する 80ms」の中身を切り分けられない。

候補として見るべき段 (推測であり、計装で確かめること):
表示テクスチャの選択・生成、`fs_upload_backlog` の消化、見開き 2 枚ぶんの合成、
連結読みの段組み、描画変換の再計算、GPU の present 待ち。

**推測で直しにいかない。** 2026-09-04 のこの調査だけで推測を 2 回外している
(サムネイル起因と読んだ後に §1.181 で否定、カラー化が UI と読んだ後に本節で否定)。
