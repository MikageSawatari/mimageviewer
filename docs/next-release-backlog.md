...
mIV から X へ指定時刻に自動投稿する。予約は `x_posts.db`、投稿は Windows タスク
スケジューラが 5 分ごとに起こす `mimageviewer.exe --x-post-worker` の一発プロセスだけが
行う (mIV を閉じていても投稿される)。作成 UI・プレビュー・費用警告・`127.0.0.1` 限定の
ローカル予約 API・OS 起動時の最小化自動起動を含む。

- 状態: 設計合意済み・**未着手**。別 worktree (branch `x-scheduled-post`) で進める。
- 利用者判断 (2026-09-06): 既定 OFF の上級者向けとして配布 / タスクスケジューラ主体で
  常駐は前提にしない / v1 は画像 1〜4・動画・GIF・長文 (連投は入れない) / 実装順は任せ。
- 未決は plan §14。まず P1 (認証と投稿) 〜 P4 (ローカル API) を先に通し、
  利用者の既存ツール (xupload) を実運用で mIV 経由へ切り替えてから UI を作る。
- pixiv への自動投稿はしない。規約を読んだうえでの判断は plan §11.1。
