...
- **鮮度の方針はユーザー裁定で決着済み (2026-07-31)。設計時にここを蒸し返さない**:
  - **スマートフォルダはセッション中スナップショットを維持する。** 滞在中にレーティングや
    タグが変わっても一覧へ反映しなくてよい。作り直すのは「スマートフォルダの外へ出て
    戻ったとき」と「ツールバーから同じスマートフォルダを開き直したとき」。
    後者が明示的な更新手段になる。
  - 既に同じ前提で動いている前例がある。`start_smart_folder_scope_nav` は現在の
    `SmartFolderViewState` を clone して `FolderNavMode::SmartFolder` へ渡し、その
    `folder_entries: Arc<Vec<PathBuf>>` は prepare 時点で凍結された並びである。
    **Ctrl+↑↓ は既に更新されない index の上を移動している。**
  - 読書履歴 / ブックマーク / レーティング一覧は軽いので、毎回更新する現行動作のままにする。
    重いスマートフォルダだけスナップショット方式にするのが、体感として最良という判断。
  - この裁定により、cache キーに `smart_folder_metadata_revision` を含める必要はなくなる
    (上の「2 だった場合」の記述より単純化できる)。
- **それでも維持すべき正しさ** (鮮度とは別問題):
  - 削除された項目を一覧に残さない。存在しないファイルのセルは、古いだけでなく開くと
    失敗する。既存の tombstone 経路 (`remove_paths_from_smart_folder_snapshots` /
    `smart_folder_removed_paths`) は snapshot に対して働くので、そのまま機能させる。
  - 定義の編集とグルーピング変更は、セッション中でも作り直す (結果の意味自体が変わるため)。
- **セッションの境界もユーザー裁定で決着済み (2026-07-31)**:
  - **セッション内 = そのスマートフォルダの結果と、そこから開いた下位フォルダ / PDF / ZIP だけ。**
  - **それ以外はすべて破棄。** アドレスバー入力、お気に入り、通常フォルダへの移動、
    別のスマートフォルダを開く、アプリ再起動。加えて **今いるスマートフォルダを
    ツールバー / メニュー / ゲームパッドから選び直した場合も破棄して作り直す**
    (これが明示的な更新手段になる)。
  - この線引きなら実装は驚くほど狭い。**`open_smart_folder()` は常にフルスキャンする現状の
    まま正しい**。メニュー ([ui_main.rs](../src/ui_main.rs) の `smart_folder_open`)、
    ツールバー (同 `toolbar_smart_folder_open`)、ゲームパッド
    ([gamepad_input.rs](../src/app/gamepad_input.rs)) はいずれもこの関数を通るので、
    明示的な開き直しは既に作り直しになっている。
  - よって直すのは **`restore_smart_folder_for_synthetic_path()` (セッション内の復帰) だけ**。
    ここが prepare 成果物まで再利用して即座に戻ればよい。
  - **セッションを出るときに snapshot を破棄する処理は必要**。破棄しないと、外へ出た後に
    戻る (Back で synthetic path へ戻る等) 経路が復帰扱いで古い結果を再利用してしまう。
    破棄点は 1 箇所が所有すること。経路ごとに破棄を書き足すと必ず漏れる。
- 設計時に決めること:
  - `smart_folder_metadata_refresh_due` の定期 prepare をセッション中どう扱うか。
    そのままだと定期的に作り直してこの方針の効果を打ち消す。
- 確認項目: 戻ったときにスクロール位置とカーソルが保たれること。ツールバーから開き直すと
  最新化されること。下位フォルダで削除した項目が戻ったときに消えていること。定義を編集して
  戻ると作り直されること。
