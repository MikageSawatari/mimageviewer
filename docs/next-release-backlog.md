...
### 1.234 完全リセットが `settings.db-shm` の削除に失敗し、無害な失敗を致命的として報告する (2026-09-13)

**P2。** 出典: 利用者報告 (2026-09-12) → **開発機で再現 (2026-09-13、開発者本人が操作)**。

#### 再現

ポータブル版 3.9.1 のデータフォルダを 3.9.0 の exe で起動し、設定の復元ダイアログから
**「完全リセット」** を選ぶ。

> 完全リセット の途中で致命的なエラーが発生しました。
> I/O エラー: プロセスはファイルにアクセスできません。別のプロセスが使用中です。(os error 32)
> 設定ファイルが不整合な状態になっている可能性があります。アプリを終了して再起動してください。

#### 失敗したファイルの特定 (残存ファイルから確定)

`full_reset` は `bak10..bak1 -> settings.db-shm -> settings.db-wal -> settings.db` の順に削除し、
**最初の失敗で即 return** する ([settings_restore.rs:474](../src/settings_restore.rs:474))。
失敗後のフォルダを確認したところ:

```
settings.db                        残 (削除順の最後)
settings.db-shm                    残  <- ここで止まった
settings.db-wal                    残 (0 バイト)
settings.db.bak1.before-restore-*  bak1 は削除済み (控えだけ残る)
settings.db.bak2.before-restore-*  bak2 も削除済み
```

**bak が消えていて `-shm` から先が残っている**ので、失敗は `settings.db-shm` の
`remove_file` で確定。

#### 要点 — strict さが、データを持たないファイルに適用されている

- **`-shm` は派生物**。SQLite がロック調整に使う共有メモリの実体で、次に DB を開けば
  作り直される。Windows ではメモリマップされるため、マッピングが外れるまで `DeleteFile` が
  `ERROR_SHARING_VIOLATION` (os error 32) になる。
- それでも `full_reset` はこの失敗を **`Terminal`** に分類する
  ([settings_restore.rs:488](../src/settings_restore.rs:488))。
- **実際の被害は世代バックアップ (bak1/bak2) の消失だけ**で、`settings.db` は無傷、
  `-wal` は 0 バイト。**データは失われていない**。`settings.db` を削除順の最後に置く設計
  (Codex P1 / 2026-05-17) が意図どおり効いている。
- にもかかわらず利用者には「設定ファイルが不整合な状態になっている可能性があります」という
  **最も強い警告**が出る。**被害の大きさと警告の強さが食い違っている。**
- `restore_from` 側が WAL/SHM 削除を strict にした根拠は「stale な `-wal` が再適用されて
  復元が部分的に巻き戻る」ことだった。**それは `-wal` の性質で、`-shm` には当てはまらない。**
  1 つの strict さを 2 つの別物へまとめて掛けている。

#### 未確認

- **`-shm` を誰がマップしたままなのか。** ダウングレード起動では boot が `db: None` を返すので
  `GLOBAL_DB` は空のはずで、**掴んでいるのは `SettingsDb` ではない**ところまでは言える。
- 符合する事実: 復元ダイアログの一覧生成 [`read_summary`](../src/settings_restore.rs:225) は
  `settings.db` を read-only で開く。WAL DB への read-only 接続も `-shm` へ書き込む。
  実際 `-shm` の更新時刻はリセット実行時刻と一致していた。**ただし確かめていない。**
- 次の観測: エラー発生中 (アプリはまだ生きている) に handle 列挙で保持者を特定する。
  併せて **`RestoreError::Io(e)` に失敗した操作と対象パスを載せる** (今はどのファイルの話か
  言わないため、残存ファイルから逆算するしかなかった)。

#### 静的読みで潰したもの (再訪不要)

- `with_db` の lazy 再オープンは起きない。`full_reset` は冒頭で `set_save_suppressed(true)` し、
  `with_db` は先頭で `save_suppressed()` を見て即 return する
  ([settings_db.rs:3813](../src/settings_db.rs:3813))。
- バックグラウンドのバックアップ worker はいない (`db_backup.rs` に `thread::spawn` は無く、
  `rotate_generation_backups` は同期呼び出し)。
- `settings.db` を開く箇所は `settings_db.rs` と `settings_restore.rs` の外に無い。

#### 対応方針

- **`-shm` の削除失敗を Terminal にしない。** データを持たないので、残っても次回起動で
  作り直される。`-wal` と `settings.db` は従来どおり strict のままにする。
- **どこまで進んだかを利用者に伝える。** 「bak は消えたが現在の設定は無事」と
  「設定が壊れたかもしれない」は別の話。**同じ文面で出さない。**
- 時間任せの retry 追加で済ませない。retry_io (50ms x 10) は既にあり、抜けていない。

#### 回帰確認

- `-shm` を掴んだ状態で完全リセット → **致命的エラーにならず**、`-wal` と `settings.db` が消える
- `-wal` の削除失敗は従来どおり Terminal のまま
- 成功時の挙動 (family 全削除 → 次回起動が clean install) が変わらない
- 失敗後も `before-restore-*` の退避が揃っている
- 規模 / 優先度: Small / P2。
