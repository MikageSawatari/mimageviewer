...
- 出典: 2026-08-26、利用者報告。**見開き表示のページ**で <kbd>Ctrl</kbd>+<kbd>G</kbd> を押して
  ...
  まったく効かなくなる。
- **2026-08-26 の調査で見つけた構造的な原因候補** (利用者の指摘「編集対象とするページを
  判断する階層がズレている可能性」から辿った):

  「見開き Double のときどちらのページを編集するか」の解決が**同じ 8 行で 4 か所に
  散っている**。

  | 場所 | `adjust_spread_target` を見るか |
  | --- | --- |
  | [app.rs `current_adjust_target_idx`](../src/app.rs) | 見る (正本のはず) |
  | [ui_adjustment_panel.rs `current_local_adjust_edit_idx`](../src/ui_adjustment_panel.rs) | 見る (写し) |
  | ui_adjustment_panel.rs の local adjust 描画内 (インライン) | 見る (写し) |
  | **[ui_fullscreen.rs のキー入場](../src/ui_fullscreen.rs)** (`enter_erase_mode(fs_idx)` / `enter_conceal_mode(fs_idx)` / `enter_text_mode(fs_idx)`) | **見ない。`fullscreen_idx` = ペアの anchor をそのまま渡す** |

  つまり <kbd>Ctrl</kbd>+<kbd>G</kbd> などキーから入ると、右ページを選んでいても左ページ
  (anchor) を対象にして入る。パネル側は右ページを見ているので、**入場した対象と操作する
  対象が食い違う**。「左右の切り替えが効かない」「マスク編集が動かない」はこの食い違いで
  説明が付く (未検証の仮説。着手時に実機で確かめること)。

- **直し方の方針**: 写しを 1 つずつ直さない。**「いま編集しているページ」を返す所有者を
  1 つにして全経路をそこへ通す**。[[feedback_one_owner_per_spelling]] と同じ形で、
  今日 2 回踏んでいる。表示単位 (見開きのペア / 横長分割の左右) は**表示の都合**であり、
  編集対象は常に元ページの index である、という切り分けを型で表せるとなお良い。

- 関連する既知の同型: 横長分割中に編集ツールへ入ると**半ページのまま**編集画面になっていた
  (2026-08-26 に `page_edit_tool_owns_canvas` で修正済み)。これも「表示単位が編集へ漏れる」
  同じ形。
