...
> **v3.4.0 から外した (2026-09-01、利用者判断)。** P0/P1 は 2026-08-31 に master へ
> マージ済みだったが、**旧 `custom_open_with_apps` を廃止して置き換える**変更なので、
> 未完成の面 (関連付けピッカーの一覧が部分的・シェル経由で起動できないハンドラがある)
> を含んだまま出すと、旧機能の利用者が中途半端な移行先を受け取ることになる。全体が
> できてから一度に出す方針にし、マージを revert した。
>
> - 実装は残っている: ブランチ `external-tool-launch` (revert 時点で master より 24 コミット先)。
>   正本ドキュメント `docs/external-tool-launch-plan.md` と各 brief も同ブランチにある。
> - **再投入の手順**: master 上で **revert コミットを revert してから**残りをマージする。
>   これを飛ばすと git は「既にマージ済み」と判断し、24 コミット分しか入らない。
> - revert では、同じブランチに載っていた無関係な 3 件 (`.cbz` を ZIP として索引に入れる、
>   選択拒否の理由表示、ゲームパッドの無言タグ経路) と、右クリックメニューの作り直し
>   (`context_menu_model.rs`) も一緒に戻っている。再投入時にまとめて入る。
...
### 1.147 360 ビュー中、ストリップ上でもホイールが 360 に取られる — **対応済み** (2026-08-31)
...
原因は [render_core.rs](../src/video/native_presenter/render_core.rs) の native wheel 変換で、
`panorama_wheel` が**他の判定より先に無条件で勝っていた**こと。360 のときは
`egui::Event::MouseWheel` を積まないので、ストリップは raw イベントすら受け取れない。
`over_seek_strip` は同じ関数の中で 3 行上に計算済みだったが、参照されずに素通りしていた。

「前後アイテムへ送らない条件」と「360 の視野角へ回さない条件」は同じ規則なので、
`pointer_region_owns_wheel` (ストリップ / 端パネル / モーダル) 1 つに集約し、
`immediate_native_wheel_command` と 360 の両方がそれを見る形にした。
`native_seek_strip_rect` は波形とサムネイル列を区別しないので、要望どおり両方が同じ扱いになる。

**端パネルとモーダルも同時に直っている。** 報告はストリップだけだが、360 が素通りしていた
のは同じ 1 行で、これらも同じように奪われていた。分けて直すと規則を 2 か所に書くことになる。
...
### 1.154 見開き縮小時、設定した 1px のページ間隔が 2px になる — **対応済み・実機確認済み** (2026-09-01)

- **v3.3.1 の縮小ボケ修正に伴う回帰。原因確定。P2。** 見開きのページ間隔を 1px に
  設定しても、縮小表示では画像によって 2px 空く。`拡大しない` で原寸表示になった場合は
  1px のままになるという報告で、コード上の丸め条件と一致する。
- 原因は、**見開き全体の配置と、各ページの最終ピクセル合わせが別々に間隔を決めている**こと。
  1. `layout_spread_page_rects` ([ui_fullscreen.rs](../src/ui_fullscreen.rs)) は、左右の小数幅を
     使って設定どおりの gap を空けた矩形を作る。縮小倍率は整数倍率ではないため、この段階では
     各ページの原点・幅を物理ピクセルへ寄せない。
  2. `draw_fs_spread_page` は左右それぞれを `RectPixelFit::Texels` の最終 transform へ通す。
     v3.3.1 の 1.0e 修正で、`snap_rect_to_physical_pixels`
     ([displayed_image_transform.rs](../src/displayed_image_transform.rs)) が縮小時にも原点を丸め、
     幅を `physical_pixel_extent` の整数 texel 数へ落とすようになった。
  3. 左右を**独立に**丸めるため、最初に作った gap が最終描画矩形では保存されない。
- 具体例: 2560x1440 の表示領域へ同じ 1249x2272 ページを 1px gap で高さフィットすると、
  理想の片ページ幅は約 791.62px。最終 transform は左を `x=488, width=791`、右を
  `x=1281` へ寄せるため、実際の gap は `1281 - (488 + 791) = 2px` になる。
  1120x1600 → 1008x1440 のように縮小後の幅が整数になる組み合わせでは 1px のままなので、
  「設定どおりの場合もある」という発症条件も説明できる。

#### 修正の不変条件

- **v3.3.1 の 1.0e 修正を戻さない。** リサンプル後テクスチャの整数サイズと貼り先矩形を
  一致させることは、Lanczos 出力へ二度目の線形補間を掛けないために必要。
- 見開きは左右ページを独立に寄せず、最終的な整数 texel サイズを確定した後で、
  **左右の見える内側端の距離が量子化済み `spread_page_gap_px` と必ず一致するよう、
  1 つのレイアウト所有者が一体で配置する。** 後段で片側だけ再スナップしない。
- gap 0 は密着、gap 1/2/3 はそれぞれ設定した物理 px 数を維持する。表示トリム中も、
  フル画像矩形ではなく実際の `paint_rect` の内側端で同じ不変条件を満たす。
- 描画・`FullscreenPageLayout` の hit-test・ナビゲータ・ルーペが、修正後の同じ最終 transform を
  参照する。見た目だけ片側をずらして入力座標を旧位置に残さない。
- 同じ配置関数を通る通常見開き、detached / frozen / holdover、連結読み内の見開きユニットを
  棚卸しし、同じ二段丸めを持つ経路をまとめて直す。

#### 回帰テスト

- **最終描画 transform の `paint_rect` で測る。** `layout_spread_page_rects` の中間矩形だけを
  検査すると今回の二段目のずれを見逃す。
...
- 寸法未確定のフォールバック分岐は移動量 0 (「最終幅が分かっている」前提が立たないため)。
  **連結読みも移動量 0 にしたが、これは誤り** (下の Codex Sol レビュー参照)。
- 回帰テストは `layout_spread_page_rects` → 最終 transform → 合わせ、まで通して
  `paint_rect` の間隔で測る。原点が物理ピクセル境界に乗ったままであること (= §1.0e を
  戻していないこと) も同じテストで見る。

##### 1 度目の修正は実機で直っていなかった (2026-08-31、利用者報告)

3 サイズ × gap 0-3 × ppp 1.0/1.25/1.5 のテストは通っていたが、**実機では直らず、
条件によっては悪化していた。** 幅を 900-1400px まで広げて全走査したところ 1,036 件失敗。

原因は `snap_rect_to_physical_pixels` の**3 分岐のうち 1 つを見落としていた**こと。あの関数は
...
で、「寄せた後だから整数に揃っている」という前提は**端数のある拡大では成り立たない**。
そこへ整数量の平行移動を足すと、`layout_spread_page_rects` が小数のまま正しく空けた間隔を
壊す (実測で 0.5px ずれ、gap 0 では**ページが重なる** = -0.375px)。

**論理倍率が 1 未満でも物理倍率は 1 以上になり得る**のが罠だった。ppp 1.25 の環境では
論理 0.9 倍の縮小表示が物理 1.125 倍の拡大になり、寄せは何もしない。利用者の環境
(高 DPI) はここに当たっていた。

修正: 3 分岐を `RectSnapMode` として名前を付け、`snap_rect_to_physical_pixels` 自身が
それを使う形にした。**同じ分岐を 2 か所に書かない。** 配置合わせは `RectSnapMode::None`
のページが片方でもあれば何もしない (レイアウトの小数の間隔がそのまま正しい)。

テストは幅 900-1400 × 左右同寸 / 異寸 × gap 0-3 × ppp 1.0/1.25/1.5/2.0 の全走査にした。
**両方向の変異で落ちる**ことを確認済み — 合わせを外すと 1,566 件 (報告どおり gap 1 が 2px)、
寄せ判定を外すと 1,036 件 (1 度目の修正の壊し方)。

##### 2 度目も直っていなかった — 自動トリムを入れていなかった (2026-08-31)

利用者が同じ本の別のページ対をキャプチャして「太さが違う」と報告。**自動トリムが ON** で、
そこが抜けていた。

利用者のファイル (`20230103_004.pdf`、26 ページ) は**全ページ寸法が違う**
(4155-4162 x 6038-6053)。スキャンした本ではこれが普通で、ページごとに丸めが変わる。
実測寸法とトリムを入れて走査すると 27/81 失敗、間隔は **-0.48px 〜 +1.70px** とばらついた
(gap 0 では**ページが重なる**)。

原因は**寄せる相手を間違えていた**こと。トリム中は `paint_rect` が全体矩形の小数分の 1 に
なり、**ラスタライズされるのは `paint_rect` の頂点**である (`paint_texture` はそこを頂点に
mesh を作る)。全体矩形の側をいくら整数へ寄せても、見えている端は端数のまま残る。
1 物理ピクセル単位の平行移動では、その端数を打ち消せない。

修正: 合わせるのは**内側の 2 辺** (= 間隔の境界) にした。左ページの右端を物理ピクセル境界へ
置き、右ページの左端をそこから `gap_px` に置く。トリムが無ければ可視幅は整数なので、
左ページの左端も従来どおり整数に乗り、§1.0e は変わらない。

テストは 2 本に分けた。`a_spread_keeps_the_configured_gap_in_the_rect_it_paints` が幅の全走査、
`a_trimmed_spread_keeps_the_gap_at_the_edges_the_viewer_sees` が実測寸法 + トリム。
後者は「全体矩形を寄せる」変異で 27/81 落ちる。

##### 3 度目 — 貼るテクスチャが差し替わると間隔が動いていた (2026-08-31)

利用者報告「線のサイズは安定した。ただ **AI アップスケールが効く前後で一瞬太さが変わる**」。

1 ページの表示は サムネイル → 元解像度 → AI アップスケール後 と差し替わる。ここで 2 つ起きる:

1. 倍率が変わるので `RectSnapMode` も変わる。「寄せていないなら動かさない」条件を置いて
   いたため、**中間の解像度でだけ合わせが効かなくなっていた**。
2. 中間テクスチャの縦横比は整数丸めで元と僅かに違う (例: 4161x6053 の 1/8 は 520x757 で
   比が 0.68692 対 0.68743)。`fs_texture_content_rect` がその比で縮めるので貼り先が
   レイアウト矩形より狭くなり、**「レイアウトが空けた間隔がそのまま正しい」も成り立たない**。

実測 1/8 解像度で 0.89px ずれ。修正は**その条件を外した**こと。見える 2 辺を直接置く形なら
寄せ済みかどうかに依らず答えは同じになるので、分岐する理由がそもそも無かった
(条件は 1 つ前の「フル画像矩形を寄せる」実装のために要ったもので、作り直したときに
一緒に消すべきだった)。

回帰テスト `the_spread_gap_does_not_change_when_the_texture_is_swapped` を追加。
テクスチャ倍率 1/16 〜 4 倍 × トリム有無 × gap 0-2 × ppp 1.0/1.25/1.5 で間隔が一定であること。
**条件を戻す変異で落ちる。**

##### Codex Sol のレビュー (2026-08-31) — **まだ終わっていない**

利用者の目には直ったが、`2db0aa75` は**最終承認されなかった**。重大 2 件 + 中 1 件。
生の回答は [sol-spread-gap-review.txt](review-v3.3.0/sol-spread-gap-review.txt)。

1. **重大: トリム時の移動が §1.0e のサンプリング位相を壊す (私が入れた退行)。**
   移動量を「物理ピクセル整数倍」と契約に書きながら、実装は整数境界から現在の
   `paint_rect` 辺を引いている。トリム境界は任意の f32 比なので、移動量は一般に端数になる。
   縮小時の Lanczos 出力は **full-image を整数サイズで**作り、それを移動後の full rect へ
   Linear sampler で貼るので、**テクセルが画素中心から外れて二度目の補間が復活する**。
   実測 fixture で原点が 253.0 → 252.82px。
   私は「トリム中はラスタライズされるのが `paint_rect` だけなので全体矩形の整列は無意味」と
   考えたが、**誤り**。`paint_rect` は全体矩形から導かれるので、全体矩形が格子から外れると
   テクセル全部がずれる。トリムは端を途中で切るだけで、位相は保たれていた。
2. **重大: 連結読みの 2 ページ見開きに元の不具合が残っている。** 私は移動量 0 にして
   「左右に並べないため」と書いたが、**確認せずに書いた誤り**。見開きモードの連結読みは
   `build_spread_display_units_for_nav` で 1 unit に左右 2 ページを入れ、
   `continuous_reading_page_rects` が gap 付きで横に並べる。1249x2272 x2 / 2560x1440 /
   gap 1 / ppp 1 で、左 `x=488 幅 791`、右 `x=1281` の**同じ 2px** が出る。
   detached の continuous frozen も同型。
3. **中: 範囲キャプチャが合わせ済み transform を再 snap する。** ページ選択は移動後の
   `FullscreenPageLayout` で行うのに、その後 Spread だけ `full_image_rect` から
   `RectPixelFit::Texels` で組み直すので、描画側 `x≈252.82` が `x=253` に戻り、同じ画面位置が
   約 0.72 source px ずれる。移動後の `page.transform` をそのまま使うのが所有権と整合する。

**Sol が言う「4 つ目の次元」は幅でも DPI でも解像度でもなく、リサンプラ分岐と所有経路**
だった。full-image / visible-region、paged / continuous / frozen、paint / capture の交差が
まだ不変条件に入っていない。**gap・出力テクスチャの位相・UV/clip を同じ配置所有者が
決める**必要がある。`RectSnapMode::None` 条件を戻すのは解決にならない (テクスチャ差し替え時の
瞬間変化が再発する)。

問題なしと確認された点: `translated_by` の `hit_rect` 再構築、transform の 2 回解決の性能
(重い GPU リサンプルは描画側 1 回だけ。永続キャッシュ不要)、paged frozen の見送り判断、
通常 paged / active detached / 2 ページ holdover が共通経路であること。

##### Sol の指摘 3 件への対応 (2026-08-31、`76148e06` ほか)

- **① トリムの端をテクセル境界へ寄せる** (`quantize_display_bbox_to_paint_pixels`)。
  端が画素境界に無いことが根なので、そこを直す。寄せると可視幅が整数になり、
  **間隔ちょうど**と**移動量が整数**が同時に成立する (今までは片方ずつしか取れなかった)。
  UV も同じ寄せ済み矩形から作る — 別々に作ると 1 テクセル分の拡大縮小が入り、寄せた意味が消える。
  ずらす量は 1 出力画素未満なので切り取り位置としては見えない。
  回帰は `a_trimmed_spread_keeps_...` に「移動量が整数」「原点が画素境界」を追加。
  **寄せを外す変異で 108 件落ちる**。
- **② 連結読みの見開き unit にも同じ合わせを適用**。`VerticalReadingPage` に `unit_id` を持たせ、
  `continuous_spread_pair_positions` が「同じ unit のちょうど 2 ページ」だけを組にする
  (並びだけで隣を相方にすると、次の unit の 1 ページ目を掴む)。
  `>= 2` に変える変異で落ちる。
- **③ 範囲コピーの transform を `Proportional` で作る**。`Texels` だと寄せ直しが入り、
  間隔合わせで動かした分が戻って、同じ画面位置が別の元画像座標を指していた
  (描画 252.82 / キャプチャ 253)。letterbox 補正はそのまま。変異で Sol の数値どおり落ちる。
- 配置合わせの**中心も関数側で決める**ようにした (呼び出し側が別々の中心を綴らないため)。

##### 5 度目 — 連結読みでは「合わせが動いていなかった」 (2026-08-31)

利用者報告「単一ページは大丈夫、連結読みだけ上下で太さが違う。ズームでも変わる」。

連結読みへ入れた合わせが**何もしていなかった**。矩形を解くときに
`fs_spread_page_paint_resource(idx, None, true)` と書き、**自分でテクスチャを選び直して**
いたため。描画ループはパススルー / 元画像プレビュー / キャッシュ済み処理結果 / final
pipeline / 遷移中から選ぶので別物になり、寸法も丸めも違う矩形で移動量を出していた
(サムネイルが無ければ移動量ゼロ)。**1 つの意味を 2 か所に綴る**という、この項目でずっと
繰り返している失敗そのもの。

修正: テクスチャの選択を**描画の前に 1 回だけ**行う専用パスへ移し、合わせも描画もその結果を
読む。アニメーションの駒送りはテクスチャを変えるので選ぶ前に済ませる。

なお `spread_page_gap_px` は**物理ピクセル量**で、拡大縮小では変わらない設計
(Z の照準ズーム中だけ意図的に一緒に拡大する)。「ズームで太さが変わる」も同じ
「合わせが効いていない継ぎ目」だった。

##### 実機確認 (2026-09-01)

利用者が確認済み: 連結読みの上下 unit で継ぎ目が同じ、スクロール / 拡大縮小でも変わらない。
単一ページ・見開き・自動トリム ON も問題なし。

##### Sol のテスト提案の消化 (2026-09-01)

- **提案 1 (出力テクセルと貼り先の一致)**: 走査に「貼り先幅がテクセル整数個」を追加。
  トリムがあっても寄せた bbox のおかげで整数になる。
- **提案 3 (production 配線)**: `the_drawn_spread_puts_the_configured_gap_between_the_pages`。
  `App` を組んで `fs_cache` へ実テクスチャを入れ、**`draw_fs_spread` を実際に走らせて**
  `FullscreenPageLayout` に積まれた最終 transform で測る。
  **合わせを外す変異で「gap=1: 描いた間隔 2px (設定 1px)」** — 利用者報告そのものが出る。
  helper 単体テストでは連結読みの取りこぼしを見抜けなかったので、これが本命。
  (このために `app::tests` を `pub(crate)` にした。兄弟モジュールから `setup_app` を使う。)
- **提案 4 (次元追加)**: 片側だけの trim、左右で違うテクスチャ倍率、rotation
  (None / Cw90 / Cw180)、非ゼロ viewport 原点を走査へ追加。**2,430 件**になり、
...
所有経路**。前 3 つは利用者の実環境を先に聞けば分かった。4 つ目は「helper を直接呼ぶ
テスト」しか書いていなかったことが原因で、production の配線 (描画 → `FullscreenPageLayout`
→ キャプチャ) を通していれば出た。**helper 単体テストで満足しない。**
...
**フル矩形が重なり、clip は重ならない**ことを要求しており、gap 0 でフル矩形同士を合わせる
今回の当て方と正面から矛盾する。
...
- 規模 / 優先度: Small〜Medium / P2。根因は確定しており、v3.3.1 の表示回帰なので次の修正版候補。

### 1.157 見開きで相方ページの GIF アニメが止まる — **対応済み** (2026-09-01)

- 出典: §1.154 の実機スモークで発見。「一覧から 1 ページ目を開くと 2 ページ目が止まる。
  何度か行き来すると動き出す」。
- **原因 (確定)**: 全フレームへの昇格 (`start_animation_promotion_if_needed`) が
  `self.fullscreen_idx != Some(idx)` で弾いており、**見開きの相方が対象外**だった。
  相方は第 1 フレームだけ (`StaticAnimationState::FirstFrameOnly`) のまま残り、
...
- **綴りは 4 か所ではなく 8 か所あった** (2026-09-01、2 度目の修正でも 3/4 ペアの右が
  止まると報告され判明)。残っていたのは:

  | 場所 | 綴り | 効果 |
  | --- | --- | --- |
  | 連結読みの先読みウィンドウ (stale 昇格) | `idx != current_idx` | 同じキャンセル |
  | 連結読みの先読みウィンドウ (backlog 掃除) | `entry.idx == current_idx` | 同じ取りこぼし |
  | 完了 upload の取り込み | `self.fullscreen_idx != Some(key)` → `continue` | **昇格が完了しても捨てる** |
  | 送信側 disconnect の後始末 | 昇格なら無条件 `mark_animation_promotion_failed` | **こちらが cancel した昇格を「失敗」扱い**。`PromotionFailed` は再試行しない終端状態なので、以後二度と動かない |

  最後の 2 つが「ページを送ると次の相方が止まる」の直接原因。前のペアで cancel された昇格が
  終端失敗になり、戻ってきても再試行されない。
- 修正: `page_is_displayed(idx, current, partner)` 1 つに集約し、**8 か所すべて**がそれを見る。
  相方は `displayed_spread_partner` が返す。cancel 由来の disconnect は失敗にせず、
  表示中のまま送信側が消えたときだけ本当の失敗として扱う。単ページ表示では従来どおり 1 枚だけ。
- 回帰テストは 3 本。判定そのもの / 先読みウィンドウを通しても昇格が残ること / そして
  **`promotion_only_asks_one_predicate_whether_a_page_is_displayed`** —
  `promotion_started_at_for` の周辺で `current_idx` や `Some(key)` と直接比べている綴りを
  ソース検査で禁止する。**同じ取りこぼしを 3 度やったので、機械的に止める。**
- **実機確認済み** (2026-09-01。一覧から開いた直後も、ページ送り後も左右とも動く)。

##### Codex Sol の 2 度目のレビュー (2026-09-01) — 対応と保留

生の回答は [sol-spread-round2-review.txt](review-v3.3.0/sol-spread-round2-review.txt)。
**重大 3 / 中 3。** 明示的な 8 判定はすべて共有述語を通っていると確認されたが、
**監査していたライフサイクル自体が足りていなかった**。

対応済み:

- **指摘 1 (重大、対応済み)**: アニメの次コマ期限が `fullscreen_idx` の 1 枚からしか
  立っていなかった。**現在ページが静止画なら相方の期限が一切立たない**ので、相方は入力が
  あるまで止まる。両方アニメでも相方は現在ページの周期に拘束される。
  `earliest_animation_deadline` で**描いたページ全部から最も早い期限**を取る形にした。
  対象を `fullscreen_page_layout` (= 実際に描いたページ) に限るのが要点で、`fs_cache` 全体から
  拾うと描いていないページの過ぎた期限で 0ms 起床を繰り返しアイドルが空転する。
- **指摘 3 (重大、対応済み)**: `displayed_spread_partner` が通常ナビ列 (動画を含む) で
  相方を解いていた。連結読みの unit は静止画列で組まれるので、動画が挟まると別のページを
  相方と答える。ページ送り側は既に同じ切り替えをしていた (`spread_page_nav`)。
  `resolve_spread_pair` を連結読み中は静止画列へ切り替えた。
- **指摘 6 (中、対応済み)**: ソース検査テストの窓が前後 3 行で、**実際に取りこぼした
  disconnect 経路 (説明コメント 4 行を挟んで 5 行下) を素通り**していた。窓を前 8 / 後 24 行へ
  広げ、コメント行を除いてから見るようにし、`fullscreen_idx == Some` も禁止綴りに加えた。
  旧綴りへ戻す変異で落ちることを確認。

保留 (いずれも**今回の変更で入った退行ではなく、既存の未解決**):

- **指摘 2 (重大)**: `RectSnapMode::None` (端数倍率の拡大) では trim 辺を寄せないが、
  visible-region Lanczos は整数サイズを出すので貼り先との間で再補間が残る。
  §1.0e の未解決部分で、縮小・整数倍率では直っている。走査もこのモードを明示的に
  除外している。**別項目として起こす。**
- **指摘 4 (中、対応済み 2026-09-01)**: `content_bbox` は要求されたままの未量子化の矩形なので、
  範囲コピーがそれを渡し直すと選択できる端が**描いた端と最大 0.5 画面 px** (縮小時は元画像で
  数 px) ずれていた。描画が寄せた後の矩形は `uv_rect` (寄せた表示矩形を元画像空間へ戻したもの)
  として transform が既に持っているので、そちらを渡すだけで一致する。
  回帰テスト `capture_region_spread_target_uses_the_trim_the_draw_quantized` は
  **テクセル境界に乗らない比率**を使い、生の bbox を渡す変異で落ちる (既存テストは整数境界の
  bbox を helper へ直接渡していたので検出できなかった)。
  **detached single snapshot は未対応** — 同じ raw bbox で再構築するので、live から passive へ
  移ると同じ差が戻る。
- **指摘 5 (中)**: 連結読みの unit 間 gap は raw bbox と量子化済み bbox が混在し、
...
#### 2 つ目の原因: ストリップの確保が「描けるか」で決まっていた (動画→動画でも出る)

- 利用者から「**動画同士の切り替えでも起きる**」との指摘。動画→動画は presenter を持ち回る
  (`attach_native_output`) 経路で固定状態を同期し直すので、上の 1 つ目では説明が付かない。
  「動画のサムネイルが一瞬出ているのでは」という推測も出たが、**フルスクリーンで動画の
  サムネイルは描かない** (`fs_display_texture_choice` が動画をフォールバック対象から外す。
  ポスターフレームで aspect がずれる問題として 2026-07-01 に外した経緯がある)。
- **原因**: 下側の確保に含めるストリップ高 (104pt) を、`seek_strip` の**スナップショットが
  今あるか**で決めていた。動画を切り替えると `reset_overlay_source_session` が新ファイル用に
...
- 出典: Codex Sol の 2 度目のレビュー (2026-09-01) 指摘 2。
  生の回答は [sol-spread-round2-review.txt](review-v3.3.0/sol-spread-round2-review.txt)。
- **これは「意図的に拡大したとき」だけの話ではない。** ppp 1.25 の環境で論理 0.9 倍表示は
  **物理 1.125 倍の拡大**になる。高 DPI では、ウィンドウよりわずかに小さい画像を出すだけで
  この分岐に入る。**通常閲覧で踏み得る。**
- 症状: `RectSnapMode::None` (物理倍率が 1 以上で整数でない) では矩形を寄せないが、
  visible-region Lanczos は**整数サイズの出力**を作る。貼り先は未量子化のままなので
  (例: 出力 1205px に対し貼り先 1205.55px)、egui/wgpu がもう一度 Linear で伸ばす。
  §1.0e で縮小時に消したのと同じ二度目の補間が、この分岐にだけ残っている。
- **§1.0e の当時から意図的に残されている** —
  [displayed_image_transform.rs](../src/displayed_image_transform.rs) の
  `snap_rect_to_physical_pixels` doc に「端数のある拡大には触れない。そちらの出力サイズは
  画像全体ではなく可視領域から決まるので、ここで全体矩形を寄せても一致しない」とある。
- **直し方**: 寄せる単位が縮小時と違う。全体矩形ではなく**可視領域の出力サイズと貼り先**を
  整数ピクセルで揃える契約にする。`paint_source_region_texture` が `full_image_rect` を
  基準に貼っているので、そこから見直す。`gpu_lanczos` の `target_and_source_region_for_branch`
  と対で決める。
- 回帰は §1.154 と同じ全走査に `RectSnapMode::None` を含める形で書ける
  (現在の走査は明示的に除外している)。
- 規模 / 優先度: 中 / **P2** (常用域で踏み得るが、症状は「わずかに眠い」で気付きにくい)。

### 1.159 連結読みのユニット間の間隔が、丸めの混在で最大 1 物理 px ばらつく

- 出典: Codex Sol の 2 度目のレビュー (2026-09-01) 指摘 5。
- ユニットの可視幅・高さとスクロール offset は**未量子化の `content_bbox`** から計算されるが、
  後段の transform は bbox の辺を量子化する。累積 offset 自体は
  `quantize_points_to_physical_pixels` を通っているので**中心間距離は揃う**が、
  **端と端の距離 (= 見える間隔) は揃わない**。
- 実害: `continuous_reading_gap_px` が既定 20px なら 19〜21px の揺れで気付きにくいが、
  **0px / 1px に設定している利用者では §1.154 とまったく同じ形の報告になる** (利用者判断)。
  gap 0 の密着も保証されない。
- 構造は §1.154 と同型 (「寄せ終わってから 1 つの所有者が置き直す」)。ただし**縦方向 ×
  N ユニット**で、位置がスクロール範囲の計算にも効くぶん慎重さが要る。
- 今回入れた合わせは**ユニット内の左右だけ**。ユニット間は対象外。
- 規模 / 優先度: 中 / **P2**。
...
- 出典: §1.154 の確認中に利用者が範囲コピー (フルスクリーン上部バーのカメラアイコンを
  **Ctrl+クリック** → ドラッグで範囲選択) を試して発見。**§1.154 とは別のバグ。**
- 症状 (見開き + 自動トリム ON):
  1. 選択範囲が**トリム分ずれ**、隣のページへ少しはみ出したところに限界がある
  2. コピーされた画像に隣のページは入らず、**トリムで切ったはずのこちら側の内容**が入る
  3. 左右にまたがる選択はできない
- **原因 (コード確認、確定)**: `capture_region_spread_transform_for_source`
  ([ui_fullscreen.rs](../src/ui_fullscreen.rs)) が **`content_bbox: None`** で transform を組む。
  トリム無しなので `paint_rect == full_image_rect` になり、
  - `hit_rect` が**見えている範囲ではなく画像全体の矩形**になるので、選択が可視領域を越える
  - `screen_to_source` は `full_image_rect` を基準に写すので、可視領域の外は
    **トリムで切った元画像の画素**を指す
  単一ページ経路は `page.transform` (bbox 付き) をそのまま使うので、この症状は出ない。
  **見開きだけがトリムを落としている。**
- 3 は仕様として未実装。`capture_region_target_at` がポインタ位置のページを 1 つ選び、
  その中だけを対象にする。左右にまたがる選択は「2 ページ分を合成して 1 枚にする」話になるので、
  1 と 2 (誤動作) とは分けて判断する。
- **直す順**: まず 1 と 2 (bbox を渡す)。3 は要望として別に扱う。
- 規模 / 優先度: 1+2 は Small / **P2** (見開き + トリムは既定に近い組み合わせで、
  取れる画が間違っている)。3 は Medium / P3。
- 回帰: トリム有りの見開きで、選択が可視領域でクランプされること、
  取れた crop がトリム後の内容と一致すること。単一ページ経路が変わっていないこと。

#### 実装 (2026-08-31) — 1 と 2 のみ。3 (横断選択) は未実装のまま

- `capture_region_spread_transform_for_source` に `content_bbox` を渡すようにした。
- あわせて `DisplayedImageTransform::resolve` をやめ、**描いた矩形の中へ元画像の縦横比で
  letterbox して `from_resolved_rect`** で組む形にした。`resolve` はトリム後の内容を
  viewport いっぱいに合わせ直すので、渡した矩形とは別の倍率になる (この形にしないと、
  bbox を渡した瞬間に選択範囲が別の縮尺になる)。
- 回帰テスト `capture_region_spread_target_cannot_reach_past_the_trim`:
  右 20% をトリムした見開きページで、画面右端まで引いても `hit_rect` が可視範囲を越えず、
  取れる crop がトリム後の内容に収まる。**`content_bbox: None` に戻す変異で落ちる。**
- 既存の letterbox テストと「描いた位置を引き継ぐ」テストも維持。

### 1.155 動画シークストリップに全体表示と高さプリセットを追加する — 専用スレ >>319 / >>325 (2026-08-31)

- **要望が具体化した。** 現在の「再生位置を中央に固定し、その周辺のサムネイル / 波形を
  流す表示」は残しつつ、動画全体を横幅へ収めたストリップも選べるようにする。
  全体表示ではサムネイル / 波形側を動かさず、現在位置を示す赤線が左から右へ移動する。
- 利用者の用途は精密なフレーム選択ではなく、**高さ 32〜48px 程度の小さなサムネイルで、
  場面や明るさの大まかな違いを把握すること**。粗い時間間隔でも問題ないとの回答を得た。
  既存の S キーによる動画タイル一覧は、別画面へ切り替えず動画を見ながら全体を把握したい
  という用途を満たさないため、シークストリップ側へ追加する。
- 既存実装との関係は §1.102 / §1.113 および
  [video-seek-strip-plan.md](video-seek-strip-plan.md)。着手時に正本側も更新する。
...
- 全体サムネイルはアスペクト比を維持し、指定された高さからセル幅を決める。利用可能幅に
  収まる枚数を算出し、動画の先頭から末尾までを等間隔で採る。横スクロールさせず、セル間に
  不自然な空白を作らない。要求は概観なので、正確な任意フレームを同期デコードせず、既存の
  近傍フレーム / キャッシュ / worker を使って段階的に埋める。
- 全体波形は動画全体を横幅へ写像する。長時間動画でも UI thread で全解析を待たず、既存の
  粗い波形データと §1.152 の永続キャッシュを利用して段階表示する。
- 赤線の位置は `current_time / duration` をストリップの左右端へ写像する。長さ不明・0・
  ライブ相当の入力では全体表示を成立させず、周辺表示へフォールバックする。

#### 高さと操作

- 高さは **大（現行）/ 中 / 小（約 48 logical pt）** の 3 段階を用意する。egui の UI は
  DPI に応じた論理座標なので、設定名は px 数を前面に出さず「大 / 中 / 小」とする。
  中の具体値は実機レイアウトを見て決める。
- `Shift+S` は、既定では「非表示 → サムネイル周辺 → サムネイル全体 → 波形周辺 →
  波形全体」を循環する。
- 右下のストリップボタンは、見開き設定と同様のメニューを開き、非表示と 4 モードを
  直接選べるようにする。高さも同じメニューまたはサブメニューから変更できるようにする。
- 環境設定では、4 モードそれぞれを **Shift+S の循環に含めるか**選べるようにする。
  これは機能自体の非表示設定にはしない。循環から外したモードも右下メニューからは直接
  選べるようにし、設定次第で到達不能にならないようにする。全解除は許可しないか、読み込み時に
  サムネイル周辺だけを有効へ正規化する。

#### 実装上の不変条件とテスト

- 現在は `SEEK_STRIP_HEIGHT` を前提に、ロック時の動画表示領域、strip rect、hover / hit-test、
  サイドパネル判定などを複数箇所で算出している。高さ可変化では、選択済みの高さを 1 つの
  layout snapshot で解決し、描画・入力・動画の予約領域が同じ値を見る。定数の局所置換で済ませない。
- ロックあり / なし、HUD 表示 / 非表示、フルスクリーン / native presenter、DPI
  100 / 125 / 150%、縦長 / 横長動画で、ストリップと映像・ボタン・hit-test がずれないこと。
- 4 モードと非表示の循環、無効化したモードの skip、右下メニューから全モードへ直接到達できる
  こと、全解除設定の正規化をテストする。
- 全体表示の先頭 / 中央 / 末尾で赤線が左右端 / 中央へ一致し、リサイズ後もサムネイルの
  アスペクト比と等間隔の時刻サンプリングを保つこと。デコード待ちで UI を止めないこと。

- 規模 / 優先度: Medium / P2。既存のストリップ worker とロック時レイアウトの拡張が中心。

### 1.158 右情報パネルをロックし、前後移動中も表示を維持する — 専用スレ >>330 (2026-09-01)

- **要望**: 静止画の右情報パネルを、前後のファイルへ移動しても開いたままにしたい。
  v2.3.0 頃までは `show_metadata_panel` のピン留めで可能だったが、左右パネルの表示方法を
  Hover / ClickToShow の 2 方式へ整理した際に Pinned を廃止し、現在の明示 open は
  ファイル移動時に閉じる per-file transient 状態になっている。
- **採用予定の仕様を利用者へ確認中。特に別の希望がなければ、以下で実装する。** 旧方式の
  「画像へ重ねたまま開状態だけを固定する」動作はそのまま戻さず、上下 HUD のロックと同じく、
  ロック中はパネル用の表示領域を確保して画像と重ならないようにする。

#### 仕様

- 静止画・本の右情報パネルのヘッダへ鍵ボタンを追加する。
- **ロック OFF**: 現行どおり画像へ重ねる一時表示。Hover / ClickToShow の召喚方法と、
  ファイル移動時に閉じる lifecycle を維持する。
- **ロック ON**: 右側にパネル幅の領域を確保し、画像の content rect を残りの領域へ再解決する。
  パネルは画像へ重ならず、前後のファイル・本のページへ移動しても開いたまま維持し、内容だけを
  新しい表示対象へ更新する。
- 鍵を外した後は現在の transient overlay へ戻す。フルスクリーンを閉じた場合はロックを解除し、
  次回起動や次回のフルスクリーン入場へは持ち越さない。永続設定にはしない。
- ロック状態は **viewer context ごとの一時状態**とする。メイン表示、F12、複数ウィンドウ間で
  共有する App-global bool にせず、別ウィンドウの操作で他方のパネルが開閉しないようにする。
- 今回の要望は以前ピン留めできた静止画の機能に対するものなので、対象はまず静止画・本とする。
  動画・音楽の左右パネルへの展開は別途要望があった場合に検討する。

#### 実装上の不変条件とテスト

- 右パネル幅、上 / 下 HUD のロック領域、画像 content rect を 1 つの layout snapshot で決め、
  描画・ズーム・パン・hit-test・ルーペ・ナビゲータが同じ矩形を見る。見た目だけ画像を左へ
  押して入力座標を旧位置に残さない。
- ウィンドウ幅が狭い場合も画像領域を負にせず、既存の最小パネル幅 / UI scaling と整合させる。
- ロック ON で前 / 次ファイル、本の前 / 次ページ、見開き、回転、表示トリムを移動しても
  パネルが維持され、情報が新しい対象へ更新されること。OFF では従来どおり閉じること。
- 上下 HUD のロックとの併用、DPI 100 / 125 / 150%、fit / 原寸 / 手動ズームを確認する。
- 複数ウィンドウで片方だけをロックし、他方の開閉・移動・終了へ影響しない回帰テストを置く。
  detached 経路へ触れる場合は [detached-rework-plan.md](detached-rework-plan.md) §2 / §11 に従い、
  App-global 状態へ症状パッチを追加しない。

- 規模 / 優先度: Medium / P2。パネル表示だけでなく、ロック時の画像表示領域と context 所有を
  一体で変更する必要がある。
