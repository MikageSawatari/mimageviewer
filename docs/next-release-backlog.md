...
- 利用者の言う「ウィンドウ表示」が `ViewerPresentation::MainWindow`
  ([app.rs:460](../src/app.rs:460)) なら、別 viewport ではなく**同一 viewport 内の描画順**の
  問題になり、経路が変わる。`main_viewer_blocked` は
  `fullscreen_idx.is_some() && !detached` ([app.rs:40468](../src/app.rs:40468)) なので
  MainWindow 表示でも真になり得る。
- 引数でパスを渡した起動が、どの presentation でビューアを開くかを先に確定する。
  `last_seen_version` を戻せば「重要な変更点」は再表示できるので、再現手順は作れる。

#### 対応方針

- 描く場所を `main_viewer_blocked` で分けるか、ビューアが別 viewport を持つ間は表示を遅らせる。
