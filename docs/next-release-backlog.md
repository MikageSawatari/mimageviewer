...
### 1.179 右クリックの最上位へファイルの「切り取り」「コピー」を追加する

- 出典: 専用スレ >>344 (2026-09-04)。「右クリックメニューの 1 ページ目に切り取りや
  コピーがほしい。長文入力以外ではキーボードを使わない」という要望。
- **新しいファイル操作基盤は作らない。** <kbd>Ctrl</kbd>+<kbd>X</kbd> / <kbd>Ctrl</kbd>+<kbd>C</kbd>
  は既に `App::invoke_shell_clipboard_verb` から Shell `IDataObject` と
  `OleSetClipboard` を使っており、Explorer と同じファイルクリップボードへ切り取り / コピーを
  設定できる。右クリックからもこの正本経路を呼ぶ。
- 実ファイル / 実フォルダの単一対象と、**実項目だけ**のチェック選択を対象に、mIV メニューの
  最上位へ「切り取り」「コピー」を追加する。複数選択では件数も表示する。
- ZIP / PDF 内ページなど単独の実パスを持たない項目は、ファイル操作として切り取り / コピー
  できない。既存の「画像をクリップボードにコピー」等とは別操作として扱い、仮想項目単独では
  ファイル操作 2 項目を出さない。実項目と仮想項目が混在するチェック選択も、一部だけを黙って
  処理せず、既存の <kbd>Ctrl</kbd>+<kbd>X</kbd> / <kbd>Ctrl</kbd>+<kbd>C</kbd> と同じ方針で無効化または
  理由を案内する。
- **「貼り付け」の新規実装は不要。** 一覧の空白右クリックには既に最上位の「貼り付け」があり、
  `invoke_shell_folder_background_verb(..., Paste)` で現在の実フォルダへ貼り付けている。
  コピー / 切り取り後に移動先の空白を右クリックすれば、マウスだけで一連の操作を完結できる。
  選択中のファイルを右クリックしたメニューへ「現在のフォルダへ貼り付け」を重複表示する変更や、
  フォルダ項目へ「この中へ貼り付け」を追加する変更は本項に含めない。
#### 実装前に確定した判断 (2026-09-05、コードと突き合わせて確認)

- **フォルダはチェックできる。** `grid_item_can_be_checked` は
  `is_checkable() || (subfolder_expansion_available() && Folder)` なので、通常のローカル
  フォルダ一覧では <kbd>Space</kbd> でフォルダをチェックできる。一方 <kbd>Ctrl</kbd>+<kbd>A</kbd>
  は `is_checkable()` だけを見るのでフォルダを含めない。「チェックできるか」の綴りが 2 つある。
  `src/ui_dialogs/context_menu.rs` の `collect_checked_indexed_paths` に付いている
  「通常の UI ではフォルダをチェックできない」というコメントは、サブフォルダ展開の追加時に
  古くなったまま残っている。**本項の実装で、このコメントを事実に合わせて直す。**
- **その結果、既存の穴が 1 つ見えている。** チェック集合の実パスを、
  <kbd>Ctrl</kbd>+<kbd>X</kbd>/<kbd>C</kbd> は `drag_source_path` (フォルダを含む) で作り、
  右クリックの `NativeGridContextMenuTarget::shell_paths` は `file_operation_path`
  (フォルダを除く) で作る。後者は 1 件でも該当しないと `None` になり、`paths` が空だと
  Windows メニュー自体が組み立てられない (`native_context_menu.rs` の `paths.is_empty()`)。
  つまり **今すでに「Space でフォルダをチェック → 右クリック」で Windows メニューが丸ごと
  消える**。同じ選択で <kbd>Ctrl</kbd>+<kbd>C</kbd> は成功する。
  **`shell_paths` のチェック分岐を `drag_source_path` に揃え、両入口を 1 つの述語にする。**
  これを直さずに最上位項目だけ足すと、フォルダを含む選択で新項目が出ない / 効かない。
- **`build_context_menu` の `has_checked` 分岐は実 / 仮想を知らない。** `ContextMenuInput` は
  `checked_count` しか持たない。`classify_checked_file_operation_selection`
  (`Empty` / `RealOnly` / `VirtualOnly` / `Mixed`) は既にあり 3 箇所で使われているので、
  その結果を input へ渡し、ゴミ箱と同じ前例 (混在はトースト全体拒否) に乗せる。
- **フルスクリーンにも出す。** 単一項目メニューの回転は Grid / Fullscreen の両方に既にある
  (`can_rotate` の `Fullscreen` 分岐)。Grid 限定なのは破壊的なゴミ箱の方だけで、
  切り取り / コピーはそれ自体では何も壊さない。フルスクリーンの実ファイルページでも出す。
- **Windows メニューとの重複**: 既定 (`show_windows_context_menu_inline = false`) では
  Windows メニューは位置 0 のサブメニューなので、同じ階層に重複は出ない。インライン展開を
  ON にすると shell 項目は **mIV 項目の後ろ (末尾)** へ差し込まれる
  (`shell_menu_insert_position`)。よって重複はメニューの上端と下端に分かれて出る。
  インライン ON のときに mIV 側を隠すと、その利用者だけ「切り取りが長いメニューの一番下」に
  戻り、本項の目的 (1 ページ目に置く) を満たせなくなる。**両方の配置で常に最上位へ出す。**

- 実装点: `MenuCommand` にファイル切り取り / コピーを追加し、`build_context_menu` の最上位へ
  配置する。dispatch は既存の Shell clipboard helper へ集約し、native / egui の両描画経路で
  同じメニューモデルを使う。対象集合は右クリック対象を解決済みの `shell_paths` を正本にし、
  `collect_shell_clipboard_paths` と述語を一致させる。
- 回帰確認: 実ファイル / 実フォルダの単一対象、実項目の複数選択、**フォルダを含むチェック
  選択**、仮想項目単独、実 / 仮想の混在選択、空白右クリックの既存「貼り付け」を model test で
  固定する。実 dispatch は <kbd>Ctrl</kbd>+<kbd>X</kbd> / <kbd>Ctrl</kbd>+<kbd>C</kbd> と同じパス集合と
  Shell verb を渡すことを確認する。フォルダ混じりのチェックで Windows メニューが消えないことも
  併せて固定する (上の既存の穴の回帰テスト)。
- 併せて更新: [docs/item-kind-capability-matrix.md](item-kind-capability-matrix.md) §6 / §7 / §8 は
  「ファイル操作 × 入口」の食い違いを追跡している表なので、入口が増えたら同時に直す。
- 規模 / 優先度: Small / P2。
...
- 出典: 専用スレ >>346 / >>349 / >>350 (2026-09-04)。「シークバーにサムネイルを出す
  機能を漫画でも使いたい」「従来のマウスオーバープレビューとストリップを選びたい」
  「動画のストリップ表示中は通常シークバーとマウスオーバープレビューを消したい」
  という要望。対応予定として扱う。
...
- **静止画のマウスオーバープレビューは、まだ存在しない。** `hover_preview_*` は
  `src/video/native_presenter/render_core.rs` にしかなく、静止画のページシークバーには
  ホバー時のサムネイル表示が無い (2026-09-05 確認)。本項はこれも**新規実装**として含む。
  下の設定はその前提で読む — 「静止画側は既存機能の設定化」ではなく、
  「ストリップとプレビューを両方作り、どちらを使うか選べるようにする」ということ。
  プレビューはシークバー上のカーソル位置が指すページのサムネイルを、バーの上へ吹き出しで出す。
  取得経路はストリップと同じ (既存サムネイルキャッシュ + 非同期 worker) にし、
  1 枚だけの要求として下の予算・抑制の議論に同じく従わせる。動画のような別 worker は作らない。
- 動画と静止画は利用目的と描画経路が異なるため、次の表示設定を**それぞれ独立して**持つ。
  **既定は動画に揃えて「プレビュー ON / ストリップ OFF」**とする (動画は
  `VideoSeekStripState::None` が既定でストリップ非表示、hover preview は常時有効)。
  静止画も同じく、プレビューは既定で出し、ストリップは既定で出さない。
  - マウスオーバーのサムネイル: `常に表示する` / `サムネイルストリップ表示中は表示しない` /
    `常に表示しない` の 3 択。非表示条件では画像を隠すだけでなく、新しいプレビュー要求も出さない。
    動画の音声波形表示は「サムネイルストリップ表示中」に含めず、従来どおり hover preview を使える。
    静止画側の既定も動画と同じ `常に表示する`。
  - サムネイルストリップ表示中の通常シークバー: `表示する` / `表示しない` の 2 択。
    `表示しない` はストリップ自身のクリック / ドラッグによる移動を残し、ストリップを閉じたときは
    通常シークバーを再表示する。動画の音声波形表示中は対象外とする。
...
- **ドラッグ中は先読みを止め、ストリップ / プレビューを最優先にする** (2026-09-05 決定)。
  旧 §1.181 (シークバー操作中の先読み過多) は v3.5.0 で解消済みで、
  `note_fullscreen_seek_activity` がシーク操作のたびに先読み時計を進め、
  `decide_prefetch_allowed` がその間の**投機的な**先読み enqueue を止める。
  ドラッグ中にシーク位置が動けば先読み結果は捨てられる可能性が高いので、**この抑制は
  そのまま効かせる**。
  一方ストリップとプレビューは投機ではなく、利用者がいま見ている前面の要求なので、
  **`enqueue_priority_thumbnail` の priority 経路に載せる**。gate は
  `!req.priority && !prefetch_ok` でしか止めない (`src/app.rs` の enqueue 判定) ため、
  priority=true の要求は抑制対象にならず、ドラッグ中でも即座に埋まる。
  **gate の条件を緩めてはならない。** 緩めると解消済みの引っかかりが戻る。要求の側を
  「投機」ではなく「前面」に分類することで解く。
  ストリップのセル数は表示幅から有界なので、priority lane が溢れない。
...
- **高さは動画と別設定にする** (2026-09-05 決定)。動画と同じく大 / 中 / 小 / 最小から選べる
  形にするが、**設定値は静止画専用**とし、動画側の変更が静止画へ波及しないようにする。
  静止画は波形も全体表示も持たず、現在ページ周辺のサムネイルだけなので、
  そもそも選ぶべき高さの基準が動画と違う。
  共有するのは `src/video/seek_strip_layout.rs` の純データ (高さ / セル幅の対応表) までとし、
  `SeekStripView` / `SeekStripShowing` / `SeekStripSpan` のような動画固有の表示状態と、
  `seek_strip_thumbs.rs` / `_wave.rs` / `_batch.rs` の worker は静止画側へ持ち込まない。
- レイアウトは 1 つの resolved geometry から、描画、hit-test、下端 hover、タッチ除外領域、
  固定時の media rect、左右パネルの下端を解く。**`FS_SEEK_BAR_HEIGHT` は現在 const として
  8 箇所以上に直書きされている** (panel rect / media rect / ページ番号 overlay / 補正パネル
  下端 / ナビゲータ / タッチ除外)。下端高を動的にするとは、この全箇所を解決済みの値へ
  切り替えることであり、そこが本項の作業の実体になる。所有者は既にテストのある
  `fullscreen_rect_excluding_fixed_bars` に置き、高さの綴りを 2 か所に作らない。
- 回帰確認: 単ページ / 見開き LTR / 見開き RTL / 連結読み、通常画像 / ZIP / PDF、
  ...
  戻していないことを確認する。
- **作業が 3 つ入っている。** 分割して出す判断もできるように分けて書く。
  1. 静止画ページシークバーのサムネイル列 (>>346)
  2. 静止画ページシークバーのマウスオーバープレビュー (新規実装、上記)
  3. 動画側の表示設定 2 つ (>>349 / >>350、native presenter 側のみ)
  1 と 2 はサムネイル取得経路と抑制の議論を共有するので一緒に作る。3 はコードを共有しない。
- 規模 / 優先度: Medium / P2。動画のフレーム抽出より簡単だが、固定バーの動的高さと
  サムネイル保持予算を同時に扱い、さらに静止画プレビューの新規実装も含むため、
  メニュー項目追加だけの Small 変更ではない。

### 1.185 「非表示 N件」の内訳に、設定で無視した書庫を明示する (2026-09-05)

- 出典: 専用スレ >>354-360。同名ファイル処理をすべて OFF にしても RAR が見えないという報告。
  原因は `環境設定 → パフォーマンス → キャッシュ → RAR / 7z / LZH の処理` が
  `無視する` だったこと。設定変更で解決し、**同名判定の不具合ではなかった**。
- 現在の `非表示 N件` は `同名など / 隠し項目 / 対象外` だけを表示する。`無視する` の RAR / 7z /
  LZH は一般の未対応形式と同じ `unsupported` へ入るため、利用者からは設定が原因だと分からず、
  同名除外と誤認しやすい。さらに、内訳に関係なく「同名ファイル設定を開く」を出している。

#### 修正方針

- `OmittedFolderEntryCounts` に、**設定で無視した変換対象書庫**の件数を独立して持たせる。
  `scan_directory_with_convertible_archives` の既存 `read_dir` ループで拡張子を判定できるため、
  追加のフォルダ走査や metadata I/O は行わない。
- `ArchiveFormat::from_extension` に該当し、`include_convertible_archives == false` の項目を
  `unsupported` ではなく新しい分類へ加える。従来の `primary_count` の合計値は変えず、
  同じ項目を二重計上しない。隠し属性が先に判定された書庫は、従来どおり `hidden` とする。
- 内訳には `RAR / 7z / LZH（設定で無視）: N件` など、原因がそのまま分かる名称を出す。
  件数がある場合だけ、キャッシュ設定の `RAR / 7z / LZH の処理` へ移動する導線を表示する。
- `同名ファイル設定を開く` は `same_name > 0` の場合だけ表示する。隠し項目・対象外だけのときに
  無関係な設定へ誘導しない。可能なら隠し項目も対応する設定への導線を同じ規則で出す。
- 通常フォルダ一覧と同じ走査結果を使う他経路、内訳の snapshot / remote 転送、テスト用 constructorを
  棚卸しし、新しい分類が途中で失われないようにする。
- 回帰確認:
  - `無視する` + RAR / 7z / LZH は一覧に出ず、新分類だけが増える。
  - `確認してからキャッシュを作成する` / `確認せずキャッシュを作成する` では一覧に出る。
  - 未対応拡張子は従来どおり「対象外」、隠し書庫は「隠し項目」、同名除外は「同名など」に入る。
  - 主表示の `非表示 N件` は修正前と同じ合計になり、内訳と一致する。
  - 内訳ごとの設定リンクが、該当件数の有無に合わせて表示される。
- 規模 / 優先度: Small〜Medium / P3。

### 1.184 お気に入りごとに表示状態を自動記憶・復元する (2026-09-05)

- **2026-09-05 の backlog 整理で完了項目と一緒に消えたため復元した。未着手。**
...
- 対象候補: 一覧のサムネイル / 詳細表示、詳細列の表示・順序・幅、一覧の並べ方、ソート条件、
  画像ビューワーの単ページ / 見開き / 連結など。実装前に既存 state の所有箇所を棚卸しし、
  1 つの型で対象を確定する。検索・絞り込み、選択、カーソル、スクロール、ズーム / パンなどの
  一時状態は記憶しない。列幅のドラッグ等は保存を debounce し、操作中に DB へ連続書き込みしない。
...
### 1.189 全体表示のストリップが埋まらない — run をまとめる前提が全体表示で逆転する (旧 §1.185)
...
- 規模 / 優先度: Small〜Medium / **P1**。1.186 と同時に入れる。

### 1.186 ストリップのセルは、一度時間切れになると二度と作り直されない
...
- 1.185 を入れる前の遅い条件を人工的に作っても、待てば最終的に埋まる
...
- **1 つ追加作業がある。** 他の 6 種は親フォルダ / ZIP タイルへ 1 段だけ集約するための
  ...
  `CropDb::load_keys(prefix)` は既にあるので、他と同じ形で `export_crop_page_keys` を足す。
  無いとページには出るがフォルダタイルには出ない。
- 規模 / 優先度: Small / P2。
