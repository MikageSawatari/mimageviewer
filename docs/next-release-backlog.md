...
### 1.48 5ch #171: 画像フォルダの見開き案内と固定バー / 設定画面 / 操作カスタマイズの小修正
...
- 画像フォルダが見開きにならない件:
  ...
  - 対応方針: まず仕様として案内する。必要なら、通常画像フォルダを単独画像として開いた場合に
    「本扱い OFF のため単ページ」トースト / 設定導線を出すか検討する。動画混在で隣動画を
    誤再生する問題を戻さないこと。
- 固定した上部情報バー / 下部シークバーの境界線:
  - 現状は `fullscreen_rect_excluding_fixed_bars` で画像領域からバー高さを除外し、
    `draw_fullscreen_seek_overlay` でバー矩形を塗った上に 1px の `hline` を描く。
    画像とバーの境界に明るい線が見えるため、線をバー内側へ収める / 色を弱める。
  - あわせて画像と固定バーの隙間を調整できる設定を追加する。初期値は現状互換に近い 0px。
    範囲は 0〜100px 程度。上部 / 下部で共通にするか個別にするかは UI 量を見て決める。
- 環境設定の右ペインスクロール:
  - 現状は左ツリーの選択が変わった時だけ `right_panel_scroll_generation` を進め、
    `ScrollArea` の id を変えて先頭へ戻している。ダイアログを閉じて同じページを開き直した場合は
    状態が残ることがある。
  - 対応方針: 環境設定を開くたびに右ペイン scroll generation を進め、毎回ページ先頭から表示する。
    設定項目としては増やさず、まず固定仕様にする。
- 操作カスタマイズの「場所」フィルタ:
  - `open_operation_assignment_editor` が `OperationAssignmentTarget::Key(action)` のとき、
    `state.operation_keyboard_context` を action の context へ上書きしている。これにより
    コマンド一覧の場所フィルタが「すべて」から勝手に特定場所へ切り替わる。
  - 対応方針: コマンド一覧の絞り込み用 context と、編集中コマンドの context / chord 割り当て候補用
    context を分離する。キー設定後も一覧フィルタは維持する。余裕があれば、コマンド一覧の
    検索欄 / 場所フィルタをスクロール外の固定ヘッダにする。
- 規模 / 優先度: Small〜Medium / P2。いずれもデータ破壊は無いが、v2.11.0 直後の
  利用者報告なので次版候補。

### 1.49 一覧に出ていないファイルの存在を知らせる (フォルダ削除の安全性を含む)
...
- 方針案 (A と B は独立に入れられる):
  ...
  - **(C) 一覧末尾に「他 N 件非表示」セル (利用者案) — 採らない**。仮想スクロールで末尾へ
    ...
    情報量に対して影響範囲が大きい。
- 決めること: 「対象外の拡張子」まで数えると `Thumbs.db` 等でノイズになる。内訳では分けて
  数え、チップの主数字は正規化 + 隠しに限る案を第一候補とする。
- 対象範囲: まず通常フォルダ一覧。サブ展開 / スマートフォルダ / 検索結果は後追い。
- 規模 / 優先度: (A) Small / **P2**、(B) Medium / P2。(A) を先に単独で出せる。

### 1.50 フルスクリーンのナビゲータ (縮小画像 + 現在表示範囲) — 利用者要望

- 出典: 利用者メール (pattier、2026-08-04)。フォトレタッチソフトのナビウィンドウ相当。
  パノラマなど大きな画像で拡大位置をすばやく動かしたい、という用途。
- 要件 (利用者案): 片隅に縮小画像 + 現在の表示範囲枠を出す / 表示位置を四隅で切り替え /
  ナビゲータ上の左ドラッグで範囲指定ズーム / 右ドラッグでパン / ダブルクリックで中心移動 /
  ホイールでナビゲータのサイズ変更。表示はキー (利用者案では Alt) で。
- 実装の足場 (座標変換を自前で組まないこと):
  - **`fullscreen_page_layout` (`displayed_image_transform::FullscreenPageLayout`) を使う**。
    ルーペと範囲キャプチャが既に同じ hit-test を共有しており、単ページ / 見開き / 連結読み /
    回転 / 補正後の「表示座標 ↔ 元画像座標」をここで一元管理している
    ([display-pipeline.md](display-pipeline.md))。
  - ズーム状態は通常の `fs_zoom` / `fs_pan` と、Z の全画面ズーム (`fs_zoom_active` /
    `fs_zoom_factor`) の 2 系統がある。どちらから呼ばれても同じ矩形計算になる形にする。
  - ナビゲータの絵はサムネイルカタログの縮小画像で足りる (位置把握が目的なので AI 拡大や
    補正の反映は不要)。回転と見開きの向きだけ合わせる。
- 連結読み (パノラマ) 対応: 全体を 1 枚に収めると極端に細長くなって役に立たないので、
  **現在ページ ± N ページ分の帯**に限定し、`fs_vertical_scroll` から現在ビュー矩形を出す。
- キーはルーペ (`FsLoupeLockToggle` = M / `FsLoupeHold` = Shift) と同じく `KeyAction` へ追加し、
  ルーペとの同時表示 / 排他を先に決める。
- 規模 / 優先度: Medium / **P2** (実装する方針)。連結読み対応は第 2 段階に分けてよい。

### 1.51 5ch #174: 一覧ツールバーのファイル名文字列フィルター
...
- UI 方針:
  ...
  - 幅は 120〜180px 程度を初期候補とし、狭い画面では縮めるか、虫眼鏡ボタンから展開する案を
    検討する。
...
- 性能方針:
  ...
  - 一覧構築時または初回使用時に basename の正規化済み文字列をキャッシュし、入力変更から
    100〜200ms 程度 debounce した後に worker で照合する。入力変更 / 一覧変更時は generation で
    古い結果を破棄する。
  - 結果保持は巨大な `HashSet<usize>` を避け、`items` 順の `Vec<usize>` または bitset を候補にする。
    `visible_indices` / `details_order` / サムネイル keep 範囲更新と統合し、詳細表示とサムネ表示で
    結果がずれないようにする。
  - IME 未確定中は検索を走らせない。処理中表示と件数表示は軽く出す。
- 既存機能との差:
...
### 1.52 見開きの高さ合わせがズーム倍率で入り切りして、レイアウトが跳ねる
...
- 方針 (利用者合意済み、2026-08-05): **判定をそのときの倍率ではなくフィット方式に移す。**
...
- ⚠ 実装時の注意: 同じ述語が [src/ui_fullscreen.rs:19664](../src/ui_fullscreen.rs) にもあり、
  そこでは `preserve_page_sizes` が **Z キーのズーム確定 (`resolve_spread_zip_zoom`) を
  走らせるかどうかの条件も兼ねている**。レイアウトの都合とズーム解決の都合が 1 つの bool に
  相乗りしているので、**まず 2 つの関心を分離してから**フィット方式ベースの判定に置き換える。
  片方だけ変えると Z ズームが壊れる。
- テスト: 解像度違いの見開きで、フィット方式ごとに倍率を変えても geometry の
  `scale_factor` が変わらないこと。同解像度の見開きで従来と同一であること。
  整数倍率でのドットバイドット (同解像度) が維持されること。
- 規模 / 優先度: Small〜Medium / **P2**。v2.12.0 に入れる。拡大の実装が落ち着いてから着手する。
