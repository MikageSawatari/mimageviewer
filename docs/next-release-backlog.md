...
### 1.268 サブ展開・検索結果・コレクション等の仮想一覧で、消しゴムの結果が表示に反映されない — 原因特定済み (2026-09-23)
...
- **ログ** (同セッションの `mimageviewer.log`、公開担当が解析): 通常表示ではサムネイルが edit preview を使い
  ...
  フルスクリーンでの保存マスク自動適用 (`erase: auto-applying saved mask`) は両方で動いていた。
- **原因 (コード、GPT-6 Sol の読み取り専用調査)**: サブ展開は合成フォルダパスで items を install し
  (`aggregate: None`、[subfolder_expansion.rs](../src/app/subfolder_expansion.rs))、読込時の `mask_pages`
  hydrate がその合成パスを DB prefix に使う ([app.rs](../src/app.rs) `resume_loading_items_after_sidecar`)。
  実画像の `page_path_key` は正しいが、index ベースの `mask_pages` に入らない。
  表示用の編集結果の組み立ては `mask_pages.contains(idx)` を条件にするため、自動 inpaint が走っても通常表示に
  選ばれない (画素比較は未実施、コード上の結論)。ツールの保存経路が `mask_pages` を立てるので、開いて閉じると直る。
- **データへの影響**: 閉じたときの誤った「編集なし」判定は、共有の **edit preview (派生キャッシュ)** の削除を
  キューに積み得る。**マスク本体 (mask DB) は消さない**。削除された場合、通常フォルダ側のサムネイルも一時的に
  未編集で出る可能性がある (このログでは既存行を消したかまでは証明できない)。
- **影響範囲 (コード上)**: 同じ経路 = サブ展開、Ctrl+G 検索結果、コレクション (v4.0.0 の新機能も該当)、
  閲覧履歴、レーティング一覧。**影響なし** = 通常フォルダ、ZIP/PDF のページ、スマートフォルダ (exact-key の
  aggregate metadata を事前に用意する)。同じ合成 prefix の問題は補正・書き出しトリミング・隠蔽・注釈にも
  及び得る (未検証)。サブ展開ではローカル調整と表示トリムは exact key で入る。
- **既存の不具合**: サブ展開 `abad79e5e` (2026-06-24)、edit preview の close 判定 `4eb6c50fd2` (2026-07-17)。
  どちらも v3.10.0 以前。v4.0.0 の退行ではない。
- 直し方の方向: 仮想一覧も smart folder と同じく **exact page key で編集状態を事前に用意する** (合成 prefix の
  hydrate をやめる)。close 判定は index 集合ではなく page key の正本から導く。修正時は第 1 層に
  「仮想一覧で開いた編集済み画像が描画に反映される」「閉じても edit preview を消さない」シナリオを足す。
- 利用者判断 (2026-09-23): 以前からの不具合ならバックログに積んで公開を進める。
- 規模 / 優先度: 中 / P2。
