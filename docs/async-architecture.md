...
| 明示メタ情報転送 | `std::thread` (`metadata-export` / `metadata-import-preview` / `metadata-import`) + mpsc + `AtomicBool` | 完全モーダル中に最大 1 | `mimageviewer.meta.miv` の再帰列挙、JSON 検証・原子的 export、rating/tags/video+audio/book bookmark DB の import を UI thread 外で行う。開始前に既存 metadata writer / rename migration を drain。export は temp 公開前なら全破棄、import は物理項目ごとの transaction 境界で cancel し完了済み項目を保持する。進捗は 50ms repaint で poll する |
