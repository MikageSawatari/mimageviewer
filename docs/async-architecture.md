...
| mIV Remote 時計なし transcode (移行 1/3、未配線) | 呼出側 worker + `Mutex` / `Condvar` / cancel flag | benchmark または将来の streaming generation につき 1 本 | `clockless_transcode` が再生器と別の FFmpeg `Input` を開き、demux / video+audio decode / 既存 H.264+AAC encoder / fMP4 segmenter を wall clock なしで直列駆動する。完成 segment が ring 上限に達したら `Condvar` で park し、解放で再開する。cancel は demux、decode、readback、scale+encode、audio encode、mux の段境界で確認する。現時点は dev-tools benchmark だけが呼び、本番 `VideoStreamSession` と既存 generation worker は変更しない |
