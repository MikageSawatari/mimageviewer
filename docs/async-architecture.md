...
| 本ブックマーク DB | `std::thread` (`book-bookmarks`) + mpsc | App-global 1 | `book_bookmarks.db` の追加 / コンテナ別一覧 / 全件一覧 / 削除 / path migration を直列処理する。製本 worker は最初のファイル変更前に元 path → 最終 path を同 DB の journal table へ書き、サービスは bookmark migration と journal 消去を同一 transaction で commit する。起動時は残存 journal を古い順に冪等再実行し、DB busy / parse failure の entry は消さず次回 retry に残す。UI は正規化前の表示値と identity を request で送り、result event からメモリ上の現在本一覧を更新する。SQLite open / schema 作成を含め UI スレッドでは行わない |
...
| 明示メタ情報転送 | `std::thread` (`metadata-export` / `metadata-import-preview` / `metadata-import`) + mpsc + `AtomicBool` | 完全モーダル中に最大 1 | `mimageviewer.meta.miv` の再帰列挙、JSON 検証・原子的 export、rating/tags/video+audio/book bookmark DB の import を UI thread 外で行う。列挙は `read_dir` を逐次消費し、メタ情報DBは一時scope表とpath indexで列挙済み項目だけを読む。開始前に既存 metadata writer / rename migration を drain。export は temp 公開前なら全破棄、import は物理項目ごとの transaction 境界で cancel し完了済み項目を保持する。進捗は 50ms repaint で poll する |
