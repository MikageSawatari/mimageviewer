...
| RAR / 7z / LZH スキャン・変換 | `std::thread` (scan / convert ごとの使い捨て) + mpsc | `ArchiveConvertState` 1 件 | 直接 RAR 判定、画像 inventory、パスワード再試行、キャッシュ ZIP 変換を行う。ブックマーク起点では `ArchiveConvertState.completion` が request ID と target identity を保持し、直接 RAR または元アーカイブ → キャッシュ ZIP の mount 後に同じ owner でページ待機へ進める。確認・パスワード・変換中は通常の 45 秒 resolve timeout を適用せず、activation / 通常 navigation / 後続ブックマークは一致する state の cancel token を立て receiver を drop する。完了は owner が現在値と一致する場合だけ表示へ適用する |
