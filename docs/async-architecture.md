...
| 明示メタ情報転送 | `std::thread` (`metadata-export` / `metadata-import-preview` / `metadata-import`) + mpsc + `AtomicBool` | 完全モーダル中に最大 1 | `mimageviewer.meta.miv` の再帰列挙、JSON 検証・原子的 export と、評価 / タグ / ブックマーク / 見開き / 表示トリム / 回転 / ページ編集 / サムネピン DB の import を UI thread 外で行う。列挙は `read_dir` を逐次消費し、メタ情報DBは一時scope表とpath indexで列挙済み項目だけを読む。開始前に pending view-trim をメモリsnapshotとしてworkerへ渡し、既存 metadata writer / rename migration を drainする。dirtyな`mimageviewer.dat`群とAppのTagsDb connectionも所有権ごとimport workerへ移してflush / journal-mode handoff / 再openを行い、いずれかのflush失敗時はDB更新を開始しない。export は temp 公開前なら全破棄、import は15ストアをrollback journal検査済みの単一attached transactionへまとめ、物理項目ごとのcommit境界でcancelし完了済み項目を保持する。commit済み項目は軽量な全体rollupに加え、開始時に表示中だった項目だけ評価・タグ・ページ状態・コンテナ状態・ピンの実値をworker結果へ載せる。UIはSQLite再読込や同期フォルダreloadをせず在メモリcacheへ適用する。進捗は50ms repaintでpollする |
...
| RAR / 7z / LZH / ZIP スキャン・変換 | `std::thread` (scan / convert ごとの使い捨て) + mpsc | `ArchiveConvertState` 1 件 | 直接 RAR 判定、画像 inventory、パスワード再試行、キャッシュ ZIP 変換を行う。`ArchiveConvertState.cancel` は事前 scan から変換完了までの単一 owner で、RAR / 7z / LZH / ZIP の各 entry 境界が同じ token を確認する。state drop、Esc / cancel、activation、競合する通常 navigation、後続 bookmark は token を立て receiver を drop し、古い worker と late result を同時に無効化する。ブックマーク起点では `completion` が request ID と target identity も保持し、直接 RAR または元アーカイブ → キャッシュ ZIP の mount 後に同じ owner でページ待機へ進める。確認・パスワード・変換中は通常の45秒 resolve timeoutを適用せず、完了はownerが現在値と一致する場合だけ表示へ適用する |
