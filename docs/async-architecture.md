...
| 動画音声 RT 出力 | `cpal::Stream` 内部スレッド | 動画 1 つにつき 1 本 | WASAPI Shared モード。コールバックで ring buffer から f32 stereo を pop し、`AvClock::set_audio_pts` でマスタークロックを更新。ただし engine_state が `LOADING` / `IDLE` (= cpal stream 起動直後の pre-fill burst 期間) と `!clock.is_playing()` (= 一時停止 / EOF) の間は silence + 早期 return で `next_pts_secs` 進行を skip し、`set_audio_pts` を呼ばない (Phase 9.A〜9.E、詳細は [docs/video-engine-redesign.md](video-engine-redesign.md) の Phase 9 シリーズの追加修正節) |
