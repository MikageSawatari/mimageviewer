...
| mIV Remote Page 最終合成 | 既存 `remote-ipc-worker-*` heavy lane 上で同期実行 | server ごとに 1〜2 本、うち prefetch 最大 1 | Page 要求ごとに committed な page / favorite / global 補正を DB から読み、raw decode 後の縮小 `ColorImage` を共有 `FinalCompositePlan` / executor へ通して WebP 化する。App の cache / `egui::Context` は触らず、remote が合成済み pixels LRU (8 件 / 128 MiB)、parsed LUT LRU (16 件)、cancel token を所有する。新しい prefetch と foreground は進行中 prefetch を cancel し、decode / colorize loop / executor 境界が同じ token を見る。cache key は source stat、target_px、解決済み全補正と LUT entry を含み、PC 側 commit 後の次要求は古い cache に当たらない。Page IPC timeout は 60 秒、各段時間は `remote_ipc: final_composite` log に出す。詳細は [web-remote-left-panel-plan.md](web-remote-left-panel-plan.md) の「段 1a 補正適用の実装結果」 |
