...
| 明示メタ情報転送 | `std::thread` (`metadata-export` / `metadata-import-preview` / `metadata-import`) + mpsc + `AtomicBool` | 完全モーダル中に最大 1 | `mimageviewer.meta.miv` の再帰列挙、JSON 検証・原子的 export と、評価 / タグ / ブックマーク / 見開き / 表示トリム / 回転 / ページ編集 / サムネピン DB の import を UI thread 外で行う。列挙は `read_dir` を逐次消費し、メタ情報DBは一時scope表とpath indexで列挙済み項目だけを読む。開始前に pending view-trim と既存 metadata writer / rename migration を drainし、dirtyな`mimageviewer.dat`群とAppのTagsDb connectionは所有権ごとimport workerへ移してflush / journal-mode handoff / 再openを行う。flush失敗時はDB更新を開始しない。export は temp 公開前なら全破棄、import は15ストアをrollback journal検査済みの単一attached transactionへまとめ、物理項目ごとのcommit境界でcancelし完了済み項目を保持する。完了後は同じ実フォルダの表示 snapshot を一度再構築する。進捗は50ms repaintでpollする |
