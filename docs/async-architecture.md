...
| スマートフォルダ snapshot | `std::thread` (`smart-folder-scan` / `smart-folder-prepare`) + mpsc | 最大 1 scan + 1 prepare | 保存済みの複数検索元から画像本 / ZIP / PDF / 対応アーカイブを列挙し、★ / タグ等を batch 取得してフラット一覧を構築する。UI は完成 snapshot のみを install し、定義変更・移動・終了時は cancel、generation と定義 snapshot の一致で stale 結果を拒否する |
