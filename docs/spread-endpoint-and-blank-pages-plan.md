...
- paged と縦横連結は同じ decoration geometry を使う。holdover は捕捉時の実 texture・白 rect・clip・色を一体で保持し、後の設定変更から再計算しない。F12 active と passive frozen snapshot にも白 decoration を保存し、実描画で一致を検証する。Remote は実 address 1 件と address の無い white decoration を送る。Web は実ページの ready presentation に従属する白い DOM 矩形として描き、白用 fetch・decode・cache・edit target を作らない。通常の §1.218 endpoint singleton の空き側は背景色のまま。
