...
| D7 | **既存 facet タグフィルタは残すが UI 二層化** (現一覧タグ + 検索) + タグ非対応アイテムは素通り | コンテナを開いても中身が消えない。★式トースト不要。§5.2 |
...
- **タグブラウザは入力欄右の「一覧▼」へ集約 (UX レビュー【中6】)**: Ctrl+T の検索欄横から、ピン留め
  タグ全件 → 最近使ったタグ 20 件 → 件数が多いタグ 20 件を選ぶ。常時チップ列は横幅を圧迫するため廃止。
  タグ 0 件時はメニュー内を空状態にする。
...
- **プルダウンの列挙は現在の表示集合に含まれるタグを優先**する ([src/ui_main.rs:1970](../src/ui_main.rs))。
  フォルダ内に実在する ad-hoc タグを隠さず、検索欄から全タグにも到達できる。
- **プルダウンにインクリメンタル検索フィールドを 1 本足す (UX レビュー【高2】)**: 現在の一覧に出ている
  タグに加え、§6.5 の補完 UI を再利用して全タグへ到達できるようにする。これが無いと「現フォルダの
  2,000 枚を ad-hoc タグ `klee` で絞る」最頻フロー (Eagle 移行組) が、facet と Ctrl+T (全ライブラリ)
  の隙間に落ちる。
...
- **実 casing の復元 (v1.4.0 リリース前レビュー)**: item_key は小文字正規化なので、キーから直接
  ...
  ディスク上の casing を復元する (`\\?\` プレフィックスは剥がす。シンボリックリンク解決で**別パス**に
  なった場合は採用しない — item_key とずれてタグ操作が空振りするため)。
- **case-sensitive ディレクトリ保護 (同上)**: DevDrive / WSL の case-sensitive フラグ付きディレクトリ
  では小文字キーの `fs::metadata` が実在ファイルでも失敗する。Missing 判定の前に親ディレクトリを
  1 回だけ走査して大小無視一致を探し、見つかれば実 casing で Existing 扱いにする (誤 prune 防止)。
...
- 「ピン留めタグの管理…」ダイアログ (語彙 = ピンの管理) で各タグに **□ メニュー・ツールバーに表示 (ピン留め)** を
  付ける (§13.2 の確定語彙。UI 文言はこちらに統一)。
- `show_shortcut = true` のタグだけが **メニュー / ツールバーのボタン** に出る。facet 絞り込みは現在の一覧に
  含まれるタグと検索候補から選ぶ。
...
アイテムを選択して **「タグを付ける/外す…」**((1) の「ピン留めタグの管理…」とは別物。§13.2 の確定語彙、
「編集」という語はどちらにも使わない) を押すと開く。高カーディナリティ前提の付与 UX:

- **自由記入 + インクリメンタル検索**: 入力に対し過去使用タグを前方一致で候補表示。
- **ピン留めタグの一覧表示**: 入力欄が空のとき、ピン留めタグを最近使ったタグとは別に先頭へ表示。
...
- **自動 seed のファイル読みは先頭 2MiB まで**。大量フォルダで大きな画像/コンテナを連続丸読みして
  ディスクを占有しないため、保険経路は bounded read にする。動画の同名 `.xmp` sidecar はメディア本体
  ではないため全体を読む。完全性が必要な場合は §7.3 の明示コマンドで読み直す。
...
- 「取り込むだけ (ファイルは変更しない)」も併置。選択中の Image / Video に対する右クリックメニュー
  およびメニュー「タグ」から実行する。フォルダ/ライブラリ再帰一括版は、進捗・キャンセル・走査範囲の
  UX を別途詰める後続 polish とする (Ctrl+A / チェック一括で現在一覧の実用代替は可能)。
...
- **削除は DB 更新成功後のみ**: 「取り込んで削除」は先に `tags.db` へ union + state 更新し、成功した item だけ
  `xmp_writer::TagOp::ClearMiv` でファイル側の `#` 要素を除去する。非 `#` の `dc:subject` は保持する。
  動画 `.xmp` は mIV 旧タグだけの空殻になった場合だけ削除し、裸タグや `xmp:Rating` 等が残る場合は保持する。
...
- **タグ一致セマンティクス = 「クリック/完全入力 = exact、入力途中 = prefix」で確定 (ユーザー決定 2026-06-12)**。
  ブラウザのタグチップクリックと、ライブラリに完全一致するタグ名の入力は **exact 一致** (チップの GROUP BY
  件数と結果件数が一致する)。タイプ途中の部分文字列は `item_keys_by_tag_prefix` の **prefix 一致**で候補を広げる。
  実装は exact 経路 (`tag_key =` 完全一致) を `item_keys_by_tag_prefix` (前方一致) と別に持つ。
- タグビューの複数タグは **AND**。`#cat #dog` は各トークンを exact 優先 (ヒット 0 なら prefix) で解決し、
  `item_key` の交差で検索する。タグ名に空白は許可せず、空白は複数タグの区切りとして扱う。
- タグビューのクエリ構文の残り: 除外 / (将来 OR) の指定方法と、Ctrl+T 入力欄の UX。
...
- **Ctrl+G 橋渡しの照合規則は実装済み (2026-06-12)** (§13.1、UX 2-7): 正の各トークンを個別に照合し、
  完全一致を優先、完全一致が無ければ前方一致候補を最大 3 件出す。`#tag` と裸の `tag` は同じタグキーへ正規化する。
...
- **メニューに「タグビュー (Ctrl+T)」を必ず置く** (Ctrl+T の文脈分裂対策。キーを知らなくても到達可能に)。

### 13.2 タグ運用 UI

- **二層は「型」でなく「ピン」として見せる** (UX レビュー【中7】): 実体は単一タグ + `show_shortcut`
  属性。ユーザー向け語彙は「**よく使うタグをピン留めするとメニュー・ツールバー・絞り込みに出ます**」に
  統一。チェックボックス文言も「□ メニュー・ツールバーに表示 (ピン留め)」。ピン留めタグ管理ダイアログ/タグ
  ビューから**その場でワンクリックでピン留め**できる導線を足す (昇格を自然に)。
- **ダイアログ名は動詞で分離** (UX レビュー【中8】、v1.0 の「タグを編集…」二義性の解消): 選択への
  付与/削除 = **「タグを付ける/外す…」**、語彙の管理 = **「ピン留めタグの管理…」**。「編集」はどちらにも使わない。
- **改名・統合 UI を v1 必須に** (UX レビュー【中9】): 「ピン留めタグの管理」に **改名** (表示名変更、tag_key 不変なら
...
- **`tags.db` に settings.db 同様の世代バックアップ** (`tags.db.bak1..bak10`) を実装済み (2026-06-12)。
  プロセス内で対象 DB パスごとの最初のタグ書き込み前に 1 回だけ snapshot を作る。「DB が壊れたら /
  PC を替えたらタグはどうなる」というロックイン不安への最低限の答え。**外部書き出し (XMP/CSV) は v1
  ではしない** (非破壊・外部書き込み無し方針の維持。将来の明示エクスポートは §10)。
