...
| A2 | A1 の窓をピン。メインで別画像を開く | ピン窓は不変。新しい active linked 窓が開く |
| A3 | ピン窓をクリックして active 化し、戻って linked 窓をクリック | linked 窓が active_context=false / independent=false に戻り、メイン選択と再連動する |
| A4 | ピン窓を foreground にして `Ctrl+↓` / `Ctrl+PageDown` | フォルダ移動は開始せず toast。メイン一覧も同時に移動しない |
| A5 | A3 の linked 窓で `V`、`Shift+Z`、消しゴムを起動/終了 | linked 窓では従来どおり動く。編集中はピンボタンが押せず tooltip が出る |
| A6 | ピン窓側で `V`、`Shift+Z`、消しゴムを軽く確認 | `V`/`Shift+Z` は動く。消しゴムなどの編集機能は無効で案内が出る。メイン操作では中身が変わらない |
