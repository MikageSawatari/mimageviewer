...
| 操作 | 現状 |
| --- | --- |
| ホイール / ↑↓ / 画像の ←→ | `visible_indices` 内の前後アイテムへ移動。通常の←→によるページ移動は、環境設定でページ表示 / 読み方向（既定）またはページシークバーの実効方向に合わせられる。縦連結ではホイール / ↑↓ / PageUp/PageDown は縦スクロールし、←→はこの設定に従って前後の静止画へ移動する。横連結ではホイール / ←→ / PageUp/PageDown は横スクロールし、↑↓ で前後の静止画へ移動する。どちらも途中の動画・音声は飛ばす。ページ単位表示では従来どおり動画・音声も前後移動の対象。独立 detached 静止画窓は、仮想一覧から開いても同じ実親または同じ ZIP / PDF を非同期に完全列挙した物理順を使い、検索で欠落した同一フォルダ画像も含める |
| 同一一覧の先頭 / 末尾 | `FsBoundaryHint::Edge` を中央表示し、「先頭です」/「末尾です」と Home / End、Ctrl+↑↓ を案内 |
...
| マウス戻る / 進む / ホイールクリック | native window / HUD 側の XButton と中ボタン短クリックを App 側のマウスボタン割り当て経路へ接続。既定は戻る / 進むが履歴、ホイールクリックは未割り当て。画像 / 動画フルスクリーンのマウスボタン候補では `C:\`〜`Z:\`、お気に入り、閲覧履歴、★一覧などの場所移動系は出さない。動画の小 / 中 / 大シーク 6 操作は v3.7.0 では候補に出さず、保存済み割り当ても実行しない（値は保持）。ウィンドウ最小化は候補に含み、native video では最小化後も再生を継続する。従来どおりを選んだ既存環境では戻る / 進むのみ Ctrl+↑↓ と同じ |
| S | 動画タイルモード ON/OFF |
...
| 同一一覧の先頭 / 末尾 | native overlay toast で「先頭です」/「末尾です」+ Ctrl+↑↓ の案内を表示 |
...
| マウス戻る / 進む / ホイールクリック | 環境設定「マウスボタン」の context 別割り当て | 同左 | 既定は戻る / 進むが履歴、ホイールクリックは未割り当て。画像 / 動画フルスクリーンのマウスボタン候補では場所移動系を出さない。動画の 6 シークは v3.7.0 では候補 / 実行対象外（保存値は保持）。ウィンドウ最小化も候補に含む。ツリー順を選んだ場合は Ctrl+↑↓ と同じ |
...
画像系と native 動画で同じ意味の案内を出す。同一一覧の境界は、内部の項目種別ではなく
利用者が見ている順序を表す「先頭です」/「末尾です」に揃える。

| 状態 | 表示する内容 |
| --- | --- |
| 同一一覧の末尾 | 「末尾です」 + `[Home] 最初に戻る` + `[Ctrl]+[↓] 次のフォルダへ` |
| 同一一覧の先頭 | 「先頭です」 + `[End] 最後に移動` + `[Ctrl]+[↑] 前のフォルダへ` |
...
環境設定で境界案内を OFF にした場合も `FsBoundaryHint::Edge` の生成・期限切れ・境界判定は
そのまま維持し、静止画 / detached / native 動画の描画だけを抑止する。他のフォルダ境界案内や
検索スコープ案内はこの設定の対象外とする。
