...
| マウス戻る / 進む / ホイールクリック | native window / HUD 側の XButton と中ボタン短クリックを App 側のマウスボタン割り当て経路へ接続。既定は戻る / 進むが履歴、ホイールクリックは未割り当て。画像 / 動画フルスクリーンのマウスボタン候補では `C:\`〜`Z:\`、お気に入り、読書履歴、★一覧などの場所移動系は出さない。従来どおりを選んだ既存環境では戻る / 進むのみ Ctrl+↑↓ と同じ |
| S | 動画タイルモード ON/OFF |
...
| マウス戻る / 進む / ホイールクリック | 環境設定「マウスボタン」の context 別割り当て | 同左 | 既定は戻る / 進むが履歴、ホイールクリックは未割り当て。画像 / 動画フルスクリーンのマウスボタン候補では場所移動系を出さない。ツリー順を選んだ場合は Ctrl+↑↓ と同じ |
