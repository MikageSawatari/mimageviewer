...
2026-09-20、利用者は速度を優先し、外部ツールの変更の自動反映を保証せず、mIV が認識した
一覧・並び順を保持する仕様変更を承認した。mIV 内の登録・参照解除・並べ替え・名前変更の追従は
維持する。再取得の入口・タイミングは実装設計で明記する。これは外部状態の全件再検査を省ける
という判断であり、実際に開く対象の検証や Remote の公開範囲検証を省略する承認ではない。
...
2026-09-20、利用者が修正を承認した。出荷前のデータ保護修正へ戻し、具体設計を進める。
外部アプリによる名前変更の自動追従を追加するものではなく、mIV 内の変更追従と未完了処理の
復旧記録を保護する。読み取り失敗時の後続操作・保存・終了の扱いは設計レビューで確認する。

同日、利用者は推奨方針を承認した。復旧記録の破損・読み取り失敗時に限り、名前変更・本棚の
移動/並べ替え・削除を実ファイル変更前に停止し、理由を表示する。閲覧と通常時の操作は維持する。
真のファイル欠損だけを空の記録と扱い、読み取り失敗時は旧記録を削除・上書きしない。
既存 admission の単一状態へ読み込み状態を統合し、再試行・終了・既に成功した操作の遅延完了も
同じ所有境界で扱う。本棚削除の pending migration invalidation も同じ整合性の検証対象とする。
これは仕様承認であり、実装・独立レビュー・検証の完了ではない。
