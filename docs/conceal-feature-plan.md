...
| `flood_fill_bitmap_mask` 関数 | `mask_db.rs` | バケツの全体 / 連結塗りと、隙間の許容を含む長方形 / 楕円 / 円への整形を消しゴム・隠蔽加工・補正レイヤーで共有 |
...
**スロット (マスク 1/2)** は隠蔽加工モード内のパネルボタンから保存 / ロードできる。
表示モードでは `F9` / `F10` でスロット 1 / 2 を quick apply し、
`Shift+F9` / `Shift+F10` で適用済み隠蔽マスクを削除する。
消しゴム側は `F7` / `F8` と `Shift+F7` / `Shift+F8` を使うためキー衝突しない。
...
| バケツ | K | 画像クリック時に 1 回だけ、開始色との許容差で全体 / 連結領域を描画 / 消去。長方形 / 楕円 / 円ではクリック位置を含み、連結領域内に収まる最大の選択図形へ整形 | ビットマップ |
...
(0..=50%、既定 10%) 以下の成分だけを文字などの小さな穴として埋めてから、クリック位置を含み
領域内に収まる最大面積の図形へ整形する。0% では穴を埋めない。
重なった図形はクリックした側を選び、重なり部分をクリックして複数候補が成立する場合は面積が
大きい方を選ぶ。半幅 / 半径が 2px 未満になる退化時だけマスクを変更せず、整形前の選択範囲を
黄色で約 1 秒表示して漏れ止めまたは色差許容値の見直しを案内する。「はみ出し」は確定後の図形
だけを 0〜8px 外側へ広げ、縁のにじみを覆う。「隙間の許容」は図形 3 モードだけで表示する。
...
- API (mask_db のスロット API と同じシグネチャ):
  ...
  - `delete_slot(slot)`
- 機能セット: **保存 / ロード / F9/F10 quick apply / Shift+F9/F10 適用済みマスク削除**
