...
- 当初は **サムネイル非反映 (バッジのみ)**。v2.5 以降は編集プレビューキャッシュが ON の場合に
  限り注釈を反映する。右 Ctrl 元画像プレビューでは注釈を隠す。
...
初期実装では吹き出しを反映せずバッジのみとしていた。v2.5 以降は「編集結果をサムネイルに保持」が
ON の場合、編集終了時に専用 worker でテキスト／スタンプを焼き込み、切り取りまで適用した編集
プレビューを保存する。色調補正は従来どおり表示時に適用し、最終 AI／ポストフィルタは含めない。
...
**自動リサイズと先端の追従 (2026-06)**: 自動サイズ (`auto_size`) は bake/幾何時に
`fit_bubble_shape` で輪郭を文字に合わせて遅延計算するため、`tip` を更新するイベントが無い。
固定距離の既定 `tip` のまま長文で本体が育つと、先端が本体内部に潜り込んで内向きツノに反転する。
対策として `tessellate::resolve_tail_tip` (内部 `project_tip_outside`) が、中心→`tip` の**方向は
保持**したまま、半径距離だけを「輪郭の出口 + 最低 overhang (`TAIL_MIN_OVERHANG`)」以上へ投影する。
描画 (`bubble_geometry`) と、ラボ/本体の先端ハンドル・AABB の双方がこの投影後の先端を使うので、
自動拡大でも手動の内側ドラッグでも先端が潜らない。保存される `tail.tip` は素の値のまま (描画/当たり
判定のみ補正)。自動サイズの縦横比上限 `MAX_ASPECT` は縦書き長文で楕円が横に水増しされすぎないよう
緩めてある (旧 1.8 は縦長文を丸い楕円へ強制し左右余白が過大だった)。
...
- 通常の thumb cache には焼き込まない。編集プレビューキャッシュが ON の場合のみ、編集終了時に
  専用 worker で反映するため、編集中の入力経路に高解像度 bake を持ち込まない。
