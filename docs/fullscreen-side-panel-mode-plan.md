...
> **現況 (2026-07)**: `FsSidePanelMode::{Hover, ClickToShow}`、設定永続化、静止画・
> native 動画・音楽の 3 面における左右別パネル、上部 `i` ボタン、per-file transient
> lifecycle は実装済み。§2 と §4 は現行仕様、§1 は実装前の問題分析、§3 / §5 / §8 は
> 実装経緯として読む。§7 の 3 面実機確認を完了した記録は本書から確認できないため未確認。
...
- 静止画は `open_fullscreen` と連結読みの直接ページ着地で `adjustment_mode` / `fs_click_info_open` を
  ...
  ファイル境界リセットに従う。
- 連結読みのスクロールに伴う現在ページの再アンカーはファイル境界ではないため、左右パネルの
  開閉状態を維持する。表示トリムの未確定状態だけは、対象ページが変わる前に保存する。
...
- **当時の実装順 (1 ブランチ内で段階的に、各段でテスト)**:
  ...
  - 当時は **3 面揃えてから実機検証 → コミット**する計画だった。3 面のコードは実装済みだが、
    実機検証の完了記録は §7 のとおり未確認。
...
用意し、**3 面 (静止画 / 動画 / 音楽) それぞれで Hover / ClickToShow を実機確認**する
こととしていた。コード実装は確認できるが、この手動確認の完了有無は未確認。

## 8. 実装前の Codex ブリーフ (履歴)
