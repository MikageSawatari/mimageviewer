...
- **コンテキストごとに「既定に戻す」**ボタン。
- 固定ボタン (A/B/X/Y、Select/Start、LB/RB、LT/RT) は、文脈ごとに `既定` / `なし` / 一発アクションへ差し替えられる。`既定` は従来のゲームパッド操作 (X 単体ならピッカーパネル、LT/RT ならスクロール・ズーム・シーク等) に戻す意味を持つ。
- X+方向リングは固定ボタンとは別の 8 方向スロットとして扱う。ただし X 単体を既定から変更した場合は、X+方向リングも起動しない。X を既定に戻すと、保存済みの X+方向リング内容を再び使える。
...
| `Settings` | `#[serde(default)] ring_shortcuts` = 8 スロット×3 context + 右ドラッグ mode 4 文脈 + マウスジェスチャ 4 文脈 + 初回案内フラグ + shift/alt wheel pair + `mouse_buttons_*` profile + `gamepad_buttons_*` profile + 互換用 `mouse_flick_enabled` / `gamepad_ring_enabled` / 旧 `mouse_back_forward_action` / `mouse_nav_prompt_done` (§8) |
