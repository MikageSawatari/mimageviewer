...
| 操作 | フルスクリーン | グリッド |
|---|---|---|
| 短タップ / 静止リリース (移動<20px) | コンテキストメニュー | コンテキストメニュー (従来) |
...
- **取消**: 中央 (不感帯) で離す / B (パッド) / Esc。**パッドで方向を選ばず離した場合は §4.2 どおり
  ピッカーパネルを開く** (取消ではない)。
...
| ← | フォルダ履歴 戻る | 回転 L | ミュート |
| → | フォルダ履歴 進む | 回転 R | キャプチャ保存 |
| ↖ | 表示↔詳細 | キャプチャ保存 | 外部プレイヤーで開く |
| ↗ | ★固定 | メタデータ表示 | ループ |
| ↙ | チェック ON/OFF | スライドショー | タイルモード |
| ↘ | (空き) | (空き) | ブックマーク追加 |
...
- **有効/無効トグルを 2 つ持つ (Codex P2-2 / P2-d)**: 「ゲームパッドの X リング/ピッカーを有効化」
  ...
  誤操作による状態変更を避けるため opt-in)。移動なし右クリックのコンテキストメニューは
  トグルに関係なく常に有効。
...
- **コンテキストごとに「既定に戻す」**ボタン。
- ピッカーパネル (X 単体) の項目は固定で**設定対象外**である旨を一文表示する (誤解防止)。
...
文言で「後で環境設定 > マウスボタンでも変更できる」を伝える。ダイアログはボタンのみで IME 配慮不要。
...
- **確定 / 取消モデル (Codex P2-a)**: 編集中は全項目ライブプレビュー。
  - トップレベル: **A または X = commit して閉じる** / **B = セッション全体を取消 (revert) して閉じる**。
  - ドリルイン内: **A = その項目を選択 (preview) してパネルへ戻る** / **B = 変更せずパネルへ戻る**。
...
| 箇所 | 変更 |
|---|---|
| マウス右クリック (既存コンテキストメニュー) | **変更なし** (ドラッグ判定だけ追加) |
...
5. **マウスフリック**: 右ボタン ジェスチャ状態機械 (§4.1) → フリック発動 (FS・グリッド)。移動なし右クリックの既存メニューは据置。
