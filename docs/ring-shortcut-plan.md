...
| `Settings` | `#[serde(default)] ring_shortcuts` = 8 スロット×3 context + 右ドラッグ mode 4 文脈 + マウスジェスチャ 4 文脈 + 初回案内フラグ + shift/alt wheel pair + `mouse_buttons_*` profile + 将来用に保持する `gamepad_buttons_*` profile + 互換用 `mouse_flick_enabled` / `gamepad_ring_enabled` / 旧 `mouse_back_forward_action` / `mouse_nav_prompt_done` (§8)。v2.2.0 の runtime/UI は `gamepad_buttons_*` を参照せず、ゲームパッド固定ボタン単体は既定動作固定 |
