...
- **戻る/進む/ホイールクリック** (`Extra1` / `Extra2`、`Browser_Back/Forward`、中ボタン短クリック)。
  ...
  画像 / 動画フルスクリーンのマウスボタンでは `C:\`〜`Z:\`、お気に入り、閲覧履歴、★一覧などの場所移動系を
...
`GridOpenSelectedAsPage` / `GridOpenSelectedAsList` は Grid context のまま維持するが、
複数ウィンドウモードではフル機能ウィンドウ用コマンドとして無効になる。リング / ジェスチャからも
キー / 右クリックと同じ App 側 open 入口を通し、理由のトーストだけを出してメイン一覧や変換状態を
変更しない。割り当て自体はモードを戻したとき再利用できるよう保存する。

### リング (一発もの) の候補

- **全 context 共通**: 本棚に追加 / 代表サムネ (フレーム) にピン留め / お気に入り巡回 /
  お気に入り 1〜20 / `C:\`〜`Z:\` / 場所▼の固定項目 (ドライブ一覧 / 閲覧履歴 / ★1〜★5 /
