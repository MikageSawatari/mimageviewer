...
- グリッドの割り当て可能リストには「メインウィンドウを閉じる」と「アプリを終了する」を含める。
  前者は [×] と同じでトレイ常駐設定に従い、後者は常駐設定を迂回して通常の保存・終了経路へ進む。
  画像 / 動画の「フルスクリーンを閉じる」とは stable ID も実行経路も分け、既定スロットには入れない。
- グリッドには「一覧の先頭へスクロール」「一覧の末尾へスクロール」も割り当て可能にする。
  Home / End と違って選択・チェックは動かさず、描画時に確定した現在の最大 offset へ pending intent を
  適用する。サムネイル / 詳細表示で共通とし、既定スロットには入れない。
