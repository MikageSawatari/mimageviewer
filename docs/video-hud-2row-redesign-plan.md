...
### 狭幅ウィンドウのフォールバック戦略 (Codex P3 指摘反映)

最小窓幅 800px 付近では controls 行が破綻し始める。「別途検討」と放置するとウィンドウ内再生時に
詰まるため、**フェーズ 3 (2 段化基盤) に畳み込み優先順を組み込む**:

`controls_row_width >= sum_of_visible_clusters` を超えた時点で、以下の順に左から削減:

| 優先順位 | 対象 | 畳み込み内容 | 削減幅目安 |
|---|---|---|---|
| 1 | **gap 縮小** | 8px → 4px | ~50px |
| 2 | **音量ラベル非表示** | 「100%」等の数値表示を隠す (slider は維持) | ~60px |
| 3 | **音量スライダー短縮** | 144px → 100px | ~44px |
| 4 | **キャプチャパレット 4 個 → カメラ単独 1 個** | 旧挙動 (click=copy) に縮退、フレームステップは消す | ~108px |
| 5 | **マーカーグループ非表示** | (キーボード J/K で代替) | ~72px |
| 6 | **連続再生 + ファイル切替を非表示** | (キーボード ↑↓ で代替) | ~108px |

判定は `overlay_width_points` の閾値 (e.g. `< 1100px` で gap 縮小、`< 950px` で音量ラベル非表示、…)
で段階的に。実装時はレイアウト計算ロジックに集約して **描画コードに `if width <` を散在させない**。

優先順位の根拠: 上位ほど「機能を失わずに圧縮」、下位ほど「機能を捨てる」。代替経路 (キーボード)
を持つ機能を後回しにする。
...
- **2026-05-25 Codex 第 1 ラウンドレビュー反映** — 以下を取り込み:
  ...
  - P3: `[↑F][↓F]` の F 衝突を回避して `[↑][↓]` 単独 + tooltip 方式に変更、狭幅
    ウィンドウのフォールバック戦略 (6 段階の削減優先順) を明記
