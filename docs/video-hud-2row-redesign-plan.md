...
**現行順序 (左 → 右、2026-05-26 実機 fb 反映後)**: ピン → ブックマーク → 一括ブックマーク

- 左端 **ピン** = 代表サムネ用 (P キー)、ユーザー要求でブックマーク系より左に寄せる
- 中央 **ブックマーク** = 通常ブックマーク追加 (B キー)
- 右端 **一括ブックマーク** = ブックマークの拡張機能 (低頻度バルク操作)

**経緯**:
1. 旧 (歴史的): 一括 (左) → ピン → ブックマーク (右)。一括が後から左端に
   ぶら下げられた経緯で、意図的な並びではなかった
2. Phase 2 (Codex 助言): ブックマーク (左) → ピン → 一括 (右) の論理読み順を採用
3. 実機 fb (2026-05-26): ユーザー要求でピンを左端、ブックマーク系を右側にまとめる
   配置に再変更 (= 現行)

### 実装

[src/video/native_presenter/overlay_draw.rs:417-487](../src/video/native_presenter/overlay_draw.rs)
の 3 つの rect の `rect.width() - X` のオフセット値:

| ボタン | X オフセット |
|---|---|
| ピン (pin_rect) | 100 (左端) |
| ブックマーク (bm_rect) | 68 (中央) |
| 一括ブックマーク (bulk_rect) | 36 (右端) |
...
| 2 | 左パネル順序入れ替え (当初案: ブックマーク → ピン → 一括ブックマーク、後の実機 fb で最終的に **ピン → ブックマーク → 一括ブックマーク** に再変更。詳細は本書「並びの判断と方針」セクション参照) | なし — レイアウト変更と独立 |
...
6. **左パネル順序入れ替え** (当初 Codex 質問は「ブックマーク → ピン → 一括」の論理読み順案。
   実機 fb で最終的に **ピン → ブックマーク → 一括** に再変更済み。詳細は本書
   「並びの判断と方針」セクション参照)
   - 頻度順 / 視覚読み順 / ユーザー直感のどの軸を優先するかが論点だった
