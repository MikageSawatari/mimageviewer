...
## 9.6. 既知の弱点 — ネットワーク上の data_dir (2026-08-16)

ポータブル版を NAS 上へ展開すると、既定の `<exe_dir>/data` もネットワーク共有上になる。
`--data-dir` は build flavor 共通なので通常版でも同じ状態を作れる。現行コードでは少なくとも
`catalog` / `settings_db` / `search_index_db` / `fts_meta` / `archive_cache` /
`edit_preview_cache` / `auto_aspect_cache` / `tags_db` が WAL を要求する。WAL の共有メモリは
同じホスト上のプロセス間で使うことが前提で、ネットワークファイルシステム上では成立しない。
画像の読み込み自体もネットワーク速度の影響を受けるため、利用者環境の症状を WAL だけに断定はしない。

現段階では data_dir の自動移動や journal mode の変更は行わない。設定読込後に UNC または
`GetDriveTypeW == DRIVE_REMOTE` を検出し、保存先のパスと、`--data-dir` でデータだけをローカルへ
置く比較手順を起動時に案内する。同じ保存先について明示的に抑止した場合だけ次回以降を省略し、
別の保存先へ変われば再案内する。

journal mode を変更する場合は NAS 実機で少なくとも起動、同じ書庫の 2 回目、終了・再起動を反復して
比較する。画像は NAS のまま data_dir だけをローカルへ変える対照条件も取り、画像 I/O と永続データ
I/O を混同しないこと。モードごとの安全性、複数プロセス経路、強制終了後の復旧も合わせて確認する。
