...
### 対象
- 通常画像ファイル (`.jpg` / `.jpeg` / `.png` / `.webp`)
- 主要なユースケース: **AI 生成画像、写真、mXDownloader 収集のツイート画像**
...
- **使用可能文字**: `#` を除く任意の Unicode 文字 (日本語可)
- **先頭文字**: `#` は使えない (mimageviewer 側で自動付与)
- **空白**: タグ名中のスペースは許可するが、非推奨 (Ctrl+G 検索でクォートが必要になる)
- **長さ**: 1〜64 文字
...
`#` で始まる要素と他のタグ要素を視覚的に区別する (色分け / バッジ)。
