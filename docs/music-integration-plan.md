...
| D4 | 右パネル | **動画アイテムの右パネル経路（タグ chips / ★ / 設定）をミラー** ＋ 音楽情報ブロックを追加。`src/ui_metadata_panel.rs` は現状 **画像のみ**（動画/ZIP/PDF は None、`ui_metadata_panel.rs:475` の注記）なので、動画のタグ/設定がどの経路で出ているか実装時に確認して合わせる（§11）。埋め込みメタ値内の HTTP(S) URL も動画と同じ `ui_text_links` でリンク化し、外部ブラウザを開く前に再生を一時停止する。 |
...
- **段階**:
  ...
  - **③ 音声ファイルの F12 / always-new 別ウィンドウ化 = Stage AUDIO で再開（2026-07-07）**:
    R1c 以降の resync gate 統一と detached lifecycle リワークで、以前の wgpu Validation Error 系を
    再調査する前提が整ったため、音声も動画と同じ **単一メディアウィンドウ**に合流する仕様へ変更。
    `viewer_item_supports_session` に Audio を追加し、F12 は no-op ではなく main / detached migration を行う。
    `detached_viewer_open_images_in_window` ON では音声も新規 open 時に detached メディアウィンドウへ入り、
    F12 で main に一時移動しても次の明示 open は設定どおり detached へ戻る。`music_*` 状態は bundle 化せず、
    メディア窓が常に 1 本という不変条件で main 側との混線を構造的に排除する。
  - **7e-残り**: long video の memory・audio 継続 smoke（① seek 音切れは 7-hidden、連続再生 EOF は 7-eof、
    ...
    VST 引き継ぎは 7e-vst、DetachedWindow(動画) は 7e-detached で解消済み。③ 音声ファイル別ウィンドウ化は Stage AUDIO で
    再開）。② 音声モード中の F12 は 7e-detached でゲート維持決着（hidden presenter の
