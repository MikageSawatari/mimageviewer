...
## 2026-09-12 利用者による動画2件の解決判断

利用者がしばらく試用し、サムネイルが一部に寄る問題とメモリ問題のいずれも再現しないと報告。
修正・既存自動回帰・S4実機確認と合わせ、§1.211 / §1.190をいったん解決済みにした。
過去ピーク全量の原因を断定せず、再発時の照合用に
[旧調査記録](video-strip-resolved-report-20260912.md) を保持する。
類似本□の§1.219は別件として継続。表紙Singleは正常、Double＋下strip表示で失敗、
strip非表示では正常という追加条件を受け、本人再現用の限定診断ログを準備する。

## 2026-09-12 類似本□の追加根因修正

本人の診断ログでDouble＋strip ONの11押下はResponseが所有していたがhover=falseとなり、
`hover_pos()`早期returnで受付へ届かないことを確認。押下をResponse所有の
`is_pointer_button_down_on()` / `interact_pointer_pos()`から受け付け、hover表示と分離した。
実eguiで修正前FAIL→修正後PASS、別widget所有時の拒否も確認し、独立最終承認済み。
main8299成功/0失敗/45 ignored、workspace/integration/doc/vendor、fmt、glyphが成功。
9/12 14:24 JSTに `build-dev.ps1 -PreserveRuntime` が成功し、確認coreのSHA256は
`E82AB30CD1C998A9F7264847FA4E1B75D16DB3DAB0B1DA940417B3715B1A571A`。
ビルド前後のsource freeze一致、アプリの起動・停止なし。
続いて利用者が「治りました。問題なさそう」と実機確認し、§1.219を完了扱いにした。

## リリース準備への引き渡し（2026-09-12）

今回合意した製品修正の実装上の阻害事項は残っていない。類似索引改善と小規模UI修正、
§1.208 / §1.210、動画§1.211 / §1.190、類似本§1.219の確認記録は本台帳と個別設計を参照。
§1.209の非同期復元と進捗modalは自動検証済みだが、実データでの47秒ケースの再確認は未実施。
自動余白カット§1.216は後続版へ延期済み。バックログにある他候補を今回の必須対象へ自動追加しない。

ClaudeCode Opusは通常のリリース準備へ進める。最新全体gateはmain8299成功/0失敗/45 ignored、
独立承認・確認core・利用者確認済み。S4は `095c82276` で実機成功。正式Ubuntu CIの再実行は
開発側からpushしていないため未実施。成功済み検証は対象source/条件が有効な範囲で再利用する。
perf smoke / idle-health、配布物の必須gate、署名・配布build・更新履歴/版番号・公開確認は
リリース手順側に残る。開発確認coreを配布成果物として扱わない。
実機操作の09:00までの了承枠は終了しており、新しい操作検証には具体suiteの再了承が必要。

作業ツリーに別系統の運用文書・Idle198自動検証等の未コミット変更がある。今回の製品commitへ
混ぜず保持した。公開担当は所有者と対象を確認して取り込みを判断する。MS Storeは問い合わせ
回答待ちの別案件で、一般公開の製品修正完了とは区別する。
