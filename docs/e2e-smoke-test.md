...
あるいは、右クリック短押しが既定の「フルスクリーンを閉じる」の場合は、
フルスクリーン中に**右クリック**でもグリッドに戻れる
...
1. 操作カスタマイズ → 設定 → 右ドラッグで、画像の「右クリック短押し」を順に切り替える
2. 「フルスクリーンを閉じる」で画像ビューアが閉じることを確認する
3. 「何もしない」で画像ビューアを維持することを確認する
4. 「右クリックメニューを表示」で現在項目の既存メニューが開くことを確認する
5. 動画についても同じ 3 動作を確認し、native presenter でもメニューの owner / focus が正しいことを確認する
6. リング / ジェスチャ有効時は短押しだけに設定動作が適用され、方向操作の発火後には適用されないことを確認する
7. 「何もしない」+ 右ドラッグ「未使用」で無反応、長押しでは従来のメニューが開くことを確認する
...
**手順:**
1. 環境設定 → ライブラリ → フォルダ・ファイル → 空フォルダスキップ上限を 1 に設定
...
**手順:**
1. 設定 → 環境設定 → 全体設定 → テーマ でライト／ダーク／システム を切り替える
2. OK を押して各テーマの適用を確認
...
#### 6-3. 本ブックマークと透過画像の Shift+B 背景切替

**手順:**
1. 画像のみフォルダ本 / ZIP / PDF をフルスクリーンで開き、<kbd>B</kbd> を押す
2. 左パネルの「ブックマーク」タブで名前を編集し、別の登録を削除してから、登録サムネイルでページ移動する
3. 「場所▼」→「ブックマーク」を開き、通常のサムネイル / 詳細表示、種類・状態フィルタを切り替える
4. 一覧の項目をダブルクリックして登録位置を開き、戻る操作で一覧へ戻る
5. 一覧で <kbd>Delete</kbd> を押し、元ファイルが残ることを確認する
6. 透過 PNG をフルスクリーンで表示し、<kbd>Shift</kbd>+<kbd>B</kbd> を複数回押す

**確認項目:**
- 同じページで <kbd>B</kbd> を繰り返しても 1 件だけ登録され、削除しても元画像は残る
- 左パネルの名前編集 / 保存 / 名称なし / キャンセル / 削除が行ジャンプに奪われず動作する
- 画像フォルダへ前のページを追加しても、登録済みブックマークは元のファイル名へ移動する
- 横断一覧は専用ダイアログではなくメイン一覧へ表示され、通常の絞り込みと登録日時順を利用できる
- 動画 / 音声は登録時刻、本は登録ページへ開き、戻ると同じブックマーク一覧へ復帰する
- 欠落項目は警告付きで残り、一覧の Delete / 右クリック削除はブックマークだけを削除する
- 背景が黒 → 白 → 市松 → 黒の順に循環
- AI アップスケール表示中は黒 ↔ 白の 2 段で切り替わる
- 次の画像を開くと黒背景に戻る（保存されない）
- 不透明画像では <kbd>Shift</kbd>+<kbd>B</kbd> で「透過画像ではないため背景は切り替えできません」と案内される
