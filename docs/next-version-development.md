...
続くclick接続の途中差分に、frame全体のToggle countと後付けApp tailの状態値だけで成功を判定する不足を親が発見。別variantで通常toggle helperを呼ぶ案も製品入力経路の証明にならないため採用せず、独立Solと既設計の維持を再確認した。実Response.clickedから特定TogglePanorama commandへgesture/named/source/owner metadataを付け、既存variantとSequencedNativeOutputEventのfeature限定metadataで全App gateを通し、通常handler直後のbefore/afterを記録する。別token/同frame無関係command/source拒否/owner再joinの偽成功を回帰対象とする。通常経路を診断別handlerへ置換しない。host checkpoint a93bd271bは確定済み、Rust接続は作業中で未検証。

Producer→App checkpointは実Response.clickedのexact command index（同frame先行無関係Toggleを除外）から既存TogglePanorama/lossless envelopeの診断metadataへ結び、通常App gate/handler直後のNone→Some(1.0)を確認する実装へ到達。通常/test-script core check成功、native_ui_smoke44/44、native_top_panorama4/4、output-bus1/1、通常App handler1/1が成功。WndProc dispatchがrenderへ先行するraceは、通常dispatch→Set/ReleaseCapture順を変えず、pre-dispatch exact identity stampとpost-capture semantic confirmationを分離して解消。最終completionは両証拠を必須としrender待ちは追加しない。独立Sol完成検収で重要未解決なし。関連修正後のsourceを凍結して全体gateを1回実行中。runner/Rhai/外部driver接続と実クリックはこの単位では未公開・未実行。

Producer→Appの全体gateはexit0/PASS（本体8034 pass/43 ignored、UI snapshot48、vendor25/9/15）、fmt-check成功。最終manifest target/next-version-work/logs/native-top-panorama-click-producer-checkpoint-20260910/manifest.json SHA D12716A580A7D62BAB76097A58CCD1539F6A74423F9D892326EBE0EB1285B446、bounded patch SHA44DB446FE695538B4416C39E863DAA02A6A6F391C810E9122522509413AAD20E。親は11sourceとevidence/patchのhash不一致0を確認。source10 Rust pathsとui-smoke plan、親台帳だけを限定保存する。既存Idle198のui-smoke.ps1差分、src/test_script.rsの内容差なし状態、他所有dirtyは対象外。Cargo所有は返却済み。normal profileやUIは未操作。診断実装の単位なので通常確認buildを自動停止付きで行わず、runner接続とportable実機準備は後続。

producer→Appをlocal master 2cb1081f1へ限定commit。後続runner/Rhai接続を同writerへ委任し、別Sol reviewerはIPC/session/PID/SID・primary failure保持・cleanup/kill順の新境界に集中する。run固有pipeとserver PIDをApp環境へ渡し、起動後exact App PIDでhelperを開始する接続が型チェックまで到達。normal/feature check、native_top_panorama6/6、PS5.1/7のrunner policy/load/approval guardが成功したとの引渡し。否定試験強化・docs・freeze/完成reviewは継続中。新liveシナリオの実行了承は未取得で、起動はしない。dupeが統合sourceを凍結し、狭域Cargo（IPC/core/remote/check/関連回帰、jobs1）枠を使用開始。masterは追加Cargoとportable prepareを返却まで待ち、非Cargo作業を進める。normal core SHA4087876E36EBA81821542B8EFFA9C513D28A5C2868F84B9A7B0C5B168C837703不変を親が再確認。

runner/Rhai接続の独立Sol最終検収を完了。終了関数内の実Finalize→Stop順をASTで確認し、primary failure＋StillRunning/Outstandingの理由保持とkill禁止の回帰を補強、重要未解決なし。最終manifest target/next-version-work/logs/native-top-panorama-click-runner-checkpoint-20260910/manifest.json SHA6FF75392616DCCE0E6F976B6CEF37EA9D27D1D2FAB8209627A77CB17090F3617、9source hash不一致0を親が確認。既存Idle198 dirtyを含むrunnerからclickだけをHEADへ移すcandidateを3wayで作成し、差分157+/6-、ValidateSetの既存Idle198名を除けば追加削除行の順序まで一致。候補SHA632FD4839736FA1B7CC0F942FA5BFD15A6A5CD5D40E1D7B522E01F02A52DC4CD。独立検収・PS5.1/7 runner policy/ASTおよびapproval guard（candidate5scenario、working6scenario）成功。working runner SHA5B3A3AA50BFC756AB35940F104D6B57BFFFACA59E76C46DD498D49456E5D3ED7を保持し、stageだけclick-only candidateとする。portable準備はdupe狭域Cargo返却待ち、実機は未了承・未実行。

runner接続を3b0f9e800へ限定commit後、dupeのCargo返却を受けてprepare-portable-smoke.ps1 -TestScriptを実行し、9分24秒でexit0。source fingerprint 8e9e229255695cd6547a127c89f686c698547a8d44e91ac7942204d1150d0854、portable core SHA E6EB5E9BABFB8F76625079961D904A56B099DFF91FACC05D49E4B94DCF8743D0、build manifest SHA 1D88F0C6E93E9AF338D7983F0402FEC32D0CBCE80660B3989908802F370DD02Fを親が照合。通常core SHA4087876E36EBA81821542B8EFFA9C513D28A5C2868F84B9A7B0C5B168C837703不変。checkpoint manifestはbuild証拠追記後SHA2223C1CB032BF71DB98F8F235C005E4F6313FB6C10158D680171711D6EA7CF17。起動・入力なし。NativeTopPanoramaClickを約5分、9/10 18:30まで、初回＋原因特定後の再試行1回、前面window/実mouse・生成動画とportable-smoke/dataのみ・通常版索引維持として具体了承を依頼した。返答までは実機未実施とする。Cargo枠はdupeへ返却し、次の実機と重い検証は開始前に調整する。
