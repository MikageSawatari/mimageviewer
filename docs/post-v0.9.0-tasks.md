...
v0.9.0 リリース時点 (2026-05-16) で残った全件をここに集約する。元の `docs/v0.9.0-release-tasks.md`
は完了記録 (frozen) として保持し、本ドキュメントが今後の作業対象の単一の真実源。

優先度カテゴリは v0.9.0 レビュー時の Claude / Codex 評価をそのまま引き継いでいる:

- **A. v0.9.0 から v0.10 に持ち越した P2** — リリースブロッカ候補だったが、修正規模が大きく
  v0.9.0 のリリース予算を超えるため明示的に延期したもの。優先度は本ドキュメント内で最上位。
- **B. P3 (T58〜T121)** — 「望ましい」修正。実害シナリオは稀 or 単発トラブルで済むが、品質の
  底上げに価値がある項目。
- **C. リファクタ負債 (R-1〜R-10)** — 巨大モジュール分割やテスト整理。コードの保守性向上が
  主目的で、エンドユーザー影響なし。
- **D. v0.9.0 で部分対応した項目の follow-on** — 主経路だけ直して残経路を v0.10 に持ち越した
  「タスク内 follow-up」。
