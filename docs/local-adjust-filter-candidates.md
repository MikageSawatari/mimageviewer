...
HighlightsShadows, Clarity, Dehaze, Blur(box), MotionBlur, SoftFocus, Mosaic, Sharpen, Look(15プリセット),
...
現状: Blur(box), MotionBlur, SoftFocus を実装済み。box blur は半径非依存コスト (積算和 O(n)) なので可変半径ぼかしの土台に最適。
