...
### `LocalEffect` (部分補正レイヤー、21種)
Tone(明度/コントラスト/γ/彩度/vibrance/色温度), ToneCurve(5点・RGB合成),
RgbToneCurve(全体+RGB別5点), Hsl(単一・全体), ColorMixer(8色帯), HighlightsShadows, Clarity,
...
- [ ] **`Tone` に tint (緑-マゼンタ) 追加** **易** — 現状 temperature のみで WB として不完全 (Lightroom)

---

## 2. ぼかし・フォーカス系 ★イラスト重要

現状: Blur(box), SoftFocus のみ。box blur は半径非依存コスト (積算和 O(n)) なので可変半径ぼかしの土台に最適。

- [ ] **移動ぼかし (モーションブラー・方向指定)** ★★★ **易〜中** — 動きの表現。イラスト/漫画で必須級 (CSP / PS / Krita)
- [ ] **被写界深度 / チルトシフト (`TiltShift`)** ★★★ **中** — 奥行きの向きを指定して奥をぼかす + ジオラマ風。**§A で詳細設計**
