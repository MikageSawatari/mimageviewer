...
HighlightsShadows, Clarity, Dehaze, Blur(box), MotionBlur, TiltShift, LensBlur, RadialBlur, SoftFocus, Mosaic, Sharpen, Look(15プリセット),
...
現状: Blur(box), MotionBlur, TiltShift, LensBlur, RadialBlur, SoftFocus を実装済み。box blur は半径非依存コスト (積算和 O(n)) なので可変半径ぼかしの土台に最適。
...
- [ ] **テクスチャ (中周波ディテール)** ★ **易** — Clarity と別系統。Lightroom は両方持つ
- [ ] **ハイパス** ★ **易** — シャープ/ディテール抽出の定番テクニック (PS Other)
