...
- 効果数が多いため、ダイアログ上部にキーワード入力欄を置き、効果名、短い表示名、
  説明文に一致する効果だけをリアルタイムに表示する。
...
- [x] **周波数分離 / ウェーブレット分解 (`FrequencySeparation`)** **中** — 低周波の色むらと高周波の質感を分け、塗り面の均し、質感抑制、細部強調を行うレタッチ向けフィルタとして追加
...
### 9-H. 出力仕上げ / フレーム
- [x] **フレーム / レターボックス / 角丸マット (`Frame`)** ★ **易** — 画像サイズは変えず、画像内側にフレーム、シネマ黒帯、角丸マットを描く保存前の仕上げ効果。色、幅、境界ぼかし、内側ライン、レターボックス比率を調整できる

### 9-I. 立体視 / 特殊光学
- [x] **レンズ汚れ / 水滴 / レンズダスト オーバーレイ (`LensDirt`)** **中** — ほこり、水滴、曇りを手続き型オーバーレイで重ねるレンズ越し演出
