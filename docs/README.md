...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 7 ヘルプキー KeyAction 化スライス実装中 / ClaudeCode レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1〜4 で既定未割り当て、競合 warning、グリッド F7-F10、フルスクリーン縦方向 resolver を段階実装。Phase 5 でツール / メニュー / hover / native 動画 overlay / ★ tooltip の shortcut 表記を keymap 追従に変更。Phase 7 では active scope から実 shortcut label を取り出す基盤と、サムネイル一覧、画像 / 動画フルスクリーン、消しゴム / 隠蔽加工 / 切り取り / 補正レイヤー / テキスト注釈のコンテキストヘルプを実装済み。現在は `HelpShowContextShortcuts` を追加し、既定 `?` のヘルプキー自体も `keymap.ini` で変更できるようにするスライス。`CommandId` や全面 dispatch resolver、メニュー構成変更は後続。Esc / Enter / 修飾なし矢印は予約警告 |
