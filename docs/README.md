...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 2 初期実装済み / レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1 は `KeyAction` 拡張 (空 `default_chords` の許可 + `GridToggleStackMode = none` + 表示用 helper) まで実装。Phase 2 初期実装では `KeyContext` を scope として再利用し、`CommandSpec` / `BindingPolicy` / active scope 隣接表 / 衝突 warning を追加。`CommandId` や dispatch resolver、UI 表示変更は後続。Esc / Enter / 修飾なし矢印は予約警告 |
