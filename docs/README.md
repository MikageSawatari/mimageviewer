...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 6 menu layout visibility render スライス実装中 / ClaudeCode レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1〜4 で既定未割り当て、競合 warning、グリッド F7-F10、フルスクリーン縦方向 resolver を段階実装。Phase 5 でツール / メニュー / hover / native 動画 overlay / ★ tooltip の shortcut 表記を keymap 追従に変更。Phase 7 では active scope から実 shortcut label を取り出す基盤と、各文脈のコンテキストヘルプ、`HelpShowContextShortcuts` によるヘルプキー自体の keymap 化まで実装済み。Phase 6 では `MenuCommandId` / `MenuCommandSpec` / `TopMenuId` の基盤、全 top menu の固定ラベル leaf 項目 catalog 化、parent 別 catalog iterator、enum ⇄ `ALL` drift テスト、stable name 文字列ベースの `MenuLayoutSettings` と resolver、`Settings.menu_layout` 永続化まで実装済み。現在は `hidden_commands` による固定 leaf 項目 / 空 top menu の描画除外を `render_menubar()` に接続するスライス。メニュー内・top menu の順序反映と編集 UI は後続。Esc / Enter / 修飾なし矢印は予約警告 |
