...
| [shell-file-operations-context-menu-plan.md](shell-file-operations-context-menu-plan.md) | **計画中**。Windows Shell の `IFileOperation` とネイティブ右クリックメニューへ寄せるファイル整理機能の実装計画。A/B クイックフォルダ、フォルダ履歴の A/B 分離、標準の上書き確認・進捗 UI、仮想 ZIP/PDF アイテム向けの native custom menu を整理 |
