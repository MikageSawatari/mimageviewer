...
| [external-tool-launch-plan.md](external-tool-launch-plan.md) | **P0/P1/P2a/P2b 実装済み (2026-09-01)、P3 以降は未実装**。backlog §1.117 の正本。既定キーなしの Grid 専用固定スロット / ピッカーと、フォルダー背景・コンテナー項目から現在のフォルダー / 本 1 件を渡す入口を持ち、右クリックの平坦なツール一覧を維持する。ツールバーは既定 OFF の動的セクション、メニューバーは「ファイル ▸ 外部ツール」から全登録を選べる。変換アーカイブは元パスを使い、1 コンテナーに定まらない集約ビュー背景と仮想ページは拒否する。外部ツール起動を引数テンプレート / 作業フォルダー / 複数選択 / **ZIP・PDF 内ページの一時実体化** / **動画の現在フレーム**まで広げる設計で、**仮想パスをそのまま渡す方針は採らない**。一時ファイルは NeeView 同型のプロセス単位ディレクトリ + 終了時削除 + 起動時の孤児回収 |
