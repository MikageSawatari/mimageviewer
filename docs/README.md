...
- [duplicate-detection-book-query-review-fixes.md](duplicate-detection-book-query-review-fixes.md) — 本照会R1/R5/R6の実装・採用検証。viewer別の要求、同一read transaction、厳密MIH、再列挙による対応付けと疎な表示結果は接続済み。実本oracle・途中取消・通常/大規模負荷・UI補助測定を検証済み。残る最終全体gate・portableと利用者実機確認。
