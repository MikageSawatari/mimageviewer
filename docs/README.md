...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 1 初期実装済み / レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1 は `CommandId` / `CommandSpec` を導入せず、既存 `KeyAction` の拡張 (空 `default_chords` の許可 + `GridToggleStackMode = none` + 表示用 helper) まで実装。`CommandId` / `CommandSpec` / scope 衝突判定は、実コードの active scope / 消費順と突き合わせる Phase 2 以降へ後送り。UI 表示変更も後回し。Esc / Enter / 矢印は初期予約 |
