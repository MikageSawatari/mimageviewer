...
| [duplicate-detection-plan.md](duplicate-detection-plan.md) | 表示中の画像と同じ絵の別バージョンを見つけ、その保存場所へ移動する機能の設計・実測・実装記録。`similar.db` には PDQ-256 だけを保存し、索引構造を作らず線形走査する。白ページ等は品質値で判定不能として区別し、画像の削除や残す版の判断は行わない |
