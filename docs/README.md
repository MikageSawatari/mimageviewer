...
| [key-customization-impl-plan.md](key-customization-impl-plan.md) | **実装済みメモ**。簡易版 (テキスト ini / GUI なし / 競合は警告のみ) の手順書と実装判断。`src/keymap.rs` の型・コメントアウト済み `keymap.ini` / `keymap.ini.default` 生成・ini 仕様 (`Action.1` 形式)・exact match ヘルパー・native 動画転送対応・エッジケース規則・`KeyAction` インベントリ (付録 A)・キー変換ホワイトリスト (付録 B) |
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 7 native 動画 overlay ヘルプスライス実装中 / レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1 は `KeyAction` 拡張 (空 `default_chords` の許可 + `GridToggleStackMode = none` + 表示用 helper) まで実装。Phase 2 初期実装では `KeyContext` を scope として再利用し、`CommandSpec` / `BindingPolicy` / active scope 隣接表 / 衝突 warning を追加。Phase 3 初期実装ではグリッド側 F7-F10 / Shift+F7-F10 のマスク一括適用・削除を `KeyAction` 化。Phase 4 初期実装ではフルスクリーン縦方向の `FsStackJumpPrev/Next` と動画 `VideoPrevFile/NextFile` を局所 resolver 化。Phase 5 では消しゴム / 隠蔽加工パネルのツールキー表示、検索 / タグビュー系のトップメニュー項目、一部 hover text、静止画フルスクリーンの上部ホバーバー tooltip / popup shortcut 表記、native 動画 overlay、★フィルタ / コンテナ★ tooltip の KeyAction 由来 shortcut 表記を keymap 追従に変更済み。Phase 7 基盤スライスでは active scope から `CommandSpec` と effective shortcut label を取り出す純ロジックを追加し、グリッドヘルプ初期スライスでは `?` でサムネイル一覧向けショートカットダイアログを開くところまで実装。未設定表示スライスではキー未設定 / 明示無効化中の操作を別枠で表示する。画像フルスクリーンヘルプスライスでは通常の画像フルスクリーンでも `?` で同ダイアログを開き、`FsCommon` / `FsImage` / `Rating` の実割り当てを表示する。消しゴム / 隠蔽加工ヘルプスライスでは各編集モード中の `?` で `Erase` / `Conceal` の実割り当てを表示する。切り取りヘルプスライスでは `Crop` の実割り当てを表示し、補正レイヤーヘルプスライスでは `LocalAdjust` の実割り当てと固定キーを表示する。egui 動画ヘルプ初期スライスでは `FsVideo` と動画で有効な一部 `FsCommon` / `Rating` を表示し、native 動画 overlay ヘルプスライスでは Windows native presenter 上でも `?` で同内容を表示する。テキスト注釈のヘルプ、`CommandId` や全面 dispatch resolver、メニュー構成変更は後続。Esc / Enter / 修飾なし矢印は予約警告 |
...
| [version-highlights-plan.md](version-highlights-plan.md) | **実装済み (v2.0.0)**。更新後 初回起動に「重要な変更点 (主要部分)」を 1 画面で表示する汎用の仕組み。標準動作の変更 (例: ツールバー左右クリック) を**個別ダイアログを増やさず display-only** で告知。`update_check` (更新前・ネットワーク・全文) とは別で、**更新後・オフライン・操作/既定の変更中心**。既存の `last_seen_version`/`previous_last_seen_version`/`version_changed` を流用。複数バージョンまたぎは累積表示。テストは**選択純関数の unit test + egui_kittest スナップショット + `--whatsnew-from` 強制表示**で実機最小。無効化設定なし |
...
| [rating-list-view-plan.md](rating-list-view-plan.md) | **Phase 1 実装済み (2026-06-23)**。場所▼ と ファイル メニューに ★1〜★5 を足し、選んだ★の付いたアイテム/コンテナを場所横断でフラット一覧する仮想ビュー。読書履歴/タグビューを雛形に `items_are_rating_view` を追加。**時刻ソートのため `rating.db` に `rated_at_ms` + `source_path` + `kind` + 仮想アイテム復元メタを後方互換追加** (リリース済み DB)。★設定時刻ソートはビュー固有 (表示中だけソートに追加、グローバル SortOrder は不変)。キー→GridItem 復元は新規行は kind/meta 直読み・旧行は parse+stat 推定 |
