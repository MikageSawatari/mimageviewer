...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 6 メニュー command catalog 基盤スライス実装中 / ClaudeCode レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1〜4 で既定未割り当て、競合 warning、グリッド F7-F10、フルスクリーン縦方向 resolver を段階実装。Phase 5 でツール / メニュー / hover / native 動画 overlay / ★ tooltip の shortcut 表記を keymap 追従に変更。Phase 7 では active scope から実 shortcut label を取り出す基盤と、サムネイル一覧、画像 / 動画フルスクリーン、消しゴム / 隠蔽加工 / 切り取り / 補正レイヤー / テキスト注釈のコンテキストヘルプ、`HelpShowContextShortcuts` によるヘルプキー自体の keymap 化まで実装済み。現在は Phase 6 初期として `MenuCommandId` / `MenuCommandSpec` / `TopMenuId` を追加し、既存 top menu の keymap 追従項目だけを catalog 化するスライス。全面 dispatch resolver、メニュー構成保存・編集 UI は後続。Esc / Enter / 修飾なし矢印は予約警告 |
