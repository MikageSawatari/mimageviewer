...
| [touch-support-plan.md](touch-support-plan.md) | **仕様確定 / Phase 1 Step 2 まで実装済み**。タブレット PC のタッチ操作対応。Step 2 は viewport + surface ごとの egui 入力相関と抑止要否の計算までで、コマンド実行・クリック抑止は未配線。3 領域タップ + 中央クローム + anchor-fraction スクロール + ピンチの設計とフェーズ別工数。タッチ / ポインタ入力を触るときに読む |
