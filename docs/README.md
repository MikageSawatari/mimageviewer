...
| [external-tool-launch-plan.md](external-tool-launch-plan.md) | **P0/P1 実装済み (2026-08-30。型・設定 DB・旧登録移行・登録 UI・引数展開・単一実ファイル起動)、P2 以降は設計中**。backlog §1.117 の正本。外部ツール起動を環境設定へ出し、引数テンプレート / 作業フォルダー / 複数選択 / **ZIP・PDF 内ページの一時実体化** / **動画の現在フレーム**まで広げる。NeeView (ソース調査) と ZipPla の仕様比較を含む。**仮想パスをそのまま渡す方針は採らない** (Win32 では解決しないことを実測、シェル名前空間は ZIP 限定)。代わりに `{container}`/`{entry}`/`{page}`/`{time}` の分解プレースホルダを用意。キー割り当てはお気に入り / ピン留めタグと同じ固定スロット方式。一時ファイルは NeeView 同型のプロセス単位ディレクトリ + 終了時削除 + 起動時の孤児回収 |
| [sns-split-export-plan.md](sns-split-export-plan.md) | **設計確定、未実装 (2026-09-01)**。1 枚の絵を X / Instagram のカルーセル投稿用に 2〜4 枚へ切り分けて書き出す。既存の切り取り (`CropSettings` / `export_crop.db`) は 30 ファイル超から参照されるため触らず、**永続化しない一度きりのモード**として分離。グループ矩形は比率固定の 1 枚の crop 矩形で、枠と隙間は導出のみ。**X の隙間を実測** (PC ブラウザ 1.588% / iOS アプリ 1.869% / モバイル Web 2.652%、隙間の絶対値は環境ごとに違う) し、枠幅比 **1.7% 固定**を採用。投稿先は X (3:4 / 1.7%) と Instagram (4:5 / 0%) の 2 択のみ。書き出しは `ExportEntry` に crop を足して既存の 1 スナップショット→N ファイル経路へ載せる。2x2 グリッドと縦並びは非対象 |
