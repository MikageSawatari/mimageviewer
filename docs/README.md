...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 7 コンテキストヘルプ基盤スライス実装中 / レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1 は `KeyAction` 拡張 (空 `default_chords` の許可 + `GridToggleStackMode = none` + 表示用 helper) まで実装。Phase 2 初期実装では `KeyContext` を scope として再利用し、`CommandSpec` / `BindingPolicy` / active scope 隣接表 / 衝突 warning を追加。Phase 3 初期実装ではグリッド側 F7-F10 / Shift+F7-F10 のマスク一括適用・削除を `KeyAction` 化。Phase 4 初期実装ではフルスクリーン縦方向の `FsStackJumpPrev/Next` と動画 `VideoPrevFile/NextFile` を局所 resolver 化。Phase 5 では消しゴム / 隠蔽加工パネルのツールキー表示、検索 / タグビュー系のトップメニュー項目、一部 hover text、静止画フルスクリーンの上部ホバーバー tooltip / popup shortcut 表記、native 動画 overlay、★フィルタ / コンテナ★ tooltip の KeyAction 由来 shortcut 表記を keymap 追従に変更済み。Phase 7 基盤スライスでは、`?` ヘルプ UI に先行して active scope から `CommandSpec` と effective shortcut label を取り出す純ロジックを追加。`CommandId` や全面 dispatch resolver、メニュー構成変更は後続。Esc / Enter / 修飾なし矢印は予約警告 |
