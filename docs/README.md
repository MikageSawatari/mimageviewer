...
| [touch-support-plan.md](touch-support-plan.md) | **仕様確定 / Phase 2 一覧スクロールまで実装済み**。タブレット PC のタッチ操作対応。静止画フルスクリーンは左右タップのページ送り、中央タップの上下クローム、2 本指ズーム / パン、上バーのタッチ専用 hit resolver を配線済み。サムネイル一覧は行スナップを維持した 1 本指スクロールを配線済み。左右パネルのタッチ導線と選択済みセルの再タップ open は後続。3 領域タップ + 中央クローム + anchor-fraction スクロール + ピンチの設計とフェーズ別工数。タッチ / ポインタ入力を触るときに読む |
