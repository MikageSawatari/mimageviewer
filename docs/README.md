...
| [key-command-catalog-plan.md](key-command-catalog-plan.md) | **Phase 6 menu catalog drift hardening スライス実装中 / ClaudeCode レビュー待ち**。簡易 keymap の次段階として、デフォルト未割り当て操作も `keymap.ini` から割り当て可能にする計画。Phase 1〜4 で既定未割り当て、競合 warning、グリッド F7-F10、フルスクリーン縦方向 resolver を段階実装。Phase 5 でツール / メニュー / hover / native 動画 overlay / ★ tooltip の shortcut 表記を keymap 追従に変更。Phase 7 では active scope から実 shortcut label を取り出す基盤と、各文脈のコンテキストヘルプ、`HelpShowContextShortcuts` によるヘルプキー自体の keymap 化まで実装済み。Phase 6 では `MenuCommandId` / `MenuCommandSpec` / `TopMenuId` の基盤、`MenuCommandId::ALL` drift テスト、全 top menu の固定ラベル leaf 項目 catalog 化まで実装済み。現在は `TopMenuId::ALL`、parent 別 catalog iterator、`TopMenuId` / `MenuCommandId` の enum ⇄ `ALL` drift テストを追加するスライス。全面 dispatch resolver、メニュー構成保存・編集 UI は後続。Esc / Enter / 修飾なし矢印は予約警告 |
