...
- **編集中はメモリ上の順序、リネームは遅延フラッシュ**。フラッシュトリガ = **並べ替え専用モード
  退出時 (+ アプリ終了)**。フラッシュは **2 パス (全 temp 名 → 全 final 名) で中断耐性**、worker、
  対象フォルダの**監視を一時停止**して実行。
- **本フォルダは検索インデックス対象外**。本ページに**タグ/★/非破壊回転も付けない**
  (= 閲覧・並べ替え・削除・Ctrl+F/ツールバー絞り込みのみ)。
- **本棚内は編集不可** (補正/レイヤー/隠蔽/注釈/AI を出さない)。補正は複製時に焼き込む。
...
- **追加トリガ = ショートカット 1 つ**で「アクティブな本」へ複製追加。文脈別:
  グリッド = カーソル位置 / 選択 (チェック) があれば選択画像すべて、画像表示 = その画像、
  動画 = 現在の再生フレーム。
- **既定は名前なしの本** 1 冊がアクティブ。製本画面でリネーム/本の追加削除/アクティブ本選択。
...
- §B の細部 (衝突サフィックス規則・エンコード品質・並べ替え実装段階・アクティブ本切替 UX・
