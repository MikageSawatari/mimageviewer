...
- **上部ホバーバー右上にカメラアイコン** を配置 (動画 HUD と静止画フルスクリーンで統一):
  ...
  - **Ctrl+クリック**: 静止画 / ZIP 内画像 / PDF ページでは一時的な範囲選択オーバーレイを開き、
    ドラッグ範囲だけをクリップボードにコピーする。永続 crop 設定は変更しない。動画 HUD は後続検討。
  - **ホバー時のツールチップ**: 3 行構成
    - 「クリック: クリップボードにコピー」
    - 「Ctrl+クリック: 範囲を選んでコピー」
...
| AI アップスケール途中の Ctrl+S | **AI 完了待ちする** (2026-06-07 確定)。最終合成が complete になるまで保存/コピー/比較は実行せず、「最終合成の完了後に再実行してください」トーストを出す。AI アップスケール済みの出力を保証する方針 (注釈の焼き込みも complete な最終合成の上で行うため整合)。`ensure_final_composite_pixels` が `entry.complete` のみ返すのがゲート |
...
- 完了: 静止画カメラアイコン + クリップボード経路統合
  ...
  - Ctrl+クリック時は範囲選択オーバーレイでドラッグした矩形だけを、一回限りの crop として
    `CapturePixelJob` に渡してクリップボードへコピーする。永続 crop は上書きしない。
- 完了: 見開き結合保存
