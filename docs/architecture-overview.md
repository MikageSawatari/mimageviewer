...
| `ime_focus.rs` | 日本語を入力できる single-line TextEdit の共通描画境界。App gate と helper が共有する viewport 別 IME event / 300ms grace 状態、キー処理由来の focus ownership 復帰、直前 pass の helper-managed focus contract、IME 候補選択中だけの Tab focus-lock、caller の focus request latch を所有する。helper field が focused または直前 pass で focused だった間の各 key press は、widget id の継続、egui focus の前後、keyboard owner / phase、左 side-panel close call site を通常 logger の `[text-input-key]` へ常時計装する。無修飾の文字 key と physical key は `Char` にマスクし、入力文字列は記録しない。通常 record は process あたり 1 MiB で抑止し、id / focus / owner / close の異常 record は継続する。pointer による focus 移動は復帰せず、理由付き allowlist 以外の raw TextEdit を unit test で禁止する |
