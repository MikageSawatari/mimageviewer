...
| `crates/remote-web/web/document-double-tap.mjs` | 文書全体の browser double-tap zoom 抑止を単一 ownership に集約する。`command-core.mjs` の時間差・距離による純粋な pair 判定を使い、成立した 2 打目の `touchend` だけを止める。`button` / `[role="button"]` は抑止で失う 2 打目の click を owner が `bubbles` / `cancelable` / `composed` 付きで 1 回だけ再送し、WebKit 等が browser click も送った場合は同一タスク境界の重複だけを capture で破棄する。単打、移動 gesture、複数接点、リンクと文字入力は既定動作へ残し、pinch、navigation、選択・caret を維持する。任意 callback には直前 tap からの時間・距離、抑止有無、除外理由、`cancelable` だけを固定形式で渡し、telemetry と相関 policy は app 側が所有する |
