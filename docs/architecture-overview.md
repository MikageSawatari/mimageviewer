...
| `keyboard_input.rs` | viewport pass 単位の `KeyboardOwner` / `TextInputPhase` / `ShortcutPermit`、bool / id snapshot だけを受ける純粋な所有者決定、pass cache、S3 期間の既存 App / Keymap 判定への互換投影を所有する。不純な snapshot 収集は `App::keyboard_ownership_snapshot` だけが担い、draft state を所有権に使わない |
...
| `ime_focus.rs` | 日本語を入力できる single-line TextEdit の共通描画境界。viewport 別の IME event 状態、Enter / Escape / Tab 由来の focus 復帰、IME 候補選択中だけの Tab focus-lock、caller の focus request latch を所有し、理由付き allowlist 以外の raw TextEdit を unit test で禁止する |
