...
| `content_identity.rs` / `content_identity/restore.rs` / `app/content_identity_detection.rs` / `app/content_identity_restore.rs` | 物理ファイルの size / 先頭 hash / 全体 hash 台帳と、通常の物理フォルダを開いたときだけ動く 3 段照合。`has_restorable_content` が復元元と検出 hash cache を分離し、段 0 のメモリ索引には復元元だけを載せる。段 1 / 2、metadata、SQLite は Low-priority worker に閉じ、同一 size / mtime の再訪は cache hash を使ってファイルを読まない。検出結果は folder key + items generation で current 判定して A3 用候補として保持する。A3a の復元入口は App / egui 非依存で共通 `STORES` の unique descriptor を copy し、sidecar / presence / cache の UI-owner 更新は report と A3b 用適用関数へ分離する |
