...
stack 展開の隣・件数判定より前で `SpreadPolicy` に従い 1〜2 件へ組み替え、`Merged` は
<kbd>Ctrl+E</kbd> と同じ経路 (`prepare_spread_export_dialog_target` → `render_export_pixels`) で
合成する。動画 `CurrentFrame` は提示済みフレームの PTS を ms 整数で載せた source を作る |
| `materializer.rs` | 外部プロセスや将来の D&D / file clipboard が使える汎用の実ファイル化境界。実画像 / ZIP entry / PDF page / 動画フレーム / 組み上げ済み画素と `TempEdited` / `TempOriginal` / `OriginalFile` から、元 path、元 bytes のコピー / 展開、または PNG を決める。**出力の形は編集の有無で変えない** (`TempEdited` は常に焼き、`TempOriginal` は常に元バイト列)。動画フレームは `video::screenshot::capture_frame` (クリップボードコピーと同じ helper) で worker 上に decode し、見開き合成は UI が組んだ画素を request で受け取って encode するだけにする。補正 DB の read-only snapshot、`CompositeSource` + `BakedEditSnapshot`、PDF 長辺、AI upscale を含めない合成を worker 上で処理する。source / 出力 stamp と完全な編集 fingerprint で process 内再利用し、`create_new` で atomic claim した request-owned RAII temp lease と process-owned reuse の分離、成功時 handoff、世代 cancel、dead-PID / 起動前から存在する現 PID 名の孤児回収、終了時 cleanup、basename sanitize と reparse point 非追従を所有する |
