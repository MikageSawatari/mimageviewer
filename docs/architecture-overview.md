...
| `context_menu_model.rs` / `ui_dialogs/context_menu.rs` / `native_context_menu.rs` | grid / fullscreen の右クリック項目を `MenuNode` の単一定義から構築し、通常は Win32 `HMENU`、構築失敗時だけ egui へ描画する。仮想項目と仮想混在選択は mIV 項目だけの native menu とし、実項目の Shell 項目は既定で末尾の「Windows のメニュー」へまとめる。`QueryContextMenu` はそのサブメニューの `WM_INITMENUPOPUP` まで遅延し、設定で従来の同階層表示へ戻せる。外部ツールは `show_in_context_menu` の平坦な一覧を共有し、フォルダー背景とコンテナー項目では対象だけをコンテナー 1 件へ切り替える。混在選択の実ファイル専用操作は target resolver の件数分類で全体を拒否する |
| `external_tool.rs` | 外部ツール登録モデル (`ExternalTool`、`ExternalToolLaunch::{Executable, Association, OsDefault}`、安定 `ExternalToolId`、対象ポリシー enum)、発火面 + checked + `current_grid_order()` から対象集合を作る純 resolver、Grid 専用の固定キースロット解決、表示時点の対象 snapshot を持つピッカー request、`effective_folder()` からコンテナー 1 件を作る resolver、`SelectionPolicy::{Single, Each, Batch}` の operation planner、`CommandLineToArgvW` 互換の引数分割と `{files}` を N 個の `OsString` へ展開する処理、1 利用者操作単位の worker / 成否集約を所有する。メニューバーとツールバーの直接起動も固定キースロットと同じ Grid resolver へ集約する。変換アーカイブは cache ZIP でなく元パスを使い、1 コンテナーに定まらない集約ビュー背景を拒否する。`Executable` だけが引数・作業フォルダーを使い、`Association::Batch` は worker から `open_with.rs` へ全 path を 1 `IDataObject` として渡す。P3 の実体化前は仮想項目が 1 件でもある対象集合を全体拒否する |
