...
| `crates/remote-web/web/app.js` | mIV Remote SPA。`gridHash` ごとの scroll / 明示復元アンカーを 64 件 LRU で持つ。復元アンカーは未操作でも 0 を持つ操作カーソルと分離し、一覧 DOM の破棄時保存、親復帰、画像・動画 viewer のページ更新を同じ復元経路へ集約する。画像 viewer / 補正 panel に加え、「端末の設定」で選ぶ端末別の静止画画質（高品質 8192 / 標準 4096 / 軽量 2048 / 最軽量 1024）、同一 target の前方 3 / 後方 1 ページ先読み、server-owned AI model catalog の選択、現在の見開きだけの自動 AI 開始、phase / page / stage / tile 進捗、取消、foreground polling と backoff、background 停止、recoverable 復帰を所有する。画像 viewer は要求位置と描画位置を別値で持ち、タイトル・ページ番号・シークは要求位置から即時導出し、描画位置が追いつくまで暫定表示にする。ページと AI result は JPEG を decode し、完成 result は Ready ページをすべて decode してから見開き DOM を一度だけ差し替え、NotApplicable ページの元画像を保持する |
