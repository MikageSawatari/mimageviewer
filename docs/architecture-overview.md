...
| `crates/remote-web/web/document-double-tap.mjs` | 文書全体の browser double-tap zoom 抑止を単一 ownership に集約する。`command-core.mjs` の時間差・距離による純粋な pair 判定を使い、成立した 2 打目の `touchend` だけを止める。単打、移動 gesture、複数接点、文字入力は既定動作へ残し、pinch と合成 click、選択・caret を維持する |
