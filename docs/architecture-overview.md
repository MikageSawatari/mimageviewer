...
| `ime_focus.rs` | 日本語を入力できる single-line TextEdit の共通描画境界。viewport 別の IME event 状態、IME 中のキー処理由来の focus 復帰、直前 pass の helper-managed focus contract、IME 候補選択中だけの Tab focus-lock、caller の focus request latch を所有する。pointer による focus 移動は復帰せず、理由付き allowlist 以外の raw TextEdit を unit test で禁止する |
