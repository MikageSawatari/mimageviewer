...
| `external_tool.rs` | 外部ツール登録モデル (`ExternalTool`、安定 `ExternalToolId`、対象ポリシー enum)、`CommandLineToArgvW` 互換の引数分割と `OsString` プレースホルダ展開、単一実ファイルの起動 request / worker / 結果通知を所有する。仮想ページの一時実体化はまだ扱わない |
...
| `open_with.rs` | Windows の関連付けアプリ列挙と EXE 選択。旧「アプリケーションで開く」の起動はファイルパスを `OsStr` のまま共通 external-tool worker へ渡す |
