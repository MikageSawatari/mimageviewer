...
| `crates/remote-web/web/app.js` | mIV Remote SPA。画像 viewer / 補正 panel に加え、server-owned AI model catalog の選択、現在の見開きだけの自動 AI 開始、phase / page / stage / tile 進捗、取消、foreground polling と backoff、background 停止、recoverable 復帰を所有する。完成 result は Ready ページをすべて decode してから見開き DOM を一度だけ差し替え、NotApplicable ページの元画像を保持する |
