...
| `external_tool.rs` | 外部ツール登録モデル (`ExternalTool`、`ExternalToolLaunch::{Executable, Association, OsDefault}`、安定 `ExternalToolId`、対象ポリシー enum)、`CommandLineToArgvW` 互換の引数分割と `OsString` プレースホルダ展開、単一実ファイルの起動 request / worker / 結果通知を所有する。`Executable` だけが引数・作業フォルダーを使い、`Association` は worker から `open_with.rs` の Shell 起動へ渡す。仮想ページの一時実体化はまだ扱わない |
