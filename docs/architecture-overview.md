...
| `remote_ipc/service.rs` | 保存済み `remote_service_enabled` の即時切替、`tailscale serve --bg <port>` の明示操作、`mimageviewer-remote.exe` の所有を専用 worker に閉じ込める。UI thread は希望状態を channel へ送るだけで、外部コマンド、spawn / kill / wait を行わない。実行ファイルは core と同じディレクトリから解決し、同じ `--data-dir`、共有既定値から本体が決めた `--port`、管理用 marker を渡す。serve 設定成功時は child 再起動で remote-web の JSON 再検出を走らせる。stdout は Bearer token を含むため破棄し、stderr の起動失敗・版不一致だけを型付き診断へ変換する。core 自身が spawn した `Child` だけを終了し、外部起動プロセスは所有しない |
