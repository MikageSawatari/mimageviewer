...
| モジュール | 役割 |
| --- | --- |
| `main.rs` | エントリポイント。フォント設定、logger 初期化、eframe 起動、`--pdf-worker` サブコマンド分岐、起動時データ移動予約の実行 (v0.7.0) |
...
| `data_dir.rs` | データディレクトリのパス解決。**bootstrap** (`%APPDATA%/mimageviewer`、設定・ログ・AI モデル・PDFium DLL 置き場) と **effective** (DB・キャッシュ置き場、`Settings::data_root` で切り替え可能、v0.7.0) の 2 段階 |
| `data_move.rs` | v0.7.0 データ移動機能。`MOVE_TARGETS` を順に copy → verify → delete する。起動時 `settings.pending_move` が有効な場合に `main.rs` から実行される |
| `instance_lock.rs` | Windows Named Mutex (`Local\mimageviewer_data_move`) と Toolhelp32 によるプロセス列挙。データ移動中に他インスタンスの起動を拒否する。通常起動では関与しない |
| `logger.rs` | パフォーマンス分析用ファイルロガー (`mimageviewer.log`、常に bootstrap 側) |
| `stats.rs` | 読み込み統計の集計 |
...
既定では `%APPDATA%/mimageviewer/` 配下 (「bootstrap」領域)。v0.7.0 以降は DB・キャッシュ系 (下表の **○** 印) は `Settings::data_root` で別ドライブに移動可能 (「effective」領域)。設定ファイル・ログ・AI モデル・PDFium DLL は常に bootstrap 側に残る。

| ファイル | 内容 | 書き込むモジュール | 移動対象 |
| --- | --- | --- | --- |
| `settings.json` | アプリ全体設定・グローバルプリセット・保存スロット・お気に入り・`data_root` | `settings.rs` | — |
...
| `pdf_passwords.json` | PDF パスワード (DPAPI 暗号化) | `pdf_passwords.rs` | ○ |
| `archive_cache.db` / `archive_cache/` | RAR/7z/LZH → ZIP 変換キャッシュ (v0.7.0 予定) | `archive_cache.rs` | ○ |
| `susie_plugins/` | Susie 画像プラグイン (v0.7.0 予定) | — | ○ |
| `pdfium.dll` | 初回起動時に exe から展開 | `pdf_loader.rs` | — |
| `models/*.onnx` | 初回起動時に exe から展開 | `ai/model_manager.rs` | — |
| `logs/mimageviewer.log` | 起動ごとに追記 | `logger.rs` | — |

### データ移動 (v0.7.0)

`Settings::data_root` は `None` で bootstrap と同じ場所 (既定)、`Some(PathBuf)` で別ドライブ (例: `D:\mimageviewer-data`)。

実際の移動は「予約 → 再起動時に実行」の 2 段階:

1. ユーザーが環境設定 → データ保存場所ページで新しい場所を指定 → `Settings::pending_move = Some({from, to})` が設定される
2. 次回起動時、`main.rs` が `pending_move` を検出し:
   - 他の mimageviewer プロセスがいないことを `instance_lock::count_other_mimageviewer_processes()` で確認
   - `instance_lock::acquire_data_move_guard()` で `Local\mimageviewer_data_move` Named Mutex を取得
   - 成功時のみ `App::default()` が開いた DB 接続をドロップしてから `data_move::MoveRunState::spawn` を呼ぶ
   - 他インスタンスが検出された場合は予約を保持したまま通常起動 (ユーザーへ Windows MessageBox で通知)
3. 移動スレッドが実行している間、メイン UI は進捗ダイアログのみ表示 (`ui_dialogs::data_move.rs`)。完了後は「終了」ボタンで App 終了 → ユーザーが再起動して通常モードへ
