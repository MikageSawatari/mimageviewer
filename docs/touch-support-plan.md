...
**初回オーバーレイヘルプを 1 回出す。常時表示の affordance は置かない。**

**タッチ操作で画像 / 本 / 動画 / 音楽を開いた最初の 1 回だけ**、その面で有効な
ジェスチャを図示したオーバーレイを重ねる。教えたら以後は何も出さないので、
**閲覧中に画像を遮るものが一切なくなる**。
...
- **3 ジェスチャ全部の実行は要求しない**。煩わしいうえ、安全上必要なのは中央タップだけ
- 必要なのは真偽値 1 個 (「クロームを一度呼び出したか」)。
  利用者向け設定ではないので §5.1-6 の無設定方針と矛盾しない
- **マウス・キーで開いた場合は出さない** (既存挙動を一切変えない)

#### 内容 — 面ごとに出し分ける

単なるテキスト箇条書きではなく、**中央領域の輪郭・左右領域・端からのスワイプ矢印を
半透明で図示**する。「クロームがある」ではなく「**どこを叩けば何が起きるか**」を伝える。
3 ジェスチャを 1 画面に詰めても、図示なら分かりにくくならない。

| 面 | オーバーレイの内容 |
| --- | --- |
| **静止画 / 本** | 中央タップ = メニュー / 左右タップ = ページ送り / 端からスワイプ = 操作パネル |
| **動画** | タップ = HUD 表示 / **左右ダブルタップ = 相対シーク** / 端からスワイプ = 操作パネル |
| **音楽** | タップ = HUD 表示 / 端からスワイプ = 操作パネル (左右タップは割り当てない) |

- **オーバーレイをタップすること自体が中央タップになる**ので、消し方を別途覚える必要がない
...
**動画にはページ送りが無い**ので、静止画と同じ割り当てはできない。

**確定 (利用者判断 2026-08-06): 左右タップは相対シーク。前後ファイル移動は下 HUD のボタンに任せる。**

- **単一タップは画面全体で HUD 表示**。動画では守るべきページ送り領域が無いので、
  中央矩形に限定する必要がない (動画プレイヤーの慣習どおり)
- **左右の「ダブル」タップで相対シーク**。単一タップにシークを載せると誤操作しやすい。
  ⚠ **HUD トグルはダブルタップ判定を待たずに即座に発火させる**こと。
  待つと単一タップの反応が鈍く感じる。2 回目のタップでシークを実行する
...
利用者判断 (2026-08-06、#3 は 2026-08-07 更新)。**未決事項はない**。
