...
#### 上部バーから開く「その他の機能」パネル (2026-08-13)

- 中央タップはクローム表示 / 非表示だけを担当し、新しいジェスチャーを重ねない。上部バーの
  `…` をタッチ / マウス共通の入口とし、既存の `prepare_bar_button_touch_targets` による
  最寄りボタンへの吸着をそのまま使う
- `…` は `×` のすぐ左に置き、パネルは上部バー直下の画面右上へ出す。機能行を 48pt、
  ヘッダーと閉じる hit target を 44pt にする。ナビゲータ位置は 4 隅を選ぶ
  1 階層サブメニューと戻る行を持つ。項目は 5 機能の固定順とする
- 描画と同じ panel rect を `touch_excluded` へ必ず加え、行、戻る、閉じるへの接触を
  `PageSide` / `ToggleChrome` へ分類しない。表示中は共通モーダル扱いで背面の wheel / key /
  pointer を止める一方、touch recognizer は接触終了まで駆動して owner を残さない
- 上部バーの 360 度ボタンは、見開きの単ページ / 2 ページやページ単位 / 連結読みで slot を
  消さない。使えない組み合わせは同じ位置の disabled button と理由 tooltip にする
