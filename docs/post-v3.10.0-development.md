...
朝の実機結果追記: 本人が起動した使い捨てportableで、互換性のない候補の無効化、bak2からの復元、完全リセット、各操作後の正常終了を確認した。ファイル検証もそれぞれ1/1 pass。§1.234は独立レビュー・全体gate・確認build・復元/reset実機が揃い、コミット可能な区切りとする。§1.231の初回markerだけは利用者の外出により後日のlive確認へ残す（headlessは成功済み）。追加のPC操作は行わない。

朝の最初のComputer Use起動要求は通常版へ誤解決したため、直ちに停止・報告し、以後は本人が正しいportableを起動した。通常版への復元/reset操作はなく、起動時自動保存は否定しない。深夜の未起動記録とは別の事象である。詳細は設定復旧planとlive-checksを参照。
