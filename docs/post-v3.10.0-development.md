...
追加要望の範囲決定: §1.237（評価増減・レーティング順）は今回に含め、§1.238（タグビューからフォルダへの移動不具合）と一覧の名前/番号降順・ツリーとの表記統一も直列で扱う。§1.239（先頭/末尾の個別配置）と§1.240（白紙ページ）は合わせて次回以降へ延期し、保留バックログへ移した。コレクションの現在の検収区切りを崩さず、共有ファイルのwriterを一人に維持する。

帰宅後の検証更新: 利用者の約5分のPC使用了承を得て、保留していた§1.231の初回起動liveを完了した。保持済みportable（core SHA `FDD49728…6535BD`）と使い捨てdataで、起動前last_seen_version=null→初回設定・File終了後3.10.0、preupgradeファイル0件を読み取り専用SQLite/ファイル列挙で検証。プロセス終了も確認。通常profileは起動せず、現在開発中のcollection成果物とは別の検証対象である。DB完全不在からのbootstrapは既存headless証拠と区別する。詳細は`target/settings-recovery-20260914/live-checks.md`。

2026-09-14 進行更新: §2.25は`614715726`、§2.24は`3438f9727`に実装をコミットした。双方とも独立レビュー、全体gate、確認ビルドが成功し、GUI確認は利用者帰宅後に残している。最新の全体gateはmain 8434 pass / 0 fail / 45 ignored、snapshot 50。設定復旧の最後の初回起動liveも引き続き保留する。利用者の続行指示により、次は§1.118コレクションの具体設計・基盤実装へ進む。既存の仕様合意を使い、保存/worker、PC一覧・操作、閲覧・連続再生、Remoteの境界を独立検収できる段階に分ける。機能全体の完了と初段基盤の完了を混同しない。
