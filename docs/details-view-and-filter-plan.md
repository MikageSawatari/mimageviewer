...
> (種類/拡張子/★/タグ/日付/サイズ/状態) とチップ式フィルタバー、遅延列 worker、
...
ファイル選択画面 (グリッド) に、サムネイルを出さず Explorer の「詳細表示」のように
...
5. サイズ
6. 更新日時
7. 編集フラグ (補正/ローカル補正/マスク/隠蔽/注釈/回転 を小アイコン束で 1 列)
...
> Explorer 互換列 (1,4,5,6) ＋ **mIV 独自列 (2 ★ / 3 タグ / 7 編集フラグ)** の構成。
> 独自列が「ただの Explorer 詳細表示」との差別化点。特に 7 は
...
- **フォルダ移動でも保持**: ★、日付、サイズ、編集状態、種別。値域が有界で、別フォルダでも
  意味が比較的安定しているため。
- **フォルダ移動時に解除、または一時停止チップとして可視化**: タグ、拡張子、カメラ名などの
  無界の値集合 facet。前フォルダ固有の値を持ち越すと 0 件表示になりやすい。
...
3. **Ph3 FacetFilter 軽量版**:
   ...
   タグ/拡張子 facet はフォルダ固有値として場所変更時に解除し、候補はその facet 自身だけを
   外した集合から再集計する。
4. **Ph4 遅延列基盤**:
...
6. **フォルダ移動時の facet 保持**:
   ★/日付/サイズ/編集状態/種別は保持。タグ/拡張子/カメラ名などの無界値集合は解除、または
   「一時停止中」チップとして可視化して 0 件化を避ける。
...
| フォルダ跨ぎ facet 永続 | §9 / §16.8 | 有界 facet は保持、無界値集合 facet は解除または一時停止チップ扱い |
...
| 指摘 | 判定 / 反映 |
| --- | --- |
| タグ facet / タグソートは `tags_cache` 依存で、未 prewarm 項目を「タグなし」と扱う | 現行でも設計課題として残る。全件タグ prewarm + 進捗/Ready gate にするか、可視範囲ベースの段階更新に留めるかは I/O 量の UX 判断が必要。今回の反映では、タグ到着時にタグ facet 有効なら表示集合、タグ列ソート中なら `details_order` を再構築して段階更新に追従する |
