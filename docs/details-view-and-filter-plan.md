...
struct FacetFilter {
    ...
    places: Option<HashSet<String>>,         // 元ファイル/元コンテナの親フォルダ
...
- 種別・拡張子・場所: distinct 値チェックボックス。場所は元ファイル/元コンテナの親フォルダで、
  ★レーティング一覧などの場所横断ビューで元フォルダを絞り込む用途を主眼にする。製本フォルダは
  内部キーは実パスのまま、表示だけ `本棚 > 本名` に寄せる。
...
| `facet_filter` | 絞り込み**条件** (種別/場所/タグ集合/日付/サイズ/編集/★) | 種類ごとに保持方針を分ける |
