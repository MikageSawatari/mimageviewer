...
> **ステータス: Ph1〜Ph4 + Ph5 画像/動画/作成日時遅延列、コンテナページ数列まで実装済み (2026-07-18)**。`GridViewMode`、列セクション内の詳細切替、
...
> v2.0.0 で実装済み (`details_name_width_auto` / `details_name_width` / `details_layout`)。
...
> ZIP / PDF / 本として扱う画像のみフォルダの共通 `ページ数` 列は、同じ遅延列基盤と
> catalog cache を使って実装済み。EXIF 撮影日時/カメラの列は後続フェーズで追加する。
