...
> **ステータス: Ph1〜Ph4 + Ph5 画像/動画/作成日時遅延列、コンテナページ数列まで実装済み (2026-07-21)**。`GridViewMode`、列セクション内の詳細切替、
> 固定列の詳細一覧骨格、右クリック列表示メニュー、`DetailsSortKey`、`details_order`、列ヘッダ 3 トグルソート、
> 詳細表示中のキーボード移動 / Shift 範囲選択の表示順対応、軽量 `FacetFilter`
> (種類/拡張子/場所/★/タグ/日付/サイズ/状態/AI モデル/生成ツール) とチップ式フィルタバー、遅延列 worker、
...
> 列境界ダブルクリックによる内容に合わせた最適幅（全行の exact max を 1 frame 最大 192 行の
> 分割 job で文字計測し、一覧 identity/order・view kind・列・font/style/DPI 変更時は破棄、
> 対象列の tag/lazy cell-content revision 変更時は先頭から再走査）、左端プレビュー列のホバーサムネイル、
> 日時の秒表示オプションは実装済み。
> 名前列の固定幅 (右端ドラッグで自動調整→固定、ヘッダ右クリック `名前の幅を自動調整` で復帰) と、
> 縦スクロールバー gutter を見込んだ列レイアウト (右端列の欠け / 不要な横スクロールバー防止) は
> v2.0.0 で実装済み (`details_name_width_auto` / `details_name_width` / `details_layout`)。自動幅では
> UI スケールに応じた最低 1 物理 px とヘッダ右端線の半幅を余白として残す。横バーの要否は
> egui の丸め後 `content_size` ではなく、アプリが所有する全列幅・gutter・右端線を物理ピクセルの
> 右端座標へ変換して決める。収まる場合は横バー・横入力を無効化して offset を 0 に戻し、手動の
> 名前幅を含め実描画が 1 物理 px でも越える場合はバーを必ず表示して scroll extent を外向きに
> 確保する。縦横スクロールバーは同じ solid style を使う。下部の選択情報バーも同じ判定・solid
> 横バー・専用レーンを使い、濃い floating bar がファイル名へ重ならないようにする。
> v2.0.0 追補レビュー後、複数選択型 facet メニュー (種類/拡張子/AIモデル/生成ツール/★/状態) は
> タグ/場所/画像色と同じく外側クリックまで開いたままに統一し、絞り込みバー右端の件数は
> `表示件数 / 全体件数 件` に変更。場所 facet の件数もタグ facet と同様に表示集合単位でキャッシュする。
> AI モデル/生成ツール facet は v1.7.0 で session-local の遅延メタデータ facet として追加済み。
> ZIP / PDF / 画像のみフォルダ / 親 ZIP 内の子 ZIP・RAR / 直読み RAR の共通 `ページ数` 列は、
> 同じ遅延列基盤と catalog cache を使って実装済み。画像のみフォルダのページ数は自動表示設定に
> 依存せず、親 ZIP 内の子書庫は列挙済みツリーから同期 I/O なしで算出する。
> EXIF 撮影日時/カメラの列は後続フェーズで追加する。
...
struct FacetFilter {
    ...
    places: Option<HashSet<String>>,         // 元ファイル/元コンテナの親フォルダ。一時状態
...
### 6.4 種別/拡張子/場所/編集/AI facet

- 種別・拡張子・場所: distinct 値チェックボックス。場所は元ファイル/元コンテナの親フォルダで、
  ★レーティング一覧などの場所横断ビューで元フォルダを絞り込む用途を主眼にする。場所は
  現在の表示スコープにだけ効く一時条件とし、フォルダ/ZIP/PDF/仮想ビューを移動したら常に解除する。
  製本フォルダは内部キーは実パスのまま、表示だけ `本棚 > 本名` に寄せる。
- 編集: `補正 / ローカル補正 / マスク / 隠蔽 / 注釈 / 回転 / タグあり / ★あり /
  ブックマークあり` の有無。それぞれ `なし` 条件も選べる。
...
- ブックマーク状態は、動画・音声では対象ファイルに 1 件以上、本の親セルでは対象コンテナに
  1 ページ以上、開いている本のページではそのページ identity に登録があるかで判定する。
  DB はフィルタ選択時に worker で軽量スナップショット化し、一覧の行判定から同期 I/O しない。
...
| `details_row_style` | 行区切り線 / 交互背景色 / 両方 / なし | 永続 |
| `facet_filter` | 絞り込み**条件** (種別/場所/タグ集合/日付/サイズ/編集/★) | 種類ごとに保持方針を分ける。場所は非永続 |
...
- **場所 facet は移動で解除**: 場所は表示中リストの元フォルダをさらに絞るための一時条件なので、
  フォルダ/ZIP/PDF/仮想ビューのスコープが変わったら現在条件と退避済み条件の両方から削除する。
  再起動後に古い場所条件で空表示になるのを防ぐため、設定にも永続化しない。
...
- **第一弾 facet は全て在メモリ** → `rebuild_visible_indices` は O(n) の述語評価で済む。
  数千件でも 1 フレーム内。distinct 値集計 (タグ/種別/拡張子/場所) も在メモリで軽いが、
...
- distinct 集計の正しさ (タグ/種別/拡張子/場所の件数)。
...
画像のみフォルダ / 直読み RAR を一度に開かない。親 ZIP 内の子書庫は保持中の列挙済みツリーを使う。
ページ数ソート時だけ全対象の取得完了を要求する。
...
- ウィンドウ下部に細いステータス行。完了時に消えても詳細一覧の上端位置を動かさない:
...
7. **進捗を常時可視化**:
   遅延列ロード中は詳細リストの上端を動かさないウィンドウ下部に進捗行を出す。
...
- ページ数列の通常表示は可視範囲 + 先読み範囲を段階取得する。この段階の `Ready` は全件取得を
  ...
  toolbar 順を維持する。画面外の未取得値を `None` のまま並べた順序を確定しない。ページ数
  ソートから別列へ切り替えた時点で全行ジョブが進行中ならキャンセルし、現在の列構成に必要な
  可視範囲の段階 request へ戻す。全行ジョブが完了済みの `Ready` なら、他の遅延列も全行分
  充足しているため状態を維持し、UI thread で全件を再評価しない。
...
- worker の `Finished` と走査セッションの完了を分ける。ページ数を可視 + 先読み範囲で段階取得する
  場合は、`Finished` 後も typed state を走査中に保ち、同じ frame の一覧描画後に更新済み対象集合を
  照合する。未処理項目があれば次 job へ接続し、無ければそこでだけ `Ready` にする。タイマーによる
  表示延長は使わない。フォルダ / 一覧内容、列構成、フィルタの変更は新しい走査セッションを始める。

### 16.5 進捗 UI の最終仕様

配置は詳細リストの下、ウィンドウ下部。パネルの出入りで詳細リストの上端を動かさず、
完了時に一覧全体が 1 行ぶん移動して見えることを避ける。
...
- セッション完了後は非表示。ただし失敗がある場合は短いチップ `詳細情報: 3件取得失敗` を出す。
  連続 job の境界では表示を外さず、セッション完了時だけ下部の予約高を解放する。
...
3. **Ph3 FacetFilter 軽量版**:
   種別/拡張子/場所/★/タグ/日付/サイズ/編集状態、チップバー、全解除。実装済み。
