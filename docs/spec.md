...
| **RAR / 7z / LZH などの対応アーカイブ** | `archive_file_handling` が `Ignore` 以外なら表示する。非ソリッドかつ入れ子なしの RAR/CBR は worker 判定後に直接 1 枚目を読み、それ以外は有効な変換キャッシュ ZIP から読む。どちらも形式バッジを表示し、📌指定した代表サムネを優先する |
