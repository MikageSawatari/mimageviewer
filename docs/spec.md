...
- 外部ツールの `PayloadPolicy` は 3 値で、**出力の形は編集の有無で変えない**。
  ...
  **常に元バイト列**をそのまま書き出す (PDF ページだけは元バイト列が無いので未加工 render)。
  ...
  カラー化 / Creative LUT / ポストフィルタ / スマートシャープ / AI アップスケールは
  表示専用として焼き込まない (製本と同じ)。
...
- 見開き表示中は `SpreadPolicy` に従う。`Merged` は <kbd>Ctrl+E</kbd> と同じ経路で合成した
  1 枚、`BothPages` は**読み順** (ページ番号昇順) の 2 件、`MainPageOnly` は現在ページ 1 件。
...
- ZIP / PDF ページと Stack は P3 の実体化対象。動画の `CurrentFrame` と見開きの
  `Merged` / `BothPages` / `MainPageOnly` の適用は P4 で、現段階では動画本体 / 現在ページ 1 件を渡す。
...
- 書き出しは `ExportEntry.crop` に枠を載せ、既存の 1 スナップショット → N ファイル経路へ流す。出力名は `_1`〜`_4` で、これが投稿順になる。隠蔽プリセットの一括出力は併用できず、理由を表示して無効にする (永続化された `export_batch_selection` は書き換えない)。既定の出力サイズは長辺 2048px 以下で、元が小さければ拡大しない。
