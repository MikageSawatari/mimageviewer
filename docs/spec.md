...
| 右ドラッグ（リング設定時） | リングショートカットのフリック。右ボタンを押した位置を中心にリングを表示し、8 方向のスロットを選んで離すと現在コンテキストの一発アクションを実行する。中央で離すと取消。右ドラッグ mode はグリッド / 画像フルスクリーン / 動画フルスクリーン / 編集モードごとに未使用・リング・ジェスチャから選ぶ。既定はすべて未使用。短い右クリックと左ドラッグのファイル D&D には干渉しない |
...
| 右半分クリック | 次の画像へ |
| 左半分クリック | 前の画像へ |
| 右ドラッグ / 長押し（リング設定時） | リングショートカットのフリック。右ボタンを押した位置を中心にリングを表示し、8 方向のスロットを選んで離すと現在コンテキストの一発アクションを実行する。中央で離すと取消。右ドラッグ mode はグリッド / 画像フルスクリーン / 動画フルスクリーン / 編集モードごとに未使用・リング・ジェスチャから選ぶ。既定はすべて未使用。短い右クリック、中ボタンズームとは別の状態として扱う |
