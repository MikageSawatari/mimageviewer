...
- 列セクションの `詳細` は環境設定 → ツールバーで表示/非表示を切り替え可能。
  非表示でも <kbd>Alt</kbd>+<kbd>-</kbd> のショートカットは使える
- 詳細モードでは縦横比セクションを非表示にする。列セクションは `詳細` 選択状態として残す
- 各セクション・個別項目の表示/非表示をカスタマイズ可能。本棚セクションも環境設定 → ツールバーで非表示にできる
...
- セルサイズ = ウィンドウの残余領域 ÷ 列数
- 選択中セルの下にはサムネイル情報ツールチップを表示する。既定はファイル名と画像解像度、動画ではファイル名と動画長さ。表示内容は環境設定 → サムネイルで、種類 / サイズ / 更新日時 / 作成日時 / 動画解像度 / 動画コーデック / 場所 / 読書履歴の最終閲覧 / 読書履歴の既読位置を切り替えられる。読書履歴ビューでは、ホバー tooltip に場所 (フルパス) / 最終閲覧日時 / 既読位置を表示する (複数フォルダ・ドライブの同名本を場所で判別できるようにする)。ツールチップ幅はセル幅固定ではなく最大約 2.5 セル分を使い、画面右端ではセル右端に合わせて画面内に収める。動画長さなど未取得の値は選択中の 1 件だけバックグラウンドで遅延取得する
- **仮想スクロール実装**（下記詳細参照）
...
- サムネ / 詳細の両モード共通で、ツールバー下のスマートフィルタバーから
  ...
  環境設定 → ツールバーで表示/非表示を切り替え可能。アクティブ条件はチップ表示し、
  `全解除` でまとめて解除できる
...
- 保存済み再生位置がある動画は通常、前回位置からレジューム再生する。挙動は
  **位置復元マトリクス**(環境設定「位置の復元」)で「メディア (動画 / ZIP・PDF・対応アーカイブ) ×
...
- 読書履歴は、フルスクリーンで読んだ画像フォルダ / ZIP / PDF / 対応アーカイブを
  ...
  移動して前後の本を探せる。
  環境設定 → 位置の復元で記録 ON/OFF、保持件数、全件クリアを操作できる。
