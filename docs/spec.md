...
サムネイル一覧、通常の画像フルスクリーン、消しゴム / 隠蔽加工 / 切り取り / 補正レイヤーモードでは `?` キーで現在の文脈のショートカット一覧を
表示する。keymap 化済み操作は現在の実割り当てで表示し、Enter / 矢印など当面固定扱いの
操作は固定キーとして補助表示する。キー未設定または明示無効化中の割り当て可能操作は
別枠で表示する。`?` 自体は初期版では固定キー扱い。動画フルスクリーンは egui 経路から
初期対応し、native 動画 overlay 上で直接開くヘルプとテキスト注釈のヘルプは後続。
