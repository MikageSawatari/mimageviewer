...
| Ctrl + B | 選択中またはチェック済みの画像 / ZIP 内画像 / PDF ページをアクティブな本へ追加する。通常画像は byte コピー、ZIP 内画像は格納 bytes 抽出、PDF/編集焼き込みが必要なページは画素保存する。動画を一覧で選んだ場合は、動画再生中に操作すれば現在フレームを追加できることを案内する |
