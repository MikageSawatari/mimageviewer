...
- **画像/動画を別ウィンドウで開く設定**: 通常画像 / ZIP 内画像 / PDF ページは、開くたびに
  ...
  表示中の active 画像は表示専用ウィンドウとして残す。この設定中は画像ビューアのピン留めボタンを表示しない。
  ...
  メイン一覧のフォルダ移動へ入らず案内だけ出す。動画表示中の <kbd>F12</kbd> は現在の動画だけを
  メインウィンドウ / 別ウィンドウへ一時切替するが、この設定自体は変更しない。次に動画を明示 open した場合は、
  設定に従って再び別ウィンドウ側で開く。
  detached 動画は、メイン一覧のフォルダ移動 / お気に入り移動などで main context が入れ替わっても
  active detached context 側で再生を継続し、以後はメイン一覧の選択変更には追従しない。
  切り離した動画窓内の前後移動は、その窓が保持する同一一覧内だけを対象にする。
  always-new 窓とピン留めした連動なし窓では、消しゴム・補正レイヤー・隠蔽加工・テキスト注釈・切り取り・
