...
- 表示モードは 1 ページ / 見開き (左→右・右→左、表紙単独あり) / 縦読みを選べる。縦読みは
  <kbd>6</kbd> キーまたはホバーバーの表示モードボタンから切り替え、画像 / ZIP 内画像 /
  PDF ページを縦方向に仮想配置してホイール・↑↓・PageUp/PageDown・ゲームパッド上下で
  スクロールする。巨大キャンバスは作らず、GPU テクスチャは可視範囲 + 上下 2 ページ程度、
  最大 20 ページ程度に抑え、同時可視ページ数が 16 ページを超えるような極端なズームアウトは
  ズーム下限を引き上げて防ぐ。
...
| キー | 動作 |
|------|------|
| ← / ↑ | 前の画像へ（フォルダはスキップ）。縦読みモードでは ↑ は上へスクロール |
| → / ↓ | 次の画像へ（同上）。縦読みモードでは ↓ は下へスクロール |
| PageUp / PageDown | 縦読みモードでは画面単位で上下スクロール |
| S | スライドショー 再生 / 停止 |
...
| マウスホイール | 前/次の画像へ。縦読みモードでは上下スクロール |
...
| 入力 | 動作 |
|------|------|
| 方向パッド / 左スティック | グリッドでは選択移動。画像フルスクリーンではページ送り。縦読みモードでは上下が縦スクロール、左右がページ送り。動画フルスクリーンでは左右がシーク / タイルカーソル移動、上下が前後ファイル移動 |
| A / B | 決定・開く・再生 / 戻る・閉じる |
...
| Select | 画像フルスクリーンの表示モード切り替え (単ページ / 見開き / 縦読み) |
