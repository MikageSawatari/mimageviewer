...
- `ページ数` は ZIP / PDF / 画像のみフォルダの共通遅延列とし、読み込み中は `...`、非対象または取得不能は `-` を表示する。ZIP はネスト ZIP を含む実際の閲覧ページ列、PDF は文書総ページ数、画像のみフォルダは本として自動表示する規則と同じ件数を使う。取得済み値はページ数ソートにも使う。パスワード付き PDF は、保存済みパスワードがある場合だけ親一覧でページ数を取得する。開く際に入力したパスワードを保存したときは、同じ起動中に旧結果を無効化して再取得する。保存しない場合は情報露出を避けるため `-` のままとする
...
| `details_show_page_count` | bool | true | 詳細表示モードで遅延ロード列 `ページ数` を表示するか。ZIP / PDF / 本として扱う画像のみフォルダが対象 |
...
| `thumb_tooltip_show_page_count` | bool | true | 選択情報にページ数を表示するか。ZIP / PDF / 本として扱う画像のみフォルダの選択時だけバックグラウンド取得する |
