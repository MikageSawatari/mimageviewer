...
- 表示はページ構成 (1 ページ / 見開き 左→右・右→左、表紙単独あり) と連結方式
  ...
  連結モードでは余白カットフィットをスキップする。ホバーバーのズーム/フィットメニューでは
...
| 右ドラッグ（設定 ON 時） | リングショートカットのフリック。右ボタンを押したまま 20px 以上動かして 8 方向のスロットを選び、離すと現在コンテキストの一発アクションを実行する。既定は OFF。短い右クリックと左ドラッグのファイル D&D には干渉しない |
...
| 右半分クリック | 次の画像へ |
| 左半分クリック | 前の画像へ |
| 右ドラッグ（設定 ON 時） | リングショートカットのフリック。右ボタンを押したまま 20px 以上動かして 8 方向のスロットを選び、離すと現在コンテキストの一発アクションを実行する。既定は OFF。短押しの終了、静止長押しのコンテキストメニュー、中ボタンズームとは別の状態として扱う |
...
例外として X リングだけは「環境設定 > リングショートカット」で 8 方向スロットを差し替えられる。
...
| Select | 画像フルスクリーンの見開きモードを巡回切り替え (単ページ / 見開き 左→右 / 左→右(表紙あり) / 右→左 / 右→左(表紙あり))。連結方式 (ページ単位/縦連結/横連結) の切替ではない |
...
| X+方向パッド / 左スティック | リングショートカット。X を押しながら方向を選び、X を離すと現在コンテキスト (グリッド / 画像フルスクリーン / 動画フルスクリーン) のスロットに設定された一発アクションを実行する。方向なしで離すと専用ピッカーパネルを開く。画像フルスクリーンのメタデータ表示は既定で画像リングの右上スロットへ割り当てる |
