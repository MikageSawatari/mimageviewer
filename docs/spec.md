...
- v2.7.0では「メタ情報をエクスポート / インポート」を安定化のため一時非表示にしたが、
  ...
  directoryへ、評価、タグ（表示順・最近使用順を保つ適用時刻を含む）、動画・音声ブックマーク、
  本ブックマーク、見開きモード・
  ...
  ページ情報がある場合は曖昧な統合をせず、対象アーカイブを示してexportを中止する。
...
- インポートは先に sidecar 全体を検証し、存在件数、欠落件数、ファイル種類・サイズ相違件数を
  ...
  reparse path、重複パス、
  未対応バージョン、上限超過データは拒否する。media kindとメタ情報区分の不正な組み合わせも
  拒否する。本ブックマークはコンテナ種別とページ種別の
