...
- サムネ / 詳細の両モード共通で、ツールバー下のスマートフィルタバーから
  種類・拡張子・★・タグ・日付・サイズ・状態・AI モデル・生成ツールを絞り込める。スマートフィルタバーは
  ツールバーの空き領域を右クリック (または `設定` メニュー → `ツールバー`) で表示/非表示を切り替え可能。
  `絞り込み:` ラベル右クリックで、バーに出すボタン (種類 / 拡張子 / AI モデル / 生成ツール /
...
| `facet_filter` | FacetFilter | default | スマートフィルタ条件。種類・拡張子・タグ・日付・サイズ・状態・AI モデル・生成ツールなど。絞り込み中に ZIP/PDF/フォルダなどのコンテナへ入ると親階層の条件を一時退避し、内側では別条件を設定できる。Backspace などで親階層へ戻ると退避した条件を復元する |
