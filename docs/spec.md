...
- 動画シークバーから上へドラッグ、または `VideoSeekStrip` で開くシークストリップは、
  左上の「場面」/「波形」で表示を切り替える。選択は `Settings.video_seek_strip_mode` に保存し、
  既定は「場面」。波形は 1 幅 60 秒の時間線形軸で、切替時は固定中央線の時刻を保つ。
  どちらもドラッグ中は seek せず、release 時の 1 回だけ精密 seek して再生 / 一時停止状態を保つ。
