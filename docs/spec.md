...
| メニュー | 項目 |
|---------|------|
| ファイル | フォルダを開く… (Ctrl+O) / 閲覧履歴を開く / レーティング一覧 / 現在地フィルタ (Ctrl+F) / メタ情報をエクスポート… / メタ情報をインポート… / キャプチャ保存フォルダを開く / ゴミ箱を開く / 外部ツール ▸ [登録済み全ツール] / 終了 |
...
- `ファイル` → `外部ツール` は登録順に全ツールを出す動的サブメニューで、各ツールの
  `右クリックメニューに表示` 設定には左右されない。右クリックメニュー側は同設定が ON の
  ツールだけを平坦に出す。どちらから起動しても、チェック済み項目を優先し、無ければ現在項目を
  対象にする共通の Grid 解決を使う。
...
- 左フォルダツリーペインの表示切替、本棚の追加先切替/追加/開く、列数（1〜10 + 詳細）、
  ...
  登録済みスマートフォルダのボタン、
  タグの付与/解除ボタンとピン留めタグのボタン、任意の登録済み外部ツールを配置
- 外部ツールセクションの既定は非表示。表示すると登録ツールを展開 / 折りたたみ / プルダウンの
  3 形式から直接起動でき、未登録時は `(未登録)` と表示する
...
| `show_toolbar_external_tools` | bool | false | ツールバーに外部ツールセクションを表示。既存のツールバーを変えないため既定 OFF |
...
| `toolbar_{cols,aspect,sort,favorites,external_tools,smart_folders,tags,bookshelf}_display` | ToolbarSectionDisplay | Buttons | 各セクションの表示形式 (展開 Buttons / 折りたたみ Collapsible / プルダウン Dropdown)。セクションのラベル右クリックで変更 |
| `toolbar_{favorites,external_tools,smart_folders,tags,bookshelf}_collapsed` | bool | false | 折りたたみ表示時の畳み状態 (永続) |
