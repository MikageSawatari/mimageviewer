...
- 動画の拡大方法は、既定の「OS に任せる」（従来表示）、拡大・縮小とも高品質補間する
  「標準（補間あり）」、拡大をくっきり補正する「シャープ拡大」、拡大時に画素を補間しない
  「ニアレスト（補間なし）」から選べる。シャープ / ニアレストも縮小時は標準と同じ高品質縮小を
  使い、「縮小時のなめらかさ」は静止画とは別に 0〜100%（10% 刻み、既定 0%）で保持する。
  選択場所は動画フルスクリーン左パネルの「画像補正」→「フィルタ」で、<kbd>T</kbd> は 4 方式を
...
| `video_scale_filter` | VideoScaleFilter | OsDefault | 動画の拡大方法。OsDefault / Standard / Sharp / Nearest。既定は実機比較前の互換性を優先して OsDefault |
...
- 確認ウィンドウは閲覧を中断しない。フルスクリーン中は表示せず、一覧へ戻るまで候補を保持する。
  照合と復元は画面操作を止めずにバックグラウンドで行う。
