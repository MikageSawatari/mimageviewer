...
- 静止画フルスクリーンの左右パネルは「通常ホバー」と「クリック表示」を選べる。通常ホバーは左端で補正パネル、右端でメタデータパネルを個別に表示し、上端は上部バーだけを表示する。クリック表示は左右最端の細い呼び出しバーをクリックして開き、明示的に閉じるまで維持する。右情報パネルの開状態と選択モードは再入場後も保持し、左編集パネルはフルスクリーン退出で閉じる。I / Tab、上部バーの i、画像リング / ゲームパッド操作は表示モード切替に使う
