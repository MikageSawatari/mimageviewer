...
| 製本 | アクティブな本に追加 (Ctrl+B) / 本棚フォルダを開く / アクティブな本を開く / この本を並べ替え / 製本の管理… / 追加先を選ぶ |
...
| Ctrl + B | 選択中またはチェック済みの画像 / ZIP 内画像 / PDF ページをアクティブな本へ追加する。通常画像は byte コピー、ZIP 内画像は格納 bytes 抽出を優先し、補正・非破壊回転・PDF など焼き込みが必要なページだけ画素保存する。動画を一覧で選んだ場合は、動画再生中に操作すれば現在フレームを追加できることを案内する |
...
| Ctrl + B | 現在表示中の静止画 / ZIP 内画像 / PDF ページをアクティブな本へ追加する。補正・消しゴム・隠蔽・テキスト・AI などが有効な場合は表示結果を焼き込んで保存する |
...
| Ctrl + B | 動画の場合、現在の再生フレームを画像としてアクティブな本へ追加する |
