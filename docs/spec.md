...
リング / ジェスチャ設定、マウス戻る / 進むボタン、ゲームパッド固定ボタンと X リングは
...
破壊的操作は標準では割り当てないが、操作カスタマイズのゲームパッド図、リングショートカット、
またはコマンド一覧の編集ダイアログから、文脈ごとに主要ボタンや X リングの割り当てを差し替えられる。
...
| A / B | 決定・開く・再生 / 戻る・閉じる |
| LB / RB | 既定では Ctrl+↑ / Ctrl+↓ と同じツリー順の前 / 次フォルダ移動。操作カスタマイズで文脈ごとに既定 / なし / 一発アクションへ差し替え可能 |
| LT / RT | 既定ではグリッドで連続スクロール、画像でズームアウト / ズームイン、動画で連続シーク。操作カスタマイズで文脈ごとに既定 / なし / 一発アクションへ差し替え可能 |
...
| A / B / X / Y / Select / Start / LB / RB / LT / RT 単体 | 既定では上記のゲームパッド操作を行う。操作カスタマイズのゲームパッド図、またはコマンド一覧の編集ダイアログから、各文脈ごとに既定 / なし / 一発アクションへ差し替え可能。X 単体を既定から変更した場合は X+方向リングも起動しない。X を既定に戻すと、保存済みの X+方向リング内容を再び使える |
