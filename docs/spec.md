...
- インポートは先に sidecar 全体を検証し、存在件数、欠落件数、ファイルサイズ相違件数を
  確認表示してから実行する。sidecar に記載された物理項目は、含まれる各メタ情報区分を
  ファイル単位で上書きする（空の区分は既存値を消す）。sidecar に記載がないファイルの
  既存メタ情報は保持する。対象が存在しない、種類が異なる、またはファイルサイズが異なる
  場合はその項目だけをスキップする。相対パス以外、root 外を指す reparse path、重複パス、
  未対応バージョン、上限超過データは拒否する。本ブックマークはコンテナ種別とページ種別の
  正しい組み合わせだけを許可し、ページ相対パスの絶対指定・親参照・コンテナ外 reparse path
  もDB反映前に拒否する。未リリース中に作成したv1〜v3 bundleとの互換は持たず、タグ適用時刻を
  含むv4だけを受け入れる。v4 import の適用後は、同じ場所に既存 `mimageviewer.dat` があれば
