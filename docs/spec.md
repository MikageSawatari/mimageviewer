...
| A / B | 決定・開く・再生 / 戻る・閉じる |
| LB / RB | 既定では Ctrl+↑ / Ctrl+↓ と同じツリー順の前 / 次フォルダ移動。固定ボタン単体は現在カスタマイズ対象外 |
| LT / RT | 既定ではグリッドで連続スクロール、画像でズームアウト / ズームイン、動画で連続シーク。固定ボタン単体は現在カスタマイズ対象外 |
...
| A / B / X / Y / Select / Start / LB / RB / LT / RT 単体 | 既定では上記のゲームパッド操作を行う。固定ボタン単体のカスタマイズは現在無効。X 単体は常にピッカーパネルを開き、X+方向リングだけを操作カスタマイズで編集できる |
