...
| `fullscreen_prefetch_status_visible` | bool | true | AI 先読み中の行と前後ページ別の点表示を切り替える。点は前のページを遠い→近い、次のページを近い→遠い順に現在位置の区切りを挟んで並べ、取得済みは緑、取得中は黄、未取得は灰色の輪郭で示す。片側 8 ページを超える場合は現在に近い 8 ページを点で残し、遠方分を状態別件数へまとめる。ホバー時は全対象の状態別件数を表示する。OFF にしても先読み処理は続ける。処理状況も OFF のとき、表示する行も点表示もなければ左下の枠自体を描画しない |
