...
- **A / B クイックフォルダボタン**: コピー元・コピー先など 2 つの作業場所を
  ...
  - 検索中、タグ / ★固定など一時ビューでは誤って検索前フォルダを記憶しないよう
    ...
    通常どおり記憶できる。
  - フォルダバー設定メニューから、最近開いたフォルダ履歴と A/B の記憶した場所を
    それぞれ確認なしでクリアできる。同じ操作は標準キーなしの Grid コマンドとして、
    キー / リング / マウス / ゲームパッド X+方向へ割り当てられる。どの入口もクリア後に
    設定を保存し、完了トーストを表示する。
...
keymap 対象にする。最近開いたフォルダ履歴と A/B の記憶した場所のクリア、および
