...
- 詳細モード中は新規サムネイル要求を抑制し、既存サムネイルテクスチャも一度だけ解放する。
  列数・縦横比の設定はサムネモードへ戻った時のために保持する
- サムネ / 詳細の両モード共通で、ツールバー下のスマートフィルタバーから
  種類・拡張子・★・タグ・日付・サイズ・状態を絞り込める。アクティブ条件はチップ表示し、
  `全解除` でまとめて解除できる
...
| `archive_cache_max_bytes` | u64 | 0 | RAR / 7z / LZH から作成した変換済み ZIP キャッシュの容量上限。`0` は無制限。上限を超えた場合は、次回の変換完了後に最終アクセスが古いキャッシュから削除する。直近で作成したキャッシュは、単体で上限を超えても削除しない |
