...
| `ui_scale_factor` | f32 | 1.0 | アプリ内 UI 表示倍率。設定メニュー「スケーリング」から 0.5〜2.0 を 0.1 刻みで選ぶ。main egui Context の `zoom_factor` が正本で、メイン UI・静止画フルスクリーン・native 動画 overlay に反映する。VST plugin GUI の別 HWND は対象外 |
