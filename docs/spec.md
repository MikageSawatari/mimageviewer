...
- **画像を毎回新しい別ウィンドウで開く設定**: 通常画像 / ZIP 内画像 / PDF ページだけに適用する。
  この設定が ON の間は、画像を開くたびに現在の active 画像ビューアを表示専用の別ウィンドウとして残し、
  次の画像を新しい active 別ウィンドウで開く。メイン一覧側の Backspace / フォルダ移動 / 再読込でも、
  ...
  メイン一覧のフォルダ移動へ入らず案内だけ出す。動画は当面 F12 の既存 host migration を維持する。
  detached 動画は、メイン一覧のフォルダ移動 / お気に入り移動などで main context が入れ替わった時点で
  active detached context 側へ切り離して再生を継続し、以後はメイン一覧の選択変更には追従しない。
...
| F12 | 別ウィンドウモード ON/OFF を切り替える。表示中セッションがあれば可能な限り再生位置・現在ページを維持して表示先を切り替える。F11 のフルスクリーン / ウィンドウ内選択は変更しない。設定「画像を毎回新しい別ウィンドウで開く」が ON の間は静止画 / ZIP画像 / PDFページでは無効化し、動画表示中のみ既存 host migration として扱う |
