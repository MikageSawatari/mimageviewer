...
- **ツールバーのカスタマイズはすべて右クリックで行う** (v2.0.0、`toolbar-customization-plan.md`):
  - **空き領域を右クリック** = 表示するセクションの ON/OFF チェックリスト + 「既定に戻す」。
    全セクションを隠した場合の入口として、`設定` メニュー → `ツールバー` からも同じメニューを開ける。
  - **セクションのラベルを右クリック** = そのセクションの設定 (表示形式 = 展開/折りたたみ/プルダウン、
    出す項目 = 列1〜10/比率/ソート、行頭に表示、このセクションを隠す、既定に戻す、本/お気に入り/タグの管理)。
...
- セクション設定の値 (表示/並び順/表示形式/出す項目) は永続化される。フォルダバー (アドレス行) も
  「並べ替えできないだけのセクション」として扱い、アドレスバー左端の `フォルダ:` ラベル右クリック、
  または `設定` メニュー → `ツールバー` → `フォルダバーの設定` から出すボタンを設定する
  (専用の環境設定ページは廃止)
