...
- 外部ツールの起動導線は、登録した全ツールを登録順に平坦に出す右クリックメニューと、
  ...
  ピッカーも表示前に同じ capability を適用し、仮想ページでは `RealFileOnly` を候補外、
  元ファイルでない出力になる編集用ツールを理由付き無効行にする。
...
- 外部ツールの `PayloadPolicy` は起動時に適用する。`AsDisplayed` は補正・回転・隠蔽・消しゴム・
  ローカル調整・注釈・切り取りが無ければ元データを渡し、加工があれば PNG に焼き込む。
  `Original` は編集を反映しない source page (実ファイル / ZIP 元 bytes、PDF は未加工 render)、
  `Container` は書庫 / PDF / 動画本体、`RealFileOnly` は実ファイルだけを渡す。
  AI アップスケール表示は焼き込まない。
- ZIP 内の無加工ページは元バイト列のまま一時展開し、PDF ページはツールごとの長辺
