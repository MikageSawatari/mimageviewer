...
| 選択済み項目を修飾なしで再クリック | エクスプローラー方式でのマウス操作に限り、Enter / ダブルクリックと同じ開く操作を実行する。クリック前から選択され、かつそのセル以外のチェック項目がない場合だけを対象とする。初回クリック、複数選択を 1 件へ畳むクリック、Ctrl / Shift 付きクリック、チェック方式、touch-derived pointer、ダイアログ中は開かない。同じ pointer release がダブルクリックにも認識された場合も open は 1 回だけ実行する |
