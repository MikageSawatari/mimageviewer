...
- 静止画フルスクリーン中にマウスを画面下部へ移動すると、フォルダ / ZIP / PDF のフルスクリーンナビ対象がすべて画像の場合だけページシークバーを表示する。LTR は右端、RTL は左端に `12/123` 形式の位置を表示し、ドラッグ中はリアルタイムにページを移動する。縦 / 横連結モードではシーク先ページを中央に置く。動画など画像以外が混在する場合はシークバーを出さず、画像 / 動画 / その他の件数サマリーを表示する
