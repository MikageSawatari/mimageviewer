...
- 静止画 / 本のページシークバーには、既定 OFF のサムネイル列を表示できる。通常画像、ZIP / CBZ、PDF、変換済みアーカイブを同じ source page 単位で扱い、現在ページを中心に表示幅ぶんと前後 2 セルだけを既存サムネイル worker の priority 要求へ載せる。見開きでは現在ユニットの 1〜2 ページを強調し、RTL では並びを反転する。セルのクリック / 横ドラッグと通常トラックは同じ位置解決と着地契約を通る。通常トラックの上ドラッグは動画と共有する `SeekRowGesture` で列を開き、列の下ドラッグは共有する `strip_drag_closes_downward` で閉じる
...
- 静止画サムネイル列の高さは大 104 / 中 72 / 小 48 / 最小 36pt を使う。列全体で共有するセル幅は、現在ページの `ThumbnailState::Loaded` texture の比率を使って `clamp(cell_height * aspect, cell_height * 0.35, preset_width)` とし、未ロード時はプリセット幅 152 / 102 / 64 / 45pt、横長でもプリセット幅を上限とする。下部バー固定中はサムネイル列と通常バーの解決済み合計高を media rect、左右パネル、タッチハンドル、パン帯、ページ番号の回避量へ渡す。非固定中は従来どおり overlay とする。サムネイル列表示中の通常バーは表示（既定）/ 非表示を選べ、非表示でも列の移動操作は残る
