...
| メニュー | 項目 |
|---------|------|
| ファイル | フォルダを開く… (Ctrl+O) / 読書履歴を開く / レーティング一覧 / 現在地フィルタ (Ctrl+F) / メタ情報をエクスポート… / メタ情報をインポート… / キャプチャ保存フォルダを開く / ゴミ箱を開く / 終了 |
...
- **← / → 履歴ボタン**: フォルダ移動履歴を戻る / 進む。履歴操作中は通常の
  フォルダ履歴に重複して積まない。読書履歴、星数別のレーティング一覧、サブ展開、
...
- **場所▼ メニュー**: ドライブ一覧 / 読書履歴 / ブックマーク / レーティング / 本棚フォルダ / デスクトップ /
...
- 通常一覧ではサブフォルダを含まない、サブ展開結果では含む設定を初期値とし、追加確認画面で
  切り替えられる。ZIP / PDF 内、検索結果、読書履歴等の実検索元を特定できないビューでは追加項目を
...
- 詳細一覧ヘッダと下部情報バーのヘッダの列右端をドラッグすると、現在その面が使う設定の列幅を
  ...
  通常一覧の `状態` と `★` は有限の固定語彙で即時確定し、ブックマークの位置列と読書履歴の
  既読位置列は実際の全行を分割計測する。既定フォント / UI 100% では、固定シードと左右余白に
...
- 読書履歴は、フルスクリーンで読んだ画像フォルダ / ZIP / PDF / 対応アーカイブを
  ...
  1 件だけ確認し、見つからない場合は移動せずトーストで通知する。missing は外付けドライブ未接続・
  NAS offline・権限エラーと恒久削除を安全に区別できないため、いずれも履歴行は残す。
...
| Select | グリッドでは場所リストを開き、ドライブ一覧 / 読書履歴 / レーティング一覧 / 本棚フォルダ / 既知フォルダ / ドライブを上下で選んで A で移動する。画像フルスクリーンでは見開きモードを巡回切り替え (単ページ / 見開き 左→右 / 左→右(表紙あり) / 右→左 / 右→左(表紙あり))。動画フルスクリーンではブックマーク / チャプター / ピンの一覧を開き、上下で選んで A で移動する |
