...
| 製本 | 追加先の本に追加 / クリップボードの画像を本に追加 / 本棚フォルダを開く / 追加先の本を開く / 読書履歴 / この本を並べ替え / 製本の管理… / 追加先の本を選ぶ |
...
- 製本メニューの「読書履歴」は、フルスクリーンで読んだ画像フォルダ / ZIP / PDF /
  対応アーカイブを最近読んだ順に並べる専用ビューを開く。動画は対象外。
  右クリックで 1 件ずつ「履歴から削除」できる。
...
- セルサイズ = ウィンドウの残余領域 ÷ 列数
- 選択中セルの下にはサムネイル情報ツールチップを表示する。既定はファイル名と画像解像度、動画ではファイル名と動画長さ。表示内容は環境設定 → サムネイルで、種類 / サイズ / 更新日時 / 作成日時 / 動画解像度 / 動画コーデック / 場所を含めて切り替えられる。読書履歴ビューでは設定に関わらず最終閲覧日時と既読位置を表示する。ツールチップ幅はセル幅固定ではなく最大約 2.5 セル分を使い、画面右端ではセル右端に合わせて画面内に収める。動画長さなど未取得の値は選択中の 1 件だけバックグラウンドで遅延取得する
- **仮想スクロール実装**（下記詳細参照）
...
- 詳細モードは固定行高の一覧で、既定では左端にプレビューアイコン列を置き、続けて
  ...
  読書履歴ビューでは同じ状態列を `既読` 列として使い、最終閲覧日時と `12 / 120`
  のような既読位置を表示する。
...
- 読書履歴は、フルスクリーンで読んだ画像フォルダ / ZIP / PDF / 対応アーカイブを
  ...
  スマートフィルタは無効。サムネイル情報ツールチップと詳細表示の `既読` 列に
  最終閲覧日時と既読位置を表示する。履歴から開く時の続きから / 先頭からは
