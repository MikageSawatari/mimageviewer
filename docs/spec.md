...
| スマートフォルダ | 新しいスマートフォルダ… / スマートフォルダを管理… / [登録済みスマートフォルダ一覧] |
...
- お気に入りとは別に、任意の複数実フォルダを検索元として直接登録する保存済みビュー。
  画像だけのフォルダ、ZIP / CBZ、PDF、対応アーカイブを、元の階層を保持しながら 1 つの
  フラット一覧へまとめる
- 専用索引や watcher は使わず、開く / 更新時にバックグラウンドで再帰走査する。表示中に
  外部で起きた変更は自動反映せず、更新操作で新しい snapshot へ一括交換する
- 保存条件は名前、種類 / 拡張子、更新日、サイズ、★、タグ。安価な条件は走査 worker、
  ★ / タグは DB の一括取得を使う準備 worker で適用し、UI スレッドでは再帰 I/O を行わない
...
| `smart_folders` | Vec\<SmartFolderDefinition\> | [] | 任意の複数実フォルダを横断する保存済み snapshot view。定義 UUID、名前、検索元 UUID/path/enabled、保存フィルタ、ソート、相対フォルダの並び単位、表示形式を保持。索引 / watcher は持たず開く・更新時に再走査する |
