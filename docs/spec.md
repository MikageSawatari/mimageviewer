...
```
┌─────────────────────────────────────────────────────────┐
│ メニューバー: ファイル | お気に入り | スマートフォルダ | 製本 | コレクション | 変換 | 動画 | タグ | 設定 │
├─────────────────────────────────────────────────────────┤
│ フォルダバー: [←][→] [A][B] [⬆][▲][▼] [フォルダパス...] [サブ展開][♡][履歴▼][📌] │
├─────────────────────────────────────────────────────────┤
│ ツールバー: [ツリー] [本棚] [コレクション] [列数/詳細] [縦横比] [ソート] [お気に入り] [スマート] │
├─────────────────────────────────────────────────────────┤
│ 任意: 左フォルダツリー │                                 │
│                                                         │
│   サムネイルグリッド (仮想スクロール)                   │
...
| タグ | ピン留めタグの管理… / タグビュー (Ctrl+T) / タグを付ける/外す… / 選択中の項目からタグをクリア / [ピン留めタグ一覧] |
| 製本 | 追加先の本に追加 / クリップボードの画像を本に追加 / 本棚フォルダを開く / 追加先の本を開く / この本を並べ替え / 製本の管理… / 追加先の本を選ぶ |
| コレクション | 追加先に追加 / 追加先を開く / 管理… / 手動順・通常ソート・シャッフル / 並べ替え… / テキストを取り込む… / テキストで書き出す… / すべてのコレクションをテキストで書き出す… |
...
- トップレベルメニューのいずれかをクリックして開いている間は、`ファイル` /
  `お気に入り` / `スマートフォルダ` / `製本` / `コレクション` / `変換` / `動画` / `タグ` / `設定` / `ヘルプ` の別項目へマウスを
  横移動して hover するだけで、開いているメニューが切り替わる。未オープン状態の
  hover だけではメニューを開かない。
- 環境設定「表示 → 通常メニュー」から、トップレベルメニューと固定メニュー項目の
  表示 / 非表示・表示順を変更できる。登録済みお気に入り一覧、タグ一覧、更新確認など
  状況で変わる項目は固定項目ではないため個別には並べ替えない。
  設定画面へ戻る入口を失わないよう、「設定 → 環境設定…」は非表示にできない。
- 環境設定「表示 → 右クリックメニュー」では、一覧 / フルスクリーン共通の静的項目を
  同一階層内で表示・並べ替え、直前の区切り線を表示 / 非表示にできる。固定の表示場面を
  選ぶとproduction menu builderの実結果から既定線、対象外、先頭を表示する。場面選択は
  確認専用で、変更は全場面共通、設定ページの閲覧だけでは保存値を変えない。
- 外部ツールの起動導線は、登録した全ツールを登録順に平坦に出す右クリックメニューと、
  Grid 専用のピッカー / 固定キースロットだけ。メニューバーとツールバーには置かない。
  ページ対象はチェック済み項目を優先し、無ければ現在項目を使う共通の Grid 解決を通す。
  ピッカーも表示前に同じ capability を適用し、仮想ページでは `OriginalFile` のツールを
  理由付き無効行にする (隠さずグレーで理由を出す)。
- 複数対象の既定は `1 件ずつ起動` (`Each`)。`1 件だけ` (`Single`) は 2 件以上なら起動せず、
  `Each` / `まとめて渡す` (`Batch`) はツールごとの確認件数 (既定 5) と上限 (既定 10) を
  対象件数に適用する。上限を超えた場合は起動しない。
- 環境設定「起動と連携 → 外部ツール」で `OS の関連付け` と `まとめて渡す` を組み合わせた
  場合、まとめて渡す API が無いため実際には対象ごとに 1 件ずつ起動し、設定欄にもその旨を表示する。
- `実行ファイル + まとめて渡す` は起動前に Windows のコマンドライン長上限を検査し、
  超える場合は OS エラーへ任せず、ファイル数が多すぎる旨を通知して起動しない。
- 外部ツールの `PayloadPolicy` は 3 値で、**出力の形は編集の有無で変えない**。
  `TempEdited` (一時ファイル / 編集を反映) は補正・回転・隠蔽・消しゴム・ローカル調整・注釈・
  切り取りを反映して**常に PNG へ焼く**。`TempOriginal` (一時ファイル / 編集前) は
  **常に元バイト列**をそのまま書き出す (PDF ページだけは元バイト列が無いので未加工 render)。
  `OriginalFile` (元のファイル) は実ファイルだけを渡し、仮想ページでは起動しない。
  焼き込む段は `Settings::bake_stage_external_tool` で選び、既定の「編集」では
  カラー化 / Creative LUT / ポストフィルタ / スマートシャープ / AI を含めない。
- 動画・音声は一時ファイルへコピーせず実ファイルを渡す (数 GB になるため)。
  `VideoPolicy::CurrentFrame` のときだけ、再生中の動画の**提示済みフレーム**を原寸で
  decode して画像として渡す (`screenshot_target_secs`。再生していなければ動画ファイル)。
- 見開き表示中は `SpreadPolicy` に従う。`Merged` は左右の source と編集 context を
  materializer worker で選択段まで合成した 1 枚、`BothPages` は**読み順**
...
- v2.7.0では「メタ情報をエクスポート / インポート」を安定化のため一時非表示にしたが、
  ...
  directoryへ、評価、タグ（表示順・最近使用順を保つ適用時刻と空タグを含む決定状態を含む）、
...
- インポートは先に sidecar 全体を検証し、存在件数、欠落件数、ファイル種別・サイズ相違件数を
  ...
  削除した後もstate行を作らず、未決定というbundle上の意味を保つ。物理項目とZIP / PDF仮想項目で
  同じ規則を使う。
...
- コレクションセクションは全表示形式で、追加先を選ぶコンボと `追加` / `開く` を表示する。コンボの
  ...
  表示形式は本棚と同じ`展開` / `折りたたみ`で、旧`プルダウン`保存値は`展開`として読み込む。
...
- ZIP / PDF 内ページ、ZIP 内フォルダ、スタック、検索集約セル、利用不能なコレクション項目は単独の
  ...
  勝手に置き換えない。
- 管理画面は本棚と同じ入れ物管理とし、作成、名前変更、追加先指定、固定、削除、開くを行う。内容は
  メイン一覧を正本とし、rootの通常`Delete` / context commandで参照だけを外す。上部`コレクション`
  メニューから手動順 / 通常ソート / シャッフル、専用サムネイル画面での手動順並べ替え、テキスト import / exportを
  操作する。参照先を変える場合は登録解除して正しい参照を再追加し、再リンクUIは提供しない。ファイル /
  フォルダの追加ピッカーは置かず、メイン一覧とツールバーの `追加` または名前の右クリックを使う。
- 追加は collection actor の最新 revision を非同期で取得してから分類・保存する。管理画面の選択対象や
  進行中の import / exportを toolbar 操作で切り替えず、保存結果は revision watch で PC の各一覧と
  認証済み mIV Remote の読み取り専用表示へ反映する。Remote からの作成・編集・追加は提供しない。
- 手動順は登録順を基に保存し、通常ソートへ切り替えても保持する。シャッフルは seed による表示順で、
  「選び直す」と順を更新する。詳細表示の列ヘッダソートは表示に限り、保存された手動順を変更しない。
  追加は 1 コレクション 10,000 件までとし、容量を超える新規参照だけを拒否する。既存の上限超過一覧も
  読み取り・削除・並べ替えは継続できる。import の UTF-8 テキストは実読込 32 MiB、非空 50,000 行を
  別の資源上限とし、超過した preview 全体を拒否する。確認画面の残り枠は説明値で、候補を先に切り詰めず、
  重複・検証・並行変更を含む最終的な追加と容量拒否は保存時にactorが決める。追加0件ですべて容量拒否なら
  成功とは表示しない。
- 既存 v2 の `collection.db` は起動時に整合性と全一覧を read-only 検証する。WAL を含む世代バックアップ
  （最大 10 世代）は、起動ごとではなく、そのセッションで最初に内容が変わる操作の transaction を開く直前に
  1 回だけ作る。同値操作、拒否、閲覧だけの再起動では世代を消費しない。未編集の空 DB は再起動後も空世代を
  作らず、最初の成功変更後に backup を有効化して次の実変更前に最初の有用な状態を保存する。
  通常のバックアップ失敗は成否と所要時間をログに記録し、同じセッションで繰り返さずに変更を続ける。
  v1 と既存 unversioned DB は旧内容を read-only 検証してから schema 書込み前バックアップを必須とし、
  失敗時は移行を中止する。未対応の将来版・破損 DB では既存バックアップをローテートしない。
  `collection.db` とそのバックアップは設定 DB の全リセット対象に含めない。
- 上部「コレクション」メニューの一括テキスト書き出しは、選択した親フォルダ内に新しい専用フォルダを作る。
  actor の単一読取版から全コレクションを順番に 1 ファイルずつ書き、欠損参照も含む有効表示順を使う。
  既存ファイルは上書きせず、`collections-index.txt` を最後に完成印として作る。失敗・取消時は成功を表示せず、
  残った未完成フォルダの場所を知らせる。単件text、全件内の各text、索引はUTF-8 BOM付きで、日本語pathを
  往復できる。書き出しはclipboardを変更しない。書き出しは変更操作とは別のread admissionを使うため、
  手動順編集画面を読み直している間も、別のoperationが無ければ開始できる。
...
- ツリー上部にはドライブ切替、ツリー専用の並び順、再読み込みを表示する。並び順は名前・番号・日付の昇順/降順6種で、既定は名前昇順。一覧のソートやお気に入り別の表示状態とは独立し、Ctrl+上下のDFS、Ctrl+PageUp/PageDownの兄弟移動、スマートフォルダ内のDFSも同じ比較規則を使う
- ツリーの並び順または隠しファイル表示を変更した場合は、処理中の旧列挙を破棄し、現在の子行・手動/自動展開・カーソルを表示したまま、選択ルートと展開済みの親をバックグラウンドで再列挙する。新しい列挙に失敗したときは旧行を残してエラーを示す。再読み込みは別操作であり、ツリーキャッシュと手動展開状態をクリアし、現在アクティブなフォルダが見える最小範囲で再列挙する
...
- 環境設定「表示 → サムネイル」の「お気に入りごとに表示状態を記憶する」(既定 OFF) を
  ON にすると、最も深く一致するお気に入りごとに、サムネイル / 詳細、サムネイルサイズ (= 列数)、
  比率 / 自動比率、カテゴリ表示順、一覧ソート、単ページ / 見開き、連結方式を自動記憶・復元する。
  サムネイル画質 (デコード解像度 `thumb_px`) は記憶しない。UI で頻繁に切り替える設定ではなく、
  場所ごとに変えるとサムネイルの再生成を招くため、全場所で共通の値とする。
  ...
  その記録は変えない。お気に入り編集から 1 件、環境設定から全件をリセットできる。OFF / リセットで
  実効値が変わった場合は、サムネイル / 詳細と比率を現在表示へ反映し、カテゴリ表示順 / 一覧ソートは
  通常フォルダなら worker 事前走査後に、仮想一覧なら各一覧の既存再構築経路で items に反映する
- **仮想スクロール実装**（下記詳細参照）
...
- 表示はページ構成 (1 ページ / 見開き 左→右・右→左、表紙単独あり) と連結方式
  ...
  見開き構成の縦連結では、表紙・横長ページ・端数の単独ページも見開きページと同じ倍率で表示する。
  本全体の先頭・末尾で相方のない単独ページだけは、それぞれ独立した全体設定と本別 override
  （全体設定に従う / 片側へ配置 / 中央表示）に従う。両設定の既定は OFF。配置 ON の端は
  §1.218 の二枠仮想 canvas を使い、本来の左右へ置く。横長ページ、末尾表紙補助による二枚表示、
  本全体の端が確定しない検索・切詰め列には適用しない。1 unit が両端なら先頭が優先する。
  page count、seek、読書位置、実ページの identity とページ送りの unit 数は変わらない。
...
- 360 度モードへ入れる条件は 1 つの述語が持ち、キー / 上部バーのボタン / セッション意図からの
  復帰が同じ答えを使う。静止画では 360 素材であることに加えて、見開きの 2 ページ表示中でなく
  ページ単位表示であることを要求する。動画は素材判定だけで決まる。
- 360 度モードの ON / OFF と投影方式の選択は、viewer ごとのセッションの意図として保持する。
  フルスクリーンを閉じても意図は残り、次に開いた対象が 360 素材なら 360 で表示する。素材でなければ
  通常表示にし、意図は保持する。利用者が明示的に解除したときだけ意図を落とす。投影方式は ON / OFF
  とは独立に保持し、環境設定の既定値は書き換えない。detached の静止スナップショットへ退避する
  park は新しいウィンドウを通常表示で始めるため意図も落とす。live context の一時退避は意図を保つ。
...
- 右情報パネルのロック: タイトルバーの鍵ボタンで固定する。ロック中は画像へ重ねず、右へ
  ...
  上下の固定バーと同じ 1 か所で決める)。前後のファイル / ページへ移動しても開いたまま
  内容だけ更新し、フルスクリーンを出ると解除する。設定として永続化しない。状態は
  viewer context ごと (`ViewerContextBundle`) に持ち、別ウィンドウのパネルへ影響しない。
  パネルを表示しないモード (補正 / トリム / 分析 / 360 / 比較 / SNS 分割 / 切り出し /
  ズーム) ではロックも効かない。非アクティブな別ウィンドウでは画像の矩形は
  そのままにパネルだけ描かない。**動画も同じ状態でロックできる** — native presenter の
  右メタ情報パネルに同じ鍵ボタンを置き、`compute_video_visual_target_rect` が固定バーと
  同じ順で右を除外する。ヘルプ / 360 / 外部ドラッグ / VST3 / メタ情報無し / 速度ポップ
  アップ中は占めない。**音楽ビューも同じ状態でロックできる** — 確保幅は音楽の
  `MUSIC_RIGHT_PANEL_WIDTH`、パネルは縮める前の右端へ置く。モーダル表示中は占めない
- 固定右パネルは本移動の読み込み待ちでも領域を予約し、現在のウィンドウサイズと設定から画像領域を解決する。画像対象が未確定の間は移動中の案内を表示し、旧画像に対する操作を表示しない。
- 類似タブの候補は明示の移動・比較に設定・長押し表示を使う（単体候補の行クリック移動も維持）。本のページ帯セルは押している間だけ対応する候補1ページを表示し、解除で戻る。帯セルのクリックは移動せず、本の移動ボタンを使う。
- 「この本と重なる本」は本のタイトルに加えてフォルダ／コンテナの完全な場所を常時表示し、別の親フォルダにある同名の本を区別できるようにする。起点と候補はそれぞれ現行ハッシュのページが 3,000 ページ以下の本を対象とし、対象候補の合計が 10,000 ページを超える場合は候補側の比較へ進まず打ち切り案内を表示する。
- 類似の閲覧履歴: viewer ごとに最初の本と訪問した本を類似タブ上部へ表示し、移動ボタンで最後に見たページへ戻る。同本を重複登録せず現在行を示す。移動成功で登録し、失敗/取消では未到達先を登録しない。同本ページ送り/タブ切替で保持、通常の別本への移動成功または真の閲覧終了で破棄し、再起動後は保存しない。
...
- 動画シークバーとシークストリップ (サムネイル / 音声波形) は、ポインタ位置の同じ 1 枚
  サムネイルプレビューを共有する。ストリップ表示中はその 14pt 上へ置き、帯へ重ねない。
  どちらの入口も「シーク時のズレ許容 (秒)」を使う。desktop は
...
- ストリップ上のホイールは上回転で範囲を 1 段狭く、下回転で 1 段広くし、その設定を即時保存する。
  サムネイルの最小間隔は 0.1 / 0.2 / 0.5 / 1 / 2 / 5 / 10 / 15 / 30 / 60 秒と 2 / 5 / 10 / 15 / 30 分 (既定 15 秒)。
...
- 通常フォルダ一覧では、一覧へ出さなかった項目数が 1 件以上なら、フォルダバー右端に
  ...
  右クリックの `show_address_bar_omitted_entries` で消せる。クリック時は
  `同名など、隠し項目、RAR / 7z / LZH（設定で無視）、対象外`
  (+ 0 件でなければ `、システム`) の内訳を表示する。**区切りは `/` ではなく `、`**:
  書庫の分類名が `RAR / 7z / LZH` を含むので、区切りも `/` にするとどこが区切りか読めない。設定で無視した書庫とシステムは
  0 件なら内訳から省く。同名・設定で無視した書庫・隠し項目は、各件数が 1 件以上のときだけ
  対応する「同名ファイル処理」「RAR / 7z / LZH の処理」「隠しファイル・フォルダを表示する」
  設定への導線を表示する。
  主数字 N は **同名で畳んだ数 + 隠し項目 + 設定で無視した変換対象書庫 +
  扱わない拡張子** の合計。`Thumbs.db` /
  ...
  設定で無視した変換対象書庫は従来「対象外」に含めていた項目を分けるだけなので、主数字は
  変えない。内訳は通常のフォルダ走査と同名除外の差分から導出し、件数取得のための追加走査はしない。
...
- 並び順は左フォルダツリー上部の設定を使う。名前・番号・日付の昇順/降順から選び、一覧のソートやお気に入り別の表示状態を変えてもツリー巡回順は変わらない。
...
| 右クリック | コンテキストメニュー。実ファイル / 実フォルダでは先頭に「切り取り」「コピー」を表示し、チェック選択時は全選択数を併記する。実選択だけなら `shell_paths` を Ctrl+X / Ctrl+C と同じ `drag_source_path` の集合から作り、同じ Shell clipboard helper へ渡す。仮想単一項目では両項目を隠し、仮想だけまたは実 / 仮想混在のチェック選択では理由付き disabled にする。その他の mIV 項目 (新しいフォルダ、名前の変更、パスコピー、画像コピー、編集内容のコピー/貼り付け、検索結果のフォルダ移動、回転、代表サムネ固定など) も共通定義から構築する。実項目では Windows Shell 項目を既定で末尾の「Windows のメニュー」サブメニューへまとめ、開くまで列挙を遅延する。設定で従来どおり同じ階層へ併記できるが、先頭の「切り取り」「コピー」は設定によらず表示する。アイテムがない場所では現在表示中の実フォルダだけ Shell 背景メニューを使い、mIV 先頭項目の「新しいフォルダ…」「貼り付け」からフォルダ作成と Shell paste を実行できる |
...
| V | 表示モードをトグル。360 候補画像 / 動画では 360 度パノラマ、通常動画ではホイール拡大縮小・左ドラッグ移動ができる拡大表示 |
...
| Space + 左ドラッグ（編集モード中） | 一時パン。切り取りモードで新規枠作成の修飾キーと同時に押した場合も Pan を優先する。すでに描画/図形編集/切り取り枠/注釈ドラッグ中の操作は横取りせず、Space を途中で離しても同じ一次ボタン hold を別操作へ再分類しない |
| 左ドラッグ（ズーム中） | パン（表示範囲を移動） |
| Ctrl + 左ドラッグ | ページ単位では任意角度回転（一時的、保存なし）。縦 / 横連結ではドラッグの軸固定を解除し、連結方向のスクロールと直交方向のパンを同時に行う。切り取りモードでは `CropForceCreateHold`（既定 Ctrl）が優先され、既存枠やハンドル上でも新規枠を作る |
...
| 右クリック長押し（設定 OFF 時） | コンテキストメニュー。実ファイルでは先頭に「切り取り」「コピー」を表示し、グリッドと同じ Shell clipboard helper へその実 path を渡す。ZIP/PDF 内ページなど仮想項目では両項目を隠し、mIV 項目（パスコピー・フォルダを開く・アプリケーションで開く等）だけを表示する。実ファイルの Windows Shell 項目は既定で「Windows のメニュー」サブメニューへまとめる。動画フルスクリーンでは動画に作用しない回転・親コンテナ代表固定を出さず、「現在のフレームを動画サムネに設定」を表示する。動画上の左クリックでメニューだけを閉じた場合、そのクリックを再生 / 一時停止へ再利用しない |
...
起動せず再利用する。比較workerの完了は100ms周期ではなく完了通知で回収する。Wipeの境界線と
中央handleは通常常時表示し、focused viewportでCtrlが押されている間だけ、drag中かどうかに
かかわらず非表示にする。
...
- **操作カスタマイズの共有・世代取り込み**: 「設定の復元」の「操作カスタマイズ」タブで
  `keymap` / `ring_shortcuts` / `menu_layout` / `context_menu_layout` を `.mivkeys.json` として書き出し・
  置換取り込みできる。差分は実効チョード単位で標準 / 現在 / 前世代と比較する。世代からの
  取り込みは設定全体を復元せずライブ適用し、直前の操作カスタマイズ一式を同形式で自動退避する。
  同タブから操作カスタマイズ一式だけを確認後に初期値へ戻すこともでき、この場合も適用前の内容を
...
チェック選択では Shell 項目を含まない mIV 項目だけの native メニューを使い、先頭の
「切り取り」「コピー」は理由付き disabled で表示する。
実項目と仮想項目が混在するチェック選択でパスコピーまたは削除を選ぶと、実項目だけへ対象を
縮めずにトーストで全体を拒否する。メニュー上の件数は仮想項目を含む全選択数とする。
...
右クリック先頭の「コピー」「切り取り」と `GridCopyFiles` / `GridCutFiles` は、対象 path だけを差し替えて同じ Shell verb helper を使う。Ctrl+V も Shell paste helper を使い、コピー / カット / ペースト時のフォルダ対応、衝突確認、進捗 UI は Windows 側に委ねる。キー入口で ZIP/PDF 内ページなどファイルとして扱えない項目が選択に含まれる場合は、実ファイルだけを部分的に処理せずトーストで中止する。
...
| `fullscreen_image_margin_color` | `[u8; 3]` | `[0, 0, 0]` | フルスクリーン表示の共通 RGB 余白色。通常 / F12 の静止画・本では単ページ / 見開き / 連結 / 比較、回転 / 表示トリム、360 度 / 分析のメインキャンバスと、サムネイルストリップのセル外空欄へ適用する。native 動画では映像表示矩形のレターボックスと、拡大・パン / scaling resolve で source 外になる画素へ適用する。透過画像内の黒 / 白 / 市松、実動画画素と素材に含まれる黒帯、字幕、音声 / 動画の音声モード、HUD / 左右パネル、ナビゲータ / ルーペなど部品内部の背景、画像画素と copy / capture / export は変更しない |
...
| `folder_tree_sort_order` | FolderTreeSortOrder | NameAsc | 左ツリーと Ctrl+上下 / Ctrl+PageUp/PageDown の並び順。NameAsc / NameDesc / NumericAsc / NumericDesc / DateAsc / DateDesc。一覧用 `sort_order` とお気に入り別表示状態から独立 |
...
| Ctrl+G | アイテム検索 | **お気に入り全体 (`auto_index_metadata=true`) の画像 / PDF / 動画 / 音声** | アイテム索引 (Tantivy bigram) で候補絞り込み → STORED 原文を `matches()` で phrase/NOT/AND 正確判定 (streaming) | ZIP はコンテナなので対象外。音声はファイル名のみ (ID3 等の埋め込みメタデータは対象外)、動画は `video_meta_text`、PDF は `pdf_meta_text`、画像と同名 JSON/TXT は `sidecar_text` (値のみの自由語検索、mIV タグとは別系統) に分けて検索対象フィルタから選べる。mIV タグは Ctrl+G の検索本文へ混ぜず、タグ候補チップからタグビューへ誘導する。結果は一覧 / 集約の 2 ビューを切替でき、ヒットが多いと自動で集約に寄る。検索中はアドレス欄と空グリッド中央に確認済み候補数とヒット数を表示し、0 件確定と区別する。検索バーは狭い幅では折り返し、Ctrl+G のドロップダウン操作中は背後のグリッドをホイールスクロールしない |
...
- フルスクリーン静止画 (`Image` / `ZipImage` / `PdfPage`) で `Ctrl+E` を押すと、元 source から `Settings::bake_stage_export` の段までを worker で合成してファイルに書き出す。既定は `DisplayAdjust`。見開き表示中は左右ページを個別に同じ段まで合成し、1 枚の見開き画像として書き出す。動画や区切りアイテムでは無効。
- `Ctrl+E` のほか、**画像補正パネルのヘッダー右側に並ぶ処理順アイコン列** (`消しゴム / 補正レイヤー / 隠蔽加工 / 切り取り / テキスト注釈 / エクスポート / SNS 分割`) のエクスポートアイコン (`draw_export_icon`) からも開ける。補正パネルはホバー表示のオーバーレイなので、クリック時は `adjustment_mode` を `false` にして閉じてから `open_export_dialog_for_current` に合流する (= ビューモードへ戻して合成結果を書き出す)。`open_export_dialog_for_current` は補正 / 消しゴム / 隠蔽 / SNS 分割 / 分析モード中は弾く (`is_overlay_edit_mode_active() || adjustment_mode || analysis_mode`) ため、`Ctrl+E` 直接押下はこれらのモード中は無効。SNS 分割中だけは「分割して書き出す」ボタンを案内し、それ以外の編集モードは従来の終了案内を出す。画像補正パネルのエクスポートアイコン経路はパネルを閉じることでこのガードを通過する。
...
- 永続化設定は `Settings::export_embed_metadata` (メタデータ保持)、`Settings::export_last_directory` (前回保存先)、`Settings::export_batch_selection` (現在 / プリセット 1〜4 のチェック状態)、`Settings::export_fallback_format` (非対応元形式の JPEG / PNG 選択)、`Settings::export_default_scale` (出力サイズ)、`Settings::bake_stage_export` (焼き込む段)。一覧からの一括エクスポートは別系統の設定を持つ (下記)。
- 保存は `ctrl-e-export` worker で順次実行する。元画像の decode、選択段までの合成、隠蔽プリセットの差し替え、見開き結合、画像エンコード、メタデータ転記、ファイル書き込みはいずれも worker 側で行い、UI は進捗モーダルを `try_recv` で更新する。キャンセルは合成中と次のエントリ開始前に反映する。
- 現在表示中の実フォルダへ保存した場合は、フルスクリーンを抜けて一覧へ戻ったタイミングでフォルダを再読み込みし、新しい出力ファイルのサムネイルを反映する。ZIP / PDF / 検索結果などの仮想フォルダでは自動再読み込みしない。
- 隠蔽プリセット出力 (`_1`〜`_4`) は `_0` と同じ source・編集 snapshot・焼き込み段を使う。`_0` は snapshot の現在 preset を使い、`_1`〜`_4` だけ隠蔽 preset を差し替えて `books::compose_export_entry` で再合成する。焼き込み済み画素へマスクを重ねないため二重適用にならない。詳細は [display-pipeline.md](display-pipeline.md) §3.0。
...
- フルスクリーン静止画で、画像補正パネル右端の `draw_sns_split_icon` から入る一度きりのモード。`KeyAction::FsSnsSplitMode` にも割り当てられるが標準キーは持たない。書き出しはパネル下部の「分割して書き出す」から進む。`KeyContext::SnsSplit` の `SnsSplitExecute` も標準キーなしで利用者が割り当てられる。`Ctrl+E` は通常の `FsExport` のままで、SNS 分割中はパネルボタンを案内する。`Esc` はモードを破棄する固定操作。
- 1 枚の表示結果を横一列の 2〜4 枚へ切り分ける。投稿先は X / Instagram の 2 択で、切り替えると継ぎ目を X = 枠幅の 1.7%、Instagram = なしへ戻す。投稿先は枠比率を決めない。継ぎ目は「なし / 1.7% / 任意」と 0.0〜10.0% (0.1% 刻み) の数値入力、枠比率は独立した「自由 (既定) / 3:4 / 4:5 / 1:1」から選ぶ。1.7% は 2026-09-01 の実測 (PC ブラウザ 1.588% / iOS アプリ 1.869% / モバイル Web 2.652%) から、PC ブラウザと iOS アプリの誤差がともに 0.4 CSS px 以内に収まる値として選んだ。根拠は [sns-split-export-plan.md](sns-split-export-plan.md) §2.1。
- 状態は `Option<SnsSplitLayout>` のみで、ページにも DB にも保存しない。`Settings` の `sns_split_target` (既定 `"x"`) / `sns_split_count` (既定 2、2..=4) / `sns_split_seam_permille` (既定 17、0..=100) / `sns_split_frame_ratio` (既定 `"free"`、`"free"` / `"3:4"` / `"4:5"` / `"1:1"`) が直前の選択だけを次回へ引き継ぐ。不明な値は既定へ戻し、既存の切り取り (`CropSettings` / `export_crop.db`) には一切書かない。
- グループ矩形は既定では比率自由で、8 ハンドル + 本体ドラッグで操作する。自由では横と縦を独立に変え、固定プリセット時だけ 1 枠の比率と現在の枚数・継ぎ目からグループ比率を求める。固定時の辺ハンドルは反対辺と直交方向の中心を、角ハンドルは反対角を固定して拡大縮小し、画像端では固定点を動かさず収まる最大寸法へ縮める。整数ピクセルへ揃える際は、先頭の実枠が丸めだけで 3:4 未満にならない高さを選ぶ。この規則は既存の切り取りと共有する `CropRect::dragged` の契約でもある。
- 枠と継ぎ目は `frames()` が整数ピクセルで導出する。全枠の高さは共通だが、割り切れない横幅は累積境界で配るため枠幅同士・継ぎ目幅同士は最大 1px 違い得る。枠と継ぎ目の総幅はグループ幅に一致し、丸めによる欠落・重複は作らない。描画・プレビュー・書き出しはすべて `frames()` を正とする。
- パネルには投稿先、枚数、継ぎ目、枠の比率、全幅の「画像全体に合わせる」、投稿後プレビュー、「枠の寸法 (横/縦)」の順で表示する。継ぎ目の数値入力は「任意」のときだけ表示する。画像全体に合わせる操作は、自由なら画像全域、固定プリセットなら比率を保って画像内で最大の中央矩形にする。寸法表示は出力サイズ適用前の元画像上の先頭枠から `1 枚 W x H (横/縦)` の形式で算出する。投稿先・枚数・継ぎ目の変更は自由ならグループを保つ。固定比率間では、同じ整数ピクセルの中心セルで画像内に置ける最大矩形に対する占有率を引き継いで同じ比率へ再適合し、異なる整数格子への初回丸め後に往復を繰り返しても寸法を累積変化させない。
- 書き出しは各 `ExportEntry::crop_override` に元画像座標の枠と作成時寸法を載せ、worker の実合成寸法へ換算して 1 source → N ファイル経路へ流す。出力名は `_1`〜`_4` で、これが投稿順になる。隠蔽プリセットの一括出力は併用できず、理由を表示して無効にする (永続化された `export_batch_selection` は書き換えない)。既定の出力サイズは長辺 2048px 以下で、元が小さければ拡大しない。
- 枠は元画像座標で並ぶのに対し書き出しは crop の後に回転を適用するため、**回転しているページでは使えない**。90 / 180 / 270 と自由回転のいずれでもボタンを無効にし、キー・モード中の回転変更・書き出し直前の 4 経路で同じ理由を表示する。
- 画像が小さすぎて枠が並ばない場合は `fits()` が false を返し、パネルに警告を出して「分割して書き出す」を無効化する。さらに Instagram では `frames()[0]` の実際の整数 crop の横/縦が 0.75 未満または 1.91 超なら、同じ拒否理由の仕組みで警告・ボタン無効化・実行直前の再確認を行う (0.75 / 1.91 ちょうどは許可)。X には比率制限を適用しない。画像が小さすぎる理由を優先する。
...
- [x] 右クリックコンテキストメニュー（mIV 先頭項目: 実ファイル / 実フォルダの切り取り・コピー。その他: パスコピー・画像コピー・フォルダを開く・回転・削除・新規フォルダ作成 / Windows 標準メニューのコピー・カット・ペースト等）
...
- [x] フルスクリーン右クリック（画像 / 動画別に短押しを「閉じる（既定）/ 何もしない / コンテキストメニュー」から選択。右ドラッグ OFF 時の長押しメニューは維持。実ファイル / 仮想項目とも native HMENU、仮想項目は mIV 項目のみ）
