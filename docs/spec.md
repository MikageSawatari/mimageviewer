...
- 360 度パノラマ表示では消しゴム・隠蔽加工・補正レイヤーを反映しない。全体の色調補正・
  ポストフィルタ・スマートシャープ・AI・自動補正は通常どおり反映する。画素編集データがあるページへ
  入ると注意を表示し、360 中の消しゴム・隠蔽加工・補正レイヤー・テキスト注釈の各モード入場は
  共通の availability gate で抑止して理由を表示する。
