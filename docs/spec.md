...
- 通常フォルダ一覧では、同名ファイル処理で代表へ畳んだ数と一覧から除いた隠し項目数の合計が
  1 件以上なら、絞り込みバーに `非表示 N 件` チップを表示する。絞り込みバーが非表示なら
  アドレスバーへ表示する。クリック時は `同名など / 隠し項目 / 対象外` の内訳と
  「同名ファイル設定を開く」導線を表示する。扱わない拡張子は内訳の対象外へ数えるが、
  チップの主数字には含めない。内訳は通常のフォルダ走査と同名除外の差分から導出し、
  件数取得のための追加走査はしない。サブ展開 / スマートフォルダ / 検索結果 / スタック表示には表示しない
