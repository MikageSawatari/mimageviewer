...
- 既存 v2 の `collection.db` は起動時に整合性と全一覧の読み取りを確認し、Ready 前に WAL 込みの
  世代バックアップ（最大 10 世代）を 1 回作る。通常のバックアップ失敗はログに記録して機能を維持する。
  v1 も旧 schema と参照整合を確認してから移行前バックアップを作り、失敗時は移行を中止する。
  新規 DB・未対応の将来版・破損 DB では既存バックアップを
  ローテートしない。`collection.db` とそのバックアップは設定 DB の全リセット対象に含めない。
