...
- ストリップ右上の鍵ボタンまたは環境設定でストリップを固定できる。固定中にストリップが表示されて
  いる場合だけ、下部 HUD のすぐ上にストリップ高さを追加確保し、映像をその内側へフィットする。
  ストリップを「なし」にすると領域は解放するが固定設定は保持し、再表示時に固定へ戻す。右上の
  範囲表示は鍵の左に置く。鍵のクリックは seek / drag を開始せず、鍵上のホイールは範囲変更に使う。
...
| `video_seek_strip_locked` | bool | false | 動画のシークストリップを固定表示する。ON は下部シークバー固定も含意し、ストリップ表示中だけその高さを映像フィット範囲から除外する。ストリップを「なし」にしている間は設定値を保持したまま領域を解放する |
