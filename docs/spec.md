...
- 静止画 / ZIP 内画像 / PDF ページと、その編集・補正・AI・注釈後の最終表示は、物理倍率が
  1.0 未満のとき level 0 から表示サイズへ Lanczos3 で直接縮小する。画像補正パネルの
  「縮小時のなめらかさ」は全画像・全ウィンドウ共通で、0〜100%（10% 刻み、既定 0%）を
  Lanczos3 の blur 1.00〜1.30 に対応させる。設定変更時は Lanczos 出力 cache を無効化するが、
  補正・AI・サムネイル cache と source texture は作り直さない。比較表示と 360 度パノラマは
  GPU mip chain を固定経路で使い続ける。サムネイル、animated image、動画、明示的なニアレスト
  表示は対象外。旧版の手動 1/2・1/4 縮小 post-filter はフィルタなしへ移行し、v2.10.0 の
