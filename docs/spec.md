...
- **中央タイムライン**: DJ スタイルの波形を「行」に分けて表示 (行長は 10 / 15 / 30 / 60 / 120 秒
  ...
  音推定 / キー + ボーカル推定) を併記する。
- **下部スペクトラム**: 108-band (20Hz〜18kHz 対数軸) の FFT を再生位置 ±1 秒窓で ~16ms ごとに
  更新表示。88 鍵ピアノ overlay がスペクトル内容に応じて発光する。ホバーで周波数 / 最寄り音名 /
  セント偏差を表示。
...
108-band スペクトラム (`SpectrumAnalyzer`) の 3 ワーカーを持つ。キャンセル規約と GPU テクスチャ
