...
- **A / B クイックフォルダボタン**: コピー元・コピー先など 2 つの作業場所を
  ...
  - 空スロットを左クリックすると現在の表示場所を登録する。登録済みスロットを
    左クリックするとその場所へ移動し、そのスロットをアクティブにする。
  - 登録済みスロットの右クリックメニューから、現在の表示場所で上書き登録、
    登録解除、エクスプローラで開く、パスコピーを実行できる。
  - A/B 間の切り替え自体はフォルダ履歴に積まない。← / → 履歴ボタンは
    アクティブなスロットごとの戻る / 進む履歴を操作し、A で進んだ履歴と B で
    進んだ履歴は混ざらない。A/B 外の通常表示では従来の通常履歴を使う。
  - 登録対象は実フォルダ、ZIP / PDF / 変換済みアーカイブのコンテナパスまでとし、
    ZIP 内エントリや PDF ページ単位の仮想パスは保存しない。
  - 検索中、タグ / ★固定など一時ビューでは誤って検索前フォルダを登録しないよう
...
| `quick_folder_slots` | `[Option<PathBuf>; 2]` | `[None, None]` | フォルダバーの A/B クイックフォルダに登録したターゲット。実フォルダまたは ZIP / PDF / 変換済みアーカイブのコンテナパスだけを永続化し、A/B 別の戻る / 進むスタックはセッション中の `App` 状態として保持する |
