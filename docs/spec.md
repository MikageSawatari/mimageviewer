...
ホバーバーの ⊞ トグルボタン（どちらも × ボタンの左）でも切り替えられ、設定は
動画・静止画で共有する。
...
- 動画タイルモード (S キー) の上部バーは通常再生時と同じ高さ・配色で、現在の動画の
  ...
  Enter でカーソル位置から再生する。Shift はタイル中の左右移動では無視し、
  Ctrl が含まれる場合だけ 1 行分移動にする。S / Esc / × で閉じた場合は
  再生位置を変更しない。
- 動画の下部 HUD では、前/次フレームボタンと現在フレームのクリップボードコピーを
  シークバー左側に配置する。前/次フレームボタンまたは Ctrl+Shift+←/→ で
