...
- **設定経路**:
  - アドレスバーの **📌 ボタン**: 選択中のアイテムを現在のコンテナの代表サムネに
...
- **対象外**: 検索アグリゲートビュー (Ctrl+G)、7z/LZH の変換キャッシュ ZIP を内側から
  ...
  設定可能)
- **設定**: `環境設定 → ツールバー → 代表サムネ固定 (📌) ボタンを表示` でアドレスバー
...
- 表示:
  ...
  - アドレスバー（ツールバー「フォルダ:」欄）の右端にも現在表示中のコンテナ★を 📁★★★ で表示
  - アドレスバー右端の件数表示は、レーティングフィルタ適用後の表示サムネイル数と
