...
| 右半分クリック | 次の画像へ |
| 左半分クリック | 前の画像へ |
| 右ドラッグ（設定 ON 時） | リングショートカットのフリック。右ボタンを押した位置を中心にリングを表示し、8 方向のスロットを選んで離すと現在コンテキストの一発アクションを実行する。中央で離すと取消。既定は OFF。移動なしの右クリック、中ボタンズームとは別の状態として扱う |
| 右クリック（移動なし） | コンテキストメニュー。実ファイルではグリッドと同じ Windows Shell 標準メニュー + mIV 独自項目を優先し、ZIP/PDF 内ページなど仮想項目や Shell メニュー取得失敗時は mIV 独自メニュー（パスコピー・フォルダを開く・アプリケーションで開く等）を表示する |
...
| `startup_folder_mode` | StartupFolderMode | Previous | 起動時に開く場所。`Previous`=前回終了した場所、`Desktop`=Windows Known Folder API のデスクトップ、`Drives`=ドライブ一覧、`Specific`=指定フォルダ |
...
- [x] フルスクリーン右クリックコンテキストメニュー（移動なし右クリック。実ファイルは Windows Shell 標準メニュー + mIV 独自項目、仮想項目は mIV 独自メニュー）
