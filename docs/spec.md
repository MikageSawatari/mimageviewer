...
| リング系の「メインウィンドウを閉じる」 / 「アプリを終了する」 | グリッド文脈だけで割り当て可能。「メインウィンドウを閉じる」は [×] と同じ close request のため、タスクトレイ常駐設定が有効ならトレイへ格納する。「アプリを終了する」は明示終了として常駐設定にかかわらず通常の保存・終了処理を通る。画像 / 動画の「フルスクリーンを閉じる」とは別操作 |
| リング系の「一覧の先頭へスクロール」 / 「一覧の末尾へスクロール」 | グリッド文脈だけで割り当て可能。現在のサムネイル / 詳細一覧を先頭 / 末尾へスクロールするが、選択・チェック・最後に選択した画像は変更しない。Home / End の選択移動とは別操作。実行時には pending intent を立て、描画時に同フレームの viewport と行高から確定した最大 offset へ適用する |
