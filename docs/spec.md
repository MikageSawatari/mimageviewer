...
- 静止画の上部ホバーバーの `…` は、画面下部の暗色固定「その他の機能」パネルを開く。中央タップには
  新しい意味を追加せず、タッチもマウスも同じ上部ボタンを入口にする。パネルの操作行は 48pt とし、
  ナビゲータ ON / OFF、4 隅から選ぶ配置サブメニュー、分析ツール、ピクセルグリッド、ルーペ固定を
  現在状態と実効ショートカット付きで表示する。サブメニューには戻る行を置く。パネル矩形はタップ分類の
  除外領域とし、表示中は共通モーダル入力規則に従って背面の wheel / key / pointer 操作を止める。
  `…` の右クリックでは、メインツールバーのカスタマイズと同じ右クリック作法で項目の表示 / 非表示と
  順序を変更する。保存順に無い新項目は末尾へ自動追加し、既定では表示する。
...
| `fullscreen_overflow_item_order` | Vec\<FullscreenOverflowItemId\> | [] | 静止画「その他の機能」パネルの保存順。空または保存順に無い既知項目は既定順の末尾へ補う。enum は `Unknown` を持ち、未知の将来項目を旧版が読んでも設定全体を壊さない。未出荷機能の加法フィールドなので `serde(default)` とし、移行は行わない |
| `fullscreen_overflow_hidden_items` | Vec\<FullscreenOverflowItemId\> | [] | 静止画「その他の機能」パネルで非表示にする項目。順序とは別に保持し、新しい既知項目は隠す指定が無いため既定で表示する。`serde(default)` の加法フィールド |
