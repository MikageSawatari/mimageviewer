...
| Ctrl + A | 現在のフィルタで表示中のチェック可能なアイテムを全てチェック（フォルダ/画像/動画/ZIP・PDF 本体/ZIP 内画像/PDF ページ/変換前アーカイブが対象） |
| Delete | 選択/チェック済みの実フォルダ / 実ファイルを削除（確認あり。通常はゴミ箱に移動。リムーバブル / ネットワーク / ゴミ箱を使わない設定のドライブ / ゴミ箱容量を超える対象では確認文言で警告。仮想ページは除外） |
| Ctrl + C | 選択/チェック済みの実フォルダ / 実ファイルをクリップボードにコピー |
| Ctrl + X | 選択/チェック済みの実フォルダ / 実ファイルをクリップボードにカット |
| Ctrl + V | クリップボードのファイル / フォルダを現在の実フォルダにペースト |
...
| 入力 | 動作 |
|------|------|
| 方向パッド / 左スティック | グリッドでは選択移動。画像フルスクリーンではページ送り。動画フルスクリーンでは左右がシーク / タイルカーソル移動、上下が前後ファイル移動 |
| A / B | 決定・開く・再生 / 戻る・閉じる |
