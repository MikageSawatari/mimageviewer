...
| Ctrl+F | 現在地フィルタ | **現グリッドの表示中アイテムのみ** (非再帰・索引なし) | worker 上のオンデマンド判定 | 構造アイテム (フォルダ / ZIP / PDF) もファイル名で一貫して絞り込む。画像 / 動画は PNG tEXt + EXIF + XMP + 動画メタ + タグ + 外部メタデータサイドカー、PDF はファイル名 + document info。PDF ページ表示中は無効、ZIP 表示中はファイル名フィルタに固定 |
