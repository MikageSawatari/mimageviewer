...
- 「サブ展開」は、現在の実フォルダ配下の実ファイル画像 / 動画を、その時点のスナップショットとして
  フラット一覧化する一時ビュー。内部的には `__subfolder_expansion__` の synthetic path を使い、
  実 root は `subfolder_expansion_root` に保持する。`last_folder`、通常フォルダ履歴、
  コンテナ★、catalog `delete_missing` など実フォルダ前提の処理には載せない。
  既存の★フィルタ、タグ、詳細表示、場所 facet、Ctrl+F 現在地フィルタは実パス item に対して使える。
  画像色フィルタは大量デコードを避けるため初期版では無効。表示順は現在のソート設定に従い、
...
- **サブ展開**: 現在の実フォルダ以下にある画像 / 動画を worker で再帰走査し、その時点の
  ...
  変換アーカイブの内部やコンテナ自体は初期版では含めない。走査中はフォルダバーに
...
| Space | チェック可能なアイテムをチェック（複数選択。画像/動画/ZIP・PDF 本体/ZIP 内画像/PDF ページ/変換前アーカイブが対象。フォルダとドライブ一覧は対象外） |
