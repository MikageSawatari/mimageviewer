...
比較ビューは単ページでは現在ページ、見開きでは左右ページを結合した見開き全体を単位に処理する。
見開きの中央には、画面上で実際に描画された gap と同じページ幅比率の黒い間隔を入れる。
結合後のキャンバスが一辺でも 8192px を超える場合は、gap を含む全体を等比縮小して上限内に収める。
上限内は原寸のまま扱う。縮小表示時の画質を保つため、比較用GPU textureのmip chainは維持する。
