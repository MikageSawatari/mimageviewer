...
- v2.7.0では「メタ情報をエクスポート / インポート」は安定化継続のためメニューから
  一時非表示にする。実装とbundle形式は保持し、リリース後に再表示して開発を継続する。
  再表示後は実フォルダ表示中だけ使用できる。既存の自動
  sidecar `mimageviewer.dat` とは別に、対象フォルダ直下の `mimageviewer.meta.miv`
  directoryへ、評価、タグ、動画・音声ブックマーク、本ブックマーク、見開きモード・
...
- インポートは先に sidecar 全体を検証し、存在件数、欠落件数、ファイルサイズ相違件数を
  ...
  もDB反映前に拒否する。v1 manifest は従来の4区分だけを適用し、v2で増えた区分の既存値を
  消さない。v2 import の適用後は、同じ場所に既存 `mimageviewer.dat` があれば現在の mtime を
