...
| 右クリック | コンテキストメニュー。実ファイル / 実フォルダと ZIP/PDF 内ページなど仮想項目のどちらも Windows と同じ見た目の native メニューを表示する。mIV 項目 (新しいフォルダ、名前の変更、パスコピー、画像コピー、編集内容のコピー/貼り付け、検索結果のフォルダ移動、回転、代表サムネ固定など) は共通定義から構築する。実項目では Windows Shell 項目を既定で末尾の「Windows のメニュー」サブメニューへまとめ、開くまで Shell 項目の列挙を遅延する。設定で従来どおり同じ階層へ併記できる。仮想項目と仮想項目を含むチェック選択では Shell 項目を出さない。実項目と仮想項目が混在するチェック選択のパスコピー / 削除は全選択数を表示するが、押すと理由と対処をトーストで示して何も実行しない。アイテムがない場所では現在表示中の実フォルダだけ Shell 背景メニューを使い、mIV 先頭項目の「新しいフォルダ…」「貼り付け」からフォルダ作成と Shell paste を実行できる |
...
| 右クリック長押し（設定 OFF 時） | コンテキストメニュー。実ファイルと ZIP/PDF 内ページなど仮想項目のどちらもグリッドと同じ native メニューを表示する。実ファイルの Windows Shell 項目は既定で「Windows のメニュー」サブメニューへまとめ、仮想項目では mIV 項目（パスコピー・フォルダを開く・アプリケーションで開く等）だけを表示する。動画フルスクリーンでは動画に作用しない回転・親コンテナ代表固定を出さず、「現在のフレームを動画サムネに設定」を表示する。動画上の左クリックでメニューだけを閉じた場合、そのクリックを再生 / 一時停止へ再利用しない |
...
チェック選択では、ファイルコピー / カット / ペーストを含まない mIV 項目だけの native メニューを使う。
