...
- 表示はページ構成 (1 ページ / 見開き 左→右・右→左、表紙単独あり) と連結方式
  ...
  表示に反映する。
- 静止画 / ZIP 内画像 / PDF ページと、その編集・補正・AI・注釈後の表示テクスチャは GPU mipmap を
  自動生成し、フィット・見開き・連結読みなどの大縮小時にスクリーントーンのモアレを抑える。
  画像補正パネルの「フィルタ」にある「縮小表示のモアレを抑制する」は既定 ON で、配下の
  「より強く抑制」は既定 0.0（標準）、範囲 0.0〜1.5。OFF は完全な mip chain を保持したまま
  level 0 固定で表示し、モアレ抑制より線のシャープさを優先する。設定は全画像・全ウィンドウ共通で、
  切替時に表示テクスチャの再生成やキャッシュ失効を行わない。比較表示と360度パノラマにも適用する。
  サムネイル、animated image、動画、明示的なニアレスト表示は対象外。旧版の手動 1/2・1/4 縮小
  post-filter が保存されていた場合は、フィルタなしへ移行する。
