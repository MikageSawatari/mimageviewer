...
| メニュー | 項目 |
|---------|------|
| ファイル | フォルダを開く… (Ctrl+O) / 読書履歴を開く / レーティング一覧 / 現在地フィルタ (Ctrl+F) / キャプチャ保存フォルダを開く / 終了 |
...
- セルサイズ = ウィンドウの残余領域 ÷ 列数
- 選択中セルの下にはサムネイル情報ツールチップを表示する。既定はファイル名と画像解像度、動画ではファイル名と動画長さ。表示内容は環境設定 → 表示 → サムネイルで、種類 / サイズ / 更新日時 / 作成日時 / 動画解像度 / 動画コーデック / 場所 / 読書履歴の最終閲覧 / 読書履歴の既読位置を切り替えられる。読書履歴ビューでは、ホバー tooltip に場所 (フルパス) / 最終閲覧日時 / 既読位置を表示する (複数フォルダ・ドライブの同名本を場所で判別できるようにする)。ツールチップ幅はセル幅固定ではなく最大約 2.5 セル分を使い、画面右端ではセル右端に合わせて画面内に収める。動画長さなど未取得の値は選択中の 1 件だけバックグラウンドで遅延取得する
- **仮想スクロール実装**（下記詳細参照）
...
- サムネ / 詳細の両モード共通で、ツールバー下のスマートフィルタバーから
  種類・拡張子・場所・★・タグ・日付・サイズ・状態・AI モデル・生成ツールを絞り込める。場所は元ファイル/元コンテナの親フォルダで、製本フォルダは `本棚 > 本名` と表示する。スマートフィルタバーは
...
| `facet_filter` | FacetFilter | default | スマートフィルタ条件。種類・拡張子・場所・タグ・日付・サイズ・状態・AI モデル・生成ツールなど。絞り込み中に ZIP/PDF/フォルダなどのコンテナへ入ると親階層の条件を一時退避し、内側では別条件を設定できる。Backspace などで親階層へ戻ると退避した条件を復元する |
