...
- 表示はページ構成 (1 ページ / 見開き 左→右・右→左、表紙単独あり) と連結方式
  ...
  ブックマークタブは現在の本に登録したページをページ順に並べ、サムネイル、ページ番号、
  ファイル名またはアーカイブ entry 名、削除ボタンを表示する。行を選ぶとそのページへ移動し、
...
- 静止画 / 動画 / 音楽フルスクリーンの左右パネルは「通常ホバー」と「クリック表示」を選べる。通常ホバーは左端と右端で対応するパネルだけを個別に表示する。クリック表示は左右最端の細い呼び出しバーをクリックして開き、現在ファイルでは明示的に閉じるまで維持する。左右ともファイル移動とフルスクリーン退出で閉じ、再入場へ開状態を持ち越さない。左右いずれかのパネルが表示中は上バーと下バー / HUD も同時表示する (音楽は上下常時表示)。I / Tab、上部バーの i、画像リング / ゲームパッド操作は永続する表示モードの切替に使う
...
- フォルダバーの `場所▼ > ブックマーク` は動画・音声・本のブックマークを登録日時の新しい順で
  ...
  本は identity を現在のページ列へ再解決して直接開き、閉じると一覧へ戻る。
...
- 動画・音声は既存 `video_bookmarks.db` (`path + pts`) を共有し、本は
  `%APPDATA%\mimageviewer\book_bookmarks.db` に `(container_key, page identity)` を一意として保存する。
