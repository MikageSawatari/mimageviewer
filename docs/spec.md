...
ファイル名順 / 番号順（区切り無視）/ 更新日昇順 / 更新日降順 / サイズ昇順 / サイズ降順。
...
記号・空白をまたいで番号本体を比較する。サイズが取得できない項目は昇順・降順とも同じ行の
末尾へ置き、実在する0バイトファイルとは区別する。同値はファイル名順で安定させる。
...
| `sort_order` | SortOrder | FileName | 通常一覧のソート順（FileName / Numeric / DateAsc / DateDesc / SizeAsc / SizeDesc）。UI 上の `Numeric` は `番号順（区切り無視）`、Sizeは小さい順 / 大きい順と表示する。サイズ不明は両方向とも同じカテゴリ行の末尾。本として表示中や閲覧履歴では UI を無効化し、文脈ごとのページ順に固定 |
