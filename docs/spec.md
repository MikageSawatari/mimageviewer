...
- 動画シークストリップの保存状態は `Settings.video_seek_strip_state` の
  ...
  下部ロックボタンの左にベクター描画のフィルムアイコンを置き、上向き menu から
  「なし」「サムネイルストリップ表示」「音声波形ストリップ表示」を直接選べる。
- 波形は最初に可視 60 秒だけを表示し、同じ中心・同じ時間解像度の 180 秒 texture へ background で
  交換する。その後は可視幅 3 倍の texture から部分矩形を scroll 描画する。replacement は端へ
