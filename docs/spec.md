...
- [x] **ポストフィルタ** (レトロ系表示エフェクト、18 プリセット)
  - 基本: 標準 / ニアレスト (補完なし)
  - CRT ブラウン管: シンプル / フル (歪み+グロー) / アーケード (高コントラスト)
  - 減色・ディザ (色数昇順):
    - 1bit ディザ (2 階調)
    - GameBoy (4 色固定、緑)
    - PC-98 (適応 16 色、median cut)
    - ゲームギア (適応 32 色、4bit/ch)
    - ファミコン (固定 52 色、NES ハードパレット)
    - メガドライブ (適応 61 色、3bit/ch の banding)
    - MSX2+ (固定 256 色、GRB 3:3:2)
    - スーパーファミコン (適応 256 色、5bit/ch)
  - 複合プリセット (非液晶機種 × CRT シンプル): ブラウン管のファミコン / PC-98 / MSX2+ / メガドライブ / スーパーファミコン
  - 適応パレット機種は各ゲームが画像ごとに色を選んだ実機挙動に合わせ、median cut で最適 N 色を抽出
