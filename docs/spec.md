...
- 非破壊編集済みページは、編集終了時または別ページへの移動時に、消しゴム・補正レイヤー・隠蔽加工済みの下地とテキスト／スタンプ注釈レイヤーへ同じ切り取りを適用し、最大辺 2048px の下地 WebP (q=90) と注釈 lossless WebP を中央キャッシュへ非同期保存して通常サムネイルより優先表示する。表示時は下地だけへ色調補正を掛けてから注釈を合成し、final AI / post-filter は含めない。編集 preview は元画像によるアイドル高画質化の対象外
