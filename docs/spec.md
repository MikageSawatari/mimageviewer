...
- **履歴▼ メニュー**: 最近開いたフォルダをフルパスで最大 20 件表示し、選択したフォルダへ移動する。最近開いたフォルダ履歴は再起動後も保持され、環境設定 → 表示 → ツールバーのフォルダバー設定からクリアできる
...
- **📌 代表サムネ固定ボタン** (フォルダバー右側): 選択中のアイテムを現在のフォルダ /
  ...
  `設定 → 環境設定 → 表示 → ツールバー → 代表サムネ固定 (📌) ボタンを表示` でオフ可能
...
- 列セクションの `詳細` は環境設定 → 表示 → ツールバーで表示/非表示を切り替え可能。
  非表示でも <kbd>Alt</kbd>+<kbd>-</kbd> のショートカットは使える
- 詳細モードでは縦横比セクションを非表示にする。列セクションは `詳細` 選択状態として残す
- 各セクション・個別項目の表示/非表示をカスタマイズ可能。本棚セクションも環境設定 → 表示 → ツールバーで非表示にできる
- ウィンドウ幅が不足する場合、ツールバー項目はボタン単位で次の行へ折り返す。ボタン内の
  日本語ラベルが縦に潰れるような内部折り返しは行わない
...
- サムネ / 詳細の両モード共通で、ツールバー下のスマートフィルタバーから
  ...
  環境設定 → 表示 → ツールバーで表示/非表示を切り替え可能。アクティブ条件はチップ表示し、
  `全解除` でまとめて解除できる
...
- メタデータパネル: AI 生成プロンプト (A1111 互換 / ComfyUI / NovelAI / InvokeAI / SwarmUI / Fooocus 系 / EasyDiffusion / JPEG EXIF UserComment) + EXIF 撮影情報 + 外部メタデータサイドカー (画像と同名 JSON/TXT を読み取り専用で表示) を表示。HTTP(S) URL はクリック可能リンクとして表示する
...
- **設定**: `環境設定 → 表示 → ツールバー → 代表サムネ固定 (📌) ボタンを表示` でフォルダバー
...
| マウス戻るボタン | `Settings.ring_shortcuts.mouse_buttons_image.back` / `mouse_buttons_video.back` に従う。新規環境と既定リセットはフォルダ履歴を戻る。従来どおりを選んだ既存環境は Ctrl+↑ と同じツリー順移動。画像系 / 動画 native 経路とも有効 |
| マウス進むボタン | `Settings.ring_shortcuts.mouse_buttons_image.forward` / `mouse_buttons_video.forward` に従う。新規環境と既定リセットはフォルダ履歴を進む。従来どおりを選んだ既存環境は Ctrl+↓ と同じツリー順移動。画像系 / 動画 native 経路とも有効 |
...
| Y | グリッドではフォルダツリーペインを表示し、現在フォルダへツリーカーソルを移す。動画フルスクリーンでは S キーと同じタイルモード切り替え。画像フルスクリーンでは Y+左右で Ctrl+左右と同じ見開き 1 ページずらし |
...
| `recent_folders` | Vec\<PathBuf\> | [] | フォルダバーの履歴▼に表示する最近開いたフォルダ履歴。最大 20 件、検索中の一時移動は記録しない。環境設定 → 表示 → ツールバーからクリアできる |
