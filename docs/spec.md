...
- **履歴▼ メニュー**: 最近開いたフォルダをフルパスで最大 20 件表示し、選択したフォルダへ移動する。最近開いたフォルダ履歴は再起動後も保持され、環境設定 → 表示 → フォルダバーからクリアできる
...
- **📌 代表サムネ固定ボタン** (フォルダバー右側): 選択中のアイテムを現在のフォルダ /
  ...
  `設定 → 環境設定 → 表示 → フォルダバー → 代表サムネ固定 (📌) ボタンを表示` でオフ可能
...
- セクション設定の値 (表示/並び順/表示形式/出す項目) は永続化される。フォルダバー (アドレス行) の
  表示項目だけは引き続き環境設定 → 表示 → フォルダバーで設定する
...
- **設定**: `環境設定 → 表示 → フォルダバー → 代表サムネ固定 (📌) ボタンを表示` でフォルダバー
...
| `recent_folders` | Vec\<PathBuf\> | [] | フォルダバーの履歴▼に表示する最近開いたフォルダ履歴。最大 20 件、検索中の一時移動は記録しない。環境設定 → 表示 → フォルダバーからクリアできる |
