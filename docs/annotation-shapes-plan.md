...
- 数字はグリフの実描画範囲 (インク bbox) の中心を円の中心に合わせる。
  `TextBlock.v_center_ink = true` は番号バッジに加え、新規作成する吹き出し全種と
  「注釈追加」の枠プリセットに設定し、横書きのときだけ有効とする。吹き出しの
  セリフタブにある「文字を上下中央に補正」でオブジェクトごとに切り替えられる。
  フィールドの既定値は `false` のため、フラグを持たない既存データは従来の行送り箱
  中心合わせのまま変化しない。空白のみ・縦書きは従来方式へフォールバックする。
