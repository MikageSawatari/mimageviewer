...
| 7 | 円の表現 | **`Ellipse` の `circle` 明示フラグで表す**。`rx == ry` から推測せず、フラグは UI 専用で描画に影響させない。番号バッジは円として作成する (§4.5) |
...
### 4.5 番号バッジ — 円フラグ + プリセット + 自動採番

- `BubbleShape::Ellipse` に `#[serde(default)] circle: bool` を加える。`true` は UI 上の
  「円」を表し、形状ピッカーの判定・半径 1 本の編集・四隅の常時等比リサイズにだけ使う。
  tessellate / raster はこのフラグを見ず、円の `rx == ry` は UI が維持する。
  `rx == ry` でも `circle: false` なら楕円のままであり、値から円を推測しない。
- 実体 = `Ellipse { circle: true } + fill 赤 (fill_opacity 1.0) + outline 白 +
  TextBlock(白・太字・中央揃え)` の Bubble。
- バッジ識別は既存の `shape_preset_link` 文字列フィールドに `"miv:step-badge"`
  を入れる。新規バッジは円として作成し、右パネルの大きさは半径 1 本で調整する。
  `circle` のない旧 JSON は `false` (楕円) とし、開発中データの移行は行わない。
...
| 線太さ (標準) | 画像長辺 × 0.25% を 3〜12px にクランプ | 座標系がソース画素空間のため画像サイズ追従が必要 (テキスト既定サイズ `sh*0.04` clamp(24,96) と同じ考え方)。1920px 画像で約 5px ≒ ShareX の 4px。細 = ×0.6 / 太 = ×1.8 / 極太 = ×3.0 (実機 FB 2026-07-14 追加) |
...
- ドラッグ中に <kbd>Ctrl</kbd> を押している間だけ、ドラッグ開始時の pivot を固定した
  ...
  操作にリアルタイムで追従する。
- ドラッグ中に <kbd>Shift</kbd> を押している間は、ドラッグ開始時のアスペクト比を維持した
  一様スケールへ切り替える。<kbd>Ctrl</kbd> と併用でき、<kbd>Ctrl</kbd>+<kbd>Shift</kbd> では
  開始時 pivot 固定の等比リサイズになる。<kbd>Shift</kbd> も毎フレーム評価する。
- 計算はドラッグ開始時の pivot・half extents・回転・角番号と現在カーソルから毎回
  再計算する。モード切替を繰り返しても増分誤差を蓄積せず、最小寸法クランプ中も
  反対角を固定する。スタンプと番号バッジは <kbd>Shift</kbd> の有無にかかわらず開始時の
  アスペクト比を維持する。番号バッジの rx / ry を右パネルで個別に変えた場合も、その
  ドラッグ開始時の比率を維持する。
...
- キャンバス選択・一覧選択・新規追加による自動選択を同じ選択変更として扱う。
- 選択先が本文の空白を除いて空の Bubble なら「本体」、それ以外は「セリフ」を既定タブにする。
  番号バッジは本文に数字が入るため「セリフ」から始まる。
- 既定タブを適用するのは選択 id が変化した瞬間だけ。同じオブジェクトを選択し続けている間は、
  ユーザーが手動で選んだタブを上書きしない。マーカーと注釈矢印は専用パネルのため対象外。
