...
## 9/10夜の追加対応：類似検索の一時休止

類似索引の全件照合が変更通知で繰り返され、長時間収束しないため、利用者は
v3.8.0では類似検索のUIとバックグラウンド処理を一時休止することを承認した。
[休止対応の設計・検証状況](similar-feature-pause-v3.8.0.md) を最新の状態として参照する。
以下の類似検索を含む統合記録・19:52の確認buildは休止対応前の履歴であり、
休止版の検証済み証拠として扱わない。休止対応は独立検収・全体gate・確認buildが完了し、
9/10 23:53 JSTの新しいdev-runtimeを作成済み（未起動）。成果物hashと検証記録は上記リンク参照。
開発側の保留は解消し、本対応を取り込んだ公開工程へ引き渡す。性能計測・公開ゲートは未免除。
更新履歴・マニュアルも公開担当が休止仕様へ合わせる。既存索引・設定は保持し、
全件照合の根本修正と再有効化は後続開発で扱う。
