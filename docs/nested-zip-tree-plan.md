...
| Ctrl+↑↓ | **DFS 前順トラバーサル** (`ZipNavState::dfs_step`)。実フォルダの Ctrl+↑↓ と同じ規則 (次 = 最初の子 → 次の兄弟 → 祖先の次の兄弟、前 = 前の兄弟の最後の子孫 → 親) を ZIP ツリーに適用。ツリーの端で `false` → 呼び出し側が ZIP を抜けて実フォルダ DFS へ続く (実機 3 巡目 #6。当初の「兄弟のみ + ルート即脱出」を置換)。FS 中は直下画像なしノードを自動スキップ |
...
(列挙が .zip/.cbz しか再帰しない / 変換器が入れ子を skip)。統一ルールで対応:
**純 ZIP-in-ZIP は無変換ツリーのまま** (entry_name 不変の利点を維持)、**非 ZIP が絡む
入れ子は変換キャッシュに再帰展開** (フラットな literal エントリ名 `"inner.rar/p01.jpg"`
で書き、ツリーは `/` split でそのまま本になる)。詳細は
[virtual-folders.md](virtual-folders.md) §「入れ子アーカイブの再帰展開」。

**実機 3 巡目フィードバックで追加実装済み (v1.3.0)**:
- **Ctrl+↑↓ を兄弟移動から DFS 前順ナビへ置換** (#6)。旧 #4 実装は「兄弟本のみ +
  ルートで即 ZIP 脱出 + 端で ZIP 内に閉じ込め」だったが、ユーザー期待は「ZIP の中身を
  実フォルダツリーの一部として Ctrl+↑↓ で出入りできる」こと。`ZipNavState::sibling` を
  `dfs_step(forward, sort) -> bool` に置換 (forward = 最初の子 → 次の兄弟 → 祖先の次の
  兄弟、backward = 前の兄弟の最後の子孫 → 親)。端では **stack 不変で false** を返し、
  呼び出し側 (グリッド 4 経路 + FS 2 経路) は既存の fall-through 配線そのままで
  `start_folder_nav` に流れて ZIP を脱出する。探索はローカルコピー上で行い確定時のみ
  書き戻す (端で遡行 pop が残ると BS / ピン / 見開きキーが表示とずれるため)。
  FS 側 (`zip_nav_dfs_fullscreen`、旧 `zip_nav_sibling_fullscreen`) は直下画像のある
  ノードまで dfs_step をループしてスキップ (グリッドは全ノード停止 = 実フォルダと同じ
  非対称)。スライドショー末尾の next-folder も同経路で、ルート直下画像の後に最初の本へ
  降りて継続するようになった。
...
- **複数本 ZIP の auto-fullscreen 着地 / cross-book resume**: ルートが ZipDir のみのとき
  自動フルスクリーンは画像を見つけられず本一覧表示に留まる。「続きから」の記録は
  **ルート階層のみ** (本の中では記録しない。idx が階層ローカルなため、再オープン時に
  ルート階層の同 idx へ誤着地するのを防ぐ)。先頭本への自動進入 / 深い階層への resume は将来。
- **ドリル中の Ctrl+G** は ZIP ルートに戻る (状態ロスト、データ破壊なし。ソート変更は修正済み)。
- **フルスクリーン本またぎで移動先がレーティングフィルタで全 hidden** の場合は、
  フルスクリーンを閉じてグリッドにその階層を見せる (= グリッド Ctrl+↓ と同じ着地)。
  直下画像なしの分岐ノードは実機 3 巡目の DFS 化で自動スキップになったため、この
  フォールバックに残るのはフィルタ全 hidden ケースのみ。

**別セッションレビュー (2026-06-10) で修正済み** (詳細はレビューブリーフ参照):
- **単一ラッパー本のピン set/lookup キー不一致** → 根治。set 側 = `pin_container_key()`
...
- **v1.2.x 保存の見開き設定**: 本キーに未保存の項目は `zip_path` キーへフォールバック
  ...
  設定を各本が継承する。
- **★固定 (snapshot) との相互不整合**: `activate_snapshot` が zip_nav を
  `saved_zip_nav` へ退避、at_origin 解除で復元。`snapshot_return_to_list_view` は破棄。
- **フルスクリーン本またぎの lock 残留** (移動先に直下画像なし) → lock 解放 +
  close_fullscreen で本一覧に着地。(実機 3 巡目の DFS 化で直下画像なしノードは
  自動スキップに変わり、このフォールバックはフィルタ全 hidden ケースのみに縮小)
- **スライドショー末尾の「次フォルダ」**が本の途中で ZIP ごと脱出 → 本を続けて継続、
  最終本でのみ ZIP を抜ける。(実機 3 巡目以降は DFS 順で次の本へ進む)
- アドレスバー ⬆ ボタンが zip_nav を経由せず ZIP ごと脱出 → BS と同じ「1 階層戻る」に統一。
- BS で本から出たとき、出てきた本の ZipDir セルを選択する (戻り先の視認性)。
