...
1. **サムネイル**: 既定は固定ドライブアイコン。ユーザーが手動でピン留めした
   ...
   そのフォルダタイルの既存サムネだけを使う。**ドライブルートの自動再帰スキャン・
   代表画像探索・read_dir・メタデータ確認・ピン先デコードは一切行わない**。
...
- サムネは「DB にピンがある時だけ」既存カタログ / 動画ピン DB を cache-only で参照。
  ...
  参照する。キャッシュ未作成なら即アイコン (read_dir / metadata / decode もしない)。
...
- **`last_folder` の保存**: ドライブ一覧表示中に終了したときの扱いを決める。
  推奨: `current_folder==None` のときは `last_folder` を上書きしない (= "前回
  フォルダ" 起動では最後に開いた実フォルダに戻る)。ドライブ一覧から起動したい
  ユーザーは専用 `Drives` モードを使う。
