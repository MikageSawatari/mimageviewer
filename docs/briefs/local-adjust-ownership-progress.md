...
ブリーフ §7 は「重なるのは `src/app.rs` だけ」としていたが、**`src/ui_fullscreen.rs` にも
3 箇所触れている** (編集状態を畳む経路の routing: 分析モードへの切替・Esc・
`reset_*` 系)。同ファイルは `external-tool-launch` worktree の担当範囲なので、
merge 時に conflict しうる。いずれも数行で、内容は
`self.fold_local_adjust_edit_state();` の追加と、破棄していた 4 行の置換。
