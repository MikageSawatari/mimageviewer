...
## ⑤ 「古い絵が出る」— 検出の置き場所の誤り (2026-08-14 夕方)

### 確定した事実

利用者の 3 回目の再現ログ (identity 検査入りビルド `4ee15943`) で:

```
4.28-4.35  continuous_draw  _002.pdf#0  idx=0  gen=4  texture=Managed(181)   ← 描かれている
4.331      fs/ready  _002.pdf#3
4.334      fs/ready  _002.pdf#2
4.342      fs/ready  _002.pdf#1
           ★ _002.pdf#0 の ready は最後まで出ない ★
```

- `display-integrity --check` は**捕まえている** (t=4.225 の 1 件)
- しかし `[display-identity]` は **0 行** = アプリ内の検査は発火しなかった
- テクスチャ ID の突き合わせでは `Managed(181)` は `_002.pdf#0` としてしか描かれていない
  → **ラベルは正しく、中身が違う**

### 検査が発火しなかった理由 (実装の誤り)

`verify_display_identity` が

1. **18 ストア中 2 つ (`edit_result_cache` / `final_composite_cache`) しか見ていない**
2. **`ensure_fs_page_load` からしか呼ばれない** — 連結表示の描画は毎スロットここを通らない

**ItemId の作りは正しい。** `item.perf_key()` の文字列を単調カウンタで interning しており、
文字列キーと判定力は同一・衝突ゼロ。**64bit ハッシュは衝突し得るので、これより悪い。**
利用者からの「文字列やハッシュにすべきか」という問いへの答えはこれ。

### 正しい形 (次にやること)

**キャッシュを探して回るのをやめ、テクスチャに素性を持たせる。**

1. **テクスチャ生成時に「どの `ItemId` のために作ったか」を記録**し、
   **描画直前に「今そこにあるページの `ItemId`」と照合**する。
   - 描画は全経路が集まる 1 点なので、**18 ストアを一度に覆える**
   - 連結表示 / 静止画 / 動画が同じ 1 箇所で捕まる
   - 比較は u64 一個のまま
2. **非同期プロデューサが開始時の `ItemId` を完了まで運ぶ**。
   今日掴んだ AI の件 (世代 N で開始 → 完了時に世代 N+1 のキーで登録) は、
   テクスチャに生成時の `ItemId` が刻んであれば**名乗り直しても素性が変わらない**ので捕まる。
3. 未対応ストア (conceal / comic / local_adjust / erase / adjustment) への横展開は、
   1 が入れば取りこぼしがそこで捕まるので**優先度が下がる**。

### 済んでいること

- `4c0779be` 合成キャッシュに `items_generation` (検出 2 件 → 0 件)
...
### ⑫-2 入力端の typed fix は「効いていない」ではなく「測る場所が違った」(2026-08-15)

Codex が `PageTurnConsumeResult` / `PageTurnKeyInput` を追加し (`consume_page_turn_action`)、
「処理時点で既に離れている auto-repeat はナビへ昇格させない」を実装した。
lib テストは通るのに再現シナリオは drift 1 のまま失敗した。

**最初に「`consume_page_turn_action` がそもそも呼ばれていない」と断定したが、これは誤り。**
呼ばれていなかったのは事実だが、それは経路が死んでいるからではなく、
**フルスクリーンの固定矢印が別の入口を通る**ため:

- 通常の ↓↑ は [ui_fullscreen.rs:17387](../../src/ui_fullscreen.rs) の
  `consume_fixed_page_turn_chord` → `consume_page_turn_chord_inner` を通る
- `consume_page_turn_action` は**通らない**。だが両者は
  `classify_page_turn_input` という同じ funnel に合流するので、
  **drop 規則自体は矢印にも効いている**

最初の probe を `consume_page_turn_action` にだけ置いたため 0 件になり、
それを「コードが実行されていない」と読んでしまった。probe を funnel
(`consume_page_turn_chord_inner` の両 return) へ移して `input/page_turn_edge`
(`chord` / `source` / `kind` / `released_after_match` / `os_held` / `outcome`) を
常時記録にしたところ、**実行されていることが確認できた**。

### 実測 (2026-08-15、利用者が報告したフォルダ)

| t | 出来事 |
| --- | --- |
| 5.180 | 最後の ↓ edge。`os_held=true` (キーは物理的にまだ下りている) |
| 5.184 | idx=30 へ移動 |
| 5.187 | idx=30 を描画、**サムネイル画質** (`pass_through`) |
| 5.474 | idx=30 がフル画質へ昇格 |

edge 30 件 / 移動 30 ページ。**行き過ぎは 1 ページも無い。**
最後の 1 ページもサムネイル段階を踏んでいる (= 約束どおり)。

**シナリオが報告していた drift 1 は測定の副作用だった。**
`hold_key` は「送り終えた時点」で返り、「送った最後のリピートが処理された時点」では返らない。
その 4ms の隙間で `fs_idx` を読むと 1 つ手前が見える。シナリオを
`at_release` / `drained` (250ms 後) / `settled` (さらに 2.5 秒後) の 3 点測定に変更し、
**在庫リピートの着地 (1 コマ) と、離した後も動き続ける本来の不具合を分離**した。
軽い設定・AI アップスケール有りの重い設定の両方で `in-flight 1, after-release 0` = PASS。

### 残る筋 (ローカルでは再現しない)

利用者の環境だけで起きているので、次の 2 つのどちらかに絞られる:

1. **最後のリピートと key-up が同じフレームのバッチに入る** → `released_after_match=true`
   → Codex の drop 規則が効いて移動しない (= 既に直っている可能性)
2. **リピートは frame F、key-up は frame F+1** → 移動時は `held=true` なので stand-in が出る
   → ハーネスと同じ挙動

いずれも `input/page_turn_edge` で判別できる。`os_held=false` の trigger、または
edge 数を超える移動が出れば 1 番目以外の原因が居る。**利用者環境で 1 本ログを取る。**

### 体感の説明 (仕様として妥当かの判断材料)

離した瞬間に目に入っているのは idx=29 (5.163 に描画)。その 4ms 後に idx=30 が
サムネイルで出て、**287ms 後にフル画質へ変わる**。287ms のサムネイル段階は
「一瞬」であり、主観的には「離してちょっとしてから 1 枚進んだフル画質になった」と映る。
これは利用者の文言と完全に一致する。

利用者の代案 (「先の画像を一旦サムネイル画質ですぐ表示してからフルサイズ画質」) は
**現状の挙動そのもの**。第一希望 (「止まった位置のまま動かない」) を満たすには、
既に発行済みのリピートを破棄する必要があるが、key-up が来る前にそれを知る術は無い。
**仕様判断が要る。**

**教訓 (今日 2 回目)**: 修正が効かないときは、まず自分の修正コードが実行されたかを
確かめる。ただし「実行されていない」と結論する前に、**計装が正しい経路に載っているか**を
確かめる。probe の 0 件は、コードが動かない証拠にも、probe が外れている証拠にもなる。
funnel (合流点) に置けばこの曖昧さは消える。
