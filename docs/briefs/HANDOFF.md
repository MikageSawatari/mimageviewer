...
**上限がリモート側にある。**本体の PDF ワーカーは 5 プロセスで、1 つを即応用に空けて
**4 本まで同時に回せる** ([src/pdf_loader.rs](../../src/pdf_loader.rs) の `worker_count - 1`)。
一方リモートは先読みを **2 本**に絞っている。先読みが枠を使い切らない設計自体は正しい
(表示要求が待たされないよう 1 枠空ける、[crates/remote-web/src/http.rs](../../crates/remote-web/src/http.rs)
の `all_limit - 1`) が、先読み専用の 2 は本体の 4 に対して低い。

**未検証**: 本体が同時 4 本で実際に速くなるか。1 ページ 2.3 秒は CPU を使う処理なので、
使い切っていれば並べても合計は変わらない。**上限の数字だけ変えれば測れる**ので、
在庫の仕組みを設計する前にこれを測る。
