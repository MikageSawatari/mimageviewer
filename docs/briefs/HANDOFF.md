...
**決定 (2026-08-13)**: リモートにフィルタのパネルを足す。4 つの描画時フィルタも**一覧に載せる**
— CRT を解除するには「なし」を選ぶしかなく、その「なし」が 4 つのうちの 1 つだから、隠すと
解除できなくなる。**注意書きは「効果なし」と書かない**: リモートでの選択は本体に記録されるので
本体の表示は変わるし、「なし」は既定値であって壊れた選択肢ではない。
**「本体の画面での拡大方法を決める項目です。リモートの表示は変わりません」**とする。
