...
1. **デコード先読み** — ブリーフ用意済み
   ([codex-remote-decode-ahead-brief.md](codex-remote-decode-ahead-brief.md))。
   表示単位で前後 2 単位ぶんデコード済みを保持する。**効果は仕様で保証されていない**ので
   測定が主目的。効かなければ canvas 方式へは進まず、**高画質の選択肢に「ページ送りが
   遅くなります」の注記を足す**方針 (利用者判断)
2. **remote-web 側の 503** (`admission_busy`)。前回ログで 39 件。入口で断る形が残っている
3. **ズーム中の先読みが 8192px を要求する件**。1 ページ 6 MB の主因
4. **診断ログのローテーションが無い**。実測 59 MB
5. リリース前: 製品ページにリモート閲覧の記載、README 更新履歴
...
- **カタログの `source_*` を編集座標の基準に使わない。** master が PDF で意味を変え
  ...
  値**でなければならない。不変条件「保存済みトリクは要求解像度が変わっても同じ割合を切り出す」
  をテストに置いた
