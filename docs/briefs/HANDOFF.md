...
**並列化の効きは 2 → 3 でほぼ終わる** (2026-08-12 の測定ビルドで確認、46 MP のページ 68 件)。
`ipc_ms` 中央値は同時 2 本で 2016ms、3 本で 2386ms、4 本で 3026ms。供給に直すと
**0.99 / 1.26 / 1.32 ページ毎秒**で、3 本目は +27%、4 本目は **+5% しかない**。
1 MP あたり 44 → 52 → 66ms と伸びており、**この機械は 3 本で CPU を使い切っている**
(本体の PDF ワーカーは 4 本受けられるので、空き待ちではない)。

**上限は 3 段が直列に並んでいて、1 つ動かしても効かない。** 2026-08-12 に
remote-web の `MAX_CONCURRENT_PAGE_PREFETCH` だけ 3 に上げて commit したが、
**実効は 2 のままだった**ので戻した (`aa145071`)。3 段とは:

| 層 | 定数 | 値 |
|---|---|---|
| 端末 | `PAGE_PREFETCH_CONCURRENCY` ([app.js](../../crates/remote-web/web/app.js)) | **2** |
| リモートサービス | `MAX_CONCURRENT_PAGE_PREFETCH` ([http.rs](../../crates/remote-web/src/http.rs)) | 2 (最も緩い) |
| 本体 | `worker_count - 1`、`remote_heavy_worker_count` が 3 で clamp ([pipe.rs](../../src/remote_ipc/pipe.rs) / [heavy_queue.rs](../../src/remote_ipc/heavy_queue.rs)) | **2** |

**実際に上げるなら 3 つ同時に動かす**必要がある。本体の clamp は「リモートに機械の
半分以上を渡さない」という判断なので、上げるかどうかは方針の決定であって漏れではない。
なお**本体側の投機的準備を作る場合も同じ 2 本の制限を受ける**ので、窓が温まる速度の
見積もりはこの値で行うこと (高画質 60 枚なら 3 本で 50 秒、2 本では約 73 秒)。
