...
**次に大きい無駄はコピー 2 回**: `display` 166ms + `to_rgba` 133ms = 299ms。46 MP のページを
DynamicImage → ColorImage → RgbaImage と 3 回材料化している。`Color32` は 4 バイトなので
ColorImage の画素列はすでに RGBA バイト列であり、JPEG 側が借用で読めれば `to_rgba` は要らない。
`display` 側は共有ローダーの中なので影響範囲が広い。
