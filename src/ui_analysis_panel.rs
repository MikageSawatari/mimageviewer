...
fn compute_hsv_histograms(
    ...
    let (px_x0, px_y0, px_x1, px_y1) =
        zc.visible_bounds(image_rect, pixels.size[0], pixels.size[1]);
    let area = (px_x1.saturating_sub(px_x0)) * (px_y1.saturating_sub(px_y0));
    let step = if area > 2_000_000 {
        3
...
fn draw_color_picker_widget(
    ...
    for gy in 0..grid {
        for gx in 0..grid {
            let col = hsv_to_color32(
                base_hue,
                gx as f32 / (grid - 1) as f32,
                1.0 - gy as f32 / (grid - 1) as f32,
            );
            painter.rect_filled(
    ...
    for (i, &v) in hist.iter().enumerate() {
        ...
        painter.rect_filled(
            egui::Rect::from_min_max(
                egui::pos2(x, rect.max.y - log_h),
                egui::pos2(x + bw, rect.max.y),
            ),
            0.0,
            hsv_to_color32(i as f32, 1.0, 0.4),
        );
        let lin_h = v as f32 / max_val as f32 * rect.height();
        painter.rect_filled(
            egui::Rect::from_min_max(
                egui::pos2(x, rect.max.y - lin_h),
                egui::pos2(x + bw, rect.max.y),
            ),
            0.0,
            hsv_to_color32(i as f32, 1.0, 1.0),
        );
    }
    painter.rect_stroke(
        rect,
...
fn draw_mosaic_grid(painter: &egui::Painter, image_rect: egui::Rect, zc: &ZoomCtx) {
    ...
    while x <= zc.origin.x + zc.disp_w {
        clip.line_segment(
            [
                egui::pos2(x, zc.origin.y),
                egui::pos2(x, zc.origin.y + zc.disp_h),
            ],
    ...
    while y <= zc.origin.y + zc.disp_h {
        clip.line_segment(
            [
                egui::pos2(zc.origin.x, y),
                egui::pos2(zc.origin.x + zc.disp_w, y),
            ],
            if i % 5 == 0 { stroke } else { dash },
        );
        y += pitch;
...
pub(crate) fn draw_guide_lines(
    ...
    for hn in -40i32..=40 {
        ...
        if !near.contains(p0) && !near.contains(p1) && !near.contains(egui::pos2(cx, cy)) {
            continue;
        }
...
fn build_color_rows(
    hc: Option<[u8; 4]>,
    pc: Option<[u8; 4]>,
) -> Vec<(&'static str, Vec<ColorRow>)> {
    let dash = "---".to_string();
    let mut sections = Vec::new();

    // RGB
    let (rh, gh, bh) = hc.map_or(
        ("---".into(), "---".into(), "---".into()),
        |[r, g, b, _]| (format!("{r:3}"), format!("{g:3}"), format!("{b:3}")),
    );
    let (rp, gp, bp) = pc.map_or(
        ("---".into(), "---".into(), "---".into()),
        |[r, g, b, _]| (format!("{r:3}"), format!("{g:3}"), format!("{b:3}")),
    );
    ...
    sections.push((
        ...
        vec![
            ColorRow {
                ...
                hover: rh,
                pinned: rp,
                diff: rgb_diff.as_ref().map_or(dash.clone(), |d| d[0].clone()),
            },
            ColorRow {
                label: "G:",
                hover: gh,
                pinned: gp,
                diff: rgb_diff.as_ref().map_or(dash.clone(), |d| d[1].clone()),
            },
            ColorRow {
                label: "B:",
                hover: bh,
                pinned: bp,
                diff: rgb_diff.as_ref().map_or(dash.clone(), |d| d[2].clone()),
            },
        ],
    ));
    ...
    let fmt_hsv = |c: Option<[u8; 4]>| -> (String, String, String) {
        c.map_or(
            (dash.clone(), dash.clone(), dash.clone()),
            |[r, g, b, _]| {
    ...
    let hsv_diff = hc.zip(pc).map(|([r1, g1, b1, _], [r2, g2, b2, _])| {
        ...
        [
            format!(
                "{:+.0}°",
                hue_diff(h1, h2) * if h1 >= h2 { 1.0 } else { -1.0 }
            ),
    ...
    sections.push((
        ...
        vec![
            ColorRow {
                ...
                hover: hh,
                pinned: hp,
                diff: hsv_diff.as_ref().map_or(dash.clone(), |d| d[0].clone()),
            },
            ColorRow {
                label: "S:",
                hover: sh,
                pinned: sp,
                diff: hsv_diff.as_ref().map_or(dash.clone(), |d| d[1].clone()),
            },
            ColorRow {
                label: "V:",
                hover: vh,
                pinned: vp,
                diff: hsv_diff.as_ref().map_or(dash.clone(), |d| d[2].clone()),
            },
        ],
    ));
    ...
    sections.push((
        ...
        vec![
            ColorRow {
                ...
                hover: lh,
                pinned: lp,
                diff: lab_diff.as_ref().map_or(dash.clone(), |d| d[0].clone()),
            },
            ColorRow {
                label: "a:",
                hover: ah,
                pinned: ap,
                diff: lab_diff.as_ref().map_or(dash.clone(), |d| d[1].clone()),
            },
            ColorRow {
                label: "b:",
                hover: bh2,
                pinned: bp2,
                diff: lab_diff.as_ref().map_or(dash.clone(), |d| d[2].clone()),
            },
        ],
    ));
    ...
    for (si, (title, rows)) in sections.iter().enumerate() {
        ...
        for (i, row) in rows.iter().enumerate() {
            ...
            painter.text(
                ...
                format!("{}{}", row.label, row.hover),
                font.clone(),
                text_col,
            );
            painter.text(
                egui::pos2(x0 + col3 * 2.0, ry),
                egui::Align2::LEFT_TOP,
                format!("{}{}", row.label, row.pinned),
                font.clone(),
                text_col,
            );
            let dc = if row.diff == "---" {
...
impl App {
    pub(crate) fn draw_analysis_panel(
        ...
        if ctx.input(|i| i.pointer.secondary_pressed()) {
            if let Some(pos) = ctx.input(|i| i.pointer.interact_pos()) {
                if image_rect.contains(pos) {
                    ...
                    if mag > 0 {
                        if let Some(c) = self.analysis_hover_color {
                            self.analysis_pinned_color = Some(c);
                        }
        ...
        if let Some(pix) = pixels {
            let overlay_mode = if self.analysis_filter_mag > 0 {
                self.analysis_filter_mag
            } else if self.analysis_grayscale {
                255
            } else {
                0
            };
            ...
            if overlay_mode > 0 {
                ...
                if !cache_valid {
                    let tex = if overlay_mode == 255 {
                        generate_grayscale_texture(ctx, pix)
                    } else {
                        generate_filter_texture(
                            ctx,
                            pix,
                            overlay_pick.unwrap_or([0, 0, 0, 255]),
                            overlay_mode as f32,
                        )
                    };
                    self.analysis_overlay_cache =
                        Some((tex, overlay_mode, overlay_pick, zoom, pan, fs_idx));
                }
            ...
            if self.analysis_mosaic_grid {
                draw_mosaic_grid(ui.painter(), image_rect, &zc);
            }
        }
        if let Some((img_start, img_end, cidx)) = self.analysis_guide_drag {
            draw_guide_lines(
                ui.painter(),
                zc.image_to_screen(img_start),
                zc.image_to_screen(img_end),
                image_rect,
                cidx,
            );
        }
        ...
        ui.painter().rect_filled(
            panel_rect,
            0.0,
            egui::Color32::from_rgba_unmultiplied(18, 18, 22, 245),
        );
        ...
        ui.painter().text(
            egui::pos2(panel_rect.min.x + 12.0, header_rect.center().y),
        ...
        let hints = format!(
            ...
            if self.analysis_filter_mag > 0 {
                format!(" [{}x]", self.analysis_filter_mag)
            } else {
                String::new()
            }
        );
        if !hints.is_empty() {
            ui.painter().text(
                egui::pos2(panel_rect.max.x - 38.0, header_rect.center().y),
        ...
        let cb_rect = egui::Rect::from_center_size(
            egui::pos2(panel_rect.max.x - 16.0, header_rect.center().y),
            egui::vec2(cb_size, cb_size),
        );
        let close_resp = ui.interact(
            cb_rect,
            egui::Id::new("analysis_close_btn"),
            egui::Sense::click(),
        );
        ui.painter().rect_filled(
            cb_rect,
            ...
            if close_resp.hovered() {
                egui::Color32::from_rgba_unmultiplied(200, 60, 60, 220)
            } else {
                egui::Color32::from_rgba_unmultiplied(60, 60, 65, 200)
            },
        );
        let xr = cb_size * 0.22;
        ...
        ui.painter().line_segment(
            [
                egui::pos2(panel_rect.min.x, header_rect.max.y),
                egui::pos2(panel_rect.max.x, header_rect.max.y),
            ],
        ...
        if let Some(c) = hover_pixel {
            self.analysis_hover_color = Some(c);
        }
        ...
        // ── カラーピッカー ──
        section_label(ui.painter(), egui::pos2(x0, y), "カラー情報");
        y += 14.0;
        let hover_hsv = self
            .analysis_hover_color
            .map(|[r, g, b, _]| rgb_to_hsv(r, g, b));
        let pinned_hsv = self
        ...
        draw_color_picker_widget(
            ui.painter(),
            egui::pos2(c1x, wheel_cy),
            outer_r,
            ring_w,
            hover_hsv,
        );
        draw_color_picker_widget(
            ui.painter(),
            egui::pos2(c2x, wheel_cy),
            outer_r,
            ring_w,
            pinned_hsv,
        );
        ui.painter().text(
            egui::pos2(c1x, wheel_cy + outer_r + 4.0),
        ...
        ui.painter().text(
            egui::pos2(c2x, wheel_cy + outer_r + 4.0),
            egui::Align2::CENTER_TOP,
            "固定(右クリック)",
        ...
        if let Some([r, g, b, _]) = self.analysis_hover_color {
            ui.painter().rect_filled(
                egui::Rect::from_min_size(egui::pos2(x0, y), egui::vec2(half - 3.0, sw_h)),
                2.0,
        ...
        if let Some([r, g, b, _]) = self.analysis_pinned_color {
            ui.painter().rect_filled(
                egui::Rect::from_min_size(egui::pos2(x0 + half, y), egui::vec2(half - 3.0, sw_h)),
                2.0,
                egui::Color32::from_rgb(r, g, b),
            );
        }
        ...
        // ── ヒストグラム ──
        if let Some(pix) = pixels {
            thin_separator(ui.painter(), egui::pos2(x0, y), pw);
            y += 4.0;
            section_label(ui.painter(), egui::pos2(x0, y), "ヒストグラム（表示範囲）");
            y += 13.0;
            ...
            let hist_valid = self
                .analysis_hist_cache
                .as_ref()
                .map_or(false, |c| c.0 == zoom && c.1 == pan && c.2 == fs_idx);
            if !hist_valid {
            ...
            small_label(ui.painter(), egui::pos2(x0, y), "色相 (H)");
            y += 11.0;
            draw_hue_histogram(
                ui.painter(),
                egui::Rect::from_min_size(egui::pos2(x0, y), egui::vec2(pw, HIST_H)),
                h_hist,
            );
            y += HIST_H + 4.0;
            small_label(ui.painter(), egui::pos2(x0, y), "彩度 (S)");
            y += 11.0;
            draw_sv_histogram(
                ui.painter(),
                egui::Rect::from_min_size(egui::pos2(x0, y), egui::vec2(pw, HIST_H)),
            ...
            y += HIST_H + 4.0;
            small_label(ui.painter(), egui::pos2(x0, y), "明度 (V)");
            y += 11.0;
            draw_sv_histogram(
                ui.painter(),
                egui::Rect::from_min_size(egui::pos2(x0, y), egui::vec2(pw, HIST_H)),
            ...
            draw_sv_map(
                ui.painter(),
                egui::Rect::from_min_size(egui::pos2(map_x, y), egui::vec2(map_side, map_side)),
                pix,
                &zc,
                image_rect,
            );
...
fn small_label(painter: &egui::Painter, pos: egui::Pos2, label: &str) {
    painter.text(
        ...
        egui::Align2::LEFT_TOP,
        label,
        egui::FontId::proportional(SECTION_FONT - 1.0),
