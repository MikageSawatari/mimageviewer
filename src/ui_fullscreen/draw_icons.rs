...
pub(super) fn handle_frame_step_button(
    ctx: &egui::Context,
    fs_idx: usize,
    direction: i32,
    ...
    match hold {
        Some(state) if state.fs_idx == fs_idx && state.direction == direction => {
            if now.saturating_duration_since(state.last_step_at)
        ...
        _ => {
            *hold = Some(crate::app::VideoFrameStepHold {
                fs_idx,
                direction,
                last_step_at: now,
            });
...
pub(super) fn draw_frame_step_icon(painter: &egui::Painter, c: egui::Pos2, r: f32, direction: i32) {
    let color = egui::Color32::WHITE;
    let stroke = egui::Stroke::new(2.0, color);
    let sign = if direction < 0 { -1.0 } else { 1.0 };
    let bar_x = c.x + sign * r * 0.9;
    painter.line_segment(
        [
            egui::pos2(bar_x, c.y - r * 1.05),
            egui::pos2(bar_x, c.y + r * 1.05),
        ],
        stroke,
    );
...
pub(super) fn draw_camera_icon(painter: &egui::Painter, c: egui::Pos2, r: f32) {
    ...
    let body =
        egui::Rect::from_center_size(c + egui::vec2(0.0, r * 0.18), egui::vec2(r * 2.1, r * 1.5));
    painter.rect_stroke(body, 2.0, stroke, egui::StrokeKind::Inside);
    let hump = egui::Rect::from_min_size(
        egui::pos2(body.min.x + r * 0.35, body.min.y - r * 0.45),
        egui::vec2(r * 0.8, r * 0.45),
    );
    painter.rect_filled(hump, 1.5, color);
    painter.circle_stroke(body.center(), r * 0.43, stroke);
    painter.circle_filled(body.center(), r * 0.17, color);
}

/// 動画 HUD のボタン (play/pause / mute) の hover ハイライト。
/// hovered = true で薄い半透明白、false で透明。
pub(super) fn draw_hud_button_bg(painter: &egui::Painter, rect: egui::Rect, hovered: bool) {
    let bg = if hovered {
...
pub(super) fn draw_speaker_icon(painter: &egui::Painter, c: egui::Pos2, r: f32, muted: bool) {
...
pub(super) fn draw_loop_icon(painter: &egui::Painter, c: egui::Pos2, r: f32, color: egui::Color32) {
...
pub(super) fn draw_replay_icon(painter: &egui::Painter, c: egui::Pos2, r: f32) {
