...
//!
//! 書き込み方式についての注意書きを常時表示し、「次回から表示しない」チェックで
//! Settings の `tag_write_warning_acknowledged` を立てる (ただし警告自体は
//! 常時出し続けている — ダイアログ内での注意喚起は消えない方が安全)。
    ...
    pub(crate) fn show_tag_editor_dialog(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("タグを編集")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(
                    egui::RichText::new(
                        "タグを付与すると、画像ファイルの XMP メタデータを書き換えます。\n\
                         書き換え中の異常終了でファイルが壊れる可能性はほぼありませんが、\n\
                         自動バックアップは作成しません。",
                    )
                ...
                ui.checkbox(
                    &mut self.settings.tag_write_warning_acknowledged,
                    "この警告を次回以降も毎回表示する場合は OFF にしてください",
                )
                .on_hover_text(
                    "ON = タグ付与時の確認プロンプトを省略\n\
                     OFF = 付与前に毎回確認 (未実装、v1.1 で追加予定)",
                );
                ui.add_space(4.0);
                ui.separator();
                ui.add_space(4.0);
