...
- **自動インデックス管理**: お気に入り単位で「名前索引」「メタ索引」「サムネイル事前キャッシュ」を個別に ON/OFF 可能。有効化するとバックグラウンドでファイルシステムを自動監視し、追加・更新・削除を差分検出してインデックスを常に最新に保つ。起動時は整合性チェックが走り、異常終了の残留を自動整理
...
- **ユーザー操作中はバックグラウンド索引を一時停止**: Ctrl+↑↓ 連打などの操作中は索引化ワーカーを停止し、ディスク I/O の取り合いを避けて UI 応答性を向上
- **PDF 連続めくりの黒画面を解消**: Ctrl+↓ 押しっぱなしで PDF を連続めくる際に 10 秒ほど黒画面が出る問題を修正。複数プロセスで並列処理し、古い要求はキャンセル
- **フォルダ切替時の XMP / EXIF / PNG メタ読み込みをワーカー化**: UI スレッドの同期 I/O を削減し、フォルダ切替の体感を改善
- **サイドカーファイルの差分チェックを高速化**: mtime キャッシュで同一フォルダの再訪問時に不要な import をスキップ
