...
- **上部バーに、使用頻度の低い操作をまとめたボタンを追加しました。** ナビゲータの 2 つの設定・ピクセルグリッド・ルーペの固定・分析モードを、指でも押しやすい大きさの一覧から選べます。これまでキーだけがあって画面上のボタンが無かったものです。分析モードは上部バーからこの一覧の中へ移りました。一覧の中身と並び順はカスタマイズできます。あわせて、360 度ビューのボタンは見開き表示や連結読みのときに消えるのではなく、押せない状態で残るようにしました（画像を送ってもボタンの位置が動かないようにするためです）。
...
- **ページ送りの表示を直しました。** ゆっくり読んでいるだけでも 1 ページごとに粗い画像が見えていたのを、キーを押したまま 2 ページ目へ進んだときだけに変えました。最初や最後のページでキーを押したときに、ページが動かないのに画像がちらつく問題、ページ送りのあとに読み込み中の表示（黒い画面に文字）が一瞬出る問題も直しています。
- **PDF のページが、一瞬だけ縦横比の違う形に引き伸ばされる問題を修正しました。**
- **切り取り（トリミング）の範囲が、画像の解像度によってずれる問題を修正しました。** 範囲を作ったときの解像度も一緒に保存するようにしました。以前に保存した範囲は、最初に読み込んだ解像度を基準として解釈します。すでにずれた状態で保存されているものは、お手数ですが取り直してください。
...
- **ジャンプパネルのマーカーのサムネイルが、縦長のときに引き伸ばされる問題を修正しました。**
