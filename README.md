...
- **書き出しにどこまで反映するかを、出力ごとに選べるようになりました。** 製本・エクスポート (1 枚 / まとめて)・外部ツールのそれぞれについて、「編集」「AI 処理」「表示用補正」の 3 段から選べます。「編集」は補正・回転・注釈・消しゴム・隠蔽加工・切り取りまで、「AI 処理」はそれに AI 拡大とノイズ除去を加えたもの、「表示用補正」はさらにスマートシャープ・カラー化・LUT・ポストフィルタまで反映します。後から別のソフトで加工するなら浅い段、そのまま見せる画像を作るなら深い段が向いています。環境設定 →「表示」→「書き出しの焼き込み」から設定します。既定はこれまでと同じ動作です。
