...
### v0.7.0
- **Susie 画像プラグイン対応**: 32bit ワーカープロセス (`mimageviewer-susie32.exe`) で `.spi` をロードし、PC-98 時代の PI / MAG / Q0 / PIC / MAKI など多数のレトロ画像形式に対応。ワーカーは本体 exe に埋め込み、初回起動時に `%APPDATA%\mimageviewer\` へ自動展開。プラグインクラッシュはワーカーに隔離されて本体には影響しない
