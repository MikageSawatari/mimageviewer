...
- **切り取りをしたページに「切」バッジが出るようになりました。** 親のフォルダやアーカイブのタイルにもまとまって表示されます。
- **大きな圧縮ファイルや PDF のページ送りが、大幅に滑らかになりました。** 読み込みの実行数に上限を設け、要らなくなった読み込みを途中で止められるようにしました。圧縮ファイルの目次を開き直さず使い回し、中断も即座に効きます。あわせて画面を組み立てる処理の作り直しを減らし、メモリの確保方法も変えています。1 コマの描画が 5 ミリ秒未満に収まる割合が 91.5% から 99.0% になり、60 ミリ秒を超えるコマは 10 分の 1 に減りました。
