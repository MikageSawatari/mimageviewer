...
### v3.2.0 (2026-08-22)
- **エクスプローラーなどでコピー・移動したファイルに、元の編集内容を引き継げるようになりました。** これまでは、ファイルを別のフォルダへ移したりコピーしたりすると、そのファイルに付けていた補正・消しゴム・モザイク・注釈・トリミング・★・タグが失われていました。フォルダを開いたときに、以前編集したファイルと中身が同じものがあれば、引き継ぐかどうかを確認します。コピー元が複数見つかったときは、どれから引き継ぐかを選べます。確認が不要なら、環境設定 →「フォルダ」で切れます。
...
- **⚠️ 白黒・セピアの原稿を消しゴムで補完したとき、補完部分の色調を画像に合わせるようになりました。** 補完した部分だけ色味が浮くのを防ぎます。明るさは保ったままです。色調を合わせる対象を狭めたい場合は、消しゴムツールパネルの「色調の許容」を下げてください（既定 12、最小 1）。
- **⚠️ フォルダの代表画像を、一覧と同じファイル名順で選ぶようになりました。** これまでは一覧の並び順とは別の順番で選んでいたため、`00表紙.jpg` と `00表紙2.jpg` のようなファイルがあると、一覧の先頭とは違うものが代表画像になっていました。更新後の初回起動で一度だけ切り替わります。以前の順番に戻すには、環境設定 →「フォルダ」→「代表画像の選択基準」で「番号順（区切り無視）」を選んでください。
...
- **ごみ箱へ移すときの削除確認を、省略できるようになりました。** 環境設定 →「ファイル操作」にあります。既定はこれまでどおり確認を表示します。完全に削除される場合は、この設定に関わらず確認します。フォルダや書庫も省略の対象になるので、中身をまとめてごみ箱へ移すことになる点にご注意ください。
