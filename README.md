...
### v1.1.1
- **連結読みを横方向にも拡張**: 画像フルスクリーン中の <kbd>6</kbd> キーを「ページ単位 → 縦連結 → 横連結」のトグルに変更しました。<kbd>1</kbd>〜<kbd>5</kbd> の単ページ/見開き構成とは独立して動くため、単ページの縦/横連結だけでなく、見開き単位の縦/横スクロールも選べます。横連結は左→右 / 右→左の両方向に対応し、可視ページ数とGPU常駐ページ数の上限で極端なズームアウト時の大量表示を防ぎます
- **補正レイヤーの境界補助を復旧**: 境界筆・多角形ツールで <kbd>Ctrl</kbd> を押している間、検出されている境界を画像上に強調表示するようにしました。多角形ツールでは候補点を常時表示し、<kbd>Ctrl</kbd> 中は近くの輪郭線へ吸着します
- **補正レイヤーの手動マスク操作をラボ版に合わせて補完**: 多角形の <kbd>Enter</kbd> 確定 / <kbd>Ctrl</kbd>+<kbd>Z</kbd> 頂点戻し、<kbd>Esc</kbd> キャンセル、選択図形の <kbd>Delete</kbd> 削除・矢印移動・<kbd>[</kbd>/<kbd>]</kbd> 回転に対応しました
- **CBZ / CB7 アーカイブ対応**: コミック向けの CBZ（中身は ZIP）と CB7（中身は 7z）を開けるようにしました。CBZ は ZIP と同じく展開不要でそのままブラウズでき、CB7 は 7z と同じくクリックで ZIP に変換して閲覧します
...
- **縦読みモード**: 画像フルスクリーン中に <kbd>6</kbd> キー、または上部ホバーバーの表示モードボタンから、ページを縦方向につなげる縦読み表示へ切り替えられるようになりました。マウスホイール・<kbd>↑</kbd>/<kbd>↓</kbd>・<kbd>PageUp</kbd>/<kbd>PageDown</kbd>・ゲームパッド上下でスクロールでき、読書位置は中央に近いページとして記憶されます。可視ページと上下少数ページだけを読み込み、極端なズームアウトで大量ページを同時表示しない安全上限を入れています
- **ZIP／PDF／RAR／CBR／7z／LZH を開いたとき自動でフルスクリーン表示**: 一覧からこれらを開いたときに、ページ一覧ではなく1ページ目をそのままフルスクリーン表示する設定を追加しました（環境設定で切り替え）。<kbd>Ctrl</kbd>+<kbd>↑</kbd>/<kbd>↓</kbd> で次のファイルへ移ったときも同じように開きます
- **RAR / CBR アーカイブ対応**: RARLAB UnRAR を利用して、RAR / CBR をクリックで無圧縮 ZIP に変換して閲覧できるようになりました。パスワード付き RAR は入力ダイアログを表示します。入力したパスワード自体は保存せず、変換済み ZIP キャッシュはパスワードなしで保存されるため、不要になったキャッシュは「変換済みアーカイブキャッシュ管理…」から削除できます。分割 RAR は先頭パートから開き、後続パートは一覧で重複表示しません
