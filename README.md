...
### v3.6.0 (2026-09-06)
- **静止画・漫画のページシークバーに、サムネイル列とプレビューが加わりました。** シークバーの上へマウスを置くと、その位置のページがプレビューで出ます。見開きなら開いたときと同じ 2 ページを表示します。フィルム状のボタンを押すと、現在ページを中心にしたサムネイル列も出せます（既定は非表示）。列の上をクリックまたはドラッグすると、通常のページシークと同じようにそこへ移動します。列の高さは大 / 中 / 小 / 最小から選べ、固定表示中は画像へ重ならないよう領域を確保します。
...
- **動画のシークバーとプレビューの表示を選べるようになりました。** マウスオーバーの場面プレビューを「常に表示する」「場面サムネイルのストリップ表示中は表示しない」「常に表示しない」から選べます。ストリップ表示中に通常シークバーを隠すこともできます。どちらも音声波形の表示中は対象外です。
