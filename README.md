...
- **ZIP／PDF を開いたとき自動でフルスクリーン表示**: 一覧から ZIP／PDF を開いたときに、ページ一覧ではなく1ページ目をそのままフルスクリーン表示する設定を追加しました（環境設定で切り替え）。<kbd>Ctrl</kbd>+<kbd>↑</kbd>/<kbd>↓</kbd> で次の ZIP／PDF へ移ったときも同じように開きます
- **フォルダのコピー／カット／新規作成**: グリッドでフォルダもコピー／カット／削除の対象になり、右クリックメニューやエクスプローラーとの貼り付けでフォルダごと扱えるようになりました。背景の右クリックから「新しいフォルダ」を作成することもできます
