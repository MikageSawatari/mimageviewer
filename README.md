...
- **PDF / ZIP の最初のページ表示が大幅高速化**: フォルダ一覧のグリッドで PDF / ZIP のサムネ (1 ページ目 / 1 枚目) は既に作られているのに、その PDF / ZIP を開いたときに同じ画像をもう一度生成し直していました (PDF は PDFium 経由で 200〜500ms かかる)。今版からはフォルダ側のサムネをそのまま中身の 1 ページ目用キャッシュにコピーするようにしたので、Ctrl+↓ で多数の PDF / ZIP を含むフォルダを順に巡るときの体感が改善します
