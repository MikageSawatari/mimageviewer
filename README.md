...
### v2.5.0 (開発中)
- **消しゴムの大きな範囲の AI 補完精度を改善しました。** MI-GAN へ渡す画像を縦横に引き伸ばさず原寸の 512px 単位で扱い、深いマスクは周囲の既知画素側から 48px ずつ複数回に分けて補完します。画像端や大きなマスクで、周囲にない赤・青の斑点、線、物体状のアーティファクトが出にくくなりました。
- **補正レイヤーに修復／塗り効果を追加しました。** スポイト色での単色塗り、元の明るさを残した着色、周囲のテクスチャからの修復、コピー元を指定するクローンを、マスク付きの非破壊レイヤーとして使えます。周囲修復のパッチサイズは自動（高品質 10px・標準 12px・高速 14px）・標準 24px・大きめ 48pxから選べます。
- **編集結果をサムネイル一覧へ保持できるようにしました。** 消しゴム・補正レイヤー・隠蔽加工・テキスト／スタンプ・切り取りの結果を、編集終了時または別ページへの移動時に高品質な派生プレビューとしてバックグラウンド保存します。下地と注釈を分離し、色調補正は下地だけへ一覧表示時に適用してから注釈を合成します。最終AIアップスケールとポストフィルタはキャッシュへ含めません。既定は有効・上限 1GB で、環境設定の「パフォーマンス → キャッシュ」から ON/OFF と容量を変更できます。元画像と非破壊編集データは変更しません。
- **固定した本棚へ追加したときのちらつきを修正しました。** ツールバーの固定本棚を右クリックしてページを追加した際、本棚ボタンが一時的に消えてツールバーが組み直される問題を修正しました。
- **隠蔽加工のモザイク境界設定を変更した際のプレビューを修正しました。** 編集中ページの旧モザイクを即時失効させるとともに、目アイコンの押下状態を画像パイプライン解決前に反映し、新しく選んだ境界処理を長押し開始フレームから再合成するようにしました。
...
- **サブフォルダをまとめて表示**: フォルダバーの「サブ展開」で、現在のフォルダ以下にある画像と動画を一時的なフラット一覧として表示できます。★・タグ・場所などの絞り込みやスタック表示と組み合わせて、複数フォルダに分かれた項目をまとめて確認できます
