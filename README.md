...
### v0.9.0
- **動画ファイルをアプリ内でそのまま再生**: 動画ファイルをフルスクリーンでクリック (または Enter キー) するだけで、外部プレイヤーを起動せずにその場で再生できるようになりました。MP4 / MKV / MOV / AVI / WMV / MPG / MPEG / HEVC / AV1 など主要フォーマットを網羅 (FFmpeg avformat ベース)。動画は「開いたら一時停止」が既定で、Space または Enter で再生開始、もう一度押すと一時停止。GPU ハードウェアデコード (D3D11VA、H.264 / HEVC / AV1 / VP9) と GPU 直接描画に対応しており、4K 60fps 動画もカクつかず再生できます。リモートデスクトップなど DX12 が使えない環境では自動的に CPU 経路にフォールバック。従来通り `Shift+Enter` で外部プレイヤーを起動することもできます
- **動画の操作機能**: シーク / 音量 / ループの基本操作に加え、再生速度切替 (0.25 倍〜4 倍、ピッチ維持)、音量を最大 150% までブースト (過大出力は内蔵 safety limiter で抑制)、フレーム単位の前後送り、現在表示フレームのクリップボードコピー、シークバーホバーでの先行サムネイル表示、`P` キーで詳細パフォーマンス オーバーレイ表示に対応。動画ごとに再生位置が記憶され、次回開いたときに続きから再開できます。シーク粒度は <kbd>Shift</kbd>+矢印で 5 秒、<kbd>Ctrl</kbd>+矢印で 30 秒に切替。<kbd>↑</kbd>/<kbd>↓</kbd> はファイル間ジャンプに割り当て
- **動画タイル モード**: フルスクリーンで <kbd>S</kbd> キー (またはホバーバーの ▦ ボタン) で動画をタイル一覧モードに切替。多数の動画を並べて中身を一気に確認したい用途向け。<kbd>Ctrl</kbd>+ホイール で 6/10/16/20/26/30 列を循環、各タイルをクリックすると該当時点へシーク、もう一度押すとタイルから抜けて続きから再生します。タイル サムネイルは SQLite に WebP で永続キャッシュされ、再訪時はキャッシュから即座に表示
- **動画ブックマーク / ピン留め / サイドカー**: 動画再生中に <kbd>B</kbd> キーで現在位置にブックマーク。左ジャンプパネル (▶ ボタン) からサムネ + 時刻 + タイトル付きで一覧でき、✏ ボタンで名称を編集できます。ピン留めは現在表示中のフレームをグリッドサムネイルとして固定し、フォルダ一覧で動画の中身が分かりやすくなります。動画と同名の画像ファイル (例: `movie.mp4` と `movie.jpg`) が並んでいる場合、その画像をサムネに使うサイドカー機能も搭載。動画に埋め込まれたチャプター・タイトル・説明文は右パネルに自動表示され、チャプター行をクリックして頭出しできます
- **動画の自動再生モード**: 環境設定→動画タブで「自動再生しない / グリッドから開いた時のみ自動再生 / 常に自動再生」の 3 モードから選択可能。デフォルトは「開いたら一時停止」
...
- **見開き表示でも左右独立に画像補正**: 見開き表示中、明るさ / コントラスト / 色調などの補正を左右ページごとに独立調整できるようになりました。スキャン本などで左右ページの色味が異なる場合に便利。両ページに同じ補正を適用したい場合は ▶ / ◀ ボタンで一方のパラメータをもう一方にコピー
- **画像の長押しで補正前のオリジナルを確認**: フルスクリーン表示中に左クリックを長押しすると、補正適用前の元画像が一時的に表示されます。離すと補正後に戻ります。補正の効果を Before / After で素早く確認できます
...
- **フォルダ一覧の並び順の安定化**: フォルダ・ZIP・PDF のサムネをユーザー指定の並び順 (名前 / 更新日時 / サイズ) で並べ替えるよう統一しました (従来は名前順固定)。同じタイムスタンプのアイテムは名前順で安定ソートされます
