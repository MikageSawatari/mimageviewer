...
- **動画音声に VST3 プラグインを挿入できるように**: 動画再生中の音声を VST3 プラグインに通せるようになりました。複数プラグインを直列接続したチェーンに対応し、LUFS 測定 + EQ + コンプ等の組み合わせを動画再生しながらリアルタイムに分析できます。デフォルトは OFF で、環境設定→動画タブで「VST3 プラグイン処理を有効にする」をチェックすると、`%COMMONPROGRAMFILES%\VST3\` などからインストール済みのプラグインを自動スキャンします。各プラグインは別プロセスで動作するためアプリ本体への影響を抑えており、プラグイン側がクラッシュしても本体や他のプラグインは無事です。プラグイン GUI は常に最前面の別ウィンドウで開かれるため、動画を全画面再生しながらでも EQ カーブの調整や LUFS の確認ができます。V キーで全プラグイン GUI の一斉表示/非表示をトグル、管理ウィンドウからチェーンの順番入れ替えや個別バイパス・削除も可能です。アプリの起動中はプラグインインスタンスが保持されるので、動画を切り替えてもプラグインの状態がリセットされることはありません。プラグイン内部状態 (EQ カーブ等) は次回起動時には初期化されるので、毎回設定し直してください (継承は今後対応予定)
