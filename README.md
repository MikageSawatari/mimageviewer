...
- **動画音声に VST3 プラグインを挿入できるように**: 動画再生中の音声を VST3 プラグイン (LUFS 測定 / EQ / コンプ等) に通せるようになりました。デフォルトは OFF で、環境設定→動画タブで「VST3 プラグイン処理を有効にする」をチェックすると、`%COMMONPROGRAMFILES%\VST3\` などからインストール済みのプラグインを自動スキャンします。プラグインは別プロセスで動作するためアプリ本体への影響を抑えており、プラグイン側がクラッシュしても本体は無事です。プラグイン GUI は別ウィンドウで開いて V キーで一斉表示/非表示をトグルできるため、動画を見ながら EQ カーブの調整や LUFS の確認ができます。プラグイン状態 (EQ カーブ等) は次回起動時には初期化されるので、毎回設定し直してください (継承は今後対応予定)。アプリの起動中はプラグインインスタンスが保持されるので、動画を切り替えてもプラグインの状態がリセットされることはありません
