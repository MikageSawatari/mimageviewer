...
- **細かな使い勝手の改善**: A / B のクイックフォルダが、それぞれ「最後に見た場所」に加えてドライブごとの最後の場所も覚えるようにしました。お気に入りの登録上限を 100 件に増やしました（番号ですぐ開ける枠は 1〜20）。詳細表示・ツールチップの「長さ」「コーデック」列を音声ファイルにも対応させました
- **その他の改善・修正**: 動画のシークバーの上でもホイール操作が効くようにし、レーティング一覧から開いた本での戻る操作、見開きのページ移動、音声モード中のシーク / 前後移動 / 右クリック操作 / ブックマーク表示、音楽タイムラインの自動スクロールやシークなど、多数の不具合を修正しました
