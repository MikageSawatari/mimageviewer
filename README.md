...
### v3.6.0 (2026-09-06)
- **静止画・漫画のページシークバーに、サムネイル列とプレビューが加わりました。** シークバーの上へマウスを置くと、その位置のページがプレビューで出ます。見開きなら開いたときと同じ 2 ページを表示します。現在ページを中心にしたサムネイル列も出せます（既定は非表示）。下部バーのフィルム状ボタン、<kbd>Shift</kbd>+<kbd>S</kbd>、シークバーを上へドラッグのいずれでも切り替えられ、端の鍵アイコンで固定できます。列の上をクリックまたはドラッグすると、通常のページシークと同じようにそこへ移動します。列の高さは大 / 中 / 小 / 最小から選べ、固定表示中は画像へ重ならないよう領域を確保します。
