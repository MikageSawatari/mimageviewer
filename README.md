...
### v4.1.0 (2026-09-25)
- **表紙ありの見開きで、「表紙の次のページ」と「最終ページ」をそれぞれ単独で表示できるようになりました。** 同人誌などで表紙の次や最後のページが単独のページとして作られていて、左右の組み合わせがずれてしまう本に使います。単独にしたページは片側に置き、空いた側は隣のページと同じ大きさの白で埋めます。「末尾に表紙を添える」が有効なら、最終ページの空いた側には白の代わりに表紙を添えます。ページ数やページ送り、読書位置は変わらず、白い面は移動先にも書き出しにも含まれません。環境設定の「表示 → 閲覧表示」で全体の既定を、フルスクリーンの見開きメニューで本ごとに「全体設定に従う / オン / オフ」を選べます。表紙なしの見開きや単ページ表示では使われません。
- **見開きの「先頭・末尾の単ページを片側に配置」を、先頭と末尾で別々に選べるようになりました。** これまでの設定は、先頭・末尾の両方にそのまま引き継ぎます。本ごとの設定も「先頭」「末尾」それぞれで選べます。
- **★の評価を 1 段階ずつ上げ下げする操作を追加しました。** 操作カスタマイズの「評価を1段階上げる / 下げる」（「星1を付ける」などと同じく、アイテム用とコンテナ用があります）にキーを割り当てて使います（既定のキーは割り当てていません）。未評価の項目は★3 を基準に、上げると★4、下げると★2 になります。★5 より上、★1 より下には変わりません。チェックした複数の項目は、それぞれの評価から 1 段階ずつ変わり、まとめて元に戻せます。
- **シーク位置のプレビューの大きさを、動画と静止画で別々に選べるようになりました。** サムネイル列の高さと同じ最小〜最大の 5 段階で、各段階の大きさも環境設定で変えられます。既定の「大」はこれまでと同じ大きさです。
- **フォルダのサムネイルに、そのフォルダ直下の画像を優先して使うようになりました。** これまでは、直下に画像があってもサブフォルダ内の画像を先に選んでいました。直下に画像が無いときは、サブフォルダの画像に加えて、サムネイルを作成済みの ZIP・PDF も使います（フォルダのサムネイルのために ZIP・PDF を開くことはしません）。この変更のため、フォルダのサムネイルはこの版で最初に表示したときに 1 度だけ作り直します（手動で選んだ画像や動画のコマはそのまま残ります）。
- **複数ウィンドウモードで、RAR を開いた別ウィンドウから <kbd>Ctrl</kbd>+<kbd>↑</kbd> / <kbd>↓</kbd> で前後のフォルダへ移動できない問題を修正しました。** 別ウィンドウからの移動先にも、変換せずに読める RAR と、変換済みの書庫を含めるようにしました。
- **複数ウィンドウモードで、コレクションの画像を別ウィンドウで開くと、前後の移動とスライドショーが元のフォルダの順番で進んでいた問題を修正しました。**
- **サブフォルダの展開、アイテム検索の結果、コレクション、レーティングの一覧から開いた画像や、ファイル名スタックを切り替えたあとの画像で、消しゴムなどの編集結果が表示に反映されないことがある問題を修正しました。**

### v4.0.0 (2026-09-23)
- **離れた場所にある画像・動画・音声・フォルダ・本を、名前を付けた一覧にまとめられるようになりました。** 上部の「コレクション」→「コレクションの管理…」で名前付きの一覧を作り、ツールバーの「コレクション:」で追加先を選んで「追加」を押すと、チェック済みの項目（チェックがなければ選択中の項目）を登録します。**登録しても元のファイルは移動もコピーもしません。**元の場所への参照を並べて閲覧する機能で、ページをコピーして 1 冊を作る製本・本棚とは別のものです。よく使う一覧は管理画面で固定するとツールバーに名前のボタンが出て、左クリックで開き、右クリックでそこへ追加できます。コレクションの一覧から画像・動画・音声・フォルダ・ZIP・PDF を開くと通常どおりの閲覧に進み、本やフォルダから戻るとコレクション内の元の位置に戻ります。並び順は、ドラッグで自由に決める「手動順」、名前・番号・日付・サイズなどの「通常ソート」、順番を混ぜる「シャッフル」から選べ、前後の移動・スライドショー・動画や音声の連続再生も同じ順番に従います。項目を右クリックすると、登録だけを外す操作と、元ファイルをゴミ箱へ移す操作を区別して選べます。パスを並べたテキストファイルからまとめて取り込む機能と、一覧をテキストへ書き出す機能も用意しました。1 つのコレクションに登録できるのは 10,000 件までです。mIV Remote からは読み取り専用で閲覧できます。操作カスタマイズに「追加先のコレクションへ追加」と「コレクション1〜20を開く」を追加しました（既定のキーは割り当てていません）。
- **⚠️ この版にしてから最初の起動で、アイテム検索（<kbd>Ctrl</kbd>+<kbd>G</kbd>）の索引を作り直します。** 下記の変更で索引の形式が変わるためです。お気に入りで索引を有効にしている場所が大きいほど時間がかかり、作り直している間はアイテム検索の結果が揃いません。タグ・★評価・閲覧履歴・サムネイルキャッシュはそのまま残ります。
- **アイテム検索が、画像の生成に実際に使われた文言だけを探すようになりました。** これまでは、差し替え前の候補をすべて並べたテンプレートのような、生成に使われたとは限らない文字列まで検索の対象にしていたため、実際には使っていない語で画像が引っかかっていました。生成情報の表示でも、生成に使われたと確かめられるものを「プロンプト」、変換前の入力を「未解決の入力」として分けて表示します。索引の大きさも小さくなります。
- **アイテム検索の進み具合が見えるようになりました。** 検索中はアドレス欄と一覧に、確認した件数とヒット数を出します。これまでは「検索中」とだけ表示していたため、除外語を含む検索のように結果が出るまで時間がかかる場合に、止まっているのか処理中なのか分かりませんでした。
- **「重なる本」が大きな本で終わらない問題を修正しました。** ページ数の多い本ほど時間がかかり、8,000 ページを超えるフォルダでは 10 分近く CPU を使い続けた上、開き直すたびに最初からやり直していました。3,000 ページを超える本は調べる側・調べられる側のどちらでも対象から外し、候補のページ数の合計が 10,000 ページを超えた時点で打ち切ります。どちらの場合も理由を表示します。
- **一覧の並べ替えにファイルサイズ順が増え、名前順・番号順で降順を選べるようになりました。**
- **フォルダツリーの並び順を、一覧の並び順と別に選べるようになりました。**
- **Visual C++ ランタイムが入っていない Windows で、起動画面のまま応答がなくなる問題を修正しました。** AI 機能用の DLL を読み込めない環境で起きていました。必要なランタイムを配布物に同梱し、それでも読み込めない場合は AI 機能なしでそのまま起動を続けます。
- **設定の復元と完全リセットが、どちらも失敗することがある問題を修正しました。** リモート閲覧の機能がお気に入りの設定を読んだままにしていたため、設定ファイルを置き換えられませんでした。
- **TensorRT を使う設定のときに、AI 用のワーカーが繰り返し起動し直す問題を修正しました。**
- **検索結果から「フォルダに移動」したときに、移動先でその項目を選ぶようになりました。** 移動できなかったときや、別の移動で取り消したときに、終わった検索の行がフォルダの表示として残ることもなくなります。
- **見開きで、横長のページが単独になったときは先頭・末尾でも中央に表示するようになりました。** 前の版で追加した「見開きの先頭・末尾の単ページを片側に配置」が、相方のいないページだけでなく、横長のため単独になったページにも掛かっていました。
- **「類似」パネルが、索引の更新中に表示位置がずれる問題を修正しました。** 更新中であることの表示をやめ、行が動かないようにしました。
- **お気に入りの画面を開いたままにしていると、何も処理していなくても CPU を使い続ける問題を修正しました。**
- **割り当てを変更できないキーを操作カスタマイズで選んだときの説明が、英語で出ていたのを日本語にしました。**
- **複数ウィンドウモードで、画像や ZIP を開いたウィンドウが表示される前に閉じてしまい、開けないことがある問題を修正しました。**
- **複数ウィンドウモードで読み込みに時間のかかるものを開いたとき、すぐにウィンドウを開いて「読込中...」と表示するようにしました。** これまでは読み込みが終わるまでウィンドウが出ず、操作が効いていないように見えていました。
- **複数ウィンドウモードで、見開きの先頭・末尾の単ページを片側に寄せているとき、別のウィンドウを選ぶと単ページが中央へ移り、戻すとまた寄っていた問題を修正しました。**
- **PDF の表示エンジンを更新しました。**

### v3.10.0 (2026-09-14)
- **右クリックメニューの項目を、並べ替えたり隠したりできるようになりました。** 環境設定の「表示 → 右クリックメニュー」で、一覧とフルスクリーンで共通に使う mImageViewer の項目を編集できます。「アプリケーションで開く…」の中の項目も同じ画面で扱えます。項目の前に区切り線を出すかどうかも指定でき、「確認する表示場面」を選ぶと、ファイル・フォルダの余白・検索結果・PDF 内ページ・フルスクリーンなどで実際にどう見えるかを確かめられます。これまでの「表示 → メニュー構成」は「表示 → 通常メニュー」に名前が変わり、メニューバー側の設定はそちらに残っています。Windows の項目、登録した外部ツール、関連付けアプリの並び順は変わりません。
- **画像を全画面で見ているときに <kbd>Ctrl</kbd>+<kbd>↑</kbd> / <kbd>↓</kbd> でフォルダを移動すると、画面がちらつく問題を修正しました。** v3.9.0 以降の設定の復元の実装時に、移動する間に別の表示が挟まるようになってしまっていました。
- **切り取り中のファイル・フォルダが、一覧で半透明になりました。** Windows のファイル操作で切り取った項目が、サムネイル表示でも詳細表示でも半透明になり、ハサミの印が付きます。どれを移動しようとしているか一目で分かります。貼り付けが終わるか、コピーや別の切り取りでクリップボードが置き換わると元に戻ります。選択枠・チェック・カーソルはそのまま見えます。
- **見開きの先頭・末尾で 1 ページだけになるページを、本来の側へ置けるようになりました。** たとえば右から左へ読む本では、先頭の表紙を左側に置き、反対側を空けて表示します。これまでは中央に表示されていました。既定は OFF で、環境設定の「表示 → 閲覧表示」→「見開きの先頭・末尾の単ページを片側に配置」で切り替えます。本ごとにフルスクリーンの表示モードメニューから選ぶこともできます。ページ数・ページ送り・シークバー・読書位置・編集対象は変わりません。画像フォルダ・ZIP・PDF、連結読み、<kbd>F12</kbd> の別ウィンドウ、mIV Remote で共通です。
- **通常の動画を、画面に収まる大きさより小さく縮小できるようになりました。** フルスクリーンで <kbd>V</kbd> を押したときの拡大表示が 10%〜1600% になり、縮小したままでも位置を動かせます。拡大・縮小表示の間は、画面の端まで動画を動かせるように左右のパネルと呼び出し部分を一時的に隠します。パネルの開閉・固定の設定は保たれ、<kbd>V</kbd> で終了すると元に戻ります。
- **切り取りモードで、枠やハンドルの上からでも新しい範囲を引けるようになりました。** <kbd>Ctrl</kbd> を押しながら左ドラッグすると、既存の枠を動かさずに新しい切り取り範囲を作れます。初めて範囲を決めるときに、画像全体の枠を掴んでしまうことがなくなります。このキーは操作カスタマイズで変更・無効化できます。<kbd>Space</kbd> の一時パンと同時に押したときはパンを優先します。
- **ワイプ比較の境界線とハンドルを、常に表示するようになりました。** これまではカーソルを近づけたときだけ現れていたため、境界を動かして見比べられることに気づきにくい状態でした。<kbd>Ctrl</kbd> を押している間は、これまでどおり線を隠して画像だけを見られます。
- **別バージョン索引を使っているとき、起動直後の確認にかかる時間を短縮しました。** 前回から変わっていない ZIP・PDF を開き直さずに済ませます。約 465 万件を対象にした環境では、起動時の確認が約 6 分半から約 40 秒になりました（環境によって異なります）。

### v3.9.1 (2026-09-12)
- **フォルダを初めて開いたときに、数秒間操作できなくなることがある問題を修正しました。** これまでフォルダを表示した後にバックグラウンドで行っていた旧バージョンのタグの確認が、前の版から、操作できるようになる前に行われるようになっていました。フォルダによっては数秒かかります。この自動確認そのものをやめたので、待ち時間はなくなります。
- **「サイドカーから設定を復元中」が、実際には復元していないときにも表示されていたのを直しました。** 処理の段階に応じて「保存中の設定を確定中」「保存されている設定を確認中」「サイドカーから設定を復元中」を出し分けます。すぐに終わるときは、これまでどおり何も表示しません。
- **旧バージョンがファイルのキーワード欄に保存していた `#` 付きタグの、自動取り込みを終了しました。** すでにアプリ内のタグとして取り込まれているものはそのまま残ります。通常のタグ、サイドカーへのバックアップと復元、レーティングや生成情報など他のメタデータの読み書きは変わりません。
- **PDF の表示エンジンを更新しました。**

### v3.9.0 (2026-09-12)
- **同じ絵の別バージョンを探せるようになりました。** 調べたい場所をお気に入りに登録し、「お気に入り」→「編集」でその行の「別バージョン索引」を ON にすると (既定は OFF)、静止画をフルスクリーンで開いて右パネルの「類似」を選べるようになります。大きさや JPEG・PNG・WebP などの形式が違っていても、同じ絵の候補を「ほぼ同一」と「別バージョン」に分けて並べ、行を押すとその画像のある場所へ移動します。画像単位と本単位の両方で探せ、本単位では一致する範囲がページ帯で分かるので、本の一部が別の本に含まれている場合でも見つけられます。索引はあとからのファイルの追加・変更・削除を自動で追い、チェックを OFF にするとその場所の索引データを削除します。
- **サイドカーファイルのあるフォルダを初めて開いたときに、アプリ全体が数十秒固まる問題を修正しました。** 補正・消しゴム・注釈などの復元をバックグラウンドへ移しました。すぐに終わるときは何も出さず、時間がかかるときだけ復元中であることを表示します。新しく使い始めたときや、ポータブル版の初回起動で起きていました。
- **動画をフルスクリーン表示しているときに ♪ ボタンで音声モードへ切り替えると、再生が終了してフルスクリーンも閉じてしまう問題を修正しました。** VST3 プラグインを使わない設定 (既定) で起きていました。ウィンドウ内表示では問題なく切り替わっていたため、気づきにくい状態でした。
- **右クリックメニューを閉じた後の左クリックで、同じメニューが出直して抜けられなくなる問題を修正しました。** 操作カスタマイズで、画像フルスクリーンの右クリック短押しにメニュー表示を割り当て、右ドラッグを未使用にしているときに起きていました。
- **動画のサムネイル列が中央半分の幅になったり、操作バーを出し入れしているうちにメモリ使用量が大きく増えたりする問題を修正しました。** 操作バーが一時的に隠れるたびにサムネイル列の準備を作り直していたのをやめ、再表示したときはそのまま使います。
- **お気に入りの索引を作る・更新するときの待ち時間を短縮しました。** フォルダ数の多い場所で起動後に行う照合を見直し、変わった範囲だけを調べるようにしました。ZIP の中身を読み直す回数も減らしています。
- **サムネイル画質の設定ダイアログが、開いた直後に画面下端まで伸びてしまう問題を修正しました。**
- **ツールバーにピン留めしたタグをもう一度押すと、タグの一覧表示を閉じて元の画面へ戻るようになりました。** 別のタグを押したときは、閉じずにそのタグへ切り替わります。右クリックの付与・解除は変わりません。
- **一覧で複数選択したときの件数表示を半透明にし、位置を少し下げました。** 右上のサムネイルのチェックマークが隠れにくくなります。

### v3.8.0 (2026-09-11)
- **表紙あり見開きで、最後のページに表紙を添えて表示できるようになりました。** 先頭と末尾がどちらも単独ページになる本で、末尾のページの隣へ表紙を並べます。ページ数・ページ送り・シーク列・読書位置は変わらず、添えた表紙は移動先になりません。既定は ON で、環境設定の「表示 → 閲覧表示」→「末尾に表紙を添える」で全体を切り替えられます。本ごとに変えたいときは、フルスクリーンの見開きメニューで「全体に従う / オン / オフ」を選びます。画像フォルダ・ZIP・PDF、連結読み、<kbd>F12</kbd> の別ウィンドウ、mIV Remote で使えます。
- **動画のシークバーにカーソルを乗せてもプレビューが出ない問題を修正しました。** 前の版で下部の操作バーに加えた表示範囲の制限が、バーの上に出すプレビューにも掛かり、絵を丸ごと切り取っていました。サムネイル列を表示していても、隠していても出るようになります。
- **見開きでページ送りが「読み込み中」のまま止まることがある問題を修正しました。** 縦横がまだ分からない画像を 2 ページと見込んで移動を始めた後、横長だと分かって 1 ページ表示へ変わると、移動の完了を待ち続けていました。ホイールのページ送りだけが効かなくなり、操作バーや別ウィンドウの動画は動いたままでした。
- **画像・動画の余白色を選べるようになりました。** 環境設定の「表示 → 閲覧表示」にあります。黒 / 灰 / 白をすぐに選べるほか、任意の色も指定できます（既定は黒）。静止画・本の画像の外側、静止画のサムネイル列の空欄、動画の映像の外側（レターボックス）に共通で使います。単ページ・見開き・連結読み・比較、フルスクリーン・ウィンドウ内表示・<kbd>F12</kbd> の別ウィンドウで共通です。透過画像の下に敷く黒 / 白 / 市松（<kbd>Shift</kbd>+<kbd>B</kbd>）、動画のシークバー上の表示（サムネイル列・音声波形）、音声画面、操作パネルは変わりません。
- **動画の戻る / 進むボタンとホイールクリックへ、小・中・大のシークを割り当てられるようになりました。** 秒数はキー操作と共通の設定を使います。押し続けると Windows のキーリピート設定に合わせて繰り返し、離すと止まります。マウスのボタンをキーボード入力へ変換するソフトを使っている場合は、そのソフトが送る入力の回数に従います。
- **静止画のサムネイル列の右端に、ボタン専用の黒い空欄ができていたのをやめました。** 列を固定する鍵と、通常バーを隠したときのフィルム状ボタンをサムネイルの上へ重ねて表示し、列の幅を最後まで使います。動画のサムネイル列と同じ見え方になります。
- **動画で <kbd>V</kbd> を押しても拡大表示に切り替わらないことがある問題を修正しました。** キーをどのウィンドウで受け取ったかによって処理が分かれ、通常の動画の拡大表示へ届かない経路がありました。

### v3.7.0 (2026-09-09)
- **詳細表示から開いた静止画・本で、シークのサムネイル列やプレビューが読み込まれない問題を修正しました。** サムネイル表示へ切り替えなくても、そのままページを確認できます。
- **「サムネイル画質」の画面が、詳細表示や狭いウィンドウで崩れる問題を修正しました。** プレビューと操作ボタンを画面内に収め、必要に応じてスクロールできます。
- **横長画像を表示していると、操作を止めても CPU 使用率が下がらないことがある問題を修正しました。** 同じサムネイルの高画質化を繰り返していました。
- **動画の長いタイトルが、上部の操作ボタンへ重なる問題を修正しました。** ボタンの手前でタイトルを省略します。
- **静止画・本・動画のサムネイル列に高さ「最大」を追加しました。** 大 / 中 / 小 / 最小と合わせて、5段階から選べます。
- **静止画・本のサムネイル列を、閲覧中のメニューから切り替えられるようになりました。** 右下のフィルム状アイコンから、表示 / 非表示と5段階の高さを選べます。
- **サムネイル列の各サイズの高さを、数値で指定できるようになりました。** 最大 / 大 / 中 / 小 / 最小それぞれに36〜320px相当の高さを設定できます。静止画・本用と動画用は別々に設定でき、Windowsの表示倍率に合わせて画面上の高さも変わります。
- **動画・音声の小・中・大シークの秒数を設定できるようになりました。** 既定は1秒 / 5秒 / 30秒で、それぞれ1〜600秒から選べます。
- **動画・音声の通常ホイール操作を、前後の項目移動と音量調整から選べるようになりました。** シーク列やパネルのスクロール、動画の拡大表示などは、引き続きそれぞれの操作を優先します。
- **動画の拡大表示やシーク列、パネルをまたぐホイール操作が、重複したり取りこぼされたりする問題を修正しました。**
- **見開きの次の両ページへ、AI・カラー化の先読みが届くようにしました。** 新規設定の既定は前方3枚 / 後方2枚です。旧版からの更新時には前方2枚の設定を一度だけ3枚へ変更し、保存済みの後方値は維持します。更新後も前方2枚へ戻せます。
- **音量ノーマライズの音声解析で、音声以外のデータを処理する無駄を減らしました。**

### v3.6.0 (2026-09-07)
- **静止画・漫画のページシークバーに、サムネイル列とプレビューが加わりました。** シークバーの上へマウスを置くと、その位置のページがプレビューで出ます。見開きなら開いたときと同じ 2 ページを表示します。現在ページを中心にしたサムネイル列も出せます（既定は非表示）。下部バーのフィルム状ボタン、<kbd>Shift</kbd>+<kbd>S</kbd>、シークバーを上へドラッグのいずれでも切り替えられ、端の鍵アイコンで固定できます。列はセルをクリックするとそのページへ移動し、横へドラッグすると動画と同じく帯そのものが左右に動きます。列の高さは大 / 中 / 小 / 最小から選べ、固定表示中は画像へ重ならないよう領域を確保します。
- **通常の動画を拡大表示できるようになりました。** 再生したまま <kbd>V</kbd> を押すと拡大表示に入り、ホイールでポインタの位置を保ったまま拡大縮小、左ドラッグで表示位置を移動できます。上部バーに現在の倍率とリセットボタンが出ます。シークストリップや左右のパネルの上ではそれぞれの操作を優先します。再生・シーク・音量・画像補正は拡大中もそのまま使えます。別の項目へ移ると解除され、倍率は次の動画へ持ち越しません。
- **右クリックメニューの先頭に「切り取り」「コピー」が加わりました。** 実ファイル・実フォルダで、一覧でもフルスクリーンでも出ます。チェック済みがあれば全件が対象になり、件数も表示します。移動先の何もない場所を右クリックして「貼り付け」すれば、マウスだけでファイルの移動・複製ができます。<kbd>Ctrl</kbd>+<kbd>X</kbd> / <kbd>Ctrl</kbd>+<kbd>C</kbd> は操作カスタマイズで変更・解除できるようになりました。
- **お気に入りごとに、一覧と閲覧の表示状態を記憶できるようになりました。** 動画の場所は 16:9 の小さめサムネイル、漫画の場所は縦長の大きめサムネイル、といった使い分けが自動になります。記憶するのはサムネイル / 詳細の別、サムネイルサイズと比率、カテゴリ表示順とソート、単ページ / 見開き、連結方式です。環境設定 →「表示」→「サムネイル」から ON にします（既定は無効）。
- **外部ツールにスクリプトを登録できるようになりました。** Windows の関連付けを使って起動します。引数と作業フォルダーを自分で指定でき、出力を読みたいときは「コンソール窓を表示する」を有効にできます。
- **動画のシークバーとプレビューの表示を選べるようになりました。** マウスオーバーの場面プレビューを「常に表示する」「場面サムネイルのストリップ表示中は表示しない」「常に表示しない」から選べます。ストリップ表示中に通常シークバーを隠すこともできます。こちらは音声波形のストリップにも効き、隠したぶん映像が広がります（再生や音量の操作はそのまま残ります）。プレビューの設定は、場面サムネイルのストリップだけを対象にします。
- **一覧に出していない書庫が、設定によるものだと分かるようになりました。** RAR / 7z / LZH の処理を「無視する」にしていると、その件数が「非表示 N 件」の内訳に分けて表示されます。これまでは「対象外」に混ざり、同名ファイルの設定を疑う原因になっていました。内訳のリンクからは、原因になっている設定の項目まで直接移動します。
- **書き出しの切り取り範囲を保存したページに「切」バッジが出るようになりました。** 親のフォルダやアーカイブのタイルにもまとまって表示されます。
- **大きな圧縮ファイルや PDF のページ送りが、大幅に滑らかになりました。** 読み込みの実行数に上限を設け、要らなくなった読み込みを途中で止められるようにしました。圧縮ファイルの目次を開き直さず使い回し、中断も即座に効きます。あわせて画面を組み立てる処理の作り直しを減らし、メモリの確保方法も変えています。1 コマあたりの処理が 5 ミリ秒未満に収まる割合が 91.5% から 99.0% になり、60 ミリ秒を超えるコマは 10 分の 1 に減りました。
- **<kbd>Ctrl</kbd>+<kbd>F</kbd> の AI プロンプト検索が速くなりました。** これまでは画像を丸ごと読んでいましたが、必要な範囲だけを読むようにしました。あるフォルダでは 1 枚あたり 3.3 MB 読んで使うのは先頭 3 KB でした。あわせて複数のファイルを並行して調べるようにしています。
- **JPEG を開くときの無駄な処理を省き、表示を速くしました。**
- **動画のシークストリップで「全体表示」の埋まり方が速くなりました。** 長い動画ほど効きます。
- **環境設定で表示の既定を変えると、お気に入りの中にいるときだけその場所の設定になってしまう問題を修正しました。** カテゴリ表示順、単ページ / 見開き、連結方式が対象です。 同じダイアログが、開いた場所によって違う意味を持っていました。環境設定は共通の設定を決める場所なので、どこで開いても共通の設定を変更します。
- **一括書き出しが、自分で書き出したファイルに反応して途中で止まる問題を修正しました。** 書き出し先を監視中のフォルダにしていると、1 つ目を書いた直後にフォルダの再読み込みが起き、残りが書かれないまま終了していました。隠蔽加工のプリセットを 5 つ選んで 1 つしか出ない、といった状態でした。
- **アニメーションではない画像で「アニメーション読み込み中」と表示されることがある問題を修正しました。**
- **シークバーをドラッグしている間に引っかかる問題を修正しました。** 動かすたびに先読みが大量に積まれていたのをやめました。

### v3.5.0 (2026-09-03)
- **外部ツール連携を追加しました。** よく使うアプリを登録しておくと、右クリックメニューやキー 1 つで、表示中の画像・動画・ページをそのアプリで開けます。圧縮ファイルや PDF の中のページも、取り出した一時ファイルとして渡せます。渡すものは「編集を反映した一時ファイル」「編集前の一時ファイル」「元のファイル」から選べ、見開きは 1 枚に合成することも両ページを渡すこともできます。複数選んだときの動作（1 件ずつ起動 / まとめて渡す）や、確認を出す件数も決められます。登録は 環境設定 →「起動と連携」→「外部ツール」から、実行ファイルを選ぶか、拡張子の関連付けアプリから選べます。詳しくはマニュアルの「外部ツール連携」をご覧ください。
- **右クリックメニューを Windows と同じ見た目に統一しました。** 実ファイル・実フォルダだけでなく、ZIP や PDF の中のページでも同じメニューが出ます。Windows の項目は既定で末尾の「Windows のメニュー」にまとめ、開くまで読み込まないので最初の表示が速くなりました。環境設定で従来どおり同じ階層へ並べることもできます。あわせて「新しいフォルダ…」と「名前の変更…」を追加しました。mImageViewer 側から行うと、作成後・変更後の一覧更新と選択の復元まで反映されます。
- **シークストリップに「全体表示」が加わりました。** 動画全体の場面サムネイルや音声波形を、帯の幅に収めて表示します。どこに明るい場面や暗い場面があるか、どこで音が続いているかを、再生を止めずに一目で見渡せます。帯の高さも大 / 中 / 小 / 最小の 4 段階から選べます。シークバー横のフィルム形ボタンはメニューになり、表示の種類と高さを直接選べます。<kbd>Shift</kbd>+<kbd>S</kbd> で巡回する表示は環境設定で絞れます。表示の種類・範囲・高さは次回起動時も引き継ぎます。
- **長い動画の音声波形が、次に開いたときは待たずに表示されるようになりました。** 10 分以上の範囲で表示している間に解析した結果を保存します。解析が進むのは波形を表示している間だけで、閉じると止まります。
- **情報パネルを固定できるようになりました。** パネルのタイトルバー右端にある鍵ボタンで固定すると、前後のファイルや本のページへ移動してもパネルが開いたままになり、中身だけが切り替わります。固定中は画像に重ならず、画面の右側にパネルの幅だけ領域を確保します。タグを付けながら次々送るような使い方ができます。動画のメタ情報パネルと音楽情報パネルにも同じ鍵ボタンがあります。
- **一覧から複数の画像をまとめて書き出せるようになりました。** サムネイル一覧で選んで <kbd>Ctrl</kbd>+<kbd>E</kbd> を押すと、一括エクスポートのダイアログが開きます。ファイル名はテンプレートで決められ（元の名前・元があったフォルダの名前・連番を差し込めます）、形式と出力サイズも指定できます。補正・消しゴム・補正レイヤー・隠蔽加工・切り取り・回転・テキスト注釈を反映した状態で書き出します。
- **書き出しにどこまで反映するかを、出力ごとに選べるようになりました。** 製本・エクスポート (まとめて)・外部ツールのそれぞれについて、「編集」「AI 処理」「表示用補正」の 3 段から選べます。「編集」は補正・回転・注釈・消しゴム・隠蔽加工・切り取りまで、「AI 処理」はそれに AI 拡大とノイズ除去を加えたもの、「表示用補正」はさらにスマートシャープ・カラー化・LUT・ポストフィルタまで反映します。後から別のソフトで加工するなら浅い段、そのまま見せる画像を作るなら深い段が向いています。環境設定 →「表示」→「書き出しの焼き込み」から設定します。既定はこれまでと同じ動作です。エクスポート (1 枚) と、外部ツールへ見開きを 1 枚に合成して渡す場合は、表示している結果をそのまま書き出します。
- **編集内容をまとめて貼り付け・まとめてリセットできるようになりました。** 一覧でチェックした複数のページへ、コピーした編集内容を一度に貼り付けられます。貼り付け先ごとに画像サイズへ合わせて変換するので、サイズが違っても使えます。リセットは消す種類（個別補正・消しゴム・モザイク・補正レイヤー・テキスト注釈・切り取り・回転）を選べます。★とタグは消しません。
- **レーティング一覧に「★設定時刻」列が加わりました。** 列ヘッダから古い順・新しい順に並べ替えられます。
- **グリッドの表示順を種類ごとに決められるようになりました。** 実フォルダ / アーカイブ類 / 画像 / 動画・音声を 4 行のどこへ表示するか選べます。同じ行にしたものは現在のソート順で混ざります。
- **360 度表示が、続けて開いた 360 度の素材でも保たれるようになりました。** 一覧へ戻ったりフォルダを移動したりしても、次に開いたものが 360 度の素材ならそのままパノラマビューで表示します。投影方式もソフトを終了するまで覚えています。
- **ゲームパッドの操作を受け付けないようにできます。** 誤って触れてしまうときに、操作カスタマイズ →「設定」→「ゲームパッド」から無効にできます。
- **見開きのページの間隔が、設定したとおりの太さで表示されるようになりました。** これまでは拡大表示しているときなど一部の条件で、1 ドットほど広く、または狭く見えることがありました。連結読みでページとページの間隔がばらつくことも直しています。
- **端数の倍率で拡大したときに、画像が少しぼやける問題を修正しました。**
- **最大化したまま起動したあと、<kbd>F11</kbd> の全画面表示が画面の左上に小さく出る問題を修正しました。**
- **切り取りで比率を固定しているとき、枠の外からドラッグして新しい枠を引くと、押した場所が枠の角に固定されずに動く問題を修正しました。**
- **切り取り枠をドラッグで引いている途中でポインタが画像の外へ出ると、画像内へ戻してもそれ以上追従しなくなる問題を修正しました。**

### v3.4.0 (2026-09-01)
- **1 枚の絵を 2〜4 枚に切り分けて書き出せるようになりました（SNS 分割）。** X や Instagram に複数枚として投稿し、横にめくると全体で 1 枚の絵に見える投稿を作るためのものです。全画面で画像補正パネルのいちばん右のアイコンから始めます。**選んだ範囲を枚数で等分する**ので、絵全体をそのまま分けることも、一部だけを切り出すこともできます。「画像全体に合わせる」で絵の全域を一度に選べます。X は画像と画像の間に隙間が入るため、既定ではその裏に隠れる帯（枠の幅の 1.7%）を取り除いて継ぎ目が繋がるようにします。取り除かずに等分することも、割合を指定することもできます。枠の比率は自由で、3:4 / 4:5 / 1:1 に固定することもできます。Instagram は表示できる比率に制限があるため、範囲外のときは書き出す前にお知らせします。ファイル名の末尾に `_1` から順に番号が付くので、その順に添付してください。回転しているページでは使えません。
- **動画のシークバーが自動で隠れただけで、サムネイル列や音声波形の表示が「なし」に戻る問題を修正しました。** 出し直すたびに波形の解析がやり直しになり、長い動画では数秒待たされていました。同じ動画を見ている間は、選んだ表示と解析結果をそのまま保ちます。
- **360 度表示のとき、サムネイル列や音声波形の上でもホイールが視野角の操作になる問題を修正しました。** ストリップの上ではストリップの時間範囲が変わります。
- **上下のバーを固定表示にしているとき、動画を開いた直後だけ全域に映ってから縮む問題を修正しました。**
- **見開きを縮小して表示したとき、ページの間隔が設定より広くなることがある問題を修正しました。** 1px に設定していても、画像によっては 2px 空いていました。左右のページをそれぞれ別に画面のドット位置へ合わせていたため、合わせたあとの間隔が設定と変わっていたものです。
- **見開きで、もう一方のページの GIF アニメーションが止まる問題を修正しました。** 一覧から 1 ページ目を開くと 2 ページ目が動かず、何度か行き来すると動き出す状態でした。
- **見開きで範囲コピーすると、選んだ場所と違うところがコピーされる問題を修正しました。** 余白の自動カットが効いているときに、選べる範囲と実際にコピーされる内容が、カットした分だけずれていました。
- **切り取りで比率を固定すると、辺や角をドラッグしても大きさが変わらない問題を修正しました。** 掴んだ場所と反対側の辺・角を固定して拡大縮小します。これまでは大きさが変わらず、ドラッグした量の半分だけ位置がずれていました。
- **補正レイヤーの筆が大幅に速くなりました。** 24 メガピクセルの画像で、塗っている間の表示が毎秒 8.5 コマから 73 コマになりました（約 11 倍）。1 コマ描くたびに編集中の画像を丸ごと複製していたのが原因でした。あわせて編集内容の保存をバックグラウンドで行うようにしたので、保存のたびに操作が止まることもなくなりました。
- **補正レイヤーの図形をキーボードで動かすと、取り消し履歴が押した回数分たまる問題を修正しました。** 矢印キーを押したままにすると自動リピートのたびに 1 件ずつ記録され、<kbd>Ctrl</kbd>+<kbd>Z</kbd> を何度も押さないと戻せませんでした。一続きの操作を 1 件にまとめます。あわせて <kbd>Esc</kbd> は、キーボードでの移動・回転を取り消さずに確定するようになりました。これまでは手を止めてから約 1.2 秒の間だけ取り消され、それを過ぎると何も起きないという、画面では区別できない違いがありました。取り消しは <kbd>Ctrl</kbd>+<kbd>Z</kbd> で行ってください。マウスでドラッグしている最中の <kbd>Esc</kbd> は、これまでどおりその操作を中止します。
- **補正レイヤーで、中身のないマスクが作られて保存データが膨らむ問題を修正しました。** 何も変わらなかった操作でも、画像と同じ大きさの空のマスクが残り、次の保存で書き出されていました。何も塗れなかった操作が取り消し履歴に入ることもなくなりました。

### v3.3.1 (2026-08-30)
- **画像を縮小して表示したときのボケを直しました。** 画面に合わせて縮小するとき、きれいに縮小した結果をもう一度引き伸ばして描いていたため、細かい線や文字がにじんでいました。他のビューアと比べて明らかに眠い、という報告をいただいて調べたものです。全画面・別ウィンドウ・連結して読む表示・見開き・分析モードのすべてで直っています。回転していても、Windows の表示スケールを 125% や 150% にしていても同じです。
- **前回終了したときのカーソル位置を復元できるようになりました。** 前回いた場所を開き直したとき、選んでいた項目にカーソルを戻し、画面の同じ高さに表示します。既定で有効です。環境設定 → 起動と連携 → 起動時の動作 で切り替えられます。ウィンドウの大きさや列数を変えていても、同じ見え方になるように位置を計算し直します。項目が無くなっていたときは先頭を選びます。
- **編集内容が保存できていないのに、保存できたことにしていた問題を修正しました。** 補正レイヤー・消しゴム・隠蔽加工・切り取り・テキスト注釈・画像補正で、保存先を開けなかったときにエラーも出さずに続けていました。次回起動時に古い内容が正しいものとして扱われ、**編集が消えたように見えることがありました。**
- **別ウィンドウ（<kbd>F12</kbd>）を続けて押すと、動画の再生が止まってウィンドウが閉じてしまう問題を修正しました。** 内部の処理で送っていたウィンドウを閉じる指示が、次に開いたウィンドウへ届き、利用者が × を押したのと同じ扱いになっていました。
- **別ウィンドウを開いているとき、ゲームパッドの操作が別のウィンドウへ届くことがある問題を修正しました。**
- **mIV Remote で、見開き表示を開くのが遅い問題を修正しました。** ページの縦横を調べるためにファイルを 1 つずつ順番に読んでいました。まとめて並行して読むようにしています。枚数が多い場所ほど効きます。
- **動画のシークバー上の表示を音声波形からサムネイル列に切り替えた後も、見えないところで波形の解析が続いていた問題を修正しました。** 長い動画では、再生やサムネイルの読み込みが遅くなる原因になっていました。
- **ツールバーの項目がウィンドウの右端からはみ出すことがある問題を修正しました。** ウィンドウの幅を変えたときに、ソートや比率などの選択欄が次の行へ移らずに切れて表示されていました。
- **補正レイヤーの図形を削除するキーを、変更できるようになりました。** これまで <kbd>Delete</kbd> で固定されていて、操作カスタマイズの一覧にも出ていませんでした。消しゴムや隠蔽加工と同じように扱えます。
- **貼り付けに失敗したとき、関係のないファイルが選ばれることがある問題を修正しました。** 貼り付けがその場で失敗しても、その後 10 秒の間にフォルダへ増えたファイルを貼り付けた結果として選んでいました。
- **大量のファイルを貼り付けたとき、途中までしか選ばれないことがある問題を修正しました。** 置き換えの確認などでコピーが途中で止まると、その後に届いたファイルが選択に入りませんでした。選択を自分で変えたときは、そこから先は上書きしません。
- **VST3 プラグインを使っているとき、設定の保存が重くなる問題を修正しました。** プラグインの状態を保存のたびに複製していました。
- **設定のバックアップが増え続ける問題を修正しました。** 更新時に作られるバックアップを消していなかったため、少しずつ溜まっていました。古いものから一定数だけ残します。

### v3.3.0 (2026-08-29)
- **横長のページを、左半分と右半分に分けて読めるようになりました。** 見開きでスキャンされたページが 1 枚の横長画像になっている本を、片側ずつ画面いっぱいに表示して読み進められます。全画面で <kbd>8</kbd> （左→右）または <kbd>9</kbd> （右→左）を押すか、上部のバーから選んでください。分割してもページ番号は変わらず、見開きのペアも変わりません。縦につないで読む表示や、回転したページでも使えます。mIV Remote でも同じように使えます。
- **動画のシークバーの上に、サムネイルの列と音声の波形を出せるようになりました。** <kbd>Shift</kbd>+<kbd>S</kbd> で、なし → サムネイル列 → 音声波形 の順に切り替わります。マウスを乗せた位置のプレビューはシークバーと共通で、ホイールで表示する時間の範囲を段階的に変えられます。右上の鍵アイコンで、映像に重ねずに固定表示できます。
- **360 度動画を、視点を動かしながら見られるようになりました。** 球面メタデータを持つ正距円筒の動画や、横：縦が 2:1 の動画で、上部バーの「360」ボタンから切り替えられます。ドラッグで視点を動かし、ホイールで視野角を変えられます。再生・一時停止・シーク・音量・画像補正はそのまま使え、前後のファイルへ移っても 360 度表示は続きます。
- **360 度表示の投影方式を 4 つから選べるようになりました。** 透視投影（既定）・立体射影・等距離射影・等立体角射影を、上部バーの投影ボタンから選べます。画面の隅ほど写り方が変わり、方式によって広げられる視野角も変わります。静止画・動画のどちらでも使えます。
- **⚠️ 最大化したまま終了すると、次回も最大化で起動するようになりました。** Windows の一般的な動きに合わせています。最大化を解いたときは、これまでどおり終了時の位置とサイズに戻ります。更新直後の初回起動は通常のウィンドウのままで、次に最大化して終了したときから効きます。常に通常のウィンドウで起動したい場合は、環境設定 →「起動と連携」→「起動時の動作」→「起動時のウィンドウ状態」で「通常ウィンドウ」を選んでください。常に最大化で起動することもできます。
- **貼り付けや新しいフォルダーの作成のあと、増えた項目が選ばれるようになりました。** 現在の並び順では追加した項目が一覧の離れた位置に入るため、どれが増えたのか分かりにくくなっていました。1 件ならその項目にカーソルが移り、複数件ならまとめてチェックが付いて、先頭の項目まで表示が動きます。名前が重なって別の名前で保存された場合も、実際に作られたものを選びます。絞り込み中は絞り込みを変えず、表示されているものだけが対象になります。
- **補正レイヤーとテキスト注釈で、見開きの左右ページを切り替えられるようになりました。** これまでは片方のページしか編集できませんでした。消しゴム・モザイクと同じように、パネルの「左ページ」「右ページ」で切り替えられます。
- **⚠️ 編集内容の保存形式が変わり、初回起動時に自動で作り直します。** 補正レイヤーのマスクを、画素ごとの数値をそのまま並べる形で保存していたため、編集を重ねると保存データが非常に大きくなり、フォルダの切り替えやページの表示が数秒止まることがありました。作り直しはバックグラウンドで行われ、数秒で終わります。編集内容はそのまま引き継がれます。実際の環境では、959 MB あったデータが 2.7 MB に、フォルダ内の保存ファイルが 734 MB から 0.6 MB になりました。
- **フォルダの切り替えや編集中に、操作を受け付けなくなる問題を修正しました。** 上の保存形式に加えて、フォルダ内の保存ファイルの書き出しをバックグラウンドで行うようにしました。書き出す間隔も、編集ツールを閉じたとき・フォルダを移ったとき・終了時を基本にして、定期的な書き出しは間隔を広げています。
- **Susie プラグインが異常終了しても、そのまま使い続けられるようになりました。** これまでは一度落ちると、mImageViewer を再起動するまでそのプラグインで開ける画像が表示できませんでした。
- **RAR 書庫を代表画像に固定したとき、親フォルダのタイルに反映されない問題を修正しました。**
- **別ウィンドウ（<kbd>F12</kbd>）の動作を軽くしました。** 動画をメインウィンドウと別ウィンドウの間で行き来させると重くなる問題と、<kbd>F12</kbd> を続けて押すと押した回数どおりに動かない問題を直しました。あわせて、動画を開いている間ずっと画面を描き直し続けていた問題も直しました。別ウィンドウかどうかに関わらず、変化があったときだけ描き直します。
- **★固定を押したとき、開いている表示が固定前の項目を指したままになる問題を修正しました。** 動画を再生しながら押すと再生が止まり、「読込中...」のまま戻らなくなることがありました。
- **見開き表示のまま <kbd>Ctrl</kbd>+<kbd>G</kbd> で補正レイヤーに入ると、何も操作できなくなる問題を修正しました。** 編集するページを 1 ページだけ表示してから始めるようになり、抜けると元の見開きに戻ります。
- **編集内容の復元を確認する画面が、ウィンドウからはみ出ることがある問題を修正しました。** コピー元の候補が複数あり、そのパスが長い場合に、画面の両端が切れて読めなくなっていました。
- **編集していない画像で、編集内容の復元を確認する画面が出ることがある問題を修正しました。** 補正プリセットを読み込んだだけで、結果が初期状態と同じだった場合に起きていました。
- **画面に短く出るメッセージの下にあるボタンが、1 回目のクリックで反応しない問題を修正しました。** メッセージが消えるまでの数秒間、動画の操作バーのボタンがクリックを受け取れませんでした。

### v3.2.0 (2026-08-23)
- **エクスプローラーなどでコピー・移動したファイルに、元の編集内容を引き継げるようになりました。** これまでは、ファイルを別のフォルダへ移したりコピーしたりすると、そのファイルに付けていた補正・消しゴム・モザイク・注釈・トリミング・★・タグが失われていました。フォルダを開いたときに、以前編集したファイルと中身が同じものがあれば、引き継ぐかどうかを確認します。コピー元が複数見つかったときは、どれから引き継ぐかを選べます。確認が不要なら、環境設定 →「フォルダ・ファイル」で切れます。
- **バケツツールで塗る範囲を選べるようになりました。** 「全体」「隣接のみ」に加えて「長方形」「楕円」「円」が増え、内側の欠けを埋めて塗れます。長方形はクリック位置の幅を保って両端へ伸ばし、交差や短い出っ張りは越えながら、広くつながる部分の手前で止まります。楕円と円は、クリック位置を含んで選択範囲の内側に収まる最大の図形に整えます。図形では「隙間の許容」も使えます。
- **⚠️ バケツの「全体」「隣接のみ」で塗る範囲が、既定で 1px 外側まで広がります。** 塗った境界に細い塗り残しが出るのを防ぐためです。以前の塗り方に戻すには、バケツの「はみ出し」を 0 にしてください。
- **⚠️ 白黒・セピアの原稿を消しゴムで補完したとき、補完部分の色調を画像に合わせるようになりました。** 補完した部分だけ色味が浮くのを防ぎます。明るさは保ったままです。以前の仕上がりに戻すには、消しゴムツールパネルの「色調の許容」を 0 にしてください（既定 12）。
- **⚠️ フォルダの代表画像を、一覧と同じファイル名順で選ぶようになりました。** これまでは一覧の並び順とは別の順番で選んでいたため、`00表紙.jpg` と `00表紙2.jpg` のようなファイルがあると、一覧の先頭とは違うものが代表画像になっていました。更新後の初回起動で一度だけ切り替わります。以前の順番に戻すには、環境設定 →「フォルダ・ファイル」→「代表画像の選択基準」で「番号順（区切り無視）」を選んでください。
- **⚠️ 動画の拡大・縮小を、mImageViewer 側で処理するようになりました。** これまでは OS に任せていました。大きな動画を小さめのウィンドウで見たときに、細かい模様がちらついて見える（モアレ）のが出にくくなります。更新後の初回起動から変わります。以前と同じ表示に戻すには、動画を全画面で開いて左パネルの「画像補正」→「フィルタ」で「OS に任せる」を選んでください。
- **動画の拡大方法を選べるようになりました。** 標準（補間あり）/ ニアレスト（補間なし）/ シャープ拡大 / アニメ塗り拡大 の 4 つから選べます。動画を全画面で開いて左パネルの「画像補正」→「フィルタ」で切り替えるか、<kbd>T</kbd> キーで順に切り替えられます。縮小のなめらかさも同じ場所で調整できます。アニメ塗り拡大は、お使いの環境で間に合う品質を自動で測って選びます。
- **動画の上部バーと下部シークバーを、固定表示にできるようになりました。** 環境設定 →「動画」→「再生画面のバー」で、上部と下部をそれぞれ選べます。固定したバーは映像に重ならず、映像はバーを除いた領域に収まります。各バーの端にある鍵のアイコンからも切り替えられます。
- **動画のシークバーにマウスを乗せたときのプレビューを、速くできるようになりました。** 環境設定 →「動画」→「シーク時のズレ許容 (秒)」です。0 秒はこれまでどおり正確な位置を出し、大きくするほど速くなります。既定は 1.0 秒です。遅く感じる場合は大きめにしてください。
- **一覧の動画サムネイルにある再生アイコンを、目立たなくできるようになりました。** 環境設定 →「表示」→「サムネイル」で、中央の再生アイコン / 左下の小さなバッジ / なし の 3 つから選べます。既定はこれまでどおり中央の再生アイコンです。音声の音楽アイコンは変わりません。
- **消しゴムやモザイクのマスク編集で、取り消した操作をやり直せるようになりました。** <kbd>Ctrl</kbd>+<kbd>Y</kbd> または <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>Z</kbd> です。
- **消しゴムやモザイクの筆の太さを、<kbd>Shift</kbd>+ホイールで変えられるようになりました。** 画面の大きさに対する割合で変わります。
- **複数ウィンドウで、アクティブでないウィンドウでもマウスジェスチャとリングが使えるようになりました。** これまでは、後から開いたウィンドウでしか使えず、先に開いたウィンドウへマウスを移しても認識されませんでした。動画を再生しているウィンドウでも同じように使えます。
- **複数ウィンドウで書庫を開いたとき、他のウィンドウの一覧まで書庫の中身に切り替わる問題を修正しました。**
- **マウスジェスチャやリングの操作を、左クリックで取り消せるようになりました。** 右ボタンを押したまま左クリックすると、そこまでの操作をやめます。
- **全画面ズーム（<kbd>Z</kbd>）で、マウスの動きと拡大位置の対応が実際の画像に合うようになりました。** これまでは画面の帯を基準にしていたため、画像の上下に余白があるときに拡大位置がずれていました。見開き表示でも合うようになっています。
- **ごみ箱へ移すときの削除確認を、省略できるようになりました。** 環境設定 →「フォルダ・ファイル」にあります。既定はこれまでどおり確認を表示します。完全に削除される場合は、この設定に関わらず確認します。フォルダや書庫も省略の対象になるので、中身をまとめてごみ箱へ移すことになる点にご注意ください。
- **PDF が多いフォルダで、開こうとした PDF が後回しにされることがある問題を改善しました。** 裏で読み込んでいる PDF が多いと、利用者が開いた PDF の順番が回ってくるまで待たされていました。同時に開く数に上限を設け、利用者が待っているものを先に通すようにしています。

### v3.1.3 (2026-08-21)
- **見開き表示でカーソルキーを長押ししてページを戻すと、操作を受け付けなくなることがある問題を修正しました。** 複数ウィンドウで起きやすく、押すのをやめても「読み込み中」のまま戻らない状態でした。ページを続けて送っている間は重い処理を後回しにする仕組みがあるのですが、その「後回し」が、いま表示しようとしているページを用意する処理まで止めていました。表示できるものが手元に無いときは、後回しにせず用意するようにしました。
- **別ウィンドウでページを続けて送るとき、これまで働いていなかった軽量化が効くようになりました。** ページ送りの速さが一定せず、重く感じることがありました。連打の間はサムネイルを使って軽く表示する仕組みがあるのですが、別ウィンドウでは一覧を表示しないという理由でサムネイルの結果を捨てていたため、一度も使われていませんでした。捨てずに使うようにしています。
- **最初に PDF を開くとき、しばらく操作を受け付けないことがある問題を修正しました。** PDF を扱う準備を、画面の描き直しと同じ場所で待っていたためです。準備は裏で行うようにしたので、開いた直後から操作できます。
- **PDF のページ表示が速くなりました。** これまでは 1 ページ描くたびにファイルを開き直していました。同じファイルを続けて表示するときは開いたまま使い回します。ページ数の多い PDF ほど差が出ます。
- **時間のかかる PDF を続けてダブルクリックしても、待ち時間が積み重ならないようにしました。** 開かないのでもう一度クリックする、を繰り返すと、そのたびに待ち時間が伸びていました。同じファイルを開き直したときは、すでに始まっている読み込みの結果を待つようにしています。
- **PDF の同時処理数を選べるようになりました。** 環境設定 →「パフォーマンス」→「並列読み込み」にあります。3〜10 で、既定は 5 です。増やすと同時に描ける数が増えますが、その分メモリを使います。変更は次回起動から有効です。
- **PDF を扱う準備に失敗したとき、理由を表示するようになりました。** これまでは黙って中途半端な状態で動き続けていました。表示中は画像・動画・書庫と、すでに読み込み済みの PDF ページはそのまま使えます。

### v3.1.2 (2026-08-19)
- **一覧でダブルクリックしても開かないことがある問題を修正しました。** 一度で開かず、何度かやり直すと開く、という形で起きていました。内部の判定が、決まった時間内の 2 回目を「ダブルクリック」、その 2 倍の時間内の 3 回目を「トリプルクリック」として数えていて、**ちょうど 2 回目のときだけ**開く扱いだったためです。1 回目が効かずにもう一度クリックすると、その 3 回目がちょうどここに落ちていました。一覧では、直前のクリックと同じ項目かどうかで組にして判断するようにしたので、続けて何度クリックしても開きます。
- **ダブルクリックと判定する間隔が、Windows のマウス設定に従うようになりました。** これまでは 0.3 秒で固定していて、Windows の初期値（0.5 秒）より短く、設定を変えている場合はさらに離れていました。「ダブルクリックしたつもりが効かない」の一因です。設定は起動時に読み込みます。
- **動画と画像が混在するフォルダで、動画を通り過ぎたあとカーソルキーが効かなくなる問題を修正しました。** <kbd>Esc</kbd> で一覧へは戻れるものの、上下左右のどれも反応しなくなる状態でした。動画へ移動したときに、表示の切り替わりを見届けるための待ちが作られるのですが、動画は別の仕組みで表示するため、その待ちが終わることがありませんでした。待ちを作る対象を、実際に画面へ描く項目だけに限りました。
- **別ウィンドウで動画を再生中に、他のアプリから戻ってくると <kbd>Z</kbd> だけ効かない問題を修正しました。** 再生・停止や矢印キーは効くのに、音声モードへの切り替えだけが反応しませんでした。キーの届き方が場面によって変わるのに、この操作だけ片方の経路にしか用意されていなかったためです。
- **動画の補正スロットの読み込み（既定 <kbd>Ctrl</kbd>+<kbd>1</kbd>〜<kbd>0</kbd>）が効かないことがある問題を修正しました。** 上と同じ理由で、別ウィンドウで再生中に他のアプリから戻ってきたときに効きませんでした。音声モードのように映像が出ていないときに読み込まないのは、これまでどおりです。
- **全画面ズーム（<kbd>Z</kbd>）で、範囲を示す枠とマウスカーソルの位置がずれて見える問題を直しました。** 押している間はカーソルを隠すようにしたので、枠だけを見て範囲を決められます。離したときに表示される範囲はこれまでと同じです。
- **書庫（RAR など）が多いフォルダで、サムネイルが出るまで待たされる問題を改善しました。** これまでは、フォルダ内のすべての書庫を調べ終えるまで 1 枚も表示できませんでした。調べ終わったものから順に表示し、調べる対象も画面に見えている範囲とその前後だけにしました。一度開いたフォルダは 2 回目以降さらに速くなります。
- **選択している項目をもう一度クリックすると開く動作を、設定で選べるようにしました。** 環境設定 →「サムネイル」→「一覧のクリック選択」にあります。既定はオフで、これまでと同じ動作です。修飾キーを押していないマウスクリックだけが対象で、複数選択しているときは 1 件に戻すだけです。
- **マウスジェスチャを実行したあとの通知を、表示しないようにできます。** 操作カスタマイズ →「設定」→「動作」にあります。既定は表示です。オフにしてもジェスチャの操作そのものは実行されます。
- **「A/B の記憶した場所をクリア」が、一覧の位置もまとめて消すようになりました。** これまでは A/B の場所だけを消していたため、フォルダへ入り直すと以前のカーソル位置が戻っていました。実行すると、いま開いている一覧も先頭に戻ります。チェックや評価、読書位置、再生位置は消えません。
- **フルスクリーン上部の「⚠ ダウンスケール表示中」という表示を、「⚠ GPU上限により解像度制限中」へ変えました。** 画面に合わせた縮小表示のことだと誤解されていました。この表示が出るのは、元画像の幅か高さが 8192px を超えていて、表示用の画像をその範囲へ縮めているときだけです。出る条件と、元画像の解像度を併記する動作は変えていません。

### v3.1.1 (2026-08-18)
- **見開きを 1 ページずらしたあと、ページ送りができなくなることがある問題を修正しました。** ずらした直後の 1 回だけ送れて、そのあとは何度押しても動かなくなる状態でした。表示が切り替わったかどうかの判定が、実際に画面へ出たページではなく、内部で使い回される目印を見ていたためです。画面に出たページそのもので判定するようにしました。
- **元画像を表示するキー（既定は右 <kbd>Ctrl</kbd>）を押したままページを送ると、戻る方向だけ極端に遅くなる問題を修正しました。** 押しっぱなしの間、ページの合間に元画像が一瞬だけ現れるちらつきも同じ原因です。ページを送っている間は元画像の表示を止めるようにしたので、送る速さは押していないときと変わらなくなりました。矢印から指を離せば、これまでどおり元画像に戻ります。
- **元画像を表示したままページを移動すると、数秒間止まることがある問題を修正しました。** 画面には加工前の画像を出しているのに、次のページへ進んでよいかの判断だけが AI 処理や色の加工の完了を待っていました。その間に押したキーはすべて捨てられていたため、操作を受け付けないように見えていました。分析モードでも同じ経路を通ります。
- **動画を音声モードにしたあと、<kbd>Z</kbd> で動画表示へ戻れない問題を修正しました。** 動画から音声モードへ入る方向は効くのに、戻る方向だけ効きませんでした。同じキーを画像の全画面ズームにも割り当てているため、音声モードでも画像側が先にキーを受け取って捨てていたのが原因です。表示中の内容に応じて、キーを受け取る側を先に決めるようにしました。
- **書庫（ZIP など）の中にある GIF / APNG も、アニメーションするようになりました。** これまで書庫の中で動いていたのは WebP だけで、GIF と APNG は 1 コマ目の静止画で表示していました。書庫かどうかで扱いを変えないようにそろえました。
- **アニメーション画像を含むフォルダや書庫で、表示が止まったりメモリを大きく消費したりする問題を修正しました。** 先読みしているページまで全コマを展開していたため、アニメーション画像が並んでいると十数本分が同時にメモリへ載ることがありました。先読みは 1 コマ目だけにし、全コマを展開するのは実際に開いたページだけにしました。あわせて、開いた直後の展開中は画面右上に進捗を表示します。
- **ウィンドウの操作中に、表示と操作の受付が止まることがある問題に対処しました。** ウィンドウからの通知を処理している最中に、そのまま画面の描き込みまで行っていたため、描き込みの待ち時間がそのまま操作の待ち時間になっていました。通知の処理と描き込みを分けたので、待ち時間が操作へ波及しにくくなります。
- **表示の更新に失敗したフレームで、使い終わった画像データが解放されずに残る問題を修正しました。** 1 回ごとの量はわずかですが、サムネイルを大量に表示し続ける使い方では積み上がります。
- **操作カスタマイズのキーボード図の表示を直しました。** 割り当て一覧の並べ替えボタンが、押せない状態のときに列ごと詰まって左右にずれる問題と、キーボード図の文字の行（Q の行・A の行）の左端が本来の位置よりわずかに左へずれる問題です。

### v3.1.0 (2026-08-16)
- **フルスクリーンの案内表示を、個別に消せるようにしました。** 環境設定の「表示 → 閲覧表示」に 3 項目を追加しました。いずれも既定は表示のままです。
  - **先頭 / 末尾の案内を表示**: 一覧の端でページを送ろうとしたときに出る案内です。文言も「最初の項目です」から「**先頭です**」（末尾も同様）へ変えました。「項目」は内部の呼び方をそのまま出していたもので、画面上の案内としては分かりにくいためです。
  - **処理状況を表示**: いま見ているページの読み込み・AI 処理・加工の状況です。
  - **先読み状況を表示**: 前後のページを先読みしている状況です。
  - どれを消しても、ページ移動・読み込み・加工・先読みの動作そのものは変わりません。また、まだ画像を表示できないときに画面中央へ出る「読込中...」は、画面が空である理由を示すため、設定にかかわらず表示します。
- **先読みの表示を、現在のページの前後に分けました。** これまでは「4 / 12」のように件数だけで、**どちら向きのページが済んでいるのかが分かりませんでした。**「次は用意できている」と思って進んだら、済んでいたのは戻る方向だけ、ということが起こります。現在位置を挟んで、左に前のページ、右に次のページを並べ、取得済み（緑）・取得中（黄）・未取得（灰）を 1 つずつ示します。先読みの枚数を増やしている場合は、近い 4 ページを丸で、それより遠い分をまとめた数で表示します。
- **「最近開いたフォルダ履歴をクリア」と「A/B の記憶した場所をクリア」を、キーやマウス操作へ割り当てられるようにしました。** フォルダバーの設定メニューにある操作と同じものです。標準のキーは割り当てていないので、操作カスタマイズからキー・リング・マウスジェスチャ・マウスボタン・ゲームパッドへ登録してお使いください。
- **リモート閲覧で、サムネイル一覧を指でゆっくり動かすと上下に震えることがある問題を修正しました。** 指が画面に触れている間かどうかの判定が正しくなく、ドラッグの途中で表示位置をそろえる処理が動いていました。
- **リモート接続の設定画面が、Tailscale の現在の状態を見るようになりました。** これまでは有効にした瞬間の状態のままで、あとから Tailscale 側の設定を変えても、いったん無効にして有効にし直すまで表示が古いままでした。「再確認」を押せば読み直します。あわせて、Tailscale が見つからないときは PIN の設定より前にその旨を表示し、リモート接続が無効のままでも証明書や公開設定を確認・変更できるようにしました。設定画面の並びも、実際に進める順番に合わせています。
- **フルスクリーンで「読込中...」が二重に表示されることがある問題を修正しました。** AI 機能を使っているとき、画像の左下と画面の左下に同じ表示が重なって出ていました。
- **キーを押した直後に他のウィンドウが前面に出ると、その 1 回が効かないことがある問題を修正しました。** 受け取った時点で前面だったキーは、そのあとで前面が変わっても処理します。
- **<kbd>R</kbd> / <kbd>L</kbd> で回転したページが、見開きで正しく単独表示されるようになりました。** 縦長のページを回転して横向きにすると、画面では横長なのに見開きの相手を探してしまい、2 ページ並べて表示していました。回転後の見た目で判断するようになり、本物の横長ページと同じく 1 ページで表示します。
- **ブラウザからのリモート閲覧でも、回転したページが回転した向きで表示されるようになりました。** これまで一覧のサムネイルには回転が反映される一方、開いたページは回転前の向きのままで、同じ画面の中で向きが食い違っていました。見開きの組み合わせも本体と一致します。なお回転したページでは、本体と同じく表示トリムは適用されません。
- **mImageViewer が使うデータの保存先がネットワーク上にある場合、起動時にお知らせするようにしました。** NAS などに本体ごと置いた場合、サムネイルや検索用の索引も同じ場所に作られます。この置き方はサポートしておらず、起動や表示が遅くなったり、終了後の再起動で応答しなくなったりすることがあります。**とくに、同じ保存先を複数の PC から使うとデータが壊れることがあります**（mImageViewer 側では検出できません）。お知らせは保存先ごとに一度だけ表示します。詳しくは[マニュアル](https://mikage.to/mimageviewer/manual/troubleshooting.html#network-data-dir)をご覧ください。
- **TensorRT を有効にしているとき、AI 処理を初めて使うまでの待ち時間が短くなりました。** これまでは準備に 15 秒ほどかかっていましたが、その大半は、実際には成功せず時間だけを消費していた手順でした。取り除いたので、待ち時間はほぼ無くなります。あわせて、この手順が原因で Windows のイベントログに残っていたエラーの記録も出なくなりました。画質や処理速度そのものは変わりません。
...
- **上部バーに、使用頻度の低い操作をまとめたボタンを追加しました。** ナビゲータの 2 つの設定・ピクセルグリッド・ルーペの固定・分析モードを、指でも押しやすい大きさの一覧から選べます。これまでキーだけで画面上のボタンが無かったものです。分析モードは上部バーからこの一覧の中へ移りました。あわせて、360 度ビューのボタンは見開き表示や連結読みのときも、消えずに押せない状態で残るようにしました。
- **消しゴムと隠蔽加工にも、塗りつぶし（バケツ）を追加しました。** ツールを「バケツ」（<kbd>K</kbd>）に切り替えると、補正レイヤーと同じく、クリックした色に近い範囲を一度に塗れます。平らな面をブラシでなぞる必要がなくなります。
- **フルスクリーンのシークバーに目盛りを表示するようにしました。** 動画・音楽の再生位置がつかみやすくなります。
- **縦向きで撮影した動画を、正しい向きで再生するようにしました。** スマートフォンで撮った動画など、回転の情報を持つ動画が、サムネイルと再生とで違う向きになっていました。
- **RAR / 7z / LZH も、親フォルダの代表サムネイルに固定できるようにしました。** 変換済みのものが対象です。
- **キーの割り当て候補一覧に検索欄を追加しました。**
- **比較表示の操作を整理しました。** ワイプの境界線は、左 <kbd>Ctrl</kbd> を押しているあいだ消えるので、線が重なっていた部分を確認できます。元画像を表示するキー（既定は右 <kbd>Ctrl</kbd>）は、比較表示中は効かなくなりました。比較は加工後の画素で作っているため、押しても画面は変わらないまま、周囲の表示だけが乱れていたためです。
- **注釈の色を選ぶときに、加算合成を選べないようにしました。** 選ぶと不透明度が 0 になり、通常合成に戻せなくなる問題があったためです。
- **ニアレスト（ドット感を残す拡大）が、画像によって効いたり効かなかったりする問題を修正しました。** どの拡大方法を使うかの判断が画像を GPU へ送る処理のあちこちに書かれていて、効くのは一部だけでした。注釈・画像補正・AI のどれを使っているかで結果が変わっていました。
...
- **絞り込みバーの ★ プルダウンでも、<kbd>Ctrl</kbd> / <kbd>Shift</kbd> を押しながらのクリックが使えるようになりました。** ツールバーの ★ と同じ選び方です。
...
- **PDF を続けて開いたときに、前の文書のページが表示されてしまうことがある問題を修正しました。** AI 拡大の結果が、それを始めた文書ではなく移動後の文書のページとして扱われていました。
- **切り取り（トリミング）の範囲が、画像の解像度によってずれる問題を修正しました。** 範囲を作ったときの解像度も一緒に保存するようにしました。以前に保存した範囲は、最初に読み込んだ解像度を基準として解釈します。すでにずれた状態で保存されているものは、お手数ですが再度設定し直してください。
- **360 度ビューでは、高画質へ切り替わったときに消しゴム・隠蔽加工・補正レイヤーの結果が反映されません。** この制限がある画像では、画質の表示の下にその旨を出すようにしました。あわせて、360 度ビュー中は画像編集モードに入れないようにしました。
- **タッチ操作を直しました。** ナビゲータに触れるとページが送られてしまいナビゲータ自体を操作できなかった問題、360 度ビューと分析モードでピンチしても拡大縮小できなかった問題、それらのモードの画面を触るとページが送られてしまう問題を修正しました。
- **PDF が「表示するファイルがありません」になったとき、理由を表示するようにしました。** 読み取りに失敗した場合とパスワードが必要な場合を、中身が空のフォルダと区別して示します。
- **連結読みの途中で、横方向のキー操作で動画に入ってしまう問題を修正しました。** ホイールと同じく動画を飛ばすようにしました。
- **ドライブ操作のコマンド名を分かりやすくしました。**

### v2.13.0 (2026-08-11)
- **タッチ画面で操作できるようにしました。** タブレット PC や タッチ対応ディスプレイで、キーボードやマウスがなくても一通りの閲覧ができます。
  - 一覧では、1本指の上下ドラッグでスクロールし、指を離すと動かしていた方向の行境界へ進んで止まります。2本指で広げると列数が減ってサムネイルが大きくなり、縮めると列数が増えます。
  - フルスクリーンでは、左右をタップしてページを移動し、中央をタップすると上部バー・下部バーと左右パネルのハンドルが出ます。2本指のピンチで拡大縮小、そのまま移動できます。縦・横の連結読みも指でスクロールできます。
  - 動画では、中央のタップで HUD、左右のタップでシーク（続けてタップすると5秒送り）ができます。音楽ビューでは、一度タッチすると上下のバーのあいだに左右パネル用のハンドルが出ます。
  - 初めてタッチしたときに、どこを触ると何が起きるかの案内を1度だけ表示します。ページの左右と読み方向の対応もそこで確認できます。
  - フルスクリーンの右上隅は × として反応するので、キーボードなしでも閉じられます。上部バーのボタンはタッチのときだけ押しやすい大きさになります。
  - ボタンが小さく感じる場合は、メニューの「スケーリング」から表示倍率を上げられます。
- **拡大中のナビゲータを改善しました。** 画像全体が見えているときも隠さず表示します（ナビゲータ上をドラッグして範囲を指定するズームは、等倍のときこそ使うためです）。表示範囲が画像より広いときは、枠を全体に置いたまま画像側を内側へ小さく描いて、いま画像より広く見えていることが分かるようにしました。比較表示中も、画面に出ているのと同じ合成結果の全体像を表示します。縮小画像にはカラー化・画像補正・AI 処理・注釈・隠蔽加工・消しゴムの結果が反映されるようになりました（これまでは加工前の元画像が出ていました）。
- **一覧に「名前の変更」と「更新」のコマンドを追加しました。** どちらも既定ではキーが割り当てられていません。操作カスタマイズから <kbd>F2</kbd> や <kbd>F5</kbd> など好きなキーに割り当てられます（既定で <kbd>F1</kbd>〜<kbd>F6</kbd> に入っているレーティングを別のキーへ移すか解除してからお使いください）。「更新」はいま開いている一覧を読み直すもので、フォルダ・スマートフォルダ・サブ展開・絞り込み中のいずれでも使えます。ファイルメニューからも実行できます。
- **削除の確認画面を矢印キーで選べるようにしました。** 左右・上下の矢印で「削除」と「キャンセル」を選び、<kbd>Enter</kbd> で決定します。ごみ箱へ移す確認は「削除」、完全に削除される可能性がある確認は「キャンセル」が最初に選ばれます。これまでどおり <kbd>Y</kbd> / <kbd>N</kbd> / <kbd>Esc</kbd> も使えます。
- **見開き表示で、同じページが左右に2枚並ぶことがある問題を修正しました。** 横長のページは1枚で表示する判定をしていますが、その判断に使うページの縦横比を、サムネイルが画面から離れると忘れてしまっていました。ページ数の多いフォルダを速くめくったときに起きていました。
- **比較表示を直しました。** ワイプの境界線は、拡大しても掴む位置とずれなくなり、画像の左右の端まで動かせます。線はドラッグ中と線の近くにポインターがあるあいだだけ出るので、比較中に邪魔になりません。<kbd>C</kbd> で通常表示に戻してからもう一度比較に入るとき、変わっていない側を作り直さなくなったので切り替えが速くなりました。極端に小さい画像と比べたときに、ピン留めした側までその大きさに縮められてぼやけていた問題も直しています。
- **比較表示（<kbd>X</kbd> / <kbd>C</kbd>）は単ページ表示専用になりました。** 見開き表示では使えません。見開きのまま操作すると、その旨を表示して比較には入りません。比較中に見開きへ切り替えると比較を終了します。
- **ルーペが画像の端で像を引き延ばす問題を修正しました。** 画像からはみ出した部分は背景のままになり、カーソル位置と倍率はそのまま保たれます。カーソルが画像の外へ出ても、拡大している範囲が画像に少しでも重なっているあいだはルーペを表示し続けます。
- **フォルダツリーの幅が、読み込み中に少しずつ広がり続ける問題を修正しました。** ディスクの入っていない光学ドライブを開こうとしたときに顕著で、広がった幅がそのまま保存されていました。
- **テキスト注釈の色を選ぶ画面が、パネルの外を最初にクリックしたときに閉じてしまう問題を修正しました。**
- **フォルダを切り替えた直後に、前の一覧のページ情報が混ざることがある構造上の問題に対処しました。** 一覧が入れ替わったときに、古い情報を確実に捨てるようにしました。

### v2.12.0 (2026-08-06)
- **画像を拡大したときの画質を上げました。** これまで拡大は近くの4点を混ぜるだけの簡易な方法だったため、線や文字の輪郭がぼやけていました。今後は縮小と同じ高品質な方法で拡大します。画面に映っている範囲だけを作り直すので、大きな写真でも待たされません。
- **拡大方法を選べるようにしました。** 画像補正パネルの「フィルタ」から、標準のほかに「シャープ拡大」「アニメ塗り拡大」を選べます。シャープ拡大は輪郭を立て、アニメ塗り拡大はイラストや漫画のような塗りの境目をなめらかに保ちます。アニメ塗り拡大には処理する大きさの上限があり、既定は長辺4096ピクセルです。
- **拡大中に画像のどこを見ているかを示すナビゲータを追加しました。** 拡大表示中に <kbd>Alt</kbd> を押しているあいだ、画面の隅に全体の縮小画像と現在の表示範囲の枠が出ます。枠をドラッグすると表示位置を動かせます。360度パノラマ表示にも対応しています。表示する隅は枠の上部から選べ、<kbd>Alt</kbd>+<kbd>N</kbd> で出したままにもできます。
- **一覧をファイル名で絞り込めるようにしました。** 絞り込みバーの「ファイル名」欄に文字を入れると、今表示している一覧をファイル名で絞り込めます。空白で区切った語はすべてを含むもの、先頭に <code>-</code> を付けた語は含まないものが残ります。欄の幅は絞り込みバーの右クリックから小・中・大を選べ、欄そのものを隠すこともできます。
- **サブ展開で、たどる階層と集める項目を選べるようにしました。** サブ展開を押すと確認画面が開き、「起点のみ」から「無制限」までの深さと、種類・サイズ・日付による絞り込みを指定できます。条件に合わない項目は最初から集めないので、深い階層に大量のファイルがあるフォルダでも必要な分だけを一覧にできます。選んだ条件は次回も引き継がれ、展開後はフォルダバーにも表示されます。
- **サブ展開した一覧で、それぞれの項目がどのフォルダから来たかが分かるようになりました。** 場所の表示がサブ展開を始めたフォルダからの相対的な位置になり、同じ名前のファイルが並んでも区別できます。詳細表示には並べ替えできる「場所」列を追加しました（既定では非表示です）。あわせて、詳細表示で行にマウスを重ねたときに出ていた説明を廃止しました。列と同じ内容を繰り返すうえ、複数行を覆い隠していたためです。
- **一覧に出していないファイルがあることを知らせるようにしました。** 同じ名前のファイルを1つにまとめた分、隠れている項目、開けない形式のファイルがあると、フォルダバーに「非表示 N 件」と表示されます。クリックすると内訳を確認でき、そこから同名ファイルの設定を開けます。フォルダを削除するときは、一覧に見えていないファイルも消えることを確認画面で必ず案内します。
- **見開き表示で、拡大率を変えるとページの配置が入れ替わる問題を修正しました。** ある倍率を境に左右のページの高さの合わせ方が切り替わり、レイアウトが跳ねていました。
- **固定表示した上部バー・下部シークバーとの境界に、明るい線が見えていた問題を修正しました。** あわせて、固定したバーと画像のあいだの間隔を0〜100ピクセルで調整できるようにしました（既定は0ピクセルです）。
- **環境設定を開くたびに、右側のパネルがページの先頭から表示されるようになりました。** これまでは前回のスクロール位置が残ることがありました。
- **操作カスタマイズで、コマンド一覧の「場所」の選択が勝手に変わらないようにしました。** 「すべて」を選んでいてもキーを設定すると特定の場所へ切り替わっていました。
- **元画像を一時的に表示するキーを変更・無効化できるようにしました。** これまで <kbd>右Ctrl</kbd> で固定でしたが、操作カスタマイズから変更できます。<kbd>Shift</kbd> や <kbd>Alt</kbd> の押しっぱなしで働く操作も同じように設定できます。
- **本体の <kbd>Enter</kbd> とテンキーの <kbd>Enter</kbd> を取り違えていた問題を修正しました。** どちらかを操作カスタマイズで押しっぱなしの操作（拡大モードなど）に割り当てていると、<kbd>Enter</kbd> で画像を開いた直後に勝手にその操作が始まる、テンキーの <kbd>Enter</kbd> を押すと表示が閉じてしまう、といったことが起きていました。既定の割り当てのままでは起きません。
- **PDFの表示と動画の再生に使っている部品を新しい版へ更新しました。**

### v2.11.0 (2026-08-04)
- **原寸表示（100%）を、画面のピクセルと画像のピクセルが1対1で対応する表示に変えました。** Windows側で拡大縮小（125%・150%など）を設定していると、これまでは原寸表示でも画像がその倍率のぶん引き伸ばされ、線や文字がぼやけていました。今後は引き伸ばさずそのまま表示し、表示位置もピクセルの境目に合わせます。そのぶん、拡大縮小を設定している環境では、これまでより小さく表示されます。「拡大しない」「縮小しない」も同じ基準になりました。見開きで左右のページの高さがわずかに違うときに、片方だけがぼやけていた問題も直っています。
- **縮小して表示するときの画質を上げました。** これまでは縮小した画像をいくつかの大きさで用意して混ぜ合わせていたため、細い線が薄くなったり、全体がやや柔らかく見えたりしていました。今後は表示する大きさへ直接作り直すので、細部が残ります。
- **「縮小時のなめらかさ」を追加しました。** 画像補正パネルの「フィルタ」にあります。0%が最もくっきりし、上げるほどスクリーントーンなどの模様に出るちらつきが減る代わりに、細部が柔らかくなります。既定は0%です。これまでの「縮小表示のモアレを抑制する」はこの設定に置き換わりました。
- **タスクトレイへ格納したときや最小化したときに、CPUを使い続ける問題を修正しました。** 画面に何も映していないあいだも表示の更新を繰り返し、CPUを1コア分使ったままになることがありました。
- **上部のバーを固定していると、画面右上などの表示が隠れてしまう問題を修正しました。** スライドショーの進み具合、背景の切り替え、元画像の表示中を示す表示が、固定したバーの下に隠れないようになりました。下部のシークバーを固定したときの比較用の表示も同様です。

### v2.10.0 (2026-08-03)
- **「読書履歴」を「閲覧履歴」に改め、動画・音声も残るようにしました。** これまで本（フォルダー・ZIP・PDF）だけだった履歴に、動画と音声のファイルが加わります。動画・音声は再生位置も一緒に記録され、一覧では「すべて／動画／音声／本」で絞り込めます。種類・拡張子・場所での絞り込みも使えるようになりました。
- **自分で選んで開いたものだけを履歴に残すようにしました。** 連続再生で次のファイルへ進んだとき、スライドショーが自動で送ったとき、スライドショーの末尾で次のフォルダーへ進んだときは記録しません。履歴は探し直すための記録なので、自動で流れていったものまで残すと、本当に残したいものが押し出されてしまうためです。一覧やキー操作で自分から移動したときは、これまでどおり記録されます。
- **縮小表示の見え方を選べるようにしました。** 画像補正の「フィルタ」に「縮小表示のモアレを抑制する」を追加しました。オフにすると線がくっきりする代わりにモアレやちらつきが出やすくなり、オンのまま強さを上げるとモアレが減って細部が柔らかくなります。既定は今までと同じ見え方のままです。
- **別のウィンドウで入力中に、キー操作がそちらへ届き続ける問題を修正しました。** 別ウィンドウ（`F12`）でブックマーク名などを入力しているとき、メインウィンドウをクリックして操作しても矢印キーなどが入力欄側へ行ってしまいました。とくに入力中のウィンドウが画面の外にあると、アプリ全体が反応しなくなったように見えていました。
- **日本語入力の変換中に `Esc` を押すと、確定していない文字が残る問題を修正しました。** テキスト注釈の本文欄で、あわせて入力欄の選択が外れてしまう問題も直っています。
- **本として扱わないフォルダーで、見開き表示になってしまう問題を修正しました。** 「画像のみのフォルダを本として扱う」がオフのとき、およびオンでも画像以外のファイルが含まれるフォルダーでは、単ページで開くようになります。そのフォルダーに見開きを保存していた場合は、これまでどおりその設定が使われます。
- **全画面で動画を再生中に VST プラグインの画面を開くと、マウスカーソルが消えたままになる問題を修正しました。** カーソルがどのウィンドウの上にあるかではなく、どのウィンドウが実際にカーソルの操作を受け取っているかで判断するようにしました。
- **ゲームパッドの操作が、日本語入力中でも発火してしまう問題を修正しました。** 全画面表示のパネルや動画の画面でブックマーク名などを変換している間は、ゲームパッドの操作を受け付けないようにしました。
- **名前の変更中に「変更中...」の表示が出ないことがある問題を修正しました。**
- **その他の改善と修正。** タスクトレイへ格納したまま再生を続けたときの負荷を抑え、名前の変更にともなう設定の引き継ぎが表示を止めないようにしました。見開き表示で片方のページの読み込みが始まらない場合がある問題も直しています。

### v2.9.1 (2026-08-01)
- **ファイル名の変更と新しいフォルダーの作成を、Windows 標準の入力画面で行うようにしました。** 文字の編集や日本語入力の扱いがエクスプローラーと同じになります。
- **見開き表示で片方のページが「読み込み中」のまま出てこないことがある問題を修正しました。** そのまま待っても表示されず、補正スライダーなどを操作してはじめて表示される状態になっていました。
- **日本語入力中に入力欄が閉じてしまう問題を修正しました。** ブックマーク名などを入力しているとき、フォーカスが外れて左パネルごと閉じることがありました。
- **名前などを入力している最中のキー操作が、背後の表示に伝わってしまう問題を修正しました。** 全画面表示の左パネルでブックマークの名前を入力中に、`Esc` で表示が閉じたり、矢印キーでページが移動したりすることがありました。
...
- **ブックマーク一覧でサムネイルのバッジが重なる問題を修正しました。** 時刻がタグに重なる問題を解消し、フォルダー名のバッジも形式バッジと釣り合う大きさに整えました。
- **名前を変更しても、編集結果を反映したサムネイルを引き継ぐようにしました。**
- **AI の切り替え待ちの間、表示が重くなることがある問題を修正しました。** 画像補正やスマートシャープを使っている場合に効果があります。
