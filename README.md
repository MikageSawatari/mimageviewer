...
- **動画のサムネイル列が中央半分の幅になったり、操作バーを出し入れしているうちにメモリ使用量が大きく増えたりする問題を修正しました。** 操作バーが一時的に隠れるたびにサムネイル列の準備を作り直していたのをやめ、再表示したときはそのまま使います。列の枚数も、実際に表示する枚数と揃うようになりました。
- **お気に入りの索引を作る・更新するときの待ち時間を短縮しました。** フォルダ数の多い場所で起動後に行う照合を見直し、変わった範囲だけを調べるようにしました。ZIP の中身を読み直す回数も減らしています。
- **サムネイル画質の設定ダイアログが、開いた直後に画面下端まで伸びてしまう問題を修正しました。** 操作項目の下に大きな空白ができなくなります。
...
- **フルスクリーンの右パネルで、本の重なりを示すページ帯を押しても相手のページが出ないことがある問題を修正しました。** ページ数の多い本で帯が圧縮されているときに、見えている枠の内側を押しても反応しないことがありました。
