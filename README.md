...
- **見開きを縮小して表示したとき、ページの間隔が設定より広くなることがある問題を修正しました。** 1px に設定していても、画像によっては 2px 空いていました。左右のページをそれぞれ別に画面のドット位置へ合わせていたため、合わせたあとの間隔が設定と変わっていたものです。なお、拡大表示しているときなど一部の条件では、継ぎ目の太さがわずかにずれる場合が残っています。こちらは今後のバージョンで修正します。
