...
### v3.8.0 (2026-09-10)
- **表示中の画像と同じ絵を、別の大きさや形式で保存したものから探せるようになりました。** メニュー「お気に入り」→「編集」で、調べたい場所の「別バージョン索引」を ON にすると使えます（既定は OFF）。フルスクリーンの右パネル「類似」に候補が並び、クリックするとその保存場所へ移動します。JPEG / PNG / WebP など形式が違っていても、大きさが違っていても照合します。索引を作った後は、その場所のファイルの追加・変更・削除を自動で追いかけます。チェックを OFF に戻すと、その場所の索引データは削除されます。検索は画像単位と本単位の両方で行います。本単位では「この本と重なる本」に、どのページがどこと一致しているかをページ帯で示すので、ある本の一部が別の本に含まれている、といった重なりも見つけられます。
