...
### v3.4.0 (2026-09-01)
- **1 枚の絵を 2〜4 枚に切り分けて書き出せるようになりました（SNS 分割）。** X や Instagram に複数枚として投稿し、横にめくると全体で 1 枚の絵に見える投稿を作るためのものです。全画面で画像補正パネルのいちばん右のアイコンから始めます。枠は全体をドラッグして移動し、四隅や辺をドラッグして拡大縮小して、パネルの「分割して書き出す」で書き出します。X は画像と画像の間に隙間が入るため、その裏に隠れる帯をあらかじめ取り除いて継ぎ目が繋がるようにします（枠の比率 3:4、取り除く幅は枠の 1.7%）。Instagram は隙間なしの 4:5 です。ファイル名の末尾に `_1` から順に番号が付くので、その順に添付してください。回転しているページでは使えません。
