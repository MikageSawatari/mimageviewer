...
- **画像を全画面で見ているときに <kbd>Ctrl</kbd>+<kbd>↑</kbd> / <kbd>↓</kbd> でフォルダを移動すると、画面が激しくちらつく問題を修正しました。** 前の版で設定の復元を待たずに操作できるようにした際、移動の途中で前の表示を保ち続ける条件から、この復元待ちが抜けていました。移動先の画像を表示できるまで、これまでどおり前の画像を表示し続けます。一覧が一瞬見えたり、黒い画面に「読み込み中」が出たりしなくなります。
