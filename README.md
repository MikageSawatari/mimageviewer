...
- EXIF 表示（日本語タグ名対応）
- 非破壊回転
- レーティング / タグ操作の Undo / Redo（<kbd>Ctrl</kbd>+<kbd>Z</kbd> / <kbd>Ctrl</kbd>+<kbd>Y</kbd>、最大 50 操作）
...
- **メタ操作の Undo / Redo**: <kbd>Ctrl</kbd>+<kbd>Z</kbd> でレーティング / タグの誤操作を取り消し、<kbd>Ctrl</kbd>+<kbd>Y</kbd> （または <kbd>Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>Z</kbd>）でやり直し。10 枚に F3 で一括★3 を付けてしまった、といった一括操作も 1 回の取り消しで全部元に戻る。最大 50 操作まで履歴を保持し、フォルダ移動・フルスクリーン遷移・フルスクリーン中の画像切替で履歴をクリア (現在ページの単位で履歴を持つ設計)。ファイル削除や移動は対象外
